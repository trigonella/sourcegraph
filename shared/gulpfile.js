...
const GRAPHQL_SCHEMA_PATH = path.join(
  __dirname,
...
async function graphQlSchema() {
  ...
  const formatOptions = await resolveConfig(__dirname, {
    config: __dirname + "/../prettier.config.js"
  });
  const typings =
    ...
    generateNamespace(
      ...
      },
      {
        ...
        enumTypeBuilder: (name, values) =>
          `export ${DEFAULT_OPTIONS.enumTypeBuilder(name, values).replace(
            /^const enum/,
            "enum"
          )}`,
...
const draftV7resolver = {
  ...
  read: () =>
    readFile(
      path.join(__dirname, "../schema/json-schema-draft-07.schema.json")
    ),
  ...
  await Promise.all(
    ["json-schema-draft-07", "settings", "site"].map(async file => {
      let schema = await readFile(
        path.join(schemaDirectory, `${file}.schema.json`),
        "utf8"
      );
      ...
      const types = await compileJSONSchema(
        JSON.parse(schema),
        "settings.schema",
        {
          cwd: schemaDirectory,
