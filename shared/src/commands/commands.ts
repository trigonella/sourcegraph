...
export function registerBuiltinClientCommands(
  ...
  context: Pick<
    PlatformContext,
    "requestGraphQL" | "telemetryService" | "settings" | "updateSettings"
  >
): Unsubscribable {
  ...
  subscription.add(
    commandRegistry.registerCommand({
      ...
      run: (id: string, uri: string, position: Position) =>
        concat(
          textDocumentLocations.getLocations(id, {
            textDocument: { uri },
            position
          }),
    ...
    commandRegistry.registerCommand({
      ...
      run: (...anyArguments: any[]): Promise<void> => {
        ...
        return updateSettings(
          context,
          convertUpdateConfigurationCommandArguments(args)
        );
      }
    })
  );
...
export function convertUpdateConfigurationCommandArguments(
  ...
  return {
    path: keyPath,
    value:
      args.length === 4 && args[3] === "json" ? JSON.parse(args[1]) : args[1]
  };
}
