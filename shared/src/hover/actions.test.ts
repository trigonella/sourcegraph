...
let urlToFile!: sinon.SinonStub<
  Parameters<PlatformContext["urlToFile"]>,
  string
>;
beforeEach(() => {
  urlToFile = sinon
    .stub<Parameters<PlatformContext["urlToFile"]>, string>()
              ...
              extensionsController: {
                services: {
                  ...
                  textDocumentDefinition: {
                    getLocations: () =>
                      cold<MaybeLoadingResult<Location[]>>(
...
describe("getDefinitionURL", () => {
  ...
  describe("if there is exactly 1 location result", () => {
    ...
    it("fails gracefully when resolveRawRepoName() fails with a PrivateRepoPublicSourcegraph error", async () => {
      const requestGraphQL = (): Observable<never> =>
        throwError(
          new PrivateRepoPublicSourcegraphComError("ResolveRawRepoName")
        );
      const urlToFile = sinon.spy();
      await getDefinitionURL(
        { urlToFile, requestGraphQL },
    ...
    describe("when the result is inside the current root", () => {
      it("emits the definition URL the user input revision (not commit SHA) of the root", () =>
        expect(
          ...
          isLoading: false,
          result: { url: "/r3@v3/-/blob/f", multiple: false }
        }));
    });
    ...
    describe("when the result is not inside the current root (different repo and/or commit)", () => {
      it("emits the definition URL with range", () =>
        expect(
          ...
          isLoading: false,
          result: { url: "/r2@c2/-/blob/f2#L3:3", multiple: false }
        }));
      ...
      it("emits the definition URL without range", () =>
        expect(
          ...
          isLoading: false,
          result: { url: "/r2@c2/-/blob/f2", multiple: false }
        }));
    });
  });
  ...
  it("emits the definition panel URL if there is more than 1 location result", () =>
    expect(
      ...
      isLoading: false,
      result: { url: "/r@v/-/blob/f#L2:2&tab=def", multiple: true }
    }));
});
...
describe("registerHoverContributions()", () => {
  ...
  let textDocumentDefinition!: Pick<
  ...
  const getHoverActions = (
    context: HoverActionsContext
  ): Promise<ActionItemAction[]> =>
    contribution
      .getContributions(undefined, context)
      .pipe(
        first(),
        map(contributions =>
          getContributedActionItems(contributions, ContributableMenu.Hover)
        )
      )
      .toPromise();
  ...
  describe("goToDefinition command", () => {
    test("reports no definition found", async () => {
      textDocumentDefinition.getLocations = () =>
        of({ isLoading: false, result: [] }); // mock
      await expect(
    ...
    test("navigates to an external URL using the global location object", async () => {
      ...
      textDocumentDefinition.getLocations = () =>
        of({
          ...
          result: [
            FIXTURE_LOCATION,
            { ...FIXTURE_LOCATION, uri: "git://r3?v3#f3" }
          ]
        }); // mock
      await commands.executeCommand({
        command: "goToDefinition",
