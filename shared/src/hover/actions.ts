...
export interface HoverActionsContext
  extends Context<TextDocumentPositionParams> {
...
export function getHoverActionsContext(
    ...
    | {
        extensionsController: {
          services: {
            ...
            textDocumentDefinition: Pick<
              Services["textDocumentDefinition"],
              "getLocations"
            >;
            textDocumentReferences: Pick<
  ...
  const parameters: TextDocumentPositionParams & URLToFileContext = {
    ...
    position: {
      line: hoverContext.line - 1,
      character: hoverContext.character - 1
    },
  ...
  return combineLatest([
    ...
    definitionURLOrError.pipe(
      emitLoading<UIDefinitionURL | ErrorLike, null>(LOADER_DELAY, null)
    ),
    ...
    merge(
      ...
      of(true).pipe(
        delay(LOADER_DELAY),
        takeUntil(
          definitionURLOrError.pipe(filter(({ result }) => result !== null))
        )
      ),
      definitionURLOrError.pipe(
  ...
  return textDocumentDefinition.getLocations(parameters).pipe(
    switchMap(
      ({
        isLoading,
        result: definitions
      }): Observable<Partial<MaybeLoadingResult<UIDefinitionURL | null>>> => {
  ...
  subscriptions.add(
    extensionsController.services.commands.registerCommand({
      ...
      run: async (parametersString: string) => {
        const parameters: TextDocumentPositionParams &
