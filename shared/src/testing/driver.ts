...
export const oncePageEvent = <E extends keyof PageEventObj>(
  page: Page,
  eventName: E
): Promise<PageEventObj[E]> =>
  new Promise(resolve => page.once(eventName, resolve));

export const percySnapshot = readEnvironmentBoolean({
  variable: "PERCY_ON",
  defaultValue: false
})
  ? realPercySnapshot
...
function findElementMatchingRegexps(
  tag: string,
  regexps: string[]
): HTMLElement | null {
...
export class Driver {
  ...
  public async setExtensionSourcegraphUrl(): Promise<void> {
    await this.page.goto(
      `chrome-extension://${BROWSER_EXTENSION_DEV_ID}/options.html`
    );
    await this.page.waitForSelector(".test-sourcegraph-url");
    await this.replaceText({
      selector: ".test-sourcegraph-url",
      newText: this.sourcegraphBaseUrl
    });
  ...
  public async selectAll(
    method: SelectTextMethod = "selectall"
  ): Promise<void> {
    switch (method) {
      case "selectall": {
        await this.page.evaluate(() =>
  ...
  public async ensureHasExternalService({
    ...
    if (externalServices.totalCount !== 0) {
      await this.page.goto(
        this.sourcegraphBaseUrl + "/site-admin/external-services"
      );
      await this.page.waitFor(".test-filtered-connection");
      await this.page.waitForSelector(".test-filtered-connection__loader", {
        hidden: true
      });
    ...
    await this.page.goto(
      this.sourcegraphBaseUrl + "/site-admin/external-services/new"
    );
    ...
    await this.page.evaluate((selector: string) => {
      ...
      if (!element) {
        throw new Error(
          `Could not find element to click on for selector ${selector}`
        );
      }
      element.click();
    }, `[data-test-external-service-card-link="${kind.toUpperCase()}"]`);
    ...
    await this.page.waitForSelector(
      ".test-external-service-editor .monaco-editor"
    );
      ...
      for (const slug of ensureRepos) {
        await this.page.goto(
          this.sourcegraphBaseUrl +
            `/site-admin/repositories?filter=cloned&query=${encodeURIComponent(
              slug
            )}`
        );
        await this.page.waitForSelector(
    ...
    if (alwaysCloning) {
      for (const slug of alwaysCloning) {
        await this.page.goto(
          this.sourcegraphBaseUrl +
            `/site-admin/repositories?filter=cloning&query=${encodeURIComponent(
              slug
            )}`
        );
        await this.page.waitForSelector(
  ...
  public async assertWindowLocation(
    location: string,
    isAbsolute = false
  ): Promise<any> {
  ...
  public async assertWindowLocationPrefix(
    locationPrefix: string,
    isAbsolute = false
  ): Promise<any> {
    const prefix = isAbsolute
      ? locationPrefix
      : this.sourcegraphBaseUrl + locationPrefix;
    await retry(async () => {
      const location: string = await this.page.evaluate(
  ...
  public async assertStickyHighlightedToken(label: string): Promise<void> {
    await this.page.waitForSelector(".selection-highlight-sticky", {
      visible: true
    }); // make sure matched token is highlighted
    await retry(async () =>
      expect(
        await this.page.evaluate(
          () =>
  ...
  public async setConfig(
    ...
    const updateConfigResponse = await this.makeGraphQLRequest<IMutation>({
      request: gql`
        mutation UpdateSiteConfigurationForTests(
          $lastID: Int!
          $input: String!
        ) {
  ...
  public async setUserSettings<S extends Settings>(settings: S): Promise<void> {
    ...
    const updateConfigResponse = await this.makeGraphQLRequest<IMutation>({
      request: gql`
        mutation OverwriteSettingsForTests(
          $subject: ID!
          $lastID: Int
          $contents: String!
        ) {
  ...
  public async findElementWithText(
    ...
    const notFoundError = (underlying?: Error): Error => {
      const debuggingExpressions = regexps.map(regexp =>
        getDebugExpressionFromRegexp(tag, regexp)
      );
      return new Error(
        `Could not find element with text ${JSON.stringify(
          text
        )}, options: ${JSON.stringify(options)}` +
          (underlying
            ? `. Underlying error was: ${JSON.stringify(underlying.message)}.`
            : "") +
...
async function getFirefoxCfgPath(): Promise<string> {
  ...
  if (process.platform === "darwin") {
    ...
    await mkdirpPromise(
      path.join(firefoxFolder, "browser", "defaults", "preferences")
    );
...
export async function createDriverForTest(
  ...
  if (options.browser === "firefox") {
    ...
    if (loadExtension) {
      const chromeExtensionPath = path.resolve(
        __dirname,
        "..",
        "..",
        "..",
