...
type ConfigFields = {
  [K in keyof Config]: Field<Config[K]> &
    (Config[K] extends string ? Partial<FieldParser> : FieldParser<Config[K]>);
};

const parseBool = (string: string): boolean => {
  if (
    ["1", "t", "true"].some(trueString => trueString === string.toLowerCase())
  ) {
    return true;
  }
  if (
    ["0", "f", "false"].some(
      falseString => falseString === string.toLowerCase()
    )
  ) {
    return false;
  }
...
const configFields: ConfigFields = {
  ...
  gitHubUserAmyPassword: {
    ...
    description:
      "Password of the GitHub user sg-test-regression-test-amy, used to log in to Sourcegraph."
  },
  gitHubUserBobPassword: {
    envVar: "GITHUB_USER_BOB_PASSWORD",
    description:
      "Password of the GitHub user sg-test-regression-test-bob, used to log in to Sourcegraph."
  },
  gitHubUserBobToken: {
  ...
  gitLabUserAmyPassword: {
    ...
    description:
      "Password of the GitLab user sg-test-regression-test-amy, used to log in to Sourcegraph."
  },
  oktaMetadataUrl: {
  ...
  oktaUserAmyPassword: {
    ...
    description:
      "Password of the Okta user, beyang+sg-test-regression-test-amy@sourcegraph.com"
  },
  sourcegraphBaseUrl: {
...
export function getConfig<T extends keyof Config>(
  ...
  for (const fieldName of Object.keys(configFields) as (keyof Config)[]) {
    ...
    if (environmentValue) {
      config[fieldName] = field.parser
        ? field.parser(environmentValue)
        : environmentValue;
    }
  }
