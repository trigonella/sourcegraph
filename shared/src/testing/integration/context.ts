...
const record = readEnvironmentBoolean({
  variable: "RECORD",
  defaultValue: false
});
...
export interface IntegrationTestContext<
  TGraphQlOperations extends Record<
...
>({
  ...
  currentTest,
  directory
}: IntegrationTestOptions): Promise<IntegrationTestContext<
  TGraphQlOperations,
  TGraphQlOperationNames
>> => {
  ...
  server
    .get(new URL("/favicon.ico", driver.sourcegraphBaseUrl).href)
  ...
  const graphQlRequests = new Subject<
    GraphQLRequestEvent<TGraphQlOperationNames>
  >();
  server
    .post(new URL("/.api/graphql", driver.sourcegraphBaseUrl).href)
    .intercept((request, response) => {
      const operationName = new URL(request.absoluteUrl).search.slice(
        1
      ) as TGraphQlOperationNames;
  ...
  server
    ...
    .on(
      ...
      (
        request,
        recording: { response: { headers: { name: string; value: string }[] } }
      ) => {
    ...
    waitForGraphQLRequest: async <O extends TGraphQlOperationNames>(
      ...
      const requestPromise = graphQlRequests
        .pipe(
          first(
            (
              request: GraphQLRequestEvent<TGraphQlOperationNames>
          ...
          timeoutWith(
            ...
            throwError(
              new Error(
                `Timeout waiting for GraphQL request "${operationName}"`
              )
            )
          )
        )
