...
interface State {
   ...
   */
  locationsOrError: MaybeLoadingResult<
    { locations: Location[]; isTruncated: boolean } | ErrorLike
  >;
 ...
 */
export class HierarchicalLocationsView extends React.PureComponent<
  HierarchicalLocationsViewProps,
  State
> {
  public state: State = {
    locationsOrError: {
        ...
        .pipe(
          switchMap(locationProviderResults =>
            locationProviderResults.pipe(
              ...
              tap(({ result }) => {
                const hasResults =
                  !isErrorLike(result) && result.locations.length > 0;
  ...
  public render(): JSX.Element | null {
    ...
    if (
      ...
      this.state.locationsOrError.result.locations.length === 0
    ) {
      return (
        <LoadingSpinner className="icon-inline m-1 test-loading-spinner" />
      );
    }
      ...
      <div className="hierarchical-locations-wrapper">
        {this.state.locationsOrError.result.isTruncated && (
          <div className="alert alert-warning py-1 px-3 m-2 text-nowrap text-center">
            <small>
              <strong>Large result set</strong> - only showing the first{" "}
              {MAXIMUM_LOCATION_RESULTS} results.
            </small>
          </div>
        )}
        <div
          className={`hierarchical-locations-view ${this.props.className ||
            ""}`}
        >
          {selectedGroups &&
            groupsToDisplay.map(
                        ...
                        {groups[index].map((group, innerIndex) => (
                          <span
                            ...
                            className={`list-group-item hierarchical-locations-view__item ${
                              selectedGroups[index] === group.key
                                ? "active"
                                : ""
                            }`}
                            onClick={event =>
  ...
  private onSelectTree = (
    ...
    this.setState({
      selectedGroups: selectedGroups.slice(0, index).concat(group)
    });
