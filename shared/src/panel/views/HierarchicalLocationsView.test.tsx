...
} from "./HierarchicalLocationsView";
...
describe("<HierarchicalLocationsView />", () => {
  const getProps = () => {
    ...
    const location: H.Location = {
      ...
      pathname:
        "/github.com/sourcegraph/sourcegraph/-/blob/browser/src/libs/phabricator/index.tsx",
      search: "",
      state: {}
    };
  ...
  test("shows a spinner before any locations emissions", () => {
    ...
    expect(
      renderer.create(<HierarchicalLocationsView {...props} />).toJSON()
    ).toMatchSnapshot();
  });
  ...
  test("registers a 'Group by file' contribution", () => {
    ...
    expect(
      services.contribution.registerContributions.getCall(0).args[0]
  ...
  test("displays a single location when complete", () => {
    const locations = of<MaybeLoadingResult<Location[]>>({
      isLoading: false,
  ...
  test("displays partial locations before complete", () => {
    const props = {
      ...getProps().props,
      locations: concat(
        of({ isLoading: false, result: [SAMPLE_LOCATION] }),
        NEVER
      )
    };
    expect(
