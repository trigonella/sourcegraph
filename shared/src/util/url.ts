 ...
 */
export interface AbsoluteRepoFile
  extends RepoSpec,
    RevisionSpec,
    ResolvedRevisionSpec,
    FileSpec {}
 ...
 */
export function toPositionHashComponent(position: {
  line: number;
  character?: number;
}): string {
  return (
...
export function lprToSelectionsZeroIndexed(
  lpr: LineOrPositionOrRange
): Selection[] {
  ...
  const start: Position = {
    ...
    character: characterZeroIndexed(range.start.character)
  };
  const end: Position = {
    line: range.end.line - 1,
    character: characterZeroIndexed(range.end.character)
  };
  return [
    {
      start,
 ...
 */
export function parseHash<V extends string>(
  hash: string
  ...
  if (!isLegacyFragment(hash)) {
    ...
    const lpr = (findLineInSearchParameters(searchParameters) ||
      {}) as LineOrPositionOrRange & {
      viewState?: V;
    };
  ...
  const lpr = parseLineOrPositionOrRange(
    lineCharModalInfo[0]
  ) as LineOrPositionOrRange & { viewState?: V };
...
function parseLineOrPositionOrRange(lineChar: string): LineOrPositionOrRange {
  ...
  if (
    ...
    (typeof endLine !== "undefined" && typeof character !== typeof endCharacter)
  ) {
    lpr = {};
 ...
 */
function findLineInSearchParameters(
...
export function encodeRepoRevision({
  repoName,
  revision
}: RepoSpec & Partial<RevisionSpec>): string {
...
export function toPrettyBlobURL(
  ...
  })}/-/blob/${target.filePath}${toRenderModeQuery(
    target
  )}${toPositionOrRangeHash(target)}${toViewStateHashComponent(
    target.viewState
  )}`;
}
...
export function toViewStateHashComponent(
...
export function makeRepoURI(parsed: ParsedRepoURI): RepoURI {
  ...
  uri += parsed.range
    ? positionString(parsed.range.start) +
      "-" +
...
export function toURIWithPath(
  context: RepoSpec & ResolvedRevisionSpec & FileSpec
): string {
...
export function withWorkspaceRootInputRevision(
  ...
  const inWorkspaceRoot = workspaceRoots.find(root => {
    ...
    return (
      rootURI.repoName === uri.repoName && rootURI.revision === uri.revision
    );
  });
  if (inWorkspaceRoot?.inputRevision !== undefined) {
    return {
      ...uri,
      commitID: undefined,
      revision: inWorkspaceRoot.inputRevision
    };
  }
 ...
 */
export function generateFiltersQuery(
  filtersInQuery: FiltersToTypeAndValue
): string {
...
export function parsePatternTypeFromQuery(
  query: string
  ...
  if (parsedQuery.type === "success") {
    for (const member of parsedQuery.token.members) {
      ...
      if (
        ...
        return {
          range: {
            start: token.filterType.range.start,
            end: token.filterValue.range.end
          },
          value: query.slice(
            token.filterValue.range.start,
            token.filterValue.range.end
          )
        };
      }
    }
...
export function parseCaseSensitivityFromQuery(
  query: string
): { range: CharacterRange; value: string } | undefined {
  const parsedQuery = parseSearchQuery(query);
  if (parsedQuery.type === "success") {
    for (const member of parsedQuery.token.members) {
      ...
      if (
        token.type === "filter" &&
        token.filterType.token.value.toLowerCase() === "case" &&
        token.filterValue
      ) {
        return {
          range: {
