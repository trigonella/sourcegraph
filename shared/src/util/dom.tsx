...
function highlightNodeHelper(
          ...
          if (nodeText.slice(0, Math.max(0, start - currentOffset))) {
            ...
            containerNode.append(
              document.createTextNode(
                nodeText.slice(0, Math.max(0, start - currentOffset))
              )
            );
          }
          ...
          if (rest.length >= length) {
            ...
            if (currentNode.classList.contains("selection-highlight")) {
              ...
              if (rest.length > length) {
                ...
                containerNode.append(
                  document.createTextNode(rest.slice(length))
                );
              }
  ...
  return {
    highlightingCompleted: false,
    charsConsumed: currentOffset - origOffset,
    charsHighlighted
  };
}
