...
describe("renderMarkdown", () => {
  ...
  it("sanitizes event handlers", () => {
    expect(renderMarkdown('<svg><rect onclick="evil()"></rect></svg>')).toBe(
      "<p><svg><rect></rect></svg></p>\n"
    );
  });
  it("sanitizes non-SVG <object> tags", () => {
    expect(renderMarkdown('<object data="something"></object>')).toBe(
      "<p></p>\n"
    );
  });
  it("allows SVG <object> tags", () => {
    expect(
      renderMarkdown('<object data="something" type="image/svg+xml"></object>')
    ).toBe('<p><object data="something" type="image/svg+xml"></object></p>\n');
  });
  it("allows <svg> tags", () => {
    const input =
  ...
  describe("allowDataUriLinksAndDownloads option", () => {
    const MARKDOWN_WITH_DOWNLOAD =
      '<a href="data:text/plain,foobar" download>D</a>\n[D2](data:text/plain,foobar)';
    test("default disabled", () => {
      expect(renderMarkdown(MARKDOWN_WITH_DOWNLOAD)).toBe(
        "<p><a>D</a>\n<a>D2</a></p>\n"
      );
    });
    test("enabled", () => {
      expect(
        renderMarkdown(MARKDOWN_WITH_DOWNLOAD, {
