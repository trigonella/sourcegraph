...
describe("content (integration)", () => {
  test("registers a link preview provider", async () => {
    ...
    expect(
      ...
      content: [
        { value: "xyz http://example.com/foo", kind: MarkupKind.PlainText }
      ],
      hover: []
    });
  });
  ...
  test("unregisters a link preview provider", async () => {
    ...
    // Register the provider and call it.
    const subscription = extensionAPI.content.registerLinkPreviewProvider(
      "http://example.com",
      {
        provideLinkPreview: () => ({
          content: { value: "bar", kind: MarkupKind.PlainText }
        })
      }
    );
    await extensionAPI.internal.sync();
  ...
  test("supports multiple link preview providers", async () => {
    ...
    extensionAPI.content.registerLinkPreviewProvider("http://example.com", {
      provideLinkPreview: () => ({
        content: { value: "qux", kind: MarkupKind.PlainText }
      })
    });
    extensionAPI.content.registerLinkPreviewProvider("http://example.com", {
      provideLinkPreview: () => ({
        content: { value: "zip", kind: MarkupKind.PlainText }
      })
    });
    await extensionAPI.internal.sync();
    expect(
