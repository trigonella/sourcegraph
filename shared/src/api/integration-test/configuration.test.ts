...
describe("Configuration (integration)", () => {
  test("is synchronously available", async () => {
    const { extensionAPI } = await integrationTestContext({
      settings: of(EMPTY_SETTINGS_CASCADE)
    });
    expect(() =>
      extensionAPI.configuration.subscribe(() => undefined)
    ).not.toThrow();
  ...
  describe("Configuration#update", () => {
    test("updates configuration", async () => {
      ...
      const { extensionAPI } = await integrationTestContext({
        settings: of({
          final: { a: 1 },
          subjects: [{ subject: {} as any, lastID: null, settings: null }]
        }),
  ...
  describe("configuration.subscribe", () => {
    test("subscribes to changes", async () => {
      const mockSettings = new BehaviorSubject<SettingsCascadeOrError>(
        EMPTY_SETTINGS_CASCADE
      );
      const { extensionAPI } = await integrationTestContext({
