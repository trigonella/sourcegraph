  ...
  describe("selection", () => {
    test("observe changes", async () => {
      ...
      const values = await from(
        extensionAPI.app.windows[0].activeViewComponentChanges
      )
        .pipe(
          switchMap(viewer =>
            viewer && viewer.type === "CodeEditor"
        ...
        .toPromise();
      assertToJSON(
        values.map(selections =>
          selections.map(selection => Selection.fromPlain(selection).toPlain())
        ),
...
async function getFirstCodeEditor(
