  ...
  describe("Window", () => {
        ...
        const viewers = await from(extensionAPI.app.activeWindowChanges)
          .pipe(
            switchMap(activeWindow =>
              activeWindow ? activeWindow.activeViewComponentChanges : of(null)
            ),
            take(4),
            toArray()
          )
    ...
    test("Window#showNotification", async () => {
      ...
      const values = collectSubscribableValues(
        services.notifications.showMessages
      );
      extensionAPI.app.activeWindow!.showNotification(
        "a",
        NotificationType.Info
      );
      await extensionAPI.internal.sync();
      expect(values).toEqual([
        { message: "a", type: NotificationType.Info }
      ] as typeof values);
    });

    test("Window#showMessage", async () => {
      const { extensionAPI, services } = await integrationTestContext();
      services.notifications.showMessageRequests.subscribe(({ resolve }) =>
      ...
      expect(await extensionAPI.app.activeWindow!.showMessage("a")).toBe(
        undefined
      );
      expect(values).toEqual([
        { message: "a", type: NotificationType.Info }
    ...
    test("Window#showInputBox", async () => {
      const { extensionAPI, services } = await integrationTestContext();
      services.notifications.showInputs.subscribe(({ resolve }) =>
        resolve(Promise.resolve("c"))
      );
      const values = collectSubscribableValues(
        services.notifications.showInputs.pipe(
          map(({ message, defaultValue }) => ({ message, defaultValue }))
        )
      );
      expect(
        await extensionAPI.app.activeWindow!.showInputBox({
          prompt: "a",
          value: "b"
        })
      ).toBe("c");
