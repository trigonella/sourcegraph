...
describe("Commands (integration)", () => {
  describe("commands.registerCommand", () => {
    test("registers and unregisters a single command", async () => {
      ...
      await expect(extensionAPI.commands.executeCommand("c")).resolves.toBe(
        "a"
      );
      await expect(
        services.commands.executeCommand({ command: "c" })
      ).resolves.toBe("a");
      ...
      await expect(
        extensionAPI.commands.executeCommand("c")
    ...
    test("supports multiple commands", async () => {
      ...
      await expect(extensionAPI.commands.executeCommand("c1")).resolves.toBe(
        "a1"
      );
      await expect(
        services.commands.executeCommand({ command: "c1" })
      ).resolves.toBe("a1");
      await expect(extensionAPI.commands.executeCommand("c2")).resolves.toBe(
        "a2"
      );
      await expect(
        services.commands.executeCommand({ command: "c2" })
      ).resolves.toBe("a2");
    });
  });
});
