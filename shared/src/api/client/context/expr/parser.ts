...
export class Parser {
  ...
  public parse(expressionString: string): ExpressionNode {
    ...
    if (token !== undefined) {
      throw new SyntaxError(
        `Unexpected token at end of input: ${JSON.stringify(token.value)} (at ${
          this.lexer.index
        })`
      );
    }
  ...
  private parseArgumentList(): ExpressionNode[] {
    ...
    while (true) {
      ...
      if (expression === undefined) {
        throw new Error(
          `Parse error on token in arguments list: ${JSON.stringify(
            this.lexer.peek()
          )} (at ${this.lexer.index})`
        );
      }
  ...
  private parseFunctionCall(name: string): ExpressionNode {
    ...
    if (!matchOp(token, "(")) {
      throw new SyntaxError(
        `Expected "(" in function call ${JSON.stringify(name)} (at ${
          this.lexer.index
        })`
      );
    }
    ...
    const args: ExpressionNode[] = matchOp(token, ")")
      ? []
  ...
  private parseTemplateParts(): ExpressionNode[] {
    ...
    while (true) {
      const token:
        | Pick<Token, "type" | "value">
        | undefined = this.lexer.peek();
      if (!token) {
        break;
      }
  ...
  protected parseTemplate(): ExpressionNode {
    ...
    throw new SyntaxError(
      `Unexpected token at beginning of template: ${JSON.stringify(
        token.value
      )} (at ${this.lexer.index})`
    );
  }
  ...
  private parsePrimary(): ExpressionNode {
    ...
    throw new SyntaxError(
      `Parse error on token: ${JSON.stringify(token.value)} (at ${
        this.lexer.index
      })`
    );
  }
...
export class TemplateParser extends Parser {
  public parse(templateString: string): ExpressionNode {
    ...
    if (token !== undefined) {
      throw new SyntaxError(
        `Unexpected token at end of template input: ${JSON.stringify(
          token.value
        )} (at ${this.lexer.index})`
      );
    }
...
function matchOp(
  token: Pick<Token, "type" | "value"> | undefined,
  operator: Operator
): boolean {
  return (
    token !== undefined &&
