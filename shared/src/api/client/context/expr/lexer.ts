...
export class Lexer {
  ...
  public next(): Token | undefined {
    ...
    throw new SyntaxError(
      `Unexpected character ${JSON.stringify(this.peekNextChar())} (at ${
        this.index
      })`
    );
  }
  ...
  public peek():
    | Pick<Token, Exclude<keyof Token, "start" | "end">>
    | undefined {
  ...
  private scanNumber(): Token | undefined {
    ...
    if (character === "e" || character === "E") {
      ...
      if (character === "+" || character === "-" || isDecimalDigit(character)) {
        ...
        throw new SyntaxError(
          `Unexpected ${character} after the exponent sign (at ${this.index})`
        );
      }
    }
