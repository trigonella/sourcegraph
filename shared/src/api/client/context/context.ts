...
export interface Context<T = never>
  extends Record<
    ...
    | Context
    | T
    | (string | number | boolean | null | Context | T)[]
  > {}
...
export function getComputedContextProperty(
  ...
  if (key.startsWith("config.")) {
    ...
    const value = isSettingsValid(settings)
      ? settings.final[property]
      : undefined;
  ...
  if (
    key === "resource" ||
    key === "component" /* BACKCOMPAT: allow 'component' */
  ) {
    return !!component;
  }
