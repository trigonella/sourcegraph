...
describe("CommandRegistry", () => {
  ...
  test("runs the specified command", async () => {
    ...
    expect(
      await registry.executeCommand({ command: "command1", arguments: [123] })
    ).toBe(456);
  });
});
...
describe("executeCommand", () => {
  test("runs the specified command with args", async () => {
    ...
    expect(
      await executeCommand(commands, { command: "command1", arguments: [123] })
    ).toBe(456);
  });
  ...
  test("runs the specified command with no args", async () => {
    // eslint-disable-next-line @typescript-eslint/require-await
    const commands: CommandEntry[] = [
      { command: "command1", run: async argument => undefined }
    ];
    expect(await executeCommand(commands, { command: "command1" })).toBe(
      undefined
    );
  });
