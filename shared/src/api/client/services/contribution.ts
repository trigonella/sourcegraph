...
export class ContributionRegistry {
   ...
   */
  public registerContributions(
    entryToRegister: ContributionsEntry
  ...
  public replaceContributions(
    ...
    this._entries.next([
      ...this._entries.value.filter(entry => entry !== previous.entry),
      next
    ]);
    return {
      unsubscribe: () => {
        ...
        return multiContributions.flat().map(contributions => {
          try {
            return filterContributions(
              evaluateContributions(computedContext, contributions)
            );
          } catch (error) {
...
export function filterContributions(
  ...
  return {
    ...contributions,
    menus: mapValues(contributions.menus, menuItems =>
      menuItems?.filter(menuItem => menuItem.when !== false)
    )
  };
}
...
export function evaluateContributions(
  ...
  return {
    ...
    menus:
      contributions.menus &&
      mapValues(contributions.menus, (menuItems):
        | Evaluated<MenuItemContribution>[]
        | undefined =>
 ...
 * Evaluates expressions in contribution definitions against the given context.
 */
export function parseContributionExpressions(
  ...
  return {
    ...
    actions:
      contributions && parseActionContributionExpressions(contributions.actions)
  };
}
...
const maybe = <T, R>(
  value: T | undefined,
  function_: (value: T) => R
): R | undefined => (value === undefined ? undefined : function_(value));

/**
 * Evaluates expressions in contribution definitions against the given context.
 */
function parseActionContributionExpressions(
  actions: Raw<Contributions["actions"]>
