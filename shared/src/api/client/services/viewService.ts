...
export interface ViewContexts {
  ...
  [ContributableViewContainer.Directory]: DeepReplace<
...
export interface ViewService {
   ...
   */
  getWhere<W extends ContributableViewContainer>(
    where: W
   ...
   */
  get<W extends ContributableViewContainer>(
    id: string,
...
export const createViewService = (): ViewService => {
  const providers = new BehaviorSubject<
    Map<string, ViewProviderEntry<ContributableViewContainer>>
  >(new Map());
  ...
  return {
    ...
    getWhere: where =>
      providers.pipe(
        map(providers =>
          [...providers.values()].filter(isTaggedUnionMember("where", where))
        ),
    ...
    get: (id, context) =>
      providers.pipe(
        ...
        distinctUntilChanged(),
        switchMap(provider =>
          provider ? provider(context).pipe(finallyReleaseProxy()) : of(null)
        )
      )
  };
};
...
export const getViewsForContainer = <W extends ContributableViewContainer>(
  ...
  viewService.getWhere(where).pipe(
    ...
    map(views =>
      views.filter(allOf(isDefined, property("view", isNot(isExactly(null)))))
    )
  );
