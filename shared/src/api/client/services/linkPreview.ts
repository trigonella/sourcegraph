...
interface MarkupContentPlainTextOnly
  extends Pick<sourcegraph.MarkupContent, "value"> {
...
export type ProvideLinkPreviewSignature = (
  url: string
) => Observable<sourcegraph.LinkPreview | null | undefined>;
...
> {
   ...
   */
  protected observeProvidersForLink(
    url: string
    ...
    return this.entries.pipe(
      map(entries =>
        entries
          .filter(entry =>
            url.startsWith(entry.registrationOptions.urlMatchPattern)
          )
...
function mergeLinkPreviews(
  values: (sourcegraph.LinkPreview | null | undefined)[]
 ...
 * plaintext contents.
 */
export function renderMarkupContents(
  contents: sourcegraph.MarkupContent[]
): ({ html: string } | string)[] {
