...
export type ViewerWithPartialModel =
  ...
  | DirectoryViewer; // Directories don't have a model
...
export function createViewerService(
  modelService: Pick<ModelService, "removeModel">
): ViewerService {
  ...
  const activeViewerUpdates = new BehaviorSubject<Viewer | undefined>(
    undefined
  );
  /**
  ...
  return {
      ...
      try {
        ...
        return viewerUpdates.pipe(
          ...
          takeWhile(updates =>
            updates.every(
              update =>
                update.viewerId !== viewerId || update.type !== "deleted"
            )
          ),
    ...
    setSelections({ viewerId }: ViewerId, selections: Selection[]): void {
      ...
      if (viewer.type !== "CodeEditor") {
        throw new Error(
          `Editor ID ${viewerId} is type ${String(
            viewer.type
          )}, expected CodeEditor`
        );
      }
    ...
    removeAllViewers(): void {
      const updates: ViewerUpdate[] = [...viewers.keys()].map(viewerId => ({
        type: "deleted",
        viewerId
      }));
      viewers.clear();
...
export function observeEditorAndModel(
  ...
  return observeViewer({ viewerId }).pipe(
    ...
    switchMap(viewer =>
      observeModel(viewer.resource).pipe(
        map(model => ({ viewerId, ...viewer, model }))
      )
    )
  );
}
