...
export function createTestViewerService({
  ...
  const viewerService = createViewerService(
    modelService || FIXTURE_MODEL_SERVICE
  );
  if (viewers) {
...
describe("viewerService", () => {
  test("addViewer", async () => {
    ...
    const editorAdded = from(viewerService.viewerUpdates)
      .pipe(first())
      .toPromise();
  ...
  describe("observeViewer", () => {
    test("emits error if editor does not exist", () => {
      scheduler().run(({ expectObservable }) => {
        const viewerService = createViewerService(FIXTURE_MODEL_SERVICE);
        expectObservable(
          from(viewerService.observeViewer({ viewerId: "x" }))
        ).toBe("#", {}, new Error("Viewer not found: x"));
      });
    });
    ...
    test("emits on selections changes", async () => {
      ...
      const changes = (viewerService.observeViewer(viewerId) as Observable<
        CodeEditorData
      >)
        .pipe(
  ...
  describe("setSelections", () => {
    test("ok", async () => {
      ...
      const selectionsSet = from(viewerService.viewerUpdates)
        .pipe(first())
        .toPromise();
      viewerService.setSelections({ viewerId }, SELECTIONS);
    ...
    test("not found", () => {
      const viewerService = createViewerService(FIXTURE_MODEL_SERVICE);
      expect(() =>
        viewerService.setSelections({ viewerId: "x" }, [])
      ).toThrowError("Viewer not found: x");
    });
  });
  ...
  describe("removeViewer", () => {
    ...
    test("not found", () => {
      const viewerService = createViewerService(FIXTURE_MODEL_SERVICE);
      expect(() => viewerService.removeViewer({ viewerId: "x" })).toThrowError(
        "Viewer not found: x"
      );
    });
  ...
  test("removeAllViewers", async () => {
    ...
    viewerService.addViewer(editor);
    viewerService.addViewer(editor);
    const editorsRemoved = from(viewerService.viewerUpdates)
      .pipe(first())
      .toPromise();
