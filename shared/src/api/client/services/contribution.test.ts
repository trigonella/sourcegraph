  ...
  describe("contributions observable", () => {
    test("emits stream of results of registrations", () => {
      scheduler().run(({ cold, expectObservable }) => {
        ...
        expectObservable(
          registry.getContributionsFromEntries(cold("-a-b-c-|", values))
        ).toBe("-a---c-|", expected);
      });
    });
    ...
    it("supports registration of an observable", () => {
      scheduler().run(({ cold, expectObservable }) => {
        ...
        expectObservable(registry.getContributionsFromEntries(entries)).toBe(
          "---b-c-|",
          expected
        );
      });
            ...
            of<ContributionsEntry[]>([
              {
                contributions: {
                  menus: {
                    commandPalette: [{ action: "a", when: parse("x == y") }]
                  }
                }
              }
            ])
      ...
      scheduler().run(({ cold, expectObservable }) => {
        const registry = new (class extends ContributionRegistry {
          ...
          public getContributionsFromEntries(
            ...
            return super.getContributionsFromEntries(
              entries,
              scope,
              undefined,
              noop
            ...
            of<ContributionsEntry[]>([
              {
                ...
                contributions: {
                  menus: {
                    commandPalette: [
                      { action: "a1", when: parse("nonExistingVar") }
                    ]
                  }
                }
              },
...
describe("mergeContributions()", () => {
  ...
  test("handles a single item", () => {
    expect(mergeContributions([FIXTURE_CONTRIBUTIONS_1])).toEqual(
...
describe("filterContributions()", () => {
  ...
  it("handles empty array of menu contributions", () => {
    ...
    expect(filterContributions({ menus: { commandPalette: [] } })).toEqual(
      expected
    );
  });
...
describe("evaluateContributions()", () => {
  ...
  test("handles empty array of command contributions", () => {
    ...
    expect(
      evaluateContributions(EMPTY_COMPUTED_CONTEXT, { actions: [] })
    ).toEqual(expected);
  });
  ...
  test("evaluates `actionItem.pressed` if present", () => {
    ...
    expect(evaluateContributions(FIXTURE_CONTEXT, input)).toEqual({
      actions: [
        { id: "a", command: "c", title: "a", actionItem: { pressed: true } }
      ]
    });
  });
});
