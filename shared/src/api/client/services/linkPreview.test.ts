...
describe("LinkPreviewProviderRegistry", () => {
  ...
  class TestLinkPreviewProviderRegistry extends LinkPreviewProviderRegistry {
    ...
    public observeProvidersForLink(
      url: string
  ...
  describe("observeProvidersForLink", () => {
    const PROVIDER: ProvideLinkPreviewSignature = () =>
      of<sourcegraph.LinkPreview>({
        hover: { value: "x", kind: MarkupKind.PlainText }
      });
        ...
        const registry = new TestLinkPreviewProviderRegistry(
          ...
          >("a", {
            a: [
              {
                provider: PROVIDER,
                registrationOptions: { urlMatchPattern: "http://example.com" }
              }
            ]
          })
        );
        expectObservable(
          registry.observeProvidersForLink("http://example.com")
        ).toBe("a", {
          a: [PROVIDER]
        });
      });
    ...
    test("no prefix match", () => {
      scheduler().run(({ cold, expectObservable }) => {
        ...
        expectObservable(
          registry.observeProvidersForLink("http://other.example.com/foo")
        ).toBe("a", {
          a: []
        });
      });
