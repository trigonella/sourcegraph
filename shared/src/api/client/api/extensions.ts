...
export class ClientExtensions {
   ...
   */
  constructor(
    private proxy: Remote<ExtExtensionsAPI>,
        ...
        .subscribe(([oldExtensions, newExtensions]) => {
          ...
          if (oldExtensions) {
            for (const extension of oldExtensions) {
              const newIndex = toActivate.findIndex(
                ({ id }) => extension.id === id
              );
              if (newIndex === -1) {
          ...
          for (const extension of toDeactivate) {
            this.proxy.$deactivateExtension(extension.id).catch(error => {
              console.warn(
                `Error deactivating extension ${JSON.stringify(extension.id)}:`,
                error
              );
            });
          }
          ...
          for (const extension of toActivate) {
            ...
            this.proxy
              ...
              .catch(error => {
                console.error(
                  `Error activating extension ${JSON.stringify(extension.id)}:`,
                  error
                );
              });
          }
