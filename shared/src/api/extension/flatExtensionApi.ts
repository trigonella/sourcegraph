...
export interface ExtState {
  ...
  // Lang
  hoverProviders: BehaviorSubject<
...
export const initNewExtensionAPI = (
  ...
  const commands: typeof sourcegraph["commands"] = {
    ...
    registerCommand: (command, callback) =>
      syncSubscription(mainAPI.registerCommand(command, proxy(callback)))
  };

  // Search
  const search: typeof sourcegraph["search"] = {
    registerQueryTransformer: transformer =>
      addWithRollback(state.queryTransformers, transformer)
  };
...
function addWithRollback<T>(
  ...
  return {
    unsubscribe: () =>
      behaviorSubject.next(behaviorSubject.value.filter(item => item !== value))
  };
}
...
export function callProviders<TProvider, TProviderResult, TMergedResult>(
  ...
  mergeResult: (
    providerResults: (TProviderResult | "loading" | null | undefined)[]
  ) => TMergedResult,
  ...
  return providersObservable
    .pipe(
      map(providers =>
        providersForDocument(document, providers, ({ selector }) => selector)
      ),
      switchMap(providers =>
          ...
          providers.map(provider =>
            concat(
              ...
              providerResultToObservable(
                ...
                defaultIfEmpty<
                  typeof LOADING | TProviderResult | null | undefined
                >(null),
                catchError(error => {
                  if (logErrors) {
