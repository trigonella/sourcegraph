...
export class ExtensionContent {
  ...
  public registerLinkPreviewProvider(
    ...
    >[1]> = comlink.proxy((url: string) =>
      toProxyableSubscribable(
        provider.provideLinkPreview(new URL(url)),
        preview => preview
      )
    );
    return syncSubscription(
      this.proxy.$registerLinkPreviewProvider(urlMatchPattern, providerFunction)
    );
  }
}
