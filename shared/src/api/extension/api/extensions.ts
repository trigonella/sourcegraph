...
export class Extensions
  implements ExtExtensionsAPI, Unsubscribable, ProxyMarked {
  ...
  public async $activateExtension(
    ...
    const extensionDeactivate =
      typeof extensionExports.deactivate === "function"
    ...
    await tryCatchPromise<void>(() =>
      extensionExports.activate({ subscriptions: extensionSubscriptions })
    ).catch(error => {
      error = asError(error);
      throw Object.assign(
        new Error(
          `error during extension ${JSON.stringify(
            extensionID
          )} activate function: ${String(error.stack || error)}`
        ),
        { error }
      );
    });
  ...
  public unsubscribe(): void {
    for (const [
      extensionID,
      deactivate
    ] of this.extensionDeactivate.entries()) {
      this.extensionDeactivate.delete(extensionID);
      tryCatchPromise(deactivate).catch(error => {
        console.warn(
          `Error deactivating extension ${JSON.stringify(extensionID)}:`,
          error
        );
      });
    }
