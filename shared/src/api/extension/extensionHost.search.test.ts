...
): Remote<Observer<string> & ProxyMarked> =>
  pretendRemote({
    next: onValue,
    error: (error: any) => {
      throw error;
...
describe("QueryTransformers", () => {
  it("returns the same query with no registered transformers", () => {
    const { exposedToMain } = initNewExtensionAPI(
      noopMain,
      emptySettings,
      noopDocuments
    );
  ...
  it("emits new value if a transformer was removed", () => {
    ...
    const transformerSubscription = search.registerQueryTransformer({
