...
function createExtensionAPI(
  ...
  const languageFeatures = new ExtensionLanguageFeatures(
    proxy.languageFeatures,
    documents
  );
  ...
  const extensionAPI: typeof sourcegraph & {
    ...
    app: {
      ...
      registerViewProvider: (id, provider) =>
        views.registerViewProvider(id, provider)
    },
    ...
    content: {
      registerLinkPreviewProvider: (
        ...
        provider: sourcegraph.LinkPreviewProvider
      ) => content.registerLinkPreviewProvider(urlMatchPattern, provider)
    },

    internal: {
      sync,
      updateContext: (updates: sourcegraph.ContextValues) =>
