...
export interface Controller extends Unsubscribable {
  ...
  executeCommand(
    ...
    suppressNotificationOnError?: boolean
  ): Promise<any>;
...
export interface ExtensionsControllerProps<
  K extends keyof Controller = keyof Controller
> {
  /**
...
export function createController(context: PlatformContext): Controller {
  ...
  subscriptions.add(() =>
    extensionHostClientPromise.then(({ subscription }) =>
  ...
  subscriptions.add(
    registerExtensionContributions(services.contribution, services.extensions)
  );

  // Show messages (that don't need user input) as global notifications.
  subscriptions.add(
    services.notifications.showMessages.subscribe(({ message, type }) =>
  ...
  if (DEBUG) {
    ...
    if (LOG_EDITORS) {
      subscriptions.add(
        services.viewer.viewerUpdates.subscribe(() =>
