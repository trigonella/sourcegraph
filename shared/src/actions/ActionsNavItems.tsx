 ...
 */
export class ActionsNavItems extends React.PureComponent<
...
> {
      ...
      combineLatest([this.scopeChanges, this.extraContextChanges])
        .pipe(
          switchMap(([scope, extraContext]) =>
            this.props.extensionsController.services.contribution.getContributions(
              scope,
              extraContext
            )
          )
  ...
  public render(): JSX.Element | React.ReactFragment | null {
    ...
    const actionItems = getContributedActionItems(
        ...
        <li className={this.props.listItemClass}>
          <ActionItem
            ...
            className={classNames(
              "actions-nav-items__action-item",
              this.props.actionItemClass
            )}
    ...
    if (this.props.wrapInList) {
      return actionItems.length > 0 ? (
        <ul className={this.props.listClass}>{actionItems}</ul>
      ) : null;
    }
    return <>{actionItems}</>;
  }
