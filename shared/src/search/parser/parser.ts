...
export interface Filter {
  ...
  filterValue:
    | Pick<ParseSuccess<Literal | Quoted>, "range" | "token">
    | undefined;
}
...
const zeroOrMore = (parseToken: Parser): Parser<Sequence> => (input, start) => {
  const members: Pick<
    ParseSuccess<Exclude<Token, Sequence>>,
    "range" | "token"
  >[] = [];
...
const pattern = <T extends Token = Literal>(
  ...
  return (input, start) => {
    ...
    if (!matchTarget) {
      return {
        type: "error",
        expected: expected || `/${regexp.source}/`,
        at: start
      };
    }
...
const followedBy = (
  ...
  if (input[end] !== undefined) {
    ...
    members.push({
      token: separatorResult.token,
      range: separatorResult.range
    });
...
const filter: Parser<Filter> = (input, start) => {
  ...
  const parsedValue =
    input[parsedDelimiter.range.end] === undefined
      ? undefined
...
const searchQuery = zeroOrMore(
  oneOf<Token>(
    ...
    ...[operator, filter, quoted, literal].map(token =>
      followedBy(
        token,
        oneOf<{ type: "whitespace" } | { type: "closingParen" }>(
          whitespace,
          closingParen
        )
      )
 ...
 */
export const parseSearchQuery = (query: string): ParserResult<Sequence> =>
  searchQuery(query, 0);
