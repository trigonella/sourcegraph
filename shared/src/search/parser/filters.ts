...
interface NegatableFilterDefinition
  extends Omit<BaseFilterDefinition, "description"> {
  negatable: true;
...
export const FILTERS: Record<NegatableFilter, NegatableFilterDefinition> &
  ...
  [FilterType.content]: {
    description: (negated: boolean): string =>
      `${
        negated ? "Exclude" : "Include only"
      } results from files if their content matches the search pattern.`,
    negatable: true,
    singular: true
  },
  [FilterType.count]: {
  ...
  [FilterType.file]: {
    ...
    description: negated =>
      `${
        negated ? "Exclude" : "Include only"
      } results from files matching the given search pattern.`,
    suggestions: "File"
  },
  [FilterType.fork]: {
  ...
  [FilterType.lang]: {
    negatable: true,
    description: negated =>
      `${negated ? "Exclude" : "Include only"} results from the given language`,
  ...
  [FilterType.repo]: {
    ...
    description: negated =>
      `${
        negated ? "Exclude" : "Include only"
      } results from repositories matching the given search pattern.`,
  ...
  [FilterType.repohasfile]: {
    ...
    description: negated =>
      `${
        negated ? "Exclude" : "Include only"
      } results from repos that contain a matching file`
  },
  [FilterType.timeout]: {
...
const isValidDiscreteValue = (
  ...
  const validDiscreteValuesForDefinition = Object.keys(
    discreteValueAliases
...
export const validateFilter = (
  ...
  if (
    ...
    (!filterValue ||
      ...
      (filterValue.token.type === "quoted" &&
        !isValidDiscreteValue(definition, filterValue.token.quotedValue)))
  ) {
    return {
      valid: false,
