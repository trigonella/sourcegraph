...
const repositoryToCompletion = (
  ...
  insertText =
    (options.isFilterValue ? insertText : `${FilterType.repo}:${insertText}`) +
    " ";
  return {
    label: name,
...
const fileToCompletion = (
  ...
  insertText =
    (options.isFilterValue ? insertText : `${FilterType.file}:${insertText}`) +
    " ";
  return {
    label: name,
    kind: isDirectory
 ...
 */
const symbolKindToCompletionItemKind: Record<
  SymbolKind,
...
const symbolToCompletion = ({
  ...
  insertText: name,
  filterText: name,
  detail: `${startCase(kind.toLowerCase())} - ${
    location.resource.repository.name
  }`
});

const languageToCompletion = ({
  name
}: ILanguage): PartialCompletionItem | undefined =>
  name
    ? {
        label: name,
...
const repoGroupToCompletion = ({
  name
}: IRepoGroup): PartialCompletionItem => ({
  label: name,
  kind: repositoryCompletionItemKind,
  insertText: name,
  filterText: name
...
export async function getCompletionItems(
  ...
  const tokenAtColumn = members.find(
    ({ range }) => range.start + 1 <= column && range.end + 1 >= column
  );
  if (!tokenAtColumn) {
    ...
    return {
      suggestions: [
        ...
        ...(await dynamicSuggestions.pipe(first()).toPromise())
          .map(suggestion =>
            suggestionToCompletionItem(suggestion, {
              isFilterValue: false,
              globbing
            })
          )
