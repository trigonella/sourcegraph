...
> extends React.PureComponent<Props<C>, State> {
  ...
  private getSize(): number {
    const storedSize = localStorage.getItem(
      `${Resizable.STORAGE_KEY_PREFIX}${this.props.storageKey}`
    );
    if (storedSize !== null) {
  ...
  public render(): React.ReactNode {
    return (
      <div
        ...
        style={{
          [isHorizontal(this.props.handlePosition)
            ? "width"
            : "height"]: `${this.state.size}px`
        }}
        className={`resizable resizable--${this.props.handlePosition} ${this
          .props.className || ""}`}
        ref={this.setContainerRef}
      >
        <div
          className={`resizable__ghost ${
            this.state.resizing ? "resizable__ghost--resizing" : ""
          }`}
        ...
        {this.props.element}
        <div
          className={`resizable__handle resizable__handle--${
