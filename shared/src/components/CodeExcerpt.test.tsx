...
export class MockVisibilitySensor extends React.Component<{
...
jest.mock(
  ...
  (): typeof _VisibilitySensor => ({ children, onChange }) => (
    <>
      <MockVisibilitySensor onChange={onChange}>
        {children}
      </MockVisibilitySensor>
    </>
  )
);
...
describe("CodeExcerpt", () => {
  ...
  it("does not disable the highlighting timeout", () => {
    /*
            ...
            ideal.
        */
    const fetchHighlightedFileLines = sinon.spy(context =>
    ...
    sinon.assert.calledWithMatch(fetchHighlightedFileLines, {
  ...
  it("displays the correct number of lines, with no highlight matches on the context lines", () => {
    const fetchHighlightedFileLines = sinon.spy(context =>
      of(HIGHLIGHTED_FILE_LINES_LONG)
    );
