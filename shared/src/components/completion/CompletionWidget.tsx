...
export type ManagedDownShiftState = Pick<
  DownshiftState<CompletionItem>,
...
export class CompletionWidget extends React.Component<
...
> {
  ...
  public componentDidMount(): void {
    this.subscriptions.add(
      fromEvent<KeyboardEvent>(
        this.props.textArea,
        "keydown"
      ).subscribe(event => this.handleKeyboardNavigationEvents(event))
    );
  ...
  private scrollDelta(delta: number): boolean {
    ...
    if (
      ...
      !isSuccessfulFetch(completionListOrError)
    ) {
    ...
    const wrappedIndex =
      ((newIndex % completionListOrError.items.length) +
   ...
   */
  private onDownshiftStateChange = ({
    highlightedIndex,
    selectedItem
  }: StateChangeOptions<CompletionItem>): void => {
    ...
    return (
      <div className={`completion-widget ${this.props.widgetClassName || ""}`}>
        <div
          ...
          className={`completion-widget__container ${this.props
