...
const EXTENSION_PERMISSIONS_ALL_URLS = Boolean(
  process.env.EXTENSION_PERMISSIONS_ALL_URLS &&
...
function copyExtensionAssets(toDirectory: string): void {
  ...
  shelljs.cp(
    "build/dist/js/extensionHostWorker.bundle.js",
    `${toDirectory}/js`
  );
...
export function copyIntegrationAssets(): void {
  ...
  shelljs.cp(
    "build/dist/js/phabricator.bundle.js",
    "build/integration/scripts"
  );
  shelljs.cp(
    "build/dist/js/integration.bundle.js",
    "build/integration/scripts"
  );
  shelljs.cp(
    "build/dist/js/extensionHostWorker.bundle.js",
...
function buildForBrowser(browser: Browser): (env: BuildEnv) => () => void {
  ...
  return environment => {
    ...
    return () => {
      ...
      const zipDestination = path.resolve(
        process.cwd(),
