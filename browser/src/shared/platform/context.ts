...
export function createPlatformContext(
  ...
  const updatedViewerSettings = new ReplaySubject<
    Pick<GQL.ISettingsCascade, "subjects" | "final">
  >(1);
    ...
    observeSourcegraphURL(isExtension).pipe(
      ...
      switchMap(sourcegraphURL => {
        if (
          ...
          const privateRepository = getContext
            ? getContext().privateRepository
            : true;
          if (privateRepository) {
            const nameMatch = request.match(/^\s*(?:query|mutation)\s+(\w+)/);
            throw new PrivateRepoPublicSourcegraphComError(
              nameMatch ? nameMatch[1] : ""
            );
          }
        }
        return requestGraphQlHelper(
          isExtension,
          sourcegraphURL
        )<T, V>({ request, variables });
      })
    );
