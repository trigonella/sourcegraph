...
export const resolveRepo = memoizeObservable(
  ({
    rawRepoName,
    requestGraphQL
  }: RawRepoSpec & Pick<PlatformContext, "requestGraphQL">): Observable<
    string
  > =>
...
export const resolveRevision = memoizeObservable(
  ({
    ...
    Pick<PlatformContext, "requestGraphQL">): Observable<string> =>
    from(
      requestGraphQL<GQL.IQuery>({
        request: gql`
...
export function retryWhenCloneInProgressError<T>(): (
  v: Observable<T>
) => Observable<T> {
      ...
      map(({ data, errors }) => {
        ...
        if (errors) {
          if (errors.length === 1) {
            ...
            const isFileContent =
              errorPath === "repository.commit.file.content" ||
              errorPath === "repository.commit.file";
