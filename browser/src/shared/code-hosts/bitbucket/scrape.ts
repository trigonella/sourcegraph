...
const getFileInfoFromLinkInSingleFileView = (
...
): Pick<FileInfo, "rawRepoName" | "filePath" | "revision"> &
  BitbucketRepoInfo => {
  const errors: Error[] = [];
  for (const selector of LINK_SELECTORS) {
    try {
      ...
      if (!pathMatch) {
        throw new Error(
          `Path of link matching selector ${selector} did not match path regex: ${path}`
        );
      }
...
const getCommitIDFromRevisionSelector = (): string => {
  const revisionSelectorSpan = document.querySelector<HTMLElement>(
    "span[data-revision-ref]"
  );
  if (!revisionSelectorSpan) {
    throw new Error("Could not find span[data-revision-ref] element");
  }
  try {
    const { latestCommit }: { latestCommit: string } = JSON.parse(
      revisionSelectorSpan.dataset.revisionRef!
    );
    return latestCommit;
  } catch {
...
export const isCommitsView = ({
  pathname
}: Pick<Location, "pathname">): boolean =>
...
const getChangeType = ({
  ...
  const className = [...changeTypeElement.classList].find(className =>
    /^change-type-[A-Z]+/.test(className)
  );
  if (!className) {
...
export const getFileInfoFromSingleFileDiffCodeView = (
  ...
  FileInfo & { baseFilePath: string; changeType: ChangeType } => {
  const {
    rawRepoName,
    project,
    repoSlug,
  ...
  const baseFilePath =
    getBaseFilePathForDiffCodeView({
      changeTypeElement,
      changeType,
      filePath
    }) || filePath;
