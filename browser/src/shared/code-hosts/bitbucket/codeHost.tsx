...
export const getToolbarMount = (codeView: HTMLElement): HTMLElement => {
  const existingMount = codeView.querySelector<HTMLElement>(
    ".sg-toolbar-mount"
  );
  if (existingMount) {
  ...
  const fileActions = codeView.querySelector<HTMLElement>(
...
const codeViewResolver: ViewResolver<CodeView> = {
  ...
  resolveView: element => {
    ...
    if (isSingleFileView(element)) {
      ...
      return isDiff
        ? { element, ...singleFileDiffCodeView }
        : { element, ...singleFileSourceCodeView };
    }
...
const getCommandPaletteMount: MountGetter = (
  container: HTMLElement
): HTMLElement | null => {
  const headerElement = querySelectorOrSelf(
    container,
  ...
  const preexisting = headerElement.querySelector<HTMLElement>(
    classNames.map(className => `.${className}`).join("")
  );
  return preexisting || create();
};

function getViewContextOnSourcegraphMount(
  container: HTMLElement
): HTMLElement | null {
  const branchSelectorButtons = querySelectorOrSelf(
    container,
    ".branch-selector-toolbar .aui-buttons"
  );
  if (!branchSelectorButtons) {
    return null;
  }
  const preexisting = branchSelectorButtons.querySelector<HTMLElement>(
    "#open-on-sourcegraph"
  );
  if (preexisting) {
    return preexisting;
