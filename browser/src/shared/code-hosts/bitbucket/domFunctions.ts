...
const getSingleFileLineElementFromLineNumber = (
...
export const singleFileDOMFunctions: DOMFunctions = {
  getCodeElementFromTarget: target => {
    ...
    return container
      ? container.querySelector<HTMLElement>('span[role="presentation"]')
      : null;
  },
  ...
  getCodeElementFromLineNumber: (codeView, line) =>
    getSingleFileLineElementFromLineNumber(codeView, line).querySelector<
      HTMLElement
    >('.CodeMirror-line span[role="presentation"]')
};

const getDiffLineElementFromLineNumber = (
  codeView: HTMLElement,
  line: number,
  part?: DiffPart
): HTMLElement => {
  for (const lineNumberElement of codeView.querySelectorAll(
    ...
    const lineNumber = parseInt(
      (lineNumberElement.textContent || "").trim(),
      10
    );
  ...
  throw new Error(
    `Could not locate line number element for line ${line}, part: ${String(
      part
    )}`
  );
};
...
export const diffDOMFunctions: DOMFunctions = {
  ...
  getLineNumberFromCodeElement: codeElement => {
    ...
    if (lineNumberFrom) {
      const lineNumber = parseInt(
        (lineNumberFrom.textContent || "").trim(),
        10
      );
      if (!isNaN(lineNumber)) {
        return lineNumber;
  ...
  getCodeElementFromLineNumber: (codeView, line, part) =>
    getDiffLineElementFromLineNumber(codeView, line, part).querySelector<
      HTMLElement
    >('.CodeMirror-line span[role="presentation"]'),
