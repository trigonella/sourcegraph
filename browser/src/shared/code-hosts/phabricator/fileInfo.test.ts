...
function mockQueryConduit(
  responseMap?: ConduitResponseMap
...
const resolveFileInfoFromFixture = async (
  ...
  const fixtureContent = await readFile(
    `${__dirname}/__fixtures__/pages/${htmlFixture}`,
    "utf-8"
  );
  ...
  if (!codeView) {
    throw new Error(
      `Code view matching selector ${codeViewSelector} not found`
    );
  }
  return resolver(
                          ...
                          uris: [
                            {
                              fields: {
                                uri: {
                                  raw:
                                    "https://phabricator.sgdev.org/gorilla/mux",
                                  normalized:
                                    "phabricator.sgdev.org/gorilla/mux",
                                  disabled: false
                                }
                              }
                            }
