...
import {
  getRepoDetailsFromCallsign,
  getRepoDetailsFromRevisionID,
  QueryConduitHelper
} from "./backend";
...
function getDiffIdFromDifferentialPage(): number {
  ...
  if (!diffsContainer) {
    throw new Error(
      "no element with id differential-review-stage found on page."
    );
  }
  ...
  const diffTitle = wrappingDiffBox.children[0]
    .querySelectorAll(".phui-header-header")
    .item(0);
  if (!diffTitle || !diffTitle.textContent) {
    throw new Error(
      "Could not find diffTitle element, or it had no text content"
    );
  }
  const matches = DIFF_PATTERN.exec(diffTitle.textContent);
  if (!matches) {
    throw new Error(
      `diffTitle element does not match pattern. Content: '${diffTitle.textContent}'`
    );
  }
...
function getBaseCommitIDFromRevisionPage(): string {
  ...
  for (const keyElement of keyElements) {
    if (keyElement.textContent === "Parents ") {
      const parentUrl = ((keyElement.nextSibling as HTMLElement).children[0]
        .children[0] as HTMLLinkElement).href;
...
export function getPhabricatorState(
  ...
  DiffusionState | DifferentialState | RevisionState | ChangeState
> {
  try {
    ...
    if (diffusionMatch) {
      ...
      if (!filePath) {
        throw new Error(
          `Could not determine file path from diffusionMatch, stateUrl: ${stateUrl}`
        );
      }
      const callsign = getCallsignFromPageTag();
      return getRepoDetailsFromCallsign(
        callsign,
        requestGraphQL,
        queryConduit
      ).pipe(
    ...
    if (differentialMatch) {
      ...
      let diffID = differentialMatch[2]
        ? parseInt(differentialMatch[2], 10)
        : undefined;
      if (!diffID) {
      ...
      return getRepoDetailsFromRevisionID(
        revisionID,
        requestGraphQL,
        queryConduit
      ).pipe(
    ...
    if (changesetMatch) {
      ...
      const differentialMatch = differentialHref
        .getAttribute("href")!
        .match(/D(\d+)/);
      if (!differentialMatch) {
      ...
      const diffMatch = diffHref
        .getAttribute("href")!
        .match(/\/differential\/diff\/(\d+)/);
      if (!diffMatch) {
    ...
    throw new Error(
      `Could not determine Phabricator state from stateUrl ${stateUrl}`
    );
  } catch (error) {
