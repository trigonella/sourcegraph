...
describe("trackViews()", () => {
  ...
  test("detects all views on the page", async () => {
    ...
    const views = await mutations
      ...
      .toPromise();
    expect(views.map(({ element }) => element.id)).toEqual([
      "view1",
      "view2",
      "view3"
    ]);
  ...
  test("detects a view if it is the added element itself", async () => {
    const mutations: Observable<MutationRecordLike[]> = of([
      {
        addedNodes: [document.querySelector<HTMLElement>("#view1")!],
        removedNodes: []
      }
    ]);
    expect(
  ...
  test("detects views added later", async () => {
    ...
    expect(
      subscriber.args[0].map(({ subscriptions, ...rest }) => rest)
    ).toEqual([{ element }]);
  });
  ...
  test("removes views", async () => {
    const mutations = from<MutationRecordLike[][]>([
      ...
      [
        {
          addedNodes: [],
          removedNodes: [document.querySelector<HTMLElement>("#view1")!]
        }
      ],
      [
        {
          addedNodes: [],
          removedNodes: [document.querySelector<HTMLElement>("#view3")!]
        }
      ]
    ]);
    await mutations
      .pipe(
        ...
        switchMap(async ([view1, view2, view3]) => {
          ...
          const v1Removed = new Promise(resolve =>
            view1.subscriptions.add(resolve)
          );
          const v3Removed = new Promise(resolve =>
            view3.subscriptions.add(resolve)
          );
  ...
  test("removes all nested views", async () => {
    const mutations = from<MutationRecordLike[][]>([
      ...
      [
        {
          addedNodes: [],
          removedNodes: [document.querySelector<HTMLElement>("#parent")!]
        }
      ]
    ]);
    await mutations
      .pipe(
        ...
        switchMap(views =>
          Promise.all(
            views.map(
              view => new Promise(resolve => view.subscriptions.add(resolve))
            )
          )
        )
      )
  ...
  test("removes a view without depending on its resolver", async () => {
    ...
    const unsubscribed = new Promise(resolve =>
      view.subscriptions.add(resolve)
    );
...
describe("delayUntilIntersecting()", () => {
  ...
  test("delays emitting views until they intersect and stops observing views as soon as they intersect", () => {
    ...
    observerCallback(
      [
        {
          target: document.querySelector<HTMLElement>("#view2")!,
          isIntersecting: true
        }
      ],
      {
    ...
    observerCallback(
      [
        {
          target: document.querySelector<HTMLElement>("#view3")!,
          isIntersecting: true
        },
        {
          target: document.querySelector<HTMLElement>("#view1")!,
          isIntersecting: true
        }
      ],
      { unobserve }
