...
interface InjectProps
  extends PlatformContextProps<
      "forceUpdateTooltip" | "settings" | "sideloadedExtensionURL"
    >,
    ExtensionsControllerProps {
  history: H.History;
...
export const renderCommandPalette = ({
  ...
  Pick<
    CommandListPopoverButtonProps,
    "inputClassName" | "popoverClassName" | "popoverInnerClassName"
  > &
...
export const applyDecorations = (
  ...
  for (const lineNumber of previousDecorations.keys()) {
    if (!decorationsByLine.has(lineNumber)) {
      ...
      const lineElement = dom.getLineElementFromLineNumber(
        codeView,
        lineNumber,
        part
      );
  ...
  for (const [lineNumber, decorationsForLine] of decorationsByLine) {
    ...
    for (const decoration of decorationsForLine) {
      ...
      if (decoration.after) {
        ...
        const annotation = decoration.after.linkURL
          ? linkTo(decoration.after.linkURL)(after)
          : after;
        annotation.dataset.part = String(part);
        annotation.className =
          "sourcegraph-extension-element line-decoration-attachment";
