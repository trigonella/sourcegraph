 ...
 */
export function createFileActionsToolbarMount(
...
): PositionAdjuster<RepoSpec &
  RevisionSpec &
  FileSpec &
  ResolvedRevisionSpec> => ({ direction, codeView, position }) =>
...
export const createFileLineContainerToolbarMount: NonNullable<CodeView["getToolbarMount"]> = (
  ...
  const existingMount = codeViewElement.querySelector(
    `.${className}`
  ) as HTMLElement;
  if (existingMount) {
    return existingMount;
...
export const fileLineContainerResolver: ViewResolver<CodeView> = {
  ...
  resolveView: (fileLineContainer: HTMLElement): CodeView | null => {
    const embeddedBlobWrapper = fileLineContainer.closest(
...
const genericCodeViewResolver: ViewResolver<CodeView> = {
  ...
  resolveView: (element: HTMLElement): CodeView | null => {
    ...
    if (
      element.closest(".discussion-item-body") ||
...
export const githubCodeHost: CodeHost = {
    ...
    return {
      ...
      revision:
        parsedURL.pageType === "blob" || parsedURL.pageType === "tree"
          ...
          : undefined,
      privateRepository:
        window.location.hostname !== "github.com" || repoHeaderHasPrivateMarker
    };
  },
  ...
  urlToFile: (sourcegraphURL, target, context) => {
      ...
      for (const container of containers) {
        ...
        if (anchorPath === target.filePath) {
          ...
          if (target.position) {
            ...
            url.hash += `${context.part === "base" ? "L" : "R"}${
    ...
    const fragment = target.position
      ? `#L${target.position.line}${
          target.position.character ? `:${target.position.character}` : ""
        }`
      : "";
