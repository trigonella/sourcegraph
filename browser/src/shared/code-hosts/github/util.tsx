...
export function getDiffFileName(
  ...
  if (fileInfoElement) {
    ...
    const link = fileInfoElement.querySelector<HTMLAnchorElement>(
      ".diffstat + a"
    );
    if (link) {
  ...
  const fileHeaderLink = container.querySelector<HTMLLinkElement>(
    ".file-header a"
  );
  if (fileHeaderLink) {
...
function getPathNamesFromElement(
  element: HTMLElement
): { headFilePath: string; baseFilePath: string | undefined } {
 ...
 */
export function getDiffResolvedRevision(
  codeView: HTMLElement
): DiffResolvedRevSpec | null {
  ...
  let baseCommitID = "";
  let headCommitID = "";
  const fetchContainers = document.querySelectorAll(
    ".js-socket-channel.js-updatable-content.js-pull-refresh-on-pjax"
  );
  const isCommentedSnippet = codeView.classList.contains(
  ...
  if (pageType === "pull") {
    ...
    const shaContainers = document.querySelectorAll(
      ".sha-block:not(.patch-diff-links)"
    );
    if (shaContainers && shaContainers.length === 2) {
      ...
      const headShaElement = shaContainers[1].querySelector(
        "span.sha"
      ) as HTMLElement;
      if (headShaElement) {
  ...
  if (baseCommitID === "" || headCommitID === "") {
    return getDiffResolvedRevisionFromPageSource(
...
function getResolvedDiffFromCommentedSnippet(
  codeView: HTMLElement
): DiffResolvedRevSpec | null {
  ...
  const linkToFile: HTMLLinkElement | null = codeView.querySelector(
    ".file-header a"
  );
  if (!linkToFile) {
    return null;
...
function getResolvedDiffForCompare(): DiffResolvedRevSpec | undefined {
  const [base, head] = document.querySelectorAll<HTMLElement>(
    ".commitish-suggester .select-menu-button span"
  );
...
export function getFilePath(): string {
  ...
  if (!permalink) {
    throw new Error(
      "Unable to determine the file path because no a.js-permalink-shortcut element was found."
    );
  }
...
export function parseURL(
  location: Pick<Location, "host" | "pathname" | "href"> = window.location
): GitHubURL {
  const { host, pathname } = location;
  const [user, ghRepoName, pageType, ...rest] = pathname.slice(1).split("/");
  if (!user || !ghRepoName) {
    throw new Error(
      `Could not parse repoName from GitHub url: ${location.href}`
    );
  }
