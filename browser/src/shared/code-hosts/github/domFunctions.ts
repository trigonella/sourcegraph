 ...
 * Prefers head.
 */
const getLineNumberElementIndex = (
  part: DiffPart,
  isSplitDiff: boolean
): number => {
  if (part === "base") {
...
const getLineNumberFromCodeElement = (codeElement: HTMLElement): number => {
  ...
  let cell = codeElement.closest("td")!
...
export const diffDomFunctions: DOMFunctions = {
  ...
  isFirstCharacterDiffIndicator: codeElement => {
    ...
    const hasDataCodeMarkerUnified = tableRow?.querySelector(
      "td[data-code-marker]"
    );
    const hasDataCodeMarkerSplit = blobCodeInner?.hasAttribute(
      "data-code-marker"
    );
    const hasDataCodeMarker =
      hasDataCodeMarkerUnified || hasDataCodeMarkerSplit;
...
const getSearchCodeSnippetCodeElementFromLineNumber = (
  ...
  const lineNumberCell = getSearchCodeSnippetLineNumberCellFromLineNumber(
    codeView,
    line
  );
...
export function isDomSplitDiff(element: HTMLElement): boolean {
  ...
  const codeView = element.classList.contains("file")
    ? element
  ...
  return (
    codeViewTable.classList.contains("js-file-diff-split") ||
