...
export const getPageKindFromPathName = (
  ...
  const pageKindMatch = pathname.match(
    new RegExp(`^/${owner}/${projectName}(/-)?/(commit|merge_requests|blob)/`)
  );
  if (!pageKindMatch) {
...
export function getPageInfo(): GitLabInfo {
  ...
  const pageKind = getPageKindFromPathName(
    owner,
    projectName,
    window.location.pathname
  );
  const hostname = isExtension
...
const getFilePathFromElement = (element: HTMLElement): string => {
  const filePath =
    element.dataset.originalTitle || element.dataset.title || element.title;
  if (!filePath) {
 ...
 */
export function getFilePathsFromCodeView(
  codeView: HTMLElement
): { headFilePath: string; baseFilePath: string } {
  const filePathElements = codeView.querySelectorAll<HTMLElement>(
    ".file-title-name"
  );
...
interface GitLabCommitPageInfo
  extends RawRepoSpec,
