...
const buildURL = (owner: string, projectName: string, path: string): string =>
  `${window.location.origin}/api/v4/projects/${encodeURIComponent(
    owner
  )}%2f${projectName}${path}`;
...
const getRepoNameFromProjectID = memoizeObservable(
  (projectId: string): Observable<string> =>
    get<{ web_url: string }>(
      `${window.location.origin}/api/v4/projects/${projectId}`
    ).pipe(
      map(({ web_url }) => {
    ...
    diffID
      ? get<DiffVersionsResponse>(
          buildURL(
            owner,
            projectName,
            `/merge_requests/${mergeRequestID}/versions/${diffID}`
          )
        ).pipe(map(({ base_commit_sha }) => base_commit_sha))
      : of(undefined),
  ({ owner, projectName, mergeRequestID, diffID }) =>
    `${owner}:${projectName}:${mergeRequestID}:${String(diffID)}`
);
...
export const getMergeRequestDetailsFromAPI = memoizeObservable(
  ({
    ...
    zip(
      get<MergeRequestResponse>(
        buildURL(owner, projectName, `/merge_requests/${mergeRequestID}`)
      ),
...
export const getBaseCommitIDForCommit: ({
  ...
  ({ owner, projectName, commitID }) =>
    get<CommitResponse>(
      buildURL(owner, projectName, `/repository/commits/${commitID}`)
    ).pipe(
