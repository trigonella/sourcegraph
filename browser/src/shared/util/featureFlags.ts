...
const createFeatureFlagStorage = ({
  get,
  set
}: FeatureFlagUtilities): FeatureFlagsStorage => ({
  set,
...
async function bextGet<K extends keyof FeatureFlags>(
  key: K
...
async function bextSet<K extends keyof FeatureFlags>(
  key: K,
...
export const featureFlags: FeatureFlagsStorage = isInPage
  ? inPageFeatureFlags
