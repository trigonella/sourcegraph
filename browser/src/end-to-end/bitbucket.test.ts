...
async function createProject(driver: Driver): Promise<void> {
  ...
  await driver.page.type(
    'form.project-settings input[name="key"]',
    "SOURCEGRAPH"
  );
  await driver.page.type(
    'form.project-settings input[name="name"]',
    "SOURCEGRAPH"
  );
  await Promise.all([
...
async function importBitbucketRepo(driver: Driver): Promise<void> {
  ...
  await driver.page.goto(
    BITBUCKET_BASE_URL + "/plugins/servlet/import-repository/SOURCEGRAPH"
  );
  ...
  await driver.page.type(
    '.source-form.git-specific input[name="url"]',
  ...
  await retry(async () => {
    ...
    expect(new URL(driver.page.url()).pathname).toBe(
      new URL(browsePage, BITBUCKET_BASE_URL).pathname
    );
 ...
 */
async function configureSourcegraphIntegration(
  driver: Driver,
  enable: boolean
): Promise<void> {
  await driver.page.goto(
    BITBUCKET_BASE_URL + "/plugins/servlet/upm?source=side_nav_manage_addons"
  );
    ...
    await retry(async () => {
      expect(
        await driver.page.evaluate(
          () =>
            document.querySelector<HTMLInputElement>("form#admin input#url")!
              .disabled
        )
      ).toBe(false);
    });
