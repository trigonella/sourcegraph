...
const TEST_NATIVE_INTEGRATION = Boolean(
  process.env.TEST_NATIVE_INTEGRATION &&
    JSON.parse(process.env.TEST_NATIVE_INTEGRATION)
);
const { gitHubToken, sourcegraphBaseUrl, ...restConfig } = getConfig(
  "gitHubToken",
  "sourcegraphBaseUrl"
);
  ...
  await retry(
    async () => {
      await driver.page.goto(
        PHABRICATOR_BASE_URL + "/source/jrpc/manage/status/"
      );
      expect(
        await driver.page.evaluate(() =>
          [
            ...document.querySelectorAll(".phui-status-item-target")
...
async function addPhabricatorRepo(driver: Driver): Promise<void> {
  ...
  const activateButton = await driver.page.waitForSelector(
    'a[href="/source/jrpc/edit/activate/"]'
  );
  const buttonLabel = ((await (
    await activateButton.getProperty("textContent")
  ).jsonValue()) as string).trim();
  // Don't click if it says "Deactivate Repository"
  if (buttonLabel === "Activate Repository") {
    await activateButton.click();
    await driver.page.waitForSelector(
      'form[action="/source/jrpc/edit/activate/"]'
    );
    await (
      await driver.page.$x('//button[text()="Activate Repository"]')
    )[0].click();
...
async function configureSourcegraphIntegration(driver: Driver): Promise<void> {
  ...
  await driver.page.goto(
    PHABRICATOR_BASE_URL + "/config/edit/sourcegraph.url/"
  );
  ...
  await driver.page.goto(
    PHABRICATOR_BASE_URL + "/config/edit/sourcegraph.callsignMappings/"
  );
  const callSignConfigString = await driver.page.evaluate(() =>
    document
      .querySelector<HTMLTextAreaElement>('textarea[name="value"]')!
      .value.trim()
  );
  ...
  await driver.page.goto(
    PHABRICATOR_BASE_URL + "/config/edit/sourcegraph.enabled/"
  );
...
async function init(driver: Driver): Promise<void> {
  if (restConfig.testUserPassword) {
    await driver.ensureLoggedIn({
      username: "test",
      password: restConfig.testUserPassword
    });
  }
...
describe("Sourcegraph Phabricator extension", () => {
  ...
  it('adds "View on Sourcegraph" buttons to files', async () => {
    await driver.page.goto(
      PHABRICATOR_BASE_URL +
        "/source/jrpc/browse/master/call_opt.go;35a74f039c6a54af5bf0402d8f7da046c3f63ba2"
    );
    await driver.page.waitForSelector(
      ".code-view-toolbar .open-on-sourcegraph"
    );
    expect(
      await driver.page.$$(".code-view-toolbar .open-on-sourcegraph")
    ).toHaveLength(1);
    await Promise.all([
      driver.page.waitForNavigation(),
  ...
  it("shows hovers when clicking a token", async () => {
    ...
    const codeElement = await driver.page.waitForXPath(
      `//tbody/tr[${lineNumber}]//span[text()="CallOption"]`
    );
