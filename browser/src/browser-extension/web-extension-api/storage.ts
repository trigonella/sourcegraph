...
export const storage: {
  [K in browser.storage.AreaName]: browser.storage.StorageArea<
    ExtensionStorageItems[K]
  >;
} & {
  onChanged: browser.CallbackEventEmitter<
    (
      changes: browser.storage.ChangeDict<
        ExtensionStorageItems[typeof areaName]
      >,
...
>(
  ...
  return concat(
    ...
    from(
      (storage[areaName] as browser.storage.StorageArea<
        ExtensionStorageItems[A]
      >).get(key)
