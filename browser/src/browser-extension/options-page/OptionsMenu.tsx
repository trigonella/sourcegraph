...
export interface OptionsMenuProps
  extends OptionsHeaderProps,
    Pick<
      ServerUrlFormProps,
      Exclude<keyof ServerUrlFormProps, "value" | "onChange" | "onSubmit">
    > {
...
const buildFeatureFlagToggleHandler = (
  key: string,
  handler: OptionsMenuProps["toggleFeatureFlag"]
) => () => handler(key);
...
export const OptionsMenu: React.FunctionComponent<OptionsMenuProps> = ({
  ...
  <div className={`options-menu ${isFullPage() ? "options-menu--full" : ""}`}>
      ...
      !PERMISSIONS_PROTOCOL_BLOCKLIST.has(currentTabStatus.protocol) && (
        <div className="options-menu__section">
          <div className="alert alert-info">
            <p>
              The Sourcegraph browser extension adds hover tooltips to code
              views on code hosts such as GitHub, GitLab, Bitbucket Server and
              Phabricator.
            </p>
            <p>
            ...
            <button
              ...
              onClick={buildRequestPermissionsHandler(
                currentTabStatus,
                requestPermissions
              )}
            >
    ...
    <div className="options-menu__section">
      <p>
        Learn more about privacy concerns, troubleshooting and extension
        features{" "}
        <a
          ...
          {featureFlags.map(({ key, value }) => (
            <div className="form-check" key={key}>
              <label className="form-check-label">
                <input
                  id={key}
                  onChange={buildFeatureFlagToggleHandler(
                    key,
                    toggleFeatureFlag
                  )}
