...
} from "../options-page/OptionsContainer";
...
class Options extends React.Component<{}, State> {
  ...
  public componentDidMount(): void {
    ...
    this.subscriptions.add(
      observeStorageKey("sync", "disableExtension").subscribe(
        disableExtension => {
          this.setState({
  ...
  public render(): React.ReactNode {
    ...
    const props: OptionsContainerProps = {
      ...
      setSourcegraphURL: (sourcegraphURL: string) =>
        storage.sync.set({ sourcegraphURL }),
      toggleExtensionDisabled: (isActivated: boolean) =>
        storage.sync.set({ disableExtension: !isActivated }),
      toggleFeatureFlag,
      featureFlags: [
