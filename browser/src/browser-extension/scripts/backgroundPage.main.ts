...
async function main(): Promise<void> {
  ...
  browser.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
    if (
      ...
      customServerOrigins.some(
        origin =>
          origin === "<all_urls>" ||
          (!!tab.url && tab.url.startsWith(origin.replace("/*", "")))
      )
    ) {
  ...
  browser.runtime.onMessage.addListener(
    async (message: {
      type: keyof BackgroundMessageHandlers;
      payload: any;
    }) => {
   ...
   */
  const browserPortPairs: Observable<Record<
    keyof EndpointPair,
    browser.runtime.PortWithSender
  >> = fromBrowserEvent(browser.runtime.onConnect).pipe(
    map(([port]) => port),
    groupBy(
...
const workerBundleURL = browser.runtime.getURL(
  "js/extensionHostWorker.bundle.js"
);
...
function handleBrowserPortPair(
  ...
  const forwardEndpoint = (from: Endpoint, to: Endpoint): void => {
    ...
    subscriptions.add(() =>
      from.removeEventListener("message", messageListener)
    );
