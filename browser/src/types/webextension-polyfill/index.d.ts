...
declare namespace browser.bookmarks {
  ...
  const onCreated: CallbackEventEmitter<(
    id: string,
...
declare namespace browser.browserAction {
  ...
  function setBadgeTextColor(
    details: SetBadgeTextColorDetails & { tabId?: number }
  ): void;
  // a union type would allow specifying both, which is not allowed.
  // eslint-disable-next-line @typescript-eslint/unified-signatures
  function setBadgeTextColor(
    details: SetBadgeTextColorDetails & { windowId?: number }
  ): void;
...
declare namespace browser.menus {
  ...
  const onClicked: CallbackEventEmitter<(
    info: OnClickData,
    tab: tabs.Tab
  ) => void>;
  ...
  const onShown: CallbackEventEmitter<(
    info: OnClickData,
    tab: tabs.Tab
  ) => void>;
}
...
declare namespace browser.omnibox {
  ...
  const onInputChanged: CallbackEventEmitter<(
    text: string,
    suggest: (arg: SuggestResult[]) => void
  ) => void>;
...
declare namespace browser.tabs {
  ...
  const onReplaced: CallbackEventEmitter<(
    addedTabId: number,
    removedTabId: number
  ) => void>;
