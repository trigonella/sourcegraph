...
const monacoEditorPaths =
    ...
    const config = {
      ...
      addons : [
        '@storybook/addon-knobs', '@storybook/addon-actions',
      ...
      webpackFinal : config => {
        ...
        const definePlugin =
            config.plugins.find(plugin => plugin instanceof DefinePlugin)
        // @ts-ignore
        ...
        // We don't use Storybook's default Babel config for our repo, it
        // doesn't include everything we need.
        ...
        // Put our style rules at the beginning so they're processed by the time
        // it gets to storybook's style rules.
        ...
        // Make sure Storybook style loaders are only evaluated for Storybook
        // styles.
        config.module.rules
            .find(rule => rule.test?.toString() === /\.css$/.toString())
            .include = storybookDirectory

        config.module.rules.unshift({
          // CSS rule for external plain CSS (skip SASS and PostCSS for build
          // perf)
          test : /\.css$/,
          // Make sure Storybook styles get handled by the Storybook config
        ...
        config.module.rules.unshift({
          // CSS rule for monaco-editor, it expects styles to be loaded with
          // `style-loader`.
          test : /\.css$/,
