...
const config = {
  ...
  coveragePathIgnorePatterns : [
    /\/node_modules\//.source, /\.(test|story)\.tsx?$/.source, /\.d\.ts$/.source
  ],
  roots : [ '<rootDir>/src' ],

  // Transform packages that do not distribute CommonJS packages (typically
  // because they only distribute ES6
  // modules). If you get an error from jest like "Jest encountered an
  // unexpected token. ... SyntaxError:
  // unexpected token import/export", then add it here. See
  // https://github.com/facebook/create-react-app/issues/5241#issuecomment-426269242
  ...
  moduleNameMapper : {
    ...
    // monaco-editor uses the "module" field in package.json, which isn't
    // supported by Jest
  ...
  // By default, don't clutter `yarn test --watch` output with the full coverage
  // table. To see it, use the
  ...
  snapshotSerializers :
      [ path.join(__dirname, 'client/shared/dev/enzymeSerializer.js') ],
}
