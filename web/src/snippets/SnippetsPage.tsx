 ...
 */
export const SnippetsPage: React.FunctionComponent<Props> = ({
  location,
  history,
  ...
  useLayoutEffect(() => setTextArea(textAreaReference.current), [
    textAreaReference
  ]);
  ...
  const [panelViews, setPanelViews] = useState<PanelViewWithComponent[] | null>(
    null
  );
  useEffect(() => {
  ...
  const allPanelViews: PanelViewWithComponent[] | null =
    initialModelLanguageId === "markdown" && modelText !== null
      ? [
          ...(panelViews || []),
          { title: "Preview", content: modelText, priority: 0 }
        ]
      : panelViews;
  ...
  return (
    <div className="container mt-3">
      <h1>
        Snippet editor{" "}
        <span className="badge badge-warning text-uppercase">Experimental</span>
      </h1>
      {viewerId && modelUri && (
        <>
          {textArea && (
