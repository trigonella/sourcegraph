...
export const GlobalAlert: React.FunctionComponent<{
  ...
  className: string;
  history: H.History;
}> = ({ alert, history, className: commonClassName }) => {
  const Icon = alertIconForType(alert.type);
  const content = (
    <>
      <Icon className="icon-inline mr-2 flex-shrink-0" />
      <Markdown
        dangerousInnerHTML={renderMarkdown(alert.message)}
        history={history}
      />
    </>
  );
  const className = `${commonClassName} alert alert-${alertClassForType(
    alert.type
  )} d-flex`;
