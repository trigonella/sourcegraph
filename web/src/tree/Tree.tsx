...
export class Tree extends React.PureComponent<Props, State> {
  ...
  private expandDirectoryChanges = new Subject<{
    path: string;
    expanded: boolean;
    node: TreeNode;
  }>();
  ...
  private keyHandlers: Record<string, () => void> = {
    ...
    [Key.ArrowUp]: () => {
      if (this.state.selectedNode) {
        this.selectNode(
          previousChild(this.state.selectedNode, this.state.selectedNode.index)
        );
      }
    },
    [Key.ArrowLeft]: () => {
    ...
    [Key.ArrowRight]: () => {
      ...
      if (nodeDomElement) {
        const isDirectory = Boolean(
          nodeDomElement.getAttribute("data-tree-is-directory")
        );
      ...
      this.expandDirectoryChanges.subscribe(({ path, expanded, node }) => {
        this.setState(previousState => ({
          resolveTo: expanded
            ...
            : previousState.resolveTo.filter(
    ...
    this.subscriptions.add(
      this.componentUpdates
        ...
        .subscribe((props: Props) => {
          const newParentPath = props.activePathIsDir
            ? props.activePath
            : dirname(props.activePath);
          const queryParameters = new URLSearchParams(
            this.props.history.location.search
          );
          const queryParametersHasSubtree =
            queryParameters.get("subtree") === "true";
          ...
          if (
            queryParametersHasSubtree &&
            !queryParameters.has("tab") &&
