...
export class UserSettingsProfilePage extends React.Component<Props, State> {
        ...
        .pipe(
          switchMap(([user]) =>
            queryUser(user.id).pipe(
              ...
              map(
                (userOrError): Pick<State, "userOrError"> => ({ userOrError })
              )
            )
          )
        )
        ...
        .pipe(
          ...
          mergeMap(() =>
            updateUser(this.props.user.id, {
              username:
                this.state.username === undefined ? null : this.state.username,
              displayName:
                this.state.displayName === undefined
                  ? null
                  : this.state.displayName,
              avatarURL:
                this.state.avatarURL === undefined ? null : this.state.avatarURL
          ...
          tap(() => {
            ...
            if (
              ...
              this.props.history.push(
                `/users/${this.state.username}/settings/profile`
              );
              return;
            }
    ...
    return (
      <div className="user-settings-profile-page">
        ...
        {isErrorLike(this.state.userOrError) && (
          <ErrorAlert
            error={this.state.userOrError.message}
            history={this.props.history}
          />
        )}
        {this.state.error && (
        ...
        {this.state.userOrError && !isErrorLike(this.state.userOrError) && (
          <Form
            className="user-settings-profile-page__form"
            onSubmit={this.handleSubmit}
          >
            <div className="form-group">
              <label htmlFor="user-settings-profile-page__form-username">
                Username
              </label>
              <UsernameInput
              ...
              <small className="form-text text-muted">
                A username consists of letters, numbers, hyphens (-), dots (.)
                and may not begin or end with a dot, nor begin with a hyphen.
              </small>
            </div>
            <div className="form-group">
              <label htmlFor="user-settings-profile-page__form-display-name">
                Display name
              </label>
              <input
            ...
            <div className="user-settings-profile-page__avatar-row">
              <div className="form-group user-settings-profile-page__field-column">
                <label htmlFor="user-settings-profile-page__form-avatar-url">
                  Avatar URL
                </label>
                <input
            ...
            {window.context.sourcegraphDotComMode && (
              <p className="mt-4">
                <a href="https://about.sourcegraph.com/contact">
                  Contact support
                </a>{" "}
  ...
  private onUsernameFieldChange = (
    event: React.ChangeEvent<HTMLInputElement>
  ): void => {
    this.setState({ username: event.target.value });
  };

  private onDisplayNameFieldChange = (
    event: React.ChangeEvent<HTMLInputElement>
  ): void => {
    this.setState({ displayName: event.target.value });
  };

  private onAvatarURLFieldChange = (
    event: React.ChangeEvent<HTMLInputElement>
  ): void => {
