...
export class SiteAdminUpdatesPage extends React.Component<Props, State> {
  ...
  public render(): JSX.Element | null {
    const autoUpdateCheckingEnabled =
      window.context.site["update.channel"] === "release";
    return (
      <div className="site-admin-updates-page">
        <PageTitle title="Updates - Admin" />
        <h2>Updates</h2>
        {this.state.error && (
          <p className="site-admin-updates-page__error">
            Error: {upperFirst(this.state.error)}
          </p>
        )}
            ...
            <div>
              {this.state.updateCheck.pending && (
                <div className="site-admin-updates-page__alert alert alert-primary">
                  <LoadingSpinner className="icon-inline" /> Checking for
                  updates... (reload in a few seconds)
                </div>
              )}
              {!this.state.updateCheck.errorMessage &&
                (this.state.updateCheck.updateVersionAvailable ? (
                  <div className="site-admin-updates-page__alert alert alert-success">
                    <CloudDownloadIcon className="icon-inline" /> Update
                    available:{" "}
        ...
        <p className="site-admin-updates_page__info">
          <small>
            <strong>Current product version:</strong>{" "}
            {this.state.productVersion} ({this.state.buildVersion})
          </small>
          <br />
          <small>
            ...
            {this.state.updateCheck?.checkedAt
              ? formatDistance(
                  parseISO(this.state.updateCheck.checkedAt),
                  new Date(),
                  {
                    addSuffix: true
                  }
          ...
          <small>
            ...
            {autoUpdateCheckingEnabled ? "on" : "off"}.{" "}
            <Link to="/site-admin/configuration">Configure</Link>{" "}
            <code>update.channel</code> to{" "}
