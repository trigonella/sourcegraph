...
const ExternalServiceNode: React.FunctionComponent<ExternalServiceNodeProps> = ({
  node,
  onDidUpdate,
  history
}) => {
  const [nextDeleteClick, deletedOrError] = useEventObservable(
    useCallback(
      (clicks: Observable<React.MouseEvent>) =>
        clicks.pipe(
          filter(() =>
            window.confirm(`Delete the external service ${node.displayName}?`)
          ),
          switchMap(() =>
            concat(
  ...
  return (
    ...
    >
      ...
      {isErrorLike(deletedOrError) && (
        <ErrorAlert className="mt-2" error={deletedOrError} history={history} />
      )}
    </li>
  );
};
 ...
 */
export class SiteAdminExternalServicesPage extends React.PureComponent<
  Props,
  State
> {
  ...
  public componentDidMount(): void {
    ...
    this.subscriptions.add(
      this.queryExternalServices({
        first: 1
      }).subscribe(externalServicesResult =>
        this.setState({
          noExternalServices: externalServicesResult.totalCount === 0
        })
      )
    );
  }
  ...
  private completeConnectedCodeHostActivation = (
    externalServices: GQL.IExternalServiceConnection
  ): void => {
  ...
  private queryExternalServices = (
    ...
    queryGraphQL(
      ...
      tap(externalServices =>
        this.completeConnectedCodeHostActivation(externalServices)
      )
    );
