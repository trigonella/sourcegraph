...
const allConfigSchema = {
  ...
  properties: {
    ...
    externalServices: {
      type: "object",
      properties: mapValues(externalServices, schema => ({
        type: "array",
        items: schema
      }))
    },
  ...
  definitions: values(externalServices)
    ...
    .reduce(
      (allDefinitions, definitions) => ({ ...allDefinitions, ...definitions }),
      {}
    )
};
...
export const SiteAdminReportBugPage: React.FunctionComponent<Props> = ({
  isLightTheme,
  telemetryService,
  history
}) => {
  const monitoringDaysBack = 7;
  const monitoringStats = useObservable(
    useMemo(() => fetchMonitoringStats(monitoringDaysBack), [])
  );
  ...
  return (
    <div>
      ...
      <p>
        ...
        , and include a description of the bug along with the info below (with
        secrets redacted). If the report contains sensitive information that
        should not be public, email the report to{" "}
        <a
          target="_blank"
          rel="noopener noreferrer"
      ...
      <div className="card-header alert alert-warning">
        <div>
          Please redact any secrets before sharing, whether on the public issue
          tracker or with support@sourcegraph.com.
        </div>
      </div>
