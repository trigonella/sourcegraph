 ...
 * Fetches all users.
 */
export function fetchAllUsers(args: {
  first?: number;
  query?: string;
  ...
  return fetchAllRepositories(args).pipe(
    ...
    repeatUntil(
      result =>
        ...
        result.nodes.every(
          nodes => !nodes.mirrorInfo.cloneInProgress && nodes.mirrorInfo.cloned
        ),
      { delay: 5000 }
    )
  );
...
export function updateMirrorRepository(args: {
  repository: GQL.ID;
}): Observable<void> {
...
export function scheduleRepositoryPermissionsSync(args: {
  repository: GQL.ID;
}): Observable<void> {
  return mutateGraphQL(
    gql`
...
export function scheduleUserPermissionsSync(args: {
  user: GQL.ID;
}): Observable<void> {
  return mutateGraphQL(
    gql`
...
type SettingsSubject = Pick<
  GQL.SettingsSubject,
  "settingsURL" | "__typename"
> & {
  contents: Settings;
};
  ...
  return queryGraphQL(
    ...
    map(data => {
      ...
      >> = data.externalServices.nodes
        ...
        .map(
          svc =>
            [svc.kind, parseJSONC(svc.config) as ExternalServiceConfig] as const
        )
        .reduce<
          Partial<{ [k in GQL.ExternalServiceKind]: ExternalServiceConfig[] }>
      ...
      const settingsSubjects = data.viewerSettings.subjects.map(settings => ({
        ...
        contents: settings.latestSettings
          ? parseJSONC(settings.latestSettings.contents)
          : null
      }));
...
export function setUserIsSiteAdmin(
  userID: GQL.ID,
  siteAdmin: boolean
): Observable<void> {
  return mutateGraphQL(
...
export function randomizeUserPassword(
  user: GQL.ID
): Observable<GQL.IRandomizeUserPasswordResult> {
  return mutateGraphQL(
    gql`
...
export function fetchMonitoringStats(
  days: number
): Observable<GQL.IMonitoringStatistics | false> {
      ...
      if (isErrorGraphQLResult(result)) {
        if (
          result.errors.find(error =>
            error.message.includes(errorPrometheusUnavailable)
          )
        ) {
          return false;
        }
