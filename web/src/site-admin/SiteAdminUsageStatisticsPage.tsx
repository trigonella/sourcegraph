...
export const UsageChart: React.FunctionComponent<UsageChartPageProps> = (
  props: UsageChartPageProps
) => (
  <div className="site-admin-usage-statistics-page">
    {props.header ? (
      props.header
    ) : (
      <h3>{chartGeneratorOptions[props.chartID].label}</h3>
    )}
    <BarChart
      showLabels={true}
...
class UserUsageStatisticsHeader extends React.PureComponent<
  UserUsageStatisticsHeaderFooterProps
> {
...
class UserUsageStatisticsFooter extends React.PureComponent<
  UserUsageStatisticsHeaderFooterProps
> {
  public render(): JSX.Element | null {
    return (
        ...
        <tr>
          ...
          <td>
            {this.props.nodes.reduce(
              (count, node) =>
                count +
                (node.usageStatistics ? node.usageStatistics.pageViews : 0),
              0
            )}
          </td>
          <td>
            {this.props.nodes.reduce(
              (count, node) =>
                count +
                (node.usageStatistics ? node.usageStatistics.searchQueries : 0),
              0
            )}
          </td>
          <td>
            {this.props.nodes.reduce(
              (count, node) =>
                count +
                (node.usageStatistics
                  ? node.usageStatistics.codeIntelligenceActions
                  : 0),
              0
            )}
          </td>
...
class UserUsageStatisticsNode extends React.PureComponent<
  UserUsageStatisticsNodeProps
> {
  public render(): JSX.Element | null {
    return (
      <tr>
        ...
        <td>
          {this.props.node.usageStatistics
            ? this.props.node.usageStatistics.pageViews
            : "n/a"}
        </td>
        <td>
          {this.props.node.usageStatistics
            ? this.props.node.usageStatistics.searchQueries
            : "n/a"}
        </td>
        <td>
          {this.props.node.usageStatistics
            ? this.props.node.usageStatistics.codeIntelligenceActions
            : "n/a"}
        </td>
        <td className="site-admin-usage-statistics-page__date-column">
...
> {
  ...
  private loadLatestChartFromStorage(): keyof ChartOptions {
    ...
    return latest && latest in chartGeneratorOptions
      ? (latest as keyof ChartOptions)
      : "daus";
  }
    ...
    return (
      <div className="site-admin-usage-statistics-page">
        ...
        <a
          ...
          download="true"
        >
          <FileDownloadIcon className="icon-inline" /> Download usage stats
          archive
        </a>
        ...
        {this.state.stats && (
          <>
            <RadioButtons
              nodes={Object.entries(chartGeneratorOptions).map(
                ([key, { label }]) => ({
                  label,
                  id: key
                })
