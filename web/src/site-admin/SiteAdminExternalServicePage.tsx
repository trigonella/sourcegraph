...
type ExternalService = Pick<
  GQL.IExternalService,
  "id" | "kind" | "displayName" | "config" | "warning" | "webhookURL"
>;
...
export const SiteAdminExternalServicePage: React.FunctionComponent<Props> = ({
  ...
  const [externalServiceOrError, setExternalServiceOrError] = useState<
    typeof LOADING | ExternalService | ErrorLike
  >(LOADING);
  ...
  const [nextSubmit, updatedServiceOrError] = useEventObservable(
    useCallback(
      (
        submits: Observable<GQL.IExternalService>
      ): Observable<typeof LOADING | ErrorLike | ExternalService> =>
        submits.pipe(
          switchMap(input =>
            concat(
              [LOADING],
              updateExternalService(input).pipe(
                catchError((error: Error) => [asError(error)])
              )
            )
          )
        ),
  ...
  useEffect(() => {
    if (
      updatedServiceOrError &&
      updatedServiceOrError !== LOADING &&
      !isErrorLike(updatedServiceOrError)
    ) {
  ...
  let externalServiceCategory =
    externalService && defaultExternalServices[externalService.kind];
  if (
    externalService &&
    [GQL.ExternalServiceKind.GITHUB, GQL.ExternalServiceKind.GITLAB].includes(
      externalService.kind
    )
  ) {
    ...
    if (
      externalService.kind === GQL.ExternalServiceKind.GITHUB &&
    ...
    if (
      externalService.kind === GQL.ExternalServiceKind.GITLAB &&
  ...
  return (
    <div className="site-admin-configuration-page">
      {externalService ? (
        <PageTitle
          title={`External service - ${externalService.displayName}`}
        />
      ) : (
      ...
      {externalService && (
        <SiteAdminExternalServiceWebhook externalService={externalService} />
      )}
    </div>
  );
};
...
function isExternalService(
  ...
  return (
    externalServiceOrError !== LOADING && !isErrorLike(externalServiceOrError)
  );
}
