...
class SurveyForm extends React.Component<SurveyFormProps, SurveyFormState> {
  ...
  public render(): JSX.Element | null {
    return (
      <Form className="survey-form" onSubmit={this.handleSubmit}>
        {this.state.error && (
          <p className="survey-form__error">{this.state.error.message}</p>
        )}
        ...
        <div>
          <small>
            Your response to this survey will be sent to Sourcegraph, and will
            be visible to your Sourcegraph site admins.
          </small>
        </div>
      </Form>
    );
  ...
  private onReasonFieldChange = (
    event: React.ChangeEvent<HTMLTextAreaElement>
  ): void => {
    this.setState({ reason: event.target.value });
  };

  private onBetterProductFieldChange = (
    event: React.ChangeEvent<HTMLTextAreaElement>
  ): void => {
...
interface SurveyPageProps
  extends RouteComponentProps<{ score?: string }>,
    ThemeProps {
  authenticatedUser: GQL.IUser | null;
}
...
const TweetFeedback: React.FunctionComponent<TweetFeedbackProps> = ({
  feedback,
  score
}) => {
  if (score >= SCORE_TO_TWEET) {
    ...
    return (
      <>
        <p className="mt-2">
          One more favor, could you share your feedback on Twitter? We'd really
          appreciate it!
        </p>
        <a
