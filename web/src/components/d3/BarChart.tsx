...
export class BarChart<T extends BarChartSeries> extends React.Component<
  Props<T>
> {
  ...
  private drawChart = (): void => {
    ...
    const barColors = this.props.isLightTheme
      ? ["#a2b0cd", "#cad2e2"]
      : ["#566e9f", "#a2b0cd"];
    ...
    const yHeights = data.map(({ yValues }) =>
      Object.keys(yValues).reduce(
        (accumulator, key) => accumulator + yValues[key],
        0
      )
    );
    ...
    const yScaleBand = scaleLinear()
      ...
      .range([height, 0]);
    const zScaleOrdinal = scaleOrdinal<string, string>()
      .domain(series)
      .range(barColors);
  ...
  public render(): JSX.Element | null {
    ...
    return (
      <svg
        ...
        ref={reference => (this.svgRef = reference)}
      />
    );
  }
}
