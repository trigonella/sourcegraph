...
/** A file diff. */
export class FileDiffNode extends React.PureComponent<
  FileDiffNodeProps,
  State
> {
  ...
  public render(): JSX.Element | null {
    ...
    if (node.oldFile?.binary || node.newFile?.binary) {
      const sizeChange =
        (node.newFile?.byteSize ?? 0) - (node.oldFile?.byteSize ?? 0);
      const className = sizeChange >= 0 ? "text-success" : "text-danger";
      stat = (
        <strong className={classNames(className, "mr-2 code")}>
          {prettyBytes(sizeChange)}
        </strong>
      );
    } else {
      stat = (
              ...
              {node.newPath &&
                ...
                node.newPath !== node.oldPath && (
                  <span className="badge badge-warning text-uppercase mr-2">
                    {dirname(node.newPath) !== dirname(node.oldPath)
                      ? "Moved"
                      : "Renamed"}
                  </span>
                )}
          ...
          {this.state.expanded &&
            (node.oldFile?.binary || node.newFile?.binary ? (
              <div className="text-muted m-2">
                Binary files can't be rendered.
              </div>
  ...
  private setRenderDeleted = (): void =>
    this.setState(() => ({ renderDeleted: true }));
}
