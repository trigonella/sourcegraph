...
export const BrandLogo: React.FunctionComponent<Props> = ({
  ...
  const sourcegraphLogoUrl = `${assetsRoot}/img/sourcegraph${
    isLightTheme ? "-light" : ""
  }-head-logo.svg?v2`;
  const customBrandingLogoUrl =
    branding?.[isLightTheme ? "light" : "dark"]?.logo;
  return props.customIcon && props.customText ? (
    ...
    <img
      {...props}
      src={props.customIcon || customBrandingLogoUrl || sourcegraphLogoUrl}
    />
  );
};
