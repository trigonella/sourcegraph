...
const { gitHubToken, sourcegraphBaseUrl } = getConfig(
  "gitHubToken",
  "sourcegraphBaseUrl"
);
...
describe("e2e test suite", () => {
  ...
  describe("Core functionality", () => {
    test("Check settings are saved and applied", async () => {
      ...
      await driver.page.waitForSelector(
        ".test-global-alert .notices .global-alerts__alert",
        { visible: true }
      );
      await driver.page.evaluate((message: string) => {
        ...
        if (!element) {
          throw new Error(
            "No .test-global-alert .notices .global-alerts__alert element found"
          );
        }
    ...
    test("Check access tokens work (create, use and delete)", async () => {
      await driver.page.goto(
        sourcegraphBaseUrl + "/users/test/settings/tokens/new"
      );
      await driver.page.waitForSelector(
        ".test-create-access-token-description"
      );
        ...
        await driver.page.waitForFunction(
          () =>
            document.querySelector<HTMLInputElement>(
              ".test-access-token input[type=text]"
            )?.value
        )
      ...
      await driver.page.waitFor(
        (name: string) =>
          !document.querySelector(
            `[data-test-access-token-description="${name}"]`
          ),
        {},
        name
      );
    ...
    test("Check allowed usernames", async () => {
      await driver.page.goto(
        sourcegraphBaseUrl + "/users/test/settings/profile"
      );
      await driver.page.waitForSelector(
        ".test-user-settings-profile-page-username"
      );
      ...
      await driver.page.click(
        ".test-user-settings-profile-page-update-profile"
      );
      await driver.page.waitForSelector(
        ".test-user-settings-profile-page-alert-success",
        { visible: true }
      );

      await driver.page.goto(
        sourcegraphBaseUrl + `/users/${name}/settings/profile`
      );
      await driver.replaceText({
        selector: ".test-user-settings-profile-page-username",
        newText: "test",
  ...
  describe("External services", () => {
    test("External service add, edit, delete", async () => {
      ...
      await driver.page.goto(
        sourcegraphBaseUrl + "/site-admin/external-services"
      );
      await (
        await driver.page.waitForSelector(
      ...
      await driver.page.waitForSelector(
        ".test-update-external-service-button:not([disabled])",
        { visible: true }
      );

      await (
        await driver.page.waitForSelector(
          '.list-group-item[href="/site-admin/external-services"]',
          {
            visible: true
          }
        )
      ...
      await driver.page.waitFor(
        () =>
          !document.querySelector(
            '[data-test-external-service-name="test-github-test-2"]'
          )
      );
    });
    ...
    const testIfAwsCredentialsSet =
      awsSecretAccessKey &&
      awsAccessKeyID &&
      awsCodeCommitUsername &&
      awsCodeCommitPassword
        ? test
        : test.skip.bind(test);
    ...
    const testIfBBSCredentialsSet =
      bbsURL && bbsToken && bbsUsername ? test : test.skip.bind(test);
    ...
    testIfBBSCredentialsSet("Bitbucket Server", async () => {
      ...
      await driver.page.goto(
        sourcegraphBaseUrl + "/bbs/SOURCEGRAPH/jsonrpc2/-/blob/.travis.yml"
      );
      const blob = (await (
        await driver.page.waitFor(
          () =>
  ...
  describe("Visual tests", () => {
    test("Repositories list", async () => {
      await driver.page.goto(
        sourcegraphBaseUrl + "/site-admin/repositories?query=gorilla%2Fmux"
      );
      await driver.page.waitForSelector('a[href="/github.com/gorilla/mux"]', {
        visible: true
      });
    ...
    test("Search results repo", async () => {
      await driver.page.goto(
        sourcegraphBaseUrl +
          "/search?q=repo:%5Egithub.com/gorilla/mux%24&patternType=regexp"
      );
      await driver.page.waitForSelector('a[href="/github.com/gorilla/mux"]', {
        visible: true
      });
    ...
    test("Search results file", async () => {
      await driver.page.goto(
        sourcegraphBaseUrl +
          "/search?q=repo:%5Egithub.com/gorilla/mux%24+file:%5Emux.go%24&patternType=regexp"
      );
      await driver.page.waitForSelector('a[href="/github.com/gorilla/mux"]', {
        visible: true
      });
    ...
    test("Search visibility:private|public", async () => {
      const privateRepos = [
        "github.com/sourcegraph/test-test-private-repository"
      ];

      await driver.page.goto(
        sourcegraphBaseUrl + "/search?q=type:repo+visibility:private"
      );
      await driver.page.waitForFunction(
        () => document.querySelectorAll(".test-search-result").length >= 1
      );
      ...
      await driver.page.goto(
        sourcegraphBaseUrl + "/search?q=type:repo+visibility:public"
      );
      await driver.page.waitForFunction(
        () => document.querySelectorAll(".test-search-result").length > 1
      );
      ...
      const publicResults = await driver.page.evaluate(() =>
        [
          ...document.querySelectorAll(".etest-search-result-label")
        ].map(label => (label.textContent || "").trim())
      );
      expect(publicResults).not.toEqual(expect.arrayContaining(privateRepos));

      await driver.page.goto(
        sourcegraphBaseUrl + "/search?q=type:repo+visibility:any"
      );
      await driver.page.waitForFunction(
        () => document.querySelectorAll(".test-search-result").length > 1
      );
    ...
    test("Site admin overview", async () => {
      ...
      await driver.page.waitForSelector(".test-site-admin-overview-menu", {
        visible: true
      });
      await driver.page.waitForSelector(".test-product-certificate", {
        visible: true
      });
  ...
  describe("Theme switcher", () => {
    test("changes the theme", async () => {
      await driver.page.goto(
        sourcegraphBaseUrl + "/github.com/gorilla/mux/-/blob/mux.go"
      );
      await driver.page.waitForSelector(
      ...
      const getActiveThemeClasses = (): Promise<string[]> =>
        driver.page.evaluate(() =>
          [...document.querySelector(".theme")!.classList].filter(className =>
  ...
  describe("Repository component", () => {
    ...
    const getHoverContents = async (): Promise<string[]> => {
      ...
      return driver.page.evaluate(() =>
        ...
        [...document.querySelectorAll(".test-tooltip-content")].map(
          content => content.textContent || ""
        )
      );
    };
    const assertHoverContentContains = async (value: string): Promise<void> => {
      expect(await getHoverContents()).toEqual(
    ...
    describe("file tree", () => {
      ...
      test("expands directory on row click (no navigation)", async () => {
        ...
        await driver.page.waitForSelector(
          '.tree__row--selected [data-tree-path="websocket"]',
          {
            visible: true
          }
        );
        await driver.page.waitForSelector(
          '.tree__row--expanded [data-tree-path="websocket"]',
          {
            visible: true
          }
        );
      ...
      test("does navigation on directory row click", async () => {
        ...
        await driver.page.waitForSelector(".tree__row-label", {
          visible: true
        });
      ...
      test("selects the current file", async () => {
        ...
        await driver.page.waitForSelector(
          '.tree__row--active [data-tree-path="async.go"]',
          {
            visible: true
          }
        );
      ...
      test("shows partial tree when opening directory", async () => {
        ...
        expect(
          await driver.page.evaluate(
            () => document.querySelectorAll(".tree__row").length
          )
        ).toEqual(1);
      });

      test("responds to keyboard shortcuts", async () => {
        const assertNumberRowsExpanded = async (
          expectedCount: number
        ): Promise<void> => {
          expect(
            await driver.page.evaluate(
              () => document.querySelectorAll(".tree__row--expanded").length
            )
        ...
        await driver.page.waitForSelector(
          '.tree__row--selected [data-tree-path="diff"]',
          { visible: true }
        );
        ...
        await driver.page.waitForSelector(
          '.tree__row--expanded [data-tree-path="diff"]',
          { visible: true }
        );
        await driver.page.waitForSelector(
          '.tree__row [data-tree-path="diff/testdata"]',
          { visible: true }
        );
        await driver.page.keyboard.press("ArrowRight"); // arrow right (move to nested 'diff/testdata' directory)
        await driver.page.waitForSelector(
          '.tree__row--selected [data-tree-path="diff/testdata"]',
          {
            visible: true
          }
        );
        ...
        await driver.page.waitForSelector(
          '.tree__row--expanded [data-tree-path="diff/testdata"]',
          {
            visible: true
          }
        );
        ...
        await driver.page.waitForSelector(
          '.tree__row [data-tree-path="diff/testdata/empty.diff"]',
          {
            visible: true
          }
        );
    ...
    describe("symbol sidebar", () => {
      ...
      for (const symbolTest of listSymbolsTests) {
        test(symbolTest.name, async () => {
          ...
          await driver.page.waitForSelector(".test-symbol-name", {
            visible: true
          });

          const symbolNames = await driver.page.evaluate(() =>
            [...document.querySelectorAll(".test-symbol-name")].map(
      ...
      const navigateToSymbolTests = [
        {
          ...
          repoPath:
            "/github.com/sourcegraph/go-diff@3f415a150aec0685cb81b73cc201e762e075006d",
          filePath: "/tree/cmd",
        ...
        },
        {
          ...
          repoPath:
            "/github.com/sourcegraph/java-langserver@03efbe9558acc532e88f5288b4e6cfa155c6f2dc",
          filePath: "/tree/src/main/java/com/sourcegraph/common",
          symbolPath:
            "/blob/src/main/java/com/sourcegraph/common/Config.java#L14:20-14:26",
          skip: true
        },
        {
          name:
          ...
          repoPath:
            "/github.com/sourcegraph/appdash@ebfcffb1b5c00031ce797183546746715a3cfe87",
          filePath: "/tree/examples",
          symbolPath:
            "/blob/examples/cmd/webapp-opentracing/main.go#L26:6-26:10",
          skip: true
        },
        {
          name:
            "displays valid symbols at different file depths for Go (./sqltrace/sql.go)",
          repoPath:
            "/github.com/sourcegraph/appdash@ebfcffb1b5c00031ce797183546746715a3cfe87",
      ...
      for (const navigationTest of navigateToSymbolTests) {
        ...
        testFunc(navigationTest.name, async () => {
          const repoBaseURL =
            sourcegraphBaseUrl + navigationTest.repoPath + "/-";
          ...
          await (
            await driver.page.waitForSelector(
              `.test-symbol-link[href*="${navigationTest.symbolPath}"]`,
              {
                visible: true
              }
            )
          ...
          await driver.assertWindowLocation(
            repoBaseURL + navigationTest.symbolPath,
            true
          );
        });
      }
    ...
    describe("directory page", () => {
      it("shows a row for each file in the directory", async () => {
        ...
        await retry(async () =>
          assert.equal(
            await driver.page.evaluate(
              () => document.querySelectorAll(".test-tree-entry-file").length
            ),
            7
          )
        );
      ...
      test("shows commit information on a row", async () => {
        ...
        await driver.page.waitForSelector(".git-commit-node__message", {
          visible: true
        });
        await retry(async () =>
          expect(
            await driver.page.evaluate(
              () =>
                document.querySelectorAll(".git-commit-node__message")[3]
                  .textContent
            )
        ...
        await retry(async () =>
          expect(
            await driver.page.evaluate(() =>
              document
                .querySelectorAll(".git-commit-node-byline")[3]
                .textContent!.trim()
            )
        ...
        await retry(async () =>
          expect(
            await driver.page.evaluate(
              () =>
                document.querySelectorAll(".git-commit-node__oid")[3]
                  .textContent
            )
          ).toEqual("2083912")
        );
    ...
    describe("revision resolution", () => {
      test("shows clone in progress interstitial page", async () => {
        await driver.page.goto(
          sourcegraphBaseUrl + "/github.com/sourcegraphtest/AlwaysCloningTest"
        );
        await driver.page.waitForSelector(".hero-page__subtitle", {
          visible: true
        });
        await retry(async () =>
          expect(
            await driver.page.evaluate(
              () => document.querySelector(".hero-page__subtitle")!.textContent
            )
      ...
      test("resolves default branch when unspecified", async () => {
        await driver.page.goto(
          sourcegraphBaseUrl +
            "/github.com/sourcegraph/go-diff/-/blob/diff/diff.go"
        );
        await driver.page.waitForSelector("#repo-revision-popover", {
          visible: true
        });
        await retry(async () => {
          expect(
            await driver.page.evaluate(() =>
              document.querySelector(".test-revision")!.textContent!.trim()
            )
          ).toEqual("master");
        });
      ...
      test("updates revision with switcher", async () => {
        ...
        // Click "Tags" tab
        await driver.page.click(
          ".revisions-popover .tab-bar__tab:nth-child(2)"
        );
        await driver.page.waitForSelector('a.git-ref-node[href*="0.5.0"]', {
          visible: true
        });
        await driver.page.click('a.git-ref-node[href*="0.5.0"]');
        await driver.assertWindowLocation(
          "/github.com/sourcegraph/go-diff@v0.5.0/-/blob/diff/diff.go"
        );
      });
    });
      ...
      describe("Blob", () => {
        ...
        describe("find references", () => {
          test("opens widget and fetches local references", async function() {
            ...
            await driver.page.waitForSelector(
              ".panel__tabs-content .file-match-children",
              {
                visible: true
              }
            );
                ...
                await driver.page.evaluate(
                  () =>
                    document.querySelectorAll(
                      ".panel__tabs-content .file-match-children__item"
                    ).length
                )
              ).toEqual(
    ...
    describe.skip('godoc.org "Uses" links', () => {
      test("resolves standard library function", async () => {
        ...
        await driver.page.goto(
          sourcegraphBaseUrl + "/-/godoc/refs?def=Compare&pkg=bytes&repo="
        );
        await driver.assertWindowLocationPrefix(
          "/github.com/golang/go/-/blob/src/bytes/bytes_decl.go"
        );
      ...
      test("resolves external package function (from gorilla/mux)", async () => {
        ...
        await driver.assertWindowLocationPrefix(
          "/github.com/gorilla/mux/-/blob/mux.go"
        );
  ...
  describe("Search component", () => {
    test("can execute search with search operators", async () => {
      await driver.page.goto(
        sourcegraphBaseUrl + "/github.com/sourcegraph/go-diff"
      );
      ...
      await driver.page.goto(
        `${sourcegraphBaseUrl}/search?q=diff+${operatorsQuery}&patternType=regexp`
      );
      await driver.page.waitForSelector(".test-search-results-stats", {
        visible: true
      });
      await retry(async () => {
    ...
    test("renders results for sourcegraph/go-diff (no search group)", async () => {
      ...
      await retry(async () => {
        expect(await driver.page.evaluate(() => window.location.href)).toEqual(
          firstFileMatchHref
        );
      });
    });
    ...
    describe("multiple revisions per repository", () => {
      ...
      test("searches", async () => {
        ...
        await retry(async () => {
          const label = await driver.page.evaluate(
            () =>
              document.querySelector(".test-search-results-stats")!
                .textContent || ""
          );
          expect(label.includes("results")).toEqual(true);
        });
        ...
        expect(
          fileMatchHrefs.filter(href =>
            checkBranches.some(branch => href.includes(`@${branch}/`))
          )
        ).toEqual(
          checkBranches.map(
            branch =>
              `/github.com/sourcegraph/go-diff@${branch}/-/blob/diff/parse.go`
          )
        );
      });
    });
    ...
    test("accepts query for sourcegraph/jsonrpc2", async () => {
      ...
      await driver.page.keyboard.type(
        "test repo:sourcegraph/jsonrpc2@c6c7b9aa99fb76ee5460ccd3912ba35d419d493d"
      );
    ...
    test("regexp toggle appears and updates patternType query parameter when clicked", async () => {
      await driver.page.goto(
        sourcegraphBaseUrl + "/search?q=test&patternType=literal"
      );
      ...
      await driver.page.goto(
        sourcegraphBaseUrl + "/search?q=test&patternType=regexp"
      );
      // Wait for monaco query input to load to avoid race condition with the intermediate input
      await driver.page.waitForSelector("#monaco-query-input");
      await driver.page.waitForSelector(".test-regexp-toggle");
  ...
  describe("Search result type tabs", () => {
    test("Search results type tabs appear", async () => {
      ...
      await driver.page.waitForSelector(".test-search-result-type-tabs", {
        visible: true
      });
      await driver.page.waitForSelector(".test-search-result-tab--active", {
        visible: true
      });
      const tabs = await driver.page.evaluate(() =>
        [...document.querySelectorAll(".test-search-result-tab")].map(
          tab => tab.textContent
        )
      );
      ...
      const label = await driver.page.evaluate(
        () =>
          document.querySelector(".test-search-result-tab--active")!
            .textContent || ""
      );
    ...
    test.skip("Clicking search results tabs updates query and URL", async () => {
      for (const searchType of ["diff", "commit", "symbol", "repo"]) {
        await driver.page.waitForSelector(
          `.test-search-result-tab-${searchType}`
        );
  ...
  describe("Saved searches", () => {
    test("Save search from search results page", async () => {
      await driver.page.goto(sourcegraphBaseUrl + "/search?q=test");
      await driver.page.waitForSelector(".test-save-search-link", {
        visible: true
      });
      ...
      await driver.assertWindowLocation(
        "/users/test/searches/add?query=test&patternType=regexp"
      );

      await driver.page.waitForSelector(
        ".test-saved-search-form-input-description",
        { visible: true }
      );
      await driver.page.click(".test-saved-search-form-input-description");
      await driver.page.keyboard.type("test query");
      await driver.page.waitForSelector(
        ".test-saved-search-form-submit-button",
        { visible: true }
      );
      ...
      expect(
        await driver.page.evaluate(
          () =>
            document.querySelector(".test-saved-search-list-page-row-title")!
              .textContent
        )
      ).toEqual("test query");
    });
    test("Delete saved search", async () => {
      ...
      await driver.page.waitForSelector(".test-delete-saved-search-button", {
        visible: true
      });
      ...
      await driver.page.waitFor(
        () => !document.querySelector(".test-saved-search-list-page-row")
      );
      const nodes = await driver.page.evaluate(
        () =>
          document.querySelectorAll(".test-saved-search-list-page-row").length
    ...
    test("Save search from saved searches page", async () => {
      await driver.page.goto(sourcegraphBaseUrl + "/users/test/searches");
      await driver.page.waitForSelector(".test-add-saved-search-button", {
        visible: true
      });
    ...
    test("Edit saved search", async () => {
      await driver.page.goto(sourcegraphBaseUrl + "/users/test/searches");
      await driver.page.waitForSelector(".test-edit-saved-search-button", {
        visible: true
      });
      ...
      await driver.page.goto(sourcegraphBaseUrl + "/users/test/searches");
      await driver.page.waitForSelector(
        ".test-saved-search-list-page-row-title"
      );
  ...
  describe("Search statistics", () => {
    beforeEach(async () => {
      await driver.setUserSettings<Settings>({
        experimentalFeatures: { searchStats: true }
      });
    });
    afterEach(async () => {
      await driver.resetUserSettings();
    ...
    test("page", async () => {
      ...
      // the stats page).
      await driver.page.waitForSelector(
        '.global-navbar a.nav-link[href="/search"]'
      );
      assert.strictEqual(
        await driver.page.evaluate(
          () => document.querySelectorAll("#monaco-query-input").length
        ),
        0
      );
  ...
  describe("Interactive search mode (feature flagged)", () => {
    ...
    before(async () => {
      await driver.page.goto(
        sourcegraphBaseUrl + "/site-admin/global-settings"
      );
      await driver.page.waitForSelector(".test-settings-file .monaco-editor");
      ...
      await retry(
        async () => {
          ...
          assert.notStrictEqual(
            await driver.page.evaluate(
              () => document.querySelectorAll(".test-search-mode-toggle").length
            ),
            0,
    ...
    test("Interactive search mode component appears", async () => {
      ...
      expect(
        await driver.page.evaluate(() => {
          const toggles = document.querySelectorAll(".test-search-mode-toggle");
    ...
    test("Interactive search mode filter buttons", async () => {
      ...
      await driver.page.waitForSelector(".test-add-filter-row", {
        visible: true
      });
      ...
      await driver.assertWindowLocation(
        "/search?q=repo:%22gorilla%22&patternType=literal"
      );
      ...
      await driver.assertWindowLocation(
        "/search?q=repo:%22gorilla/mux%22&patternType=literal"
      );
      ...
      await driver.page.keyboard.press("Enter");
      await driver.assertWindowLocation(
        "/search?q=repo:%22gorilla/mux%22+file:%22README%22&patternType=literal"
      );

      // Delete filter
      await driver.page.goto(
        sourcegraphBaseUrl + "/search?q=repo:gorilla/mux&patternType=literal"
      );
      await driver.page.waitForSelector(".test-filter-input__delete-button", {
        visible: true
      });
    ...
    test("Interactive search mode updates query when searching from directory page", async () => {
      await driver.page.goto(
        sourcegraphBaseUrl + "/github.com/sourcegraph/jsonrpc2"
      );
      await driver.page.waitForSelector(".filter-input");
      const filterInputValue = () =>
        driver.page.evaluate(() => {
          const filterInput = document.querySelector<HTMLButtonElement>(
            ".filter-input__button-text"
          );
          return filterInput ? filterInput.textContent : null;
        });
      assert.strictEqual(
        await filterInputValue(),
        "repo:^github\\.com/sourcegraph/jsonrpc2$"
      );
    });
    ...
    test("Interactive search mode filter dropdown and finite-option filter inputs", async () => {
      ...
      await driver.assertWindowLocation(
        "/search?q=fork:%22no%22+test&patternType=literal"
      );
      // Edit filter
      await driver.page.waitForSelector(".filter-input");
      ...
      await driver.page.click(".test-confirm-filter-button");
      await driver.assertWindowLocation(
        "/search?q=fork:%22only%22+test&patternType=literal"
      );
  ...
  describe("Structural search toggle", () => {
    test("Clicking toggle turns on structural search", async () => {
      ...
      await driver.assertWindowLocation(
        "/search?q=test&patternType=structural"
      );
    });
