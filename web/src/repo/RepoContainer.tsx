...
export interface RepoContainerContext
  ...
  onDidUpdateExternalLinks: (
    externalLinks: GQL.IExternalLink[] | undefined
  ) => void;
...
export interface RepoContainerRoute
  extends RouteDescriptor<RepoContainerContext> {}

const RepoPageNotFound: React.FunctionComponent = () => (
  <HeroPage
    icon={MapSearchIcon}
    title="404: Not Found"
    subtitle="The repository page was not found."
  />
);
...
> {
  ...
  public componentDidMount(): void {
    ...
    this.subscriptions.add(
      this.revResolves.subscribe(resolvedRevisionOrError =>
        this.setState({ resolvedRevisionOrError })
      )
    );
    ...
    this.subscriptions.add(
      parsedRouteChanges.subscribe(({ repoName, revision, rawRevision }) => {
        ...
        const query = searchQueryForRepoRevision(
          repoName,
          this.props.globbing,
          revision
        );
    ...
    this.subscriptions.add(
      this.repositoryUpdates.subscribe(update =>
        this.setState(({ repoOrError }) => ({
          repoOrError: { ...repoOrError, ...update } as GQL.IRepository
        }))
      )
    );
          ...
          if (this.props.splitSearchModes && this.props.interactiveSearchMode) {
            const filters: FiltersToTypeAndValue = {
              [uniqueId("repo")]: {
                ...
                value: repoFilterForRepoRevision(
                  repoName,
                  this.props.globbing,
                  revision
                ),
            ...
            let query = searchQueryForRepoRevision(
              repoName,
              this.props.globbing,
              revision
            );
      ...
      <div className="repo-container test-repo-container w-100 d-flex flex-column">
        <RepoHeader
          ...
          onLifecyclePropsChange={
            this.onRepoHeaderContributionsLifecyclePropsChange
          }
        />
  ...
  private onDidUpdateRepository = (update: Partial<GQL.IRepository>): void =>
    this.repositoryUpdates.next(update);

  private onDidUpdateExternalLinks = (
    externalLinks: GQL.IExternalLink[] | undefined
  ): void => this.setState({ externalLinks });

  private onResolvedRevOrError = (
    value: ResolvedRevision | ErrorLike | undefined
  ): void => this.revResolves.next(value);
...
function parseURLPath(
  repoRevisionAndRest: string
): ParsedRepoRevision & { rest?: string } {
