...
export const GitReferenceNode: React.FunctionComponent<GitReferenceNodeProps> = ({
  ...
  const mostRecentSig =
    ...
    node.target.commit.committer.date > node.target.commit.author.date
      ? node.target.commit.committer
  ...
  return (
    <LinkOrSpan
      key={node.id}
      className="git-ref-node list-group-item"
    ...
    >
      ...
      {behindAhead && (
        <small className="text-muted">
          {numberWithCommas(behindAhead.behind)} behind,{" "}
          {numberWithCommas(behindAhead.ahead)} ahead
        </small>
      )}
      {children}
    </LinkOrSpan>
...
export const queryGitReferences = memoizeObservable(
  (args: {
    ...
    queryGraphQL(
      ...
      `,
      {
        ...args,
        withBehindAhead:
          args.withBehindAhead !== undefined
            ? args.withBehindAhead
            : args.type === GQL.GitRefType.GIT_BRANCH
      }
    ).pipe(
      map(({ data, errors }) => {
  ...
  args =>
    `${args.repo}:${String(args.first)}:${String(args.query)}:${args.type}`
);
