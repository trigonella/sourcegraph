...
class UpdateMirrorRepositoryActionContainer extends React.PureComponent<
...
> {
    ...
    if (this.props.repo.mirrorInfo.cloneInProgress) {
      ...
      title = (
        <>
          ...
          {updateSchedule && (
            <div>
              Next scheduled update <Timestamp date={updateSchedule.due} />{" "}
              (position {updateSchedule.index + 1} out of {updateSchedule.total}{" "}
              in the schedule)
            </div>
          )}
          {this.props.repo.mirrorInfo.updateQueue &&
            !this.props.repo.mirrorInfo.updateQueue.updating && (
              <div>
                Queued for update (position{" "}
                {this.props.repo.mirrorInfo.updateQueue.index + 1} out of{" "}
      ...
      if (!updateSchedule) {
        description =
          "This repository is automatically updated when accessed by a user.";
      } else {
        description =
      ...
      description =
        "This repository has not yet been cloned from its remote repository.";
  ...
  private updateMirrorRepository = async (): Promise<void> => {
    await updateMirrorRepository({
      repository: this.props.repo.id
    }).toPromise();
    this.props.onDidUpdateRepository();
  };
}
...
> {
  public state: CheckMirrorRepositoryConnectionActionContainerState = {
    loading: false
  };
  ...
  public render(): JSX.Element | null {
    return (
      <BaseActionContainer
        ...
        description={
          <span>
            Diagnose problems cloning or updating from the remote repository.
          </span>
        }
        action={
          <button
          ...
          <>
            ...
            {this.state.loading && (
              <div className="alert alert-primary action-container__alert">
                <LoadingSpinner className="icon-inline" /> Checking
                connection...
              </div>
            )}
            {this.state.result &&
              (this.state.result.error === null ? (
                <div className="alert alert-success action-container__alert">
                  <CheckIcon className="icon-inline" /> The remote repository is
                  reachable.
                </div>
              ) : (
  ...
  private checkMirrorRepositoryConnection = (): void =>
    this.checkRequests.next();
}
...
export class RepoSettingsMirrorPage extends React.PureComponent<Props, State> {
  ...
  public render(): JSX.Element | null {
    return (
      <div className="repo-settings-mirror-page">
        ...
        {this.state.error && (
          <ErrorAlert error={this.state.error} history={this.props.history} />
        )}
          ...
          <div className="alert alert-info repo-settings-mirror-page__troubleshooting">
            ...
            <ul className="repo-settings-mirror-page__steps">
              <li className="repo-settings-mirror-page__step">
                Inspect the <strong>Check connection</strong> error log output
                to see why the remote repository is not reachable.
              </li>
              <li className="repo-settings-mirror-page__step">
                <code>
                  <strong>
                ...
                for how to provide an SSH <code>known_hosts</code> file with the
                remote host's SSH host key.
              </li>
              <li className="repo-settings-mirror-page__step">
                Consult{" "}
                <Link to="/help/admin/repo/add">
                  Sourcegraph repositories documentation
                </Link>{" "}
                for resolving other authentication issues (such as HTTPS
  ...
  private onDidUpdateReachability = (reachable: boolean | undefined): void =>
