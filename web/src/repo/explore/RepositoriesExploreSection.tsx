...
export class RepositoriesExploreSection extends React.PureComponent<
  ...
  State
> {
  private static QUERY_REPOSITORIES_ARGS: { first: number } & Pick<
    GQL.IRepositoriesOnQueryArguments,
    "names"
  > = {
  ...
  public componentDidMount(): void {
    this.subscriptions.add(
      queryRepositories(RepositoriesExploreSection.QUERY_REPOSITORIES_ARGS)
        ...
        .subscribe(repositoriesOrError =>
          this.setState({ repositoriesOrError })
        )
    );
  }
  ...
  public render(): JSX.Element | null {
    const repositoriesOrError:
      | (typeof LOADING | GQL.IRepository)[]
      | ErrorLike =
      this.state.repositoriesOrError === LOADING
        ? new Array(
            RepositoriesExploreSection.QUERY_REPOSITORIES_ARGS.first
          ).fill(LOADING)
