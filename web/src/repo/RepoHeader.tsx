...
import {
  displayRepoName,
  splitPath
} from "../../../shared/src/components/RepoFileLink";
...
class RepoHeaderContributionStore {
  constructor(
    ...
    private setState: (
      callback: (
        prevState: RepoHeaderContributionsProps
      ) => RepoHeaderContributionsProps
    ) => void
  ) {}
  ...
  public readonly props: RepoHeaderContributionsLifecycleProps = {
    repoHeaderContributionsLifecycleProps: {
      ...
      onRepoHeaderContributionRemove: this.onRepoHeaderContributionRemove.bind(
        this
      )
    }
  };
...
export interface RepoHeaderActionButton
  extends ActionButtonDescriptor<RepoHeaderContext> {}
...
export class RepoHeader extends React.PureComponent<Props, State> {
  ...
  public render(): JSX.Element | null {
    const navActions = this.state.repoHeaderContributions.filter(
      ({ position }) => position === "nav"
    );
    const leftActions = this.state.repoHeaderContributions.filter(
      ({ position }) => position === "left"
    );
    const rightActions = this.state.repoHeaderContributions.filter(
      ({ position }) => position === "right"
    );

    const [repoDirectory, repoBase] = splitPath(
      displayRepoName(this.props.repo.name)
    );
        ...
        <ul className="navbar-nav">
          {this.props.actionButtons.map(
            ({ condition = () => true, label, tooltip, icon: Icon, to }) =>
              condition(context) && (
                <li
                  className="nav-item repo-header__action-list-item"
                  key={label}
                >
          ...
          {this.props.repo.viewerCanAdminister && (
            <li className="nav-item repo-header__action-list-item">
              <LinkOrButton
                to={`/${this.props.repo.name}/-/settings`}
  ...
  private repoHeaderContributionStore = new RepoHeaderContributionStore(
