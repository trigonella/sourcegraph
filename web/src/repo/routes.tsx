...
const RepositoryCommitsPage = lazyComponent(
  () => import("./commits/RepositoryCommitsPage"),
  "RepositoryCommitsPage"
);
const FilePathBreadcrumb = lazyComponent(
  () => import("./FilePathBreadcrumb"),
  "FilePathBreadcrumb"
);
const RepoRevisionSidebar = lazyComponent(
  ...
  ...["", "/-/:objectType(blob|tree)/:filePath*"].map(routePath => ({
    ...
    render: ({
      ...
      if (match.params.objectType && filePath.replace(/\/+$/g, "") === "") {
        return (
          <Redirect to={toRepoURL({ repoName, revision: context.revision })} />
        );
      }
