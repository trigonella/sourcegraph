...
const TreeEntriesSection: React.FunctionComponent<{
  ...
  entries.length > 0 ? (
    <section className="tree-page__section test-tree-entries">
      ...
      <div
        className={
          entries.length > MIN_ENTRIES_FOR_COLUMN_LAYOUT
            ? "tree-page__entries--columns"
            : undefined
        }
      >
...
const fetchTreeCommits = memoizeObservable(
  ...
  args =>
    `${args.repo}:${args.revspec}:${String(args.first)}:${String(
      args.filePath
    )}:${String(args.after)}`
);
...
export const TreePage: React.FunctionComponent<Props> = ({
  ...
  const queryCommits = useCallback(
    (args: { first?: number }): Observable<GQL.IGitCommitConnection> => {
      const after: string | undefined = showOlderCommits
        ? undefined
        : formatISO(subYears(Date.now(), 1));
  ...
  const TotalCountSummary: React.FunctionComponent<{ totalCount: number }> = ({
    ...
    <div className="mt-2">
      {showOlderCommits ? (
        ...
        <>
          {totalCount} {pluralize("commit", totalCount)} in this tree in the
          past year.
          <br />
          <button
            type="button"
    ...
    <div className="tree-page">
      ...
      {treeOrError === undefined ? (
        ...
        /not a directory/i.test(treeOrError.message) ? (
          <Redirect
            to={toPrettyBlobURL({ repoName, revision, commitID, filePath })}
          />
        ) : (
        ...
        <>
              ...
              <>
                ...
                <div className="btn-group mb-3">
                  ...
                  <Link
                    ...
                    to={
                      revision
                        ? `/${repoName}/-/compare/...${encodeURIComponent(
                            revision
                          )}`
                        : `/${repoName}/-/compare`
                    }
                  ...
                  <Link
                    className="btn btn-secondary"
                    to={`/${repoName}/-/stats/contributors`}
                  >
          ...
          <div className="tree-page__section">
            ...
            <FilteredConnection<
              GQL.IGitCommit,
              Pick<GitCommitNodeProps, "className" | "compact">
            >
              ...
              updateOnChange={`${repoName}:${revision}:${filePath}:${String(
                showOlderCommits
              )}`}
              defaultFirst={7}
              useURLQuery={false}
