...
const fetchRepositoryCommits = memoizeObservable(
  (
    args: RevisionSpec & { repo: GQL.ID; first?: number; query?: string }
  ): Observable<GQL.IGitCommitConnection> =>
    queryGraphQL(
      gql`
        query RepositoryGitCommit(
          $repo: ID!
          $first: Int
          $revision: String!
          $query: String
...
const GitReferencePopoverNode: React.FunctionComponent<GitRefPopoverNodeProps> = ({
  ...
  if (currentRevision) {
    isCurrent =
      node.name === currentRevision || node.abbrevName === currentRevision;
  } else {
...
const GitCommitNode: React.FunctionComponent<GitCommitNodeProps> = ({
  ...
  return (
    <li
      key={node.oid}
      className="connection-popover__node revisions-popover-git-commit-node"
    >
      <Link
        to={replaceRevisionInURL(
          location.pathname + location.search + location.hash,
          node.oid
        )}
      ...
      >
        <code
          className="revisions-popover-git-commit-node__oid"
          title={node.oid}
        >
          {node.abbreviatedOID}
        </code>
        <span className="revisions-popover-git-commit-node__message">
  ...
  private static TABS: RevisionsPopoverTab[] = [
    {
      ...
      noun: "branch",
      pluralNoun: "branches",
      type: GQL.GitRefType.GIT_BRANCH
    },
    {
      id: "tags",
      label: "Tags",
        ...
        >
          {RevisionsPopover.TABS.map(tab =>
            tab.type ? (
              <FilteredConnection<
                GQL.IGitRef,
                Omit<GitRefPopoverNodeProps, "node">
              >
                key={tab.id}
