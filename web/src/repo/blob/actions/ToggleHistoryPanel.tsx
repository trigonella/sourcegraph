...
export class ToggleHistoryPanel extends React.PureComponent<{
   ...
   * the given value.
   */
  private static locationWithVisibility(
    location: H.Location,
    visible: boolean
    ...
    return {
      hash:
        toPositionOrRangeHash({ range: lprToRange(hash) }) +
        toViewStateHashComponent(hash.viewState)
    };
  }
    ...
    this.subscriptions.add(
      this.toggles.subscribe(() => {
        ...
        this.props.history.push(
          ToggleHistoryPanel.locationWithVisibility(
            this.props.location,
            !visible
          )
        );
