...
export class BlobPanel extends React.PureComponent<Props> {
  ...
  constructor(props: Props) {
    ...
    const entryForViewProviderRegistration = <
      P extends TextDocumentPositionParams
    >(
      id: string,
      title: string,
      priority: number,
      registry: TextDocumentLocationProviderRegistry<P>,
      extraParameters?: Pick<
        P,
        Exclude<keyof P, keyof TextDocumentPositionParams>
      >
    ): Entry<
      PanelViewProviderRegistrationOptions,
      ProvidePanelViewSignature
    > => ({
      ...
      provider: from(
        this.props.extensionsController.services.viewer.activeViewerUpdates
      ).pipe(
        map(activeEditor =>
