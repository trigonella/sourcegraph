...
function fetchBlobCacheKey(
  parsed: ParsedRepoURI & { isLightTheme: boolean; disableTimeout: boolean }
): string {
  return (
    makeRepoURI(parsed) +
    String(parsed.isLightTheme) +
    String(parsed.disableTimeout)
  );
}
...
export class BlobPage extends React.PureComponent<Props, State> {
    ...
    this.subscriptions.add(
      combineLatest([
        this.propsUpdates.pipe(
          map(props =>
            pick(props, "repoName", "commitID", "filePath", "isLightTheme")
          ),
          ...
          <RepoHeaderContributionPortal
            ...
            element={
              <ToggleLineWrap
                ...
                onDidUpdate={this.onDidUpdateLineWrap}
              />
            }
            repoHeaderContributionsLifecycleProps={
              this.props.repoHeaderContributionsLifecycleProps
  ...
  private onDidUpdateLineWrap = (value: boolean): void =>
    this.setState({ wrapCode: value });

  private onExtendHighlightingTimeoutClick = (): void =>
    this.extendHighlightingTimeoutClicks.next();

  private getPageTitle(): string {
    const repoNameSplit = this.props.repoName.split("/");
    const repoString =
      repoNameSplit.length > 2
        ? repoNameSplit.slice(1).join("/")
        : this.props.repoName;
