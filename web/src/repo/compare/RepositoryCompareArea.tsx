 ...
 */
export class RepositoryCompareArea extends React.Component<
  RepositoryCompareAreaProps,
  State
> {
  ...
  private nextRepositoryCompareAreaElement = (
    element: HTMLElement | null
  ): void => this.repositoryCompareAreaElements.next(element);
  ...
  constructor(props: RepositoryCompareAreaProps) {
    ...
    >({
      ...
      hoverOverlayRerenders: this.componentUpdates.pipe(
        withLatestFrom(
          this.hoverOverlayElements,
          this.repositoryCompareAreaElements
        ),
      ...
      getHover: hoveredToken =>
        getHover(
          this.getLSPTextDocumentPositionParams(hoveredToken),
          this.props
        ),
  ...
  public shouldComponentUpdate(
    nextProps: Readonly<RepositoryCompareAreaProps>,
  ...
  public render(): JSX.Element | null {
    ...
    if (this.props.match.params.spec) {
      spec = parseComparisonSpec(
        decodeURIComponent(this.props.match.params.spec)
      );
    }
    ...
    return (
      <div
        className="repository-compare-area container"
        ref={this.nextRepositoryCompareAreaElement}
      >
        <RepoHeaderContributionPortal
          position="nav"
          element={
            <RepoHeaderBreadcrumbNavItem key="compare">
              Compare
            </RepoHeaderBreadcrumbNavItem>
          }
          repoHeaderContributionsLifecycleProps={
  ...
  private onUpdateComparisonSpec = (
    newBaseSpec: string,
    newHeadSpec: string
  ): void => {
    this.props.history.push(
      `/${this.props.repo.name}/-/compare${
        newBaseSpec || newHeadSpec
          ? `/${escapeRevspecForURL(newBaseSpec || "")}...${escapeRevspecForURL(
              newHeadSpec || ""
            )}`
          : ""
      }`
...
function parseComparisonSpec(
  spec: string
): { base: string | null; head: string | null } | null {
  if (!spec.includes("...")) {
    return null;
  }
