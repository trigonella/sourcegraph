...
export class GitCommitNode extends React.PureComponent<
  GitCommitNodeProps,
  State
> {
  public state: State = {
  ...
  public render(): JSX.Element | null {
        ...
        {this.props.compact && (
          <small className="text-muted git-commit-node__message-timestamp">
            <Timestamp
              noAbout={true}
              date={
                this.props.node.committer
                  ? this.props.node.committer.date
                  : this.props.node.author.date
              }
            />
          </small>
        )}
    ...
    const oidElement = (
      <code className="git-commit-node__oid">
        {this.props.node.abbreviatedOID}
      </code>
    );
    return (
      <div
        ...
        className={`git-commit-node ${
          this.props.compact ? "git-commit-node--compact" : ""
                ...
                {!this.props.showSHAAndParentsRow && (
                  <div className="btn-group btn-group-sm mr-2" role="group">
                    ...
                    <button
                      ...
                      data-tooltip={
                        this.state.flashCopiedToClipboardMessage
                          ? "Copied!"
                          : "Copy full SHA"
                      }
                    >
  ...
  private toggleShowCommitMessageBody = (): void => {
    ...
    this.setState(previousState => ({
      showCommitMessageBody: !previousState.showCommitMessageBody
    }));
  };
