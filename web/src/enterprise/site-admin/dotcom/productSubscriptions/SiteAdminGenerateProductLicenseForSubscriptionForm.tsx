...
export const SiteAdminGenerateProductLicenseForSubscriptionForm: React.FunctionComponent<Props> = ({
  ...
  const onUserCountChange = useCallback<
    React.ChangeEventHandler<HTMLInputElement>
  >(
    event =>
      setFormData(formData => ({
        ...formData,
        userCount: event.currentTarget.valueAsNumber
      })),
    []
  );
  ...
  const setValidDays = useCallback((validDays: number | null): void => {
    setFormData(formData => ({
      ...
      validDays,
      expiresAt:
        validDays !== null ? addDaysAndRoundToEndOfDay(validDays || 0) : null
    }));
  }, []);
  const onValidDaysChange = useCallback<
    React.ChangeEventHandler<HTMLInputElement>
  >(
    event =>
      setValidDays(
  ...
  const dismissAlert = useCallback(
    (): void => setFormData(EMPTY_FORM_DATA),
    []
  );
  ...
  const disableForm = Boolean(
    creation === LOADING || (creation && !isErrorLike(creation))
  );
  ...
  return (
    <div className="site-admin-generate-product-license-for-subscription-form">
      {creation && !isErrorLike(creation) && creation !== LOADING ? (
        ...
        <Form onSubmit={onSubmit}>
          <div className="form-group">
            <label htmlFor="site-admin-create-product-subscription-page__tags">
              Tags
            </label>
            <input
              type="text"
            ...
            <small className="form-text text-muted">
              Tags restrict a license. Recognized tags: <code>true-up</code>{" "}
              (allow user creation beyond limit), <code>starter</code>{" "}
              (Enterprise Starter), <code>trial</code> (shows a "trial"
              indicator), <code>dev</code> (shows a "for development only"
              indicator). Separate multiple with commas and no spaces (like{" "}
              <code>starter,trial</code>
              ). Order does not matter.
            </small>
            <small className="form-text text-muted mt-2">
              To find the exact license tags used for licenses generated by
              self-service payment, view the <code>licenseTags</code> product
              plan metadata item in the billing system.
            </small>
          </div>
          <div className="form-group">
            <label htmlFor="site-admin-create-product-subscription-page__userCount">
              Users
            </label>
            <input
              type="number"
      ...
      {isErrorLike(creation) && (
        <ErrorAlert className="mt-3" error={creation} history={history} />
      )}
    </div>
  );
};
