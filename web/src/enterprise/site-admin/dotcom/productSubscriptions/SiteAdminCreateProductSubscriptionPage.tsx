  ...
  const [onSubmit, createdSubscription] = useEventObservable(
    useCallback(
      (
        ...
        Pick<GQL.IProductSubscription, "urlForSiteAdmin"> | "saving" | ErrorLike
      > =>
        submits.pipe(
    ...
    <>
      {createdSubscription &&
        ...
        createdSubscription.urlForSiteAdmin && (
          <Redirect to={createdSubscription.urlForSiteAdmin} />
        )}
      <li className="list-group-item py-2">
        <div className="d-flex align-items-center justify-content-between">
          <div>
            <Link to={`/users/${props.node.username}`}>
              {props.node.username}
            </Link>{" "}
