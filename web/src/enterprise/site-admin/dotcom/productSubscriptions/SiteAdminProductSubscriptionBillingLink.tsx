...
interface Props {
  ...
  productSubscription: Pick<
    GQL.IProductSubscription,
    "id" | "urlForSiteAdminBilling"
  >;
...
export const SiteAdminProductSubscriptionBillingLink: React.FunctionComponent<Props> = ({
  ...
  const onLinkBillingClick = useCallback(() => {
    const billingSubscriptionID = window.prompt(
      "Enter new Stripe billing subscription ID:",
      "sub_ABCDEF12345678"
    );
  ...
  const productSubscriptionHasLinkedBilling =
    productSubscription.urlForSiteAdminBilling !== null;
  return (
    <div className="site-admin-product-subscription-billing-link">
      <div className="d-flex align-items-center">
        {productSubscription.urlForSiteAdminBilling && (
          <a
            href={productSubscription.urlForSiteAdminBilling}
        ...
        <button
          ...
          onClick={
            productSubscriptionHasLinkedBilling
              ? onUnlinkBillingClick
              : onLinkBillingClick
          }
...
function setProductSubscriptionBilling(
  ...
  return mutateGraphQL(
    gql`
      mutation SetProductSubscriptionBilling(
        $id: ID!
    ...
    map(({ data, errors }) => {
      if (
        !data ||
        !data.dotcom ||
        !data.dotcom.setProductSubscriptionBilling ||
