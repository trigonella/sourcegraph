...
interface SiteAdminCustomerNodeProps {
  node: Pick<
    GQL.IUser,
    "id" | "username" | "displayName" | "urlForSiteAdminBilling"
  >;
  onDidUpdate: () => void;
}
 ...
 */
const SiteAdminCustomerNode: React.FunctionComponent<SiteAdminCustomerNodeProps> = ({
  node,
  onDidUpdate
}) => (
  <li className="list-group-item py-2">
    <div className="d-flex align-items-center justify-content-between">
      <span className="mr-3">
        <AccountName
          account={node}
          link={`${userURL(node.username)}/subscriptions`}
        />
      </span>
...
class FilteredSiteAdminCustomerConnection extends FilteredConnection<
  ...
  Pick<
    SiteAdminCustomerNodeProps,
    Exclude<keyof SiteAdminCustomerNodeProps, "node">
  >
> {}
