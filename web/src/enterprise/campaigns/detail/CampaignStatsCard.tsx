...
interface CampaignStatsCardProps
  extends Pick<CampaignFields["changesets"], "stats"> {}
...
export const CampaignStatsCard: React.FunctionComponent<CampaignStatsCardProps> = ({
  ...
  const percentComplete =
    stats.total === 0
      ? 0
      : (((stats.closed + stats.merged) / stats.total) * 100).toFixed(0);
    ...
    <div className="card mt-2">
      <div className="card-body">
        <div className="d-flex justify-content-between align-items-center">
          ...
          <div className="d-flex align-items-center">
            <SourceBranchIcon className="icon-inline text-muted mr-2" />{" "}
            {stats.unpublished} unpublished
          </div>
          <div className="d-flex align-items-center">
            <SourceBranchIcon className="icon-inline text-success mr-2" />{" "}
            {stats.open} open
          </div>
          <div className="d-flex align-items-center">
            <SourceMergeIcon className="icon-inline text-merged mr-2" />{" "}
            {stats.merged} merged
          </div>
          <div className="d-flex align-items-center">
            <SourceBranchIcon className="icon-inline text-danger mr-2" />{" "}
            {stats.closed} closed
          </div>
        </div>
      </div>
