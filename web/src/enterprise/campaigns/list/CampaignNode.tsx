...
import {
  changesetExternalStateIcons,
  changesetExternalStateColorClasses
} from "../detail/changesets/presentation";
...
export const CampaignNode: React.FunctionComponent<CampaignNodeProps> = ({
  ...
  const OpenChangesetIcon =
    changesetExternalStateIcons[ChangesetExternalState.OPEN];
  const ClosedChangesetIcon =
    changesetExternalStateIcons[ChangesetExternalState.CLOSED];
  const MergedChangesetIcon =
    changesetExternalStateIcons[ChangesetExternalState.MERGED];
  return (
    <li className="list-group-item">
      <div className="d-flex align-items-center p-2">
        <CampaignsIcon
          className={classNames(
            "icon-inline mr-2 flex-shrink-0 align-self-stretch",
            campaignIconClass
          )}
        ...
        <div className="flex-grow-1 campaign-node__content">
          ...
          <Markdown
            className={classNames(
              "text-truncate",
              !node.description && "text-muted font-italic"
            )}
            dangerousInnerHTML={
              node.description
                ? renderMarkdown(node.description, { plainText: true })
                : "No description"
            }
            history={history}
          />
        ...
        <div className="flex-shrink-0" data-tooltip="Open changesets">
          ...
          <OpenChangesetIcon
            className={`text-${changesetExternalStateColorClasses.OPEN} ml-1 mr-2`}
          />
        </div>
        <div className="flex-shrink-0" data-tooltip="Closed changesets">
          {node.changesets.stats.closed}{" "}
          <ClosedChangesetIcon
            className={`text-${changesetExternalStateColorClasses.CLOSED} ml-1 mr-2`}
          />
        </div>
        <div className="flex-shrink-0" data-tooltip="Merged changesets">
          {node.changesets.stats.merged}{" "}
          <MergedChangesetIcon
            className={`text-${changesetExternalStateColorClasses.MERGED} ml-1`}
          />
        </div>
      </div>
    </li>
