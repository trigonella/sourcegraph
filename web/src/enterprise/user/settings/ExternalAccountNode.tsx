...
export class ExternalAccountNode extends React.PureComponent<
  ExternalAccountNodeProps,
  ExternalAccountNodeState
> {
    ...
    this.subscriptions.add(
      this.deletes
        .pipe(
          filter(() =>
            window.confirm(
              "Really delete the association with this external account?"
            )
          ),
          switchMap(() =>
            deleteExternalAccount(this.props.node.id).pipe(
              ...
              startWith<Pick<ExternalAccountNodeState, "deletionOrError">>({
      ...
      <li className="list-group-item py-2">
        <div className="d-flex align-items-center justify-content-between">
          ...
          <div className="text-nowrap">
            ...
            {this.props.node.refreshURL && (
              <a
                className="btn btn-secondary"
                href={this.props.node.refreshURL}
              >
                Refresh
              </a>
            )}{" "}
        ...
        {this.state.showData && (
          <pre className="p-2 mt-2 mb-4">
            <small>
              {JSON.stringify(this.props.node.accountData, null, 2)}
            </small>
          </pre>
        )}
      </li>
  ...
  private toggleShowData = (): void =>
    this.setState(previous => ({ showData: !previous.showData }));
}
