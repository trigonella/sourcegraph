...
import {
  formatUserCount,
  mailtoSales
} from "../../productSubscription/helpers";
...
type PreviewInvoiceOrError =
  | GQL.IProductSubscriptionPreviewInvoice
  | null
  | typeof LOADING
  | ErrorLike;

const previewInvoiceValidity = (
  previewInvoice: PreviewInvoiceOrError
): PaymentValidity =>
  previewInvoice === null ||
...
const undefinedIsLoading = <T extends any>(
  value: T | undefined
): T | typeof LOADING => (value === undefined ? LOADING : value);
        ...
        {previewInvoice === LOADING ? (
          ...
          <>
            ...
            {previewInvoice.isDowngradeRequiringManualIntervention ? (
              ...
              !isEqual(
                previewInvoice.beforeInvoiceItem,
                previewInvoice.afterInvoiceItem
              ) && (
                <div className="mb-2">
                  Amount due: ${numberWithCommas(previewInvoice.price / 100)}
                </div>
              )
            )}
          </>
          ...
          <>
            ...
            {formatDistanceStrict(
              parseISO(previewInvoice.afterInvoiceItem.expiresAt),
              Date.now()
            )}{" "}
