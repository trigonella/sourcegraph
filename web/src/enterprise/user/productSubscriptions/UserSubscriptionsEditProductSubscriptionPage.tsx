...
interface Props
  extends RouteComponentProps<{ subscriptionUUID: string }>,
    ThemeProps {
...
type ProductSubscription = Pick<
  GQL.IProductSubscription,
  "id" | "name" | "invoiceItem" | "url"
>;
          ...
          <ProductSubscriptionForm
            ...
            initialValue={
              productSubscription.invoiceItem
                ? {
                    billingPlanID:
                      productSubscription.invoiceItem.plan.billingPlanID,
  ...
  return mutateGraphQL(
    ...
    map(({ data, errors }) => {
      if (
        !data ||
        !data.dotcom ||
        !data.dotcom.updatePaidProductSubscription ||
        (errors && errors.length > 0)
      ) {
        throw createAggregateError(errors);
      }
