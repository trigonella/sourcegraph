...
interface State {
  ...
  extensionsOrError:
    | typeof LOADING
    | GQL.IRegistryExtensionConnection
    | ErrorLike;
}
 ...
 */
export class ExtensionsExploreSection extends React.PureComponent<
  Props,
  State
> {
    ...
    this.subscriptions.add(
      queryExtensions({
        ...
        prioritizeExtensionIDs:
          ExtensionsExploreSection.QUERY_EXTENSIONS_ARG_EXTENSION_IDS
      })
  ...
  public render(): JSX.Element | null {
    const extensionsOrError:
      | (typeof LOADING | GQL.IRegistryExtension)[]
      | ErrorLike =
      this.state.extensionsOrError === LOADING
        ? new Array(ExtensionsExploreSection.QUERY_EXTENSIONS_ARG_FIRST).fill(
            LOADING
          )
        ...
        {isErrorLike(extensionsOrError) ? (
          ...
          <div className="list-group list-group-flush">
            {extensionsOrError
              ...
              .filter(
                (extension): extension is GQL.IRegistryExtension =>
                  extension !== LOADING
              )
              .map(extension => (
...
function queryExtensions(
  args: Pick<
    GQL.IExtensionsOnExtensionRegistryArguments,
    "first" | "prioritizeExtensionIDs"
  >
