...
const publishExtension = (
  args: Pick<
    GQL.IPublishExtensionOnExtensionRegistryMutationArguments,
    "extensionID" | "manifest" | "bundle"
  >
): Promise<GQL.IExtensionRegistryCreateExtensionResult> =>
  mutateGraphQL(
    gql`
      mutation PublishRegistryExtension(
        $extensionID: String!
        $manifest: String!
        $bundle: String!
      ) {
...
export const RegistryExtensionNewReleasePage = withAuthenticatedUser<Props>(
  ({ extension, isLightTheme, telemetryService, history }) => {
    ...
    const manifestWithoutUrl = extension.rawManifest
      ? JSON.parse(extension.rawManifest)
      : { ...DEFAULT_MANIFEST };
    delete manifestWithoutUrl.url;
    const [manifest, setManifest] = useState(
      JSON.stringify(manifestWithoutUrl, null, 2)
    );
        ...
        (bundleChanges: Observable<string>) =>
          concat(
            isErrorLike(extension.manifest) || !extension.manifest?.url
              ? of(DEFAULT_SOURCE)
              : fromFetch(extension.manifest.url, {
    ...
    const [showEditor, setShowEditor] = useLocalStorage(
      "RegistryExtensionNewReleasePage.showEditor",
      false
    );
    const onShowEditorClick = useCallback(() => setShowEditor(true), [
      setShowEditor
    ]);

    return !extension.registryExtension ||
      ...
      <div className="registry-extension-new-release-page">
        ...
        {showEditor ? (
          <>
            ...
            <p>
              Edit or paste in an extension JSON manifest and JavaScript bundle.
              The JavaScript bundle source must be self-contained; dependency
              bundling and TypeScript transpilation is not yet supported.
            </p>
            <Form onSubmit={onSubmit} className="mb-3">
              ...
              <div className="d-flex align-items-center">
                <button
                  type="submit"
                  disabled={
                    updateOrError === LOADING || isErrorLike(bundleOrError)
                  }
                ...
                {updateOrError &&
                  ...
                  (updateOrError === LOADING ? (
                    ...
                    <span className="text-success">
                      <CheckCircleIcon className="icon-inline" /> Published
                      release successfully.
                    </span>
                  ))}
              </div>
              {isErrorLike(updateOrError) && (
                <ErrorAlert error={updateOrError} history={history} />
              )}
            </Form>
          </>
        ) : (
