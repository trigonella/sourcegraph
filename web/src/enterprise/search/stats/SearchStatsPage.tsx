...
export const SearchStatsPage: React.FunctionComponent<Props> = ({
  ...
  const onUncommittedQueryChange = useCallback<
  ...
  const onSubmit = useCallback<React.FormEventHandler<HTMLFormElement>>(
    event => {
      ...
      history.push({
        ...location,
        search: new URLSearchParams({ q: uncommittedQuery }).toString()
      });
    },
  ...
  const queryWithCount = query.includes("count:")
    ? query
  ...
  const stats = useObservable(
    useMemo(
      () =>
        _querySearchResultsStats(queryWithCount).pipe(
    ...
    <div className="search-stats-page container mt-4">
      <header className="d-flex align-items-center justify-content-between mb-3">
        <h2 className="d-flex align-items-center mb-0">
          ...
          <small className="badge badge-secondary text-uppercase ml-2">
            Experimental
          </small>
        </h2>
      </header>
      <Form onSubmit={onSubmit} className="form">
        <div className="form-group d-flex align-items-stretch">
          ...
          {uncommittedQuery !== query && (
            <button
              type="submit"
              className="btn btn-primary ml-2 test-stats-query-update"
            >
              Update
            </button>
          )}
      ...
      {stats === undefined ? (
        ...
        <div className="alert alert-warning">
          Limit hit. Add <code>count:{DEFAULT_COUNT * 5}</code> (or an even
          larger number) to your query to retry with a higher limit.
        </div>
      ) : (
