...
export const noSuggestions: State["suggestions"] = {
  values: [],
  cursorPosition: 0
};
...
export class QueryInput extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    this.subscriptions.add(
      this.inputValues.subscribe(queryState => this.props.onChange(queryState))
    );
      ...
      this.subscriptions.add(
        this.componentUpdates
          .pipe(
            ...
            distinctUntilChanged(
              (previous, current) =>
                dedupeWhitespace(previous.value.query) ===
                dedupeWhitespace(current.value.query)
            ),
            switchMap(({ value: queryState }) => {
              ...
              const fullQuery = this.props.prependQueryForSuggestions
                ? this.props.prependQueryForSuggestions +
                  " " +
                  queryForFuzzySearch
                : queryForFuzzySearch;
                ...
                map((suggestions): Suggestion[] =>
                  suggestions
                    .map((item: SearchSuggestion) =>
                      createSuggestion(item, this.props.globbing)
                    )
                    .filter(isDefined)
                    .map(
              ...
              const currentSuggestions = {
                ...
                values: staticSuggestions.values.concat(
                  this.state.suggestions.values.filter(
                    ({ fromFuzzySearch }) => fromFuzzySearch
                  )
                )
              };
              ...
              return merge(
                ...
                }).pipe(delay(1000), takeUntil(fuzzySearchSuggestions)),
  ...
  public componentDidUpdate(previousProps: Props): void {
    if (
      this.props.value.cursorPosition &&
      previousProps.value.cursorPosition !== this.props.value.cursorPosition
    ) {
  ...
  public render(): JSX.Element | null {
    ...
    return (
      <>
        ...
        {this.props.keyboardShortcutForFocus?.keybindings.map(
          (keybinding, index) => (
            <Shortcut
              key={index}
              {...keybinding}
              onMatch={this.focusInputAndPositionCursorAtEnd}
            />
          )
        )}
      </>
  ...
  private downshiftScrollIntoView = (
    node: HTMLElement,
  ...
  private setShowSuggestions = (showSuggestions: boolean): void => {
    this.setState(
      { showSuggestions },
      () => !showSuggestions && this.suggestionsHidden.next()
    );
  };
