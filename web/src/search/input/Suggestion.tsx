 ...
 */
export const isolatedFuzzySearchFilters: FilterSuggestionTypes[] = [
  FilterType.repo,
  FilterType.repogroup
];
    ...
    case "Symbol": {
      return {
        ...
        description: `${item.containerName ||
          item.location.resource.path} — ${basename(
...
const SuggestionIcon: React.FunctionComponent<SuggestionIconProps> = ({
  suggestion,
  children,
  ...props
}) => {
...
export const SuggestionItem: React.FunctionComponent<SuggestionProps> = ({
  ...
  >
    <SuggestionIcon
      className="icon-inline suggestion__icon"
      suggestion={suggestion}
    />
