...
export class FilterInput extends React.Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    this.subscriptions.add(
      this.inputValues.subscribe(query => this.setState({ inputValue: query }))
    );
    ...
    this.subscriptions.add(
      this.inputValues
        .pipe(
          ...
          distinctUntilChanged(
            (previous, current) =>
              dedupeWhitespace(previous) === dedupeWhitespace(current)
          ),
          switchMap(inputValue => {
            ...
            fullQuery = formatInteractiveQueryForFuzzySearch(
              fullQuery,
              filterType,
              inputValue
            );
            ...
            return merge(
              of({ suggestions: LOADING }).pipe(
                delay(1000),
                takeUntil(suggestions)
              ),
              suggestions
            );
          }),
  ...
  private onSubmitInput = (event: React.FormEvent<HTMLFormElement>): void => {
    ...
    if (
      ...
      this.props.filterType === FilterType.type
    ) {
  ...
  private onSuggestionSelect = (suggestion: Suggestion | undefined): void => {
    ...
    this.setState({ suggestions: noSuggestions, showSuggestions: false }, () =>
  ...
  private onClickFilterChip = (): void => {
    ...
    this.inputValues.next(
      isQuoted(inputValue) ? JSON.parse(inputValue) : inputValue
    );
  ...
  private downshiftItemToString = (suggestion?: Suggestion): string =>
    ...
    return (
      <div
        className={`${classNames(
          ...
          {
            "filter-input--active-homepage":
              isEditableAndText && this.props.isHomepage
          },
          {
            ...
            return (
              <div>
                <div className="filter-input__form">
                  {isNegatableFilter(this.props.filterType) &&
                    this.props.editable && (
                      <button
                        type="button"
                        className={classNames(
                          "btn",
                          "btn-icon",
                          "filter-input__negation-button",
                          {
                            "filter-input__negation-button--active": this.props
                              .negated
                          }
                        )}
              ...
              finiteFilters[this.props.filterType].values.map(value => (
                <div key={value.value} className="filter-input__radio">
                  ...
                  <label
                    htmlFor={value.value}
                    tabIndex={0}
                    className="filter-input__radio-label"
                  >
                    {value.displayValue
                      ? startCase(value.displayValue)
