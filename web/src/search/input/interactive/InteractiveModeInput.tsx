...
interface InteractiveModeProps
  extends SettingsCascadeProps,
    ...
    Pick<
      InteractiveSearchProps,
      "filtersInQuery" | "onFiltersInQueryChange" | "toggleSearchMode"
    >,
    VersionContextProps {
  location: H.Location;
  history: H.History;
...
export class InteractiveModeInput extends React.Component<
...
> {
  constructor(props: InteractiveModeProps) {
    ...
    if (query !== null && query.length > 0) {
      const {
        ...
        navbarQuery
      } = convertPlainTextToInteractiveQuery(query);
  ...
  private onFilterDeleted = (filterKey: string): void => {
    const filterWasEmpty =
      this.props.filtersInQuery[filterKey].value === "" ||
      !this.props.filtersInQuery[filterKey];
    const newFiltersInQuery = { ...this.props.filtersInQuery };
  ...
  public render(): JSX.Element | null {
    const homepageMode =
      ...
      (this.props.location.pathname === "/search" &&
        !parseSearchURLQuery(this.props.location.search));

    let logoSource = "/.assets/img/sourcegraph-mark.svg";
    let logoLinkClassName =
      "global-navbar__logo-link global-navbar__logo-animated";
      ...
      <div className="interactive-mode-input test-interactive-mode-input">
        <div className={!homepageMode ? "interactive-mode-input__top-nav" : ""}>
          ...
          <div
            className={`d-none d-sm-flex flex-row ${
              !homepageMode
                ? "interactive-mode-input__search-box-container"
                : ""
            }`}
          >
        ...
        {!this.props.lowProfile && (
          <div>
            ...
            />
            <AddFilterRow
              onAddNewFilter={this.addNewFilter}
              isHomepage={homepageMode}
            />
          </div>
        )}
