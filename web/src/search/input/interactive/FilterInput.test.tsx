...
describe("FilterInput", () => {
  ...
  const filterHandler = (
    newFiltersInQuery: FiltersToTypeAndValue,
    value: string
  ) => {
  ...
  describe("For type filters", () => {
    it("successfully updates when submitting with an empty value", () => {
      ...
      const codeRadioButton = container.querySelector(
        ".test-filter-input-radio-button-"
      );
      expect(codeRadioButton).toBeTruthy();
      fireEvent.click(codeRadioButton!);
      const confirmButton = container.querySelector(
        ".test-confirm-filter-button"
      );
  ...
  describe("For all other filters", () => {
    ...
    it("calls onFilterEdited with escaped and quoted value", () => {
      ...
      fireEvent.click(confirmButton!);
      expect(
        defaultProps.onFilterEdited.calledWith("content", '"\\"\\"\\""')
      ).toBe(true);
    });
    ...
    it("calls onFilterEdited with quoted value", () => {
      ...
      fireEvent.click(confirmButton!);
      expect(
        defaultProps.onFilterEdited.calledWith("after", '"2 months ago"')
      ).toBe(true);
    });
  });
});
