...
export function search(
  ...
  const transformedQuery = from(extensionHostPromise).pipe(
    switchMap(extensionHost =>
      wrapRemoteObservable(extensionHost.transformSearchQuery(query))
    )
  );
...
export function fetchSuggestions(
  ...
  return combineLatest([
    ...
    map(([repogroups, dynamicSuggestions]) => [
      ...repogroups,
      ...dynamicSuggestions
    ])
  );
  ...
  return queryGraphQL(
    ...
    map(({ data, errors }) => {
      if (
        ...
        !data.search ||
        !data.search.results ||
        !data.search.results.repositories
      ) {
        throw createAggregateError(errors);
      }
...
export const highlightCode = memoizeObservable(
  ...
  context =>
    `${context.code}:${context.fuzzyLanguage}:${String(
 ...
 */
export function shouldDisplayPerformanceWarning(
