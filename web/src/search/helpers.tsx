...
export function submitSearch({
  ...
  eventLogger.log("SearchSubmitted", {
    query: [query, generateFiltersQuery(filtersInQuery || {})]
...
export function queryIndexOfScope(query: string, scope: string): number {
  ...
  while (true) {
    ...
    const boundAtEnd =
      index + scope.length === query.length ||
      query[index + scope.length] === " ";
...
export function toggleSearchFilter(
  ...
  if (index === -1) {
    ...
    return (
      [query.trim(), searchFilter].filter(string => string).join(" ") + " "
    );
  }

  // Scope exists in the search query, so remove it now.
  return (
    query.slice(0, index).trim() +
    " " +
    query.slice(index + searchFilter.length).trim()
  ).trim();
}
...
export const resolveFilterType = (
  filter: string = ""
): FilterSuggestionTypes | null => {
  const absoluteFilter = filter.replace(/^-/, "");
  return (
    filterAliases[absoluteFilter] ??
    (isValidFilter(absoluteFilter) ? absoluteFilter : null)
  );
};
 ...
 */
const getFilterAndValueBeforeCursor = (
  queryState: QueryState
): FilterAndValueMatch => {
  const { firstPart } = splitStringAtPosition(
    queryState.query,
    queryState.cursorPosition
  );
...
export const validFilterAndValueBeforeCursor = (
  queryState: QueryState
): ValidFilterAndValueMatch | null => {
  const filterAndValueBeforeCursor = getFilterAndValueBeforeCursor(queryState);
  const resolvedFilterType = resolveFilterType(
    filterAndValueBeforeCursor.matchedFilter
  );
...
export const filterStaticSuggestions = (
  ...
  const {
    ...
    value,
    filterAndValue
  } = getFilterAndValueBeforeCursor(queryState);
  ...
  if (
    ...
    return suggestionsToShow.values.filter(suggestion =>
      suggestion.value.startsWith(value)
    );
  }

  // Suggest filter types
  return suggestions.filters.values.filter(({ value }) =>
    value.startsWith(matchedFilter)
  );
};
 ...
 * "l:go archived:" => false
 */
const isTypingWordAndNotFilterValue = (value: string): boolean =>
  Boolean(value.match(/\s+([^:]?)+$/));
...
export const insertSuggestionInQuery = (
  ...
  const { firstPart, lastPart } = splitStringAtPosition(
    queryToInsertIn,
    cursorPosition
  );
  const isFiltersSuggestion =
    selectedSuggestion.type === NonFilterSuggestionType.Filters;
  // Know where to place the suggestion later on
  const separatorIndex = firstPart.lastIndexOf(
    !isFiltersSuggestion ? ":" : " "
  );
  // If a filter value or separate word suggestion was selected, then append a whitespace
  const valueToAppend =
    selectedSuggestion.value + (isFiltersSuggestion ? "" : " ");
  ...
  const newFirstPart = (() => {
    ...
    if (
      ...
      return (
        firstPart.slice(0, lastWordOfFirstPartMatch.index) +
        " " +
        valueToAppend +
        lastPart
      );
...
export const formatInteractiveQueryForFuzzySearch = (
  ...
  if (filterSearchAlias) {
    return fullQuery.replace(
      `${filterType}:${value}`,
      `${filterSearchAlias}:${value}`
    );
  }

  return isolatedFuzzySearchFiltersFilterType.includes(filterType)
    ? filterType + ":" + value
    : fullQuery;
};
