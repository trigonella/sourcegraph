...
const ScopeNotFound: React.FunctionComponent = () => (
  <HeroPage
    ...
    subtitle={
      <>
        No search page found with this scope ID. Add an ID and description to a
        search scope to create a search page. See{" "}
        <Link to="/help/user/search/scopes#creating-search-scope-pages">
 ...
 */
export const ScopePage: React.FunctionComponent<Props> = ({
  settingsCascade,
  onNavbarQueryChange,
  ...props
}) => {
  ...
  const searchScopes = useMemo(
    () =>
      ...
      settingsCascade.final["search.scopes"],
    [settingsCascade]
  );
  const searchScope = useMemo(
    () =>
      searchScopes
        ? searchScopes.find(scope => scope.id === props.match.params.id)
        : undefined,
  ...
  const scopeRepositories = useObservable(
    useMemo(() => {
      if (
        searchScope?.value.includes("repo:") ||
        searchScope?.value.includes("repogroup:")
      ) {
        return fetchReposByQuery(searchScope.value).pipe(
  ...
  const showMoreRepositories = useCallback(
    () => setRepositoriesFirst(previousValue => previousValue + 50),
    []
  );
    ...
    <div className="container mt-3">
      ...
      <header>
        ...
        {searchScope.description && (
          <Markdown
            dangerousInnerHTML={renderMarkdown(searchScope.description)}
