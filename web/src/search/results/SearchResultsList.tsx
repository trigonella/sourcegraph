...
export class SearchResultsList extends React.PureComponent<
  SearchResultsListProps,
  State
> {
  /** Emits when a result was either scrolled into or out of the page */
  private visibleItemChanges = new Subject<{
    isVisible: boolean;
    index: number;
  }>();
  private nextItemVisibilityChange = (
    isVisible: boolean,
    index: number
  ): void => this.visibleItemChanges.next({ isVisible, index });
  ...
  private nextVirtualListContainerElement = (
    reference: HTMLElement | null
  ): void => this.virtualListContainerElements.next(reference);
    ...
    this.subscriptions.add(
      this.visibleItemChanges
        .pipe(
          filter(
            ({ isVisible, index }) =>
              isVisible && !this.state.visibleItems.has(index)
          )
        )
        .subscribe(({ index }) => {
      ...
      this.virtualListContainerElements
        ...
        .subscribe(({ container, checkpoint }) => {
          ...
          if (this.scrollableElementRef === null) {
            console.error(
              "scrollableElement ref was null when trying to scroll to a list item"
            );
            return;
          }
    ...
    this.subscriptions.add(
      this.componentUpdates
        .pipe(
          ...
          filter(isDefined),
          filter(
            (resultsOrError): resultsOrError is GQL.ISearchResults =>
              !isErrorLike(resultsOrError)
          ),
          map(({ results }) => results),
          map((results): GQL.IFileMatch[] =>
            results.filter(
              (result): result is GQL.IFileMatch =>
  ...
  public componentDidMount(): void {
    ...
    this.subscriptions.add(
      ...
      ).subscribe(displayPerformanceWarning =>
  ...
  public componentDidUpdate(): void {
    const lowestIndex = [...this.state.visibleItems].reduce(
      (low, index) => Math.min(index, low),
      Infinity
    );
                ...
                <>
                  ...
                  <SearchResultsInfoBar
                    ...
                    displayPerformanceWarning={
                      this.state.displayPerformanceWarning
                    }
                    // This isn't always correct, but the penalty for a false-positive is
                    // low.
                    hasRepoishField={
                      parsedQuery
                        ? parsedQuery.includes("repo:") ||
                          parsedQuery.includes("repogroup:")
                        : false
                    }
                  />
                    ...
                    <div className="alert alert-info m-2">
                      ...
                      {results.alert.proposedQueries && (
                        <>
                          ...
                          <ul className="list-unstyled">
                            {results.alert.proposedQueries.map(
                              proposedQuery => (
                      ...
                      {results.timedout.length > 0 &&
                        ...
                        this.renderRecommendations(
                          ["dev", "docker-container"].includes(
                            window.context.deployType
                          )
                            ? [
                                <>
      ...
      case "FileMatch":
        return (
          <FileMatch
            ...
            repoDisplayName={this.state.fileMatchRepoDisplayNames.get(
  ...
  private setCheckpoint = (checkpoint: number): void => {
    if (
      ...
      this.props.resultsOrError.limitHit
    ) {
      return;
    }
