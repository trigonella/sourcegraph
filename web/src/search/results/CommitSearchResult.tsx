...
export const CommitSearchResult: React.FunctionComponent<Props> = (
  ...
  const title: React.ReactChild = (
    <div className="commit-search-result__title">
      ...
      >
        <UserAvatar
          user={props.result.commit.author.person}
          size={32}
          className="mr-1 icon-inline"
        />
      ...
      <Link
        ...
        onMouseDown={logClick}
      >
        {commitMessageSubject(props.result.commit.message) ||
          "(empty commit message)"}
      </Link>
      <span className="commit-search-result__title-signature">
        {uniqueReferences([
          ...props.result.refs,
          ...props.result.sourceRefs
        ]).map((reference, index) => (
          <GitReferenceTag
            key={index}
        ...
        >
          {formatDistance(
            parseISO(props.result.commit.author.date),
            new Date(),
            {
              addSuffix: true
            }
  ...
  if (props.result.diffPreview) {
    ...
    interface AbsoluteRepoFilePositionNonReadonly
      extends Pick<
        AbsoluteRepoFilePosition,
        Exclude<keyof AbsoluteRepoFilePosition, "position">
      > {
    ...
    for (const [index, line] of lines.entries()) {
      ...
      if (line.startsWith("diff ")) {
        ...
        lineClasses.push({
          ...
          url: toPrettyBlobURL(rhsContext)
        });
