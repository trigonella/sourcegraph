...
export const SearchResultsInfoBar: React.FunctionComponent<SearchResultsInfoBarProps> = props => {
  const excludeForksFilter = props.results.dynamicFilters.find(
    filter => filter.value === "fork:yes"
  );
  const excludedForksCount = excludeForksFilter?.count || 0;
  const excludeArchivedFilter = props.results.dynamicFilters.find(
    filter => filter.value === "archived:yes"
  );
  ...
  return (
    <div className="search-results-info-bar" data-testid="results-info-bar">
          ...
          <div className="search-results-info-bar__row-left">
            <div className="search-results-info-bar__notice test-search-results-stats">
              <span>
                ...
                {props.results.approximateResultCount}{" "}
            ...
            {excludedForksCount > 0 && (
              ...
              >
                <span>
                  ...
                  {excludedForksCount} forked{" "}
                  {pluralize("repository", excludedForksCount, "repositories")}{" "}
                  excluded
                </span>
              </div>
            )}
            ...
            {excludedArchivedCount > 0 && (
              ...
              >
                <span>
                  <AlertCircleIcon className="icon-inline" />{" "}
                  {excludedArchivedCount} archived{" "}
                  {pluralize(
                    "repository",
            ...
            {props.results.missing.length > 0 && (
              <div
                className="search-results-info-bar__notice"
                data-tooltip={props.results.missing
                  .map(repo => repo.name)
                  .join("\n")}
              >
                <span>
                  <MapSearchIcon className="icon-inline" />{" "}
                  {props.results.missing.length}{" "}
                  {pluralize(
                    "repository",
                    props.results.missing.length,
                    "repositories"
            ...
            {props.results.timedout.length > 0 && (
              <div
                className="search-results-info-bar__notice"
                data-tooltip={props.results.timedout
                  .map(repo => repo.name)
                  .join("\n")}
              >
                <span>
                  <TimerSandIcon className="icon-inline" />{" "}
                  {props.results.timedout.length}{" "}
                  {pluralize(
                    "repository",
                    props.results.timedout.length,
                    "repositories"
                  )}{" "}
                  timed out (reload to try again, or specify a longer "timeout:"
                  in your query)
                </span>
              </div>
            )}
            ...
            {props.results.cloning.length > 0 && (
              <div
                className="search-results-info-bar__notice"
                data-tooltip={props.results.cloning
                  .map(repo => repo.name)
                  .join("\n")}
              >
                <span>
                  <CloudDownloadIcon className="icon-inline" />{" "}
                  {props.results.cloning.length}{" "}
                  {pluralize(
                    "repository",
                    props.results.cloning.length,
                    "repositories"
              ...
              <li className="nav-item">
                ...
                >
                  {props.allExpanded ? (
                    ...
                    <>
                      <ArrowExpandVerticalIcon className="icon-inline" /> Expand
                      all
                    </>
                  )}
                </button>
              ...
              <li className="nav-item">
                ...
                >
                  {props.didSave ? (
                    ...
                    <>
                      <DownloadIcon className="icon-inline test-save-search-link" />{" "}
      ...
      {!props.results.alert && props.displayPerformanceWarning && (
