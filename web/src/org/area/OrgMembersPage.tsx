          ...
          filter(() =>
            window.confirm(
              this.isSelf
                ...
                : `Remove the user ${this.props.node.username}?`
            )
          ),
          switchMap(() =>
            removeUserFromOrganization({
              user: this.props.node.id,
              organization: this.props.org.id
            }).pipe(
              catchError(error => [asError(error)]),
              map(removalOrError => ({
                removalOrError: removalOrError || null
              })),
              tap(() => {
              ...
              startWith<Pick<UserNodeState, "removalOrError">>({
                removalOrError: undefined
              })
            )
          )
        )
    ...
    return (
      <li className="list-group-item py-2">
        ...
        {isErrorLike(this.state.removalOrError) && (
          <ErrorAlert
            className="mt-2"
            error={this.state.removalOrError}
