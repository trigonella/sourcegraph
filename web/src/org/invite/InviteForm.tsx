...
const InvitedNotification: React.FunctionComponent<{
  ...
  <div className={`${className} invited-notification`}>
    <div className="invited-notification__message">
      {sentInvitationEmail ? (
        <>
          Invitation sent to {username}. You can also send {username} the
...
interface SubmittedInvite
  extends Pick<
    GQL.IInviteUserToOrganizationResult,
    "sentInvitationEmail" | "invitationURL"
  > {
  username: string;
}
...
export class InviteForm extends React.PureComponent<Props, State> {
  ...
  public componentDidMount(): void {
    const orgChanges = this.componentUpdates.pipe(
      distinctUntilChanged((a, b) => a.orgID !== b.orgID)
    );

    type Update = (prevState: State) => State;

    this.subscriptions.add(
      this.usernameChanges.subscribe(username => this.setState({ username }))
    );

    // Invite clicks.
    this.subscriptions.add(
      merge(
        this.submits.pipe(filter(() => !this.viewerCanAddUserToOrganization)),
        this.inviteClicks
      )
        .pipe(
        ...
        {this.props.authenticatedUser?.siteAdmin &&
          !window.context.emailEnabled && (
            <DismissibleAlert
              ...
              partialStorageKey="org-invite-email-config"
            >
              <p className=" mb-0">
                Set <code>email.smtp</code> in{" "}
                <Link to="/site-admin/configuration">site configuration</Link>{" "}
                to send email notfications about invitations.
              </p>
            </DismissibleAlert>
          )}
        {this.state.invited?.map(
          ({ username, sentInvitationEmail, invitationURL }, index) => (
  ...
  private onUsernameChange: React.ChangeEventHandler<
    HTMLInputElement
  > = event => this.usernameChanges.next(event.currentTarget.value);
  private onSubmit: React.FormEventHandler<HTMLFormElement> = event =>
    this.submits.next(event);
  private onInviteClick: React.MouseEventHandler<HTMLButtonElement> = event =>
    this.inviteClicks.next(event);

  private dismissNotification = (dismissedIndex: number): void => {
    this.setState(previousState => ({
      invited: (previousState.invited || []).filter(
        (invite, index) => dismissedIndex !== index
      )
    }));
  };
}
