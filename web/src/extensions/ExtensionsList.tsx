...
export class ExtensionsList extends React.PureComponent<Props, State> {
  ...
  public componentDidMount(): void {
    ...
    const debouncedQueryChanges = this.queryChanges.pipe(
    ...
    this.subscriptions.add(
      this.componentUpdates
        .pipe(
          ...
          filter(
            ([urlQuery, { query: debouncedStateQuery }]) =>
              urlQuery !== debouncedStateQuery
          )
        )
    ...
    return (
      <div className="extensions-list">
        ...
        {this.state.data.resultOrError === LOADING ? (
          ...
          <ErrorAlert
            error={this.state.data.resultOrError}
  ...
  private onQueryChangeEvent: React.FormEventHandler<
    HTMLInputElement
  > = event => this.onQueryChange({ query: event.currentTarget.value });

  private onQueryChangeImmediate = (query: string): void =>
    this.queryChanges.next({ query, immediate: true });
  ...
  private onQueryChange = ({
    ...
    query: string;
    immediate?: boolean;
  }): void => this.queryChanges.next({ query, immediate });

  private queryRegistryExtensions = (args: {
    query?: string;
  }): Observable<ExtensionsResult> =>
