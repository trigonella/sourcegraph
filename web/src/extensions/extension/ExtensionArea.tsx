...
import {
  ExtensionAreaHeader,
  ExtensionAreaHeaderNavItem
} from "./ExtensionAreaHeader";
...
export interface ExtensionAreaRoute
  extends RouteDescriptor<ExtensionAreaRouteContext> {}
...
interface ExtensionAreaState {
  ...
  extensionOrError?:
    | ConfiguredRegistryExtension<GQL.IRegistryExtension>
    | ErrorLike;
}
        ...
        .pipe(
          switchMap(([extensionID, forceRefresh]) => {
            ...
            return queryExtension(extensionID).pipe(
              ...
              map(
                (extensionOrError): PartialStateUpdate => ({ extensionOrError })
              ),

              // Don't clear old data while we reload, to avoid unmounting all components during
              // loading.
              startWith<PartialStateUpdate>(
                forceRefresh ? { extensionOrError: undefined } : {}
              )
            );
          })
        )
    ...
    if (isErrorLike(this.state.extensionOrError)) {
      return (
        <HeroPage
          ...
          subtitle={
            <ErrorMessage
              error={this.state.extensionOrError}
      ...
      <div className="registry-extension-area">
        ...
        <div className="container pt-3">
          <ErrorBoundary location={this.props.location}>
            <React.Suspense
              fallback={<LoadingSpinner className="icon-inline m-2" />}
            >
              <Switch>
...
function queryExtension(
  extensionID: string
): Observable<ConfiguredRegistryExtension<GQL.IRegistryExtension>> {
  return queryGraphQL(
    gql`
    ...
    map(({ data, errors }) => {
      if (
        ...
        !data.extensionRegistry.extension
      ) {
