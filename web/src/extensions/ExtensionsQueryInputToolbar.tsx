 ...
 */
export class ExtensionsQueryInputToolbar extends React.PureComponent<
  Props,
  State
> {
  public state: State = {};
  ...
  private toggleIsOpen = (menu: DropdownMenuID): void =>
    this.setState(previousState => ({
      open: previousState.open === menu ? undefined : menu
    }));
          ...
          <DropdownMenu right={true}>
            {EXTENSION_CATEGORIES.map(category => (
              <DropdownItem
                ...
                onClick={() =>
                  this.props.onQueryChange(extensionsQuery({ category }))
                }
                key={category}
        ...
        >
          ...
          <DropdownMenu right={true}>
            <DropdownItem
              // eslint-disable-next-line react/jsx-no-bind
              onClick={() =>
                this.props.onQueryChange(extensionsQuery({ enabled: true }))
              }
              disabled={this.props.query.includes(
                extensionsQuery({ enabled: true })
              )}
            >
            ...
            <DropdownItem
              // eslint-disable-next-line react/jsx-no-bind
              onClick={() =>
                this.props.onQueryChange(extensionsQuery({ disabled: true }))
              }
              disabled={this.props.query.includes(
                extensionsQuery({ disabled: true })
              )}
            >
