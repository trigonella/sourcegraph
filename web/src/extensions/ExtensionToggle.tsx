...
export class ExtensionToggle extends React.PureComponent<Props> {
        ...
        .pipe(
          switchMap(enabled => {
            ...
            eventLogger.log("ExtensionToggled", {
              extension_id: this.props.extension.id
            });

            return from(
              this.props.platformContext.updateSettings(
  ...
  public render(): JSX.Element | null {
    ...
    const highestPrecedenceSubjectWithExtensionAdded = isErrorLike(cascade)
      ? undefined
      : last(
          cascade.subjects.filter(subject =>
            isExtensionAdded(subject.settings, this.props.extension.id)
          )
        );
    ...
    if (highestPrecedenceSubjectWithExtensionAdded) {
      ...
      title = `${
        isExtensionEnabled(
          highestPrecedenceSubjectWithExtensionAdded.settings,
          this.props.extension.id
        )
          ? "Enabled"
          : "Disabled"
...
function extractErrors(
  ...
  const found = settingsCascade.subjects.find(
    property("settings", isErrorLike)
  );
  if (found) {
