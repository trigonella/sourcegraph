...
export class UserNavItem extends React.PureComponent<Props, State> {
  private supportsSystemTheme = Boolean(
    window.matchMedia?.(
      "not all and (prefers-color-scheme), (prefers-color-scheme)"
    ).matches
  );
      ...
      >
        ...
        <DropdownMenu right={true} className="user-nav-item__dropdown-menu">
          ...
          <div className="px-2 py-1">
            ...
            {this.props.themePreference === ThemePreference.System &&
              !this.supportsSystemTheme && (
            ...
            {this.props.keyboardShortcutForSwitchTheme?.keybindings.map(
              (keybinding, index) => (
                <Shortcut
                  key={index}
                  {...keybinding}
                  onMatch={this.onThemeCycle}
                />
              )
            )}
          </div>
  ...
  private onThemeChange: React.ChangeEventHandler<
    HTMLSelectElement
  > = event => {
