...
const renderShallow = (element: React.ReactElement<NavLinks["props"]>): any => {
  ...
  const getDisplayName = (element: React.ReactChild): string | string[] => {
    ...
    if (typeof element.type === "symbol" || typeof element.type === "string") {
      return React.Children.map(element.props.children, element =>
  ...
  return React.Children.map<string | string[], React.ReactChild>(
...
describe("NavLinks", () => {
  ...
  const SETTINGS_CASCADE: SettingsCascadeProps["settingsCascade"] = {
    final: null,
    subjects: null
  };
