...
describe("Settings", () => {
  ...
  describe("User settings page", () => {
    it("updates user settings", async () => {
      ...
      const getSettingsEditorContent = async (): Promise<
      ...
      await driver.page.goto(
        driver.sourcegraphBaseUrl + "/users/test/settings"
      );
      ...
      assert.strictEqual(
        await driver.page.evaluate(
          () =>
            document.querySelector<HTMLButtonElement>(".test-save-toolbar-save")
              ?.disabled
        ),
        true,
      ...
      const overrideSettingsVariables = await testContext.waitForGraphQLRequest(
        async () => {
