  ...
  describe("index page", () => {
    it("loads when accessed with a repo url", async () => {
      ...
      testContext.overrideGraphQL({
        ...
        TreeEntries: () =>
          createTreeEntriesResult(repositorySourcegraphUrl, fileEntries),
                ...
                nodes: [
                  ...
                  },
                  {
                    ...
                    author: {
                      person: {
                        avatarURL:
                          "https://avatars0.githubusercontent.com/u/1976?v=4",
                        name: "Quinn Slack",
      ...
      await driver.page.goto(
        driver.sourcegraphBaseUrl + repositorySourcegraphUrl
      );
      ...
      const numberOfFileEntries = await driver.page.evaluate(
        () =>
          document.querySelectorAll<HTMLButtonElement>(".test-tree-entry-file")
            ?.length
      );
    ...
    it("works with files with spaces in the name", async () => {
      ...
      testContext.overrideGraphQL({
        ...
        FileExternalLinks: ({ filePath, repoName, revision }) =>
          createFileExternalLinksResult(
            `https://${repoName}/blob/${revision}/${filePath
              .split("/")
      ...
      assert.strictEqual(
        await driver.page.evaluate(
          () => document.querySelector(".test-tree-file-link")?.textContent
        ),
        fileName
      );
      ...
      assert.strictEqual(
        await driver.page.evaluate(
          () =>
            document.querySelector(".test-breadcrumb-part-directory")
              ?.textContent
        ),
        directoryName
      );
      assert.strictEqual(
        await driver.page.evaluate(
          () =>
            document.querySelector(".test-breadcrumb-part-last")?.textContent
        ),
        fileName
      );
      ...
      assert.strictEqual(
        await driver.page.evaluate(
          () =>
            document.querySelector<HTMLAnchorElement>(".test-go-to-code-host")
              ?.href
        ),
