...
function formatLineOrPositionOrRange(lpr: LineOrPositionOrRange): string {
  ...
  return emptyRange
    ...
    : `${toPositionHashComponent(range.start)}-${toPositionHashComponent(
        range.end
      )}`;
}
 ...
 */
export function replaceRevisionInURL(
  href: string,
  newRevision: string
): string {
  ...
  url.pathname = `/${encodeRepoRevision({
    ...parsed,
...
export function parseBrowserRepoURL(href: string): ParsedRepoURI {
  ...
  if (treeSeparator !== -1) {
    filePath = decodeURIComponent(
      pathname.slice(treeSeparator + "/-/tree/".length)
    );
  }
  if (blobSeparator !== -1) {
    filePath = decodeURIComponent(
      pathname.slice(blobSeparator + "/-/blob/".length)
    );
  }
...
export function parseRepoRevision(repoRevision: string): ParsedRepoRevision {
  const [repository, revision] = repoRevision.split("@", 2) as [
    string,
    string | undefined
  ];
  return {
