...
> {
      ...
      >
        ...
        {window.context.sourcegraphDotComMode && (
          <p className="mt-1 mb-0">
            Create a public account to search/navigate open-source code and
            manage Sourcegraph subscriptions.
          </p>
        )}
        {signupTerms && (
          <p className="mt-1 mb-0">
  ...
  private onRequestTrialFieldChange = (
    event: React.ChangeEvent<HTMLInputElement>
  ): void => {
    ...
    this.subscriptions.add(
      from(
        this.props
          ...
          .catch(error =>
            this.setState({ error: asError(error), loading: false })
          )
      ).subscribe()
    );
