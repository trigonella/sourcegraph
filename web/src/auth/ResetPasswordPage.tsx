 ...
 */
class ResetPasswordInitForm extends React.PureComponent<
  ResetPasswordInitFormProps,
  ResetPasswordInitFormState
> {
  ...
  public render(): JSX.Element | null {
    if (this.state.submitOrError === null) {
      return (
        <div className="alert alert-success">Password reset email sent.</div>
      );
    }
    ...
    return (
      ...
      >
        <p>
          Enter the email address associated with your account. You'll receive
        ...
        {isErrorLike(this.state.submitOrError) && (
          <ErrorAlert
            className="mt-2"
            error={this.state.submitOrError}
  ...
  private onEmailFieldChange = (
    event: React.ChangeEvent<HTMLInputElement>
  ): void => {
    this.setState({ email: event.target.value });
  };

  private handleSubmitResetPasswordInit = (
      ...
      .then(response => {
        if (response.status === 200) {
          ...
          response
            ...
            .then(text =>
              this.setState({
                submitOrError: new Error(text || "Unknown error")
              })
            )
...
class ResetPasswordCodeForm extends React.PureComponent<
  ResetPasswordCodeFormProps,
  ResetPasswordCodeFormState
> {
    ...
    if (this.state.submitOrError === null) {
      return (
        <div className="alert alert-success">
          ...
          <Link to="/sign-in">Sign in with your new password</Link> to continue.
        </div>
      );
    }
  ...
  private handleSubmitResetPassword = (
    event: React.FormEvent<HTMLFormElement>
  ): void => {
    event.preventDefault();
    this.setState({ submitOrError: "loading" });
    fetch("/-/reset-password-code", {
      ...
      .then(response => {
        if (response.status === 200) {
          ...
          this.setState({
            submitOrError: new Error(
              "Password reset code was invalid or expired."
            )
          });
        } else {
    ...
    if (this.props.authenticatedUser) {
      body = (
        <div className="alert alert-danger">
          Authenticated users may not perform password reset.
        </div>
      );
      ...
      body = (
        <div className="alert alert-warning">
          Password reset is disabled. Ask a site administrator to manually reset
          your password.
        </div>
      );
    }
