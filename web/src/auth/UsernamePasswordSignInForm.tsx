...
export class UsernamePasswordSignInForm extends React.Component<Props, State> {
  ...
  public render(): JSX.Element | null {
    return (
      <Form
        className="signin-signup-form signin-form test-signin-form"
        onSubmit={this.handleSubmit}
      >
        {window.context.allowSignup ? (
          <p>
            <Link to={`/sign-up${this.props.location.search}`}>
              Don't have an account? Sign up.
            </Link>
          </p>
        ) : (
          <p className="text-muted">
            To create an account, contact the site admin.
          </p>
        )}
        {this.state.error && (
          <ErrorAlert
    ...
    fetch("/-/sign-in", {
      ...
      .then(response => {
        if (response.status === 200) {
          if (
            new URLSearchParams(this.props.location.search).get("close") ===
            "true"
          ) {
            window.close();
          } else {
