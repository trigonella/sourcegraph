...
async function getActivationStatus(
  driver: Driver
): Promise<{ complete: number; total: number }> {
  ...
  return driver.page.evaluate(() => {
    ...
    const lineItems = [
      ...dropdownMenu.querySelectorAll(".activation-dropdown-item")
    ];
    const complete = lineItems.flatMap(element => [
      ...element.querySelectorAll(".mdi-icon.text-success")
    ]).length;
    const incomplete = lineItems.flatMap(element => [
      ...element.querySelectorAll(".mdi-icon.text-muted")
    ]).length;
    return {
      complete,
...
describe("Onboarding", () => {
  ...
  testAdminOnboarding("Site-admin onboarding", async function() {
    ...
    const testUser = await getUser(
      gqlClient,
      "test-onboarding-regression-test-user"
    );
    if (!testUser) {
    ...
    if (activationStatus.total !== 4) {
      throw new Error(
        `Expected 4 onboarding steps for site admins, but only found ${activationStatus.total}`
      );
    }
    ...
    await retry(async () => {
      ...
      await driver.page.click(activationNavBarSelector);
      await (
        await driver.findElementWithText("Connect your code host")
      ).click();
    });
    await driver.waitUntilURL(
      driver.sourcegraphBaseUrl + "/site-admin/external-services"
    );
  ...
  test("Non-admin user onboarding", async function() {
    ...
    await waitForRepos(
      gqlClient,
      ["github.com/" + repoSlugs[repoSlugs.length - 1]],
      config
    );
