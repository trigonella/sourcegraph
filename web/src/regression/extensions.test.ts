...
describe("Sourcegraph extensions regression test suite", () => {
  ...
  before(async function() {
    ...
    ({ driver, gqlClient: graphQLClient, resourceManager } = await getTestTools(
      config
    ));
    resourceManager.add(
      "User",
  ...
  test("Codecov extension", async function() {
    ...
    await driver.findElementWithText("Codecov: Show line hit/branch counts", {
      action: "click"
    });
    ...
    await retry(async () =>
      expect(await driver.page.$$(".line-decoration-attachment")).toHaveLength(
        264
      )
    );
  ...
  test("Datadog extension", async function() {
    ...
    await retry(async () =>
      expect(
        await driver.page.$$('[data-contents=" View metric (Datadog) » "]')
      ).toHaveLength(9)
    );
  });
  ...
  test("Sentry extension", async function() {
    ...
    await retry(async () =>
      expect(
        await driver.page.$$('[data-contents=" View logs in Sentry » "]')
      ).toHaveLength(1)
    );
  });
});
