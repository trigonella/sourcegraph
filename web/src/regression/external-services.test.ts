...
describe("External services GUI", () => {
  ...
  test("External services: GitHub.com GUI and repositoryPathPattern", async () => {
    ...
    resourceManager.add(
      ...
      await (async () => {
        await driver.page.goto(
          config.sourcegraphBaseUrl + "/site-admin/external-services"
        );
        await driver.findElementWithText("Add repositories", {
          action: "click",
          wait: { timeout: 500 }
        });
        await driver.findElementWithText("GitHub.com", {
          action: "click",
          wait: { timeout: 500 }
        });
    ...
    // Redirect
    await driver.page.goto(
      config.sourcegraphBaseUrl + "/github.com/gorilla/mux"
    );
...
describe("External services API", () => {
  ...
  test("External services: GitLab", async function() {
    ...
    resourceManager.add(
      ...
      await ensureTestExternalService(
        gqlClient,
        { ...externalService, waitForRepos: repos },
        config
      )
    );
  });
  ...
  test("External services: Bitbucket Cloud", async function() {
    ...
    resourceManager.add(
      ...
      await ensureTestExternalService(
        gqlClient,
        { ...externalServiceInput, waitForRepos: repos },
        config
      )
    );
    // Update eternal service with an "exclude" property
    const { id } = (
      await getExternalServices(gqlClient, { uniqueDisplayName })
    )[0];
    ...
    await waitForRepos(gqlClient, ["sg-test-regression-test-bob/jsonrpc2"], {
      ...config,
      shouldNotExist: true
    });
  });
...
describe("External services permissions", () => {
  ...
  test("External services permissions: GitHub", async function() {
    ...
    await login(
      driver,
      { ...config, authProviderDisplayName: authProvider.displayName },
      () =>
        loginToGitHub(
          driver,
          "sg-test-regression-test-amy",
    ...
    await driver.findElementWithText(
      "sg-test-regression-test-bob/shared-with-amy",
      {
        wait: { timeout: 2 * 1000 }
      }
    );
