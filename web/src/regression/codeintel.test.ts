...
describe("Code intelligence regression test suite", () => {
  ...
  const prometheusCommonFallbackCommit =
    "e8215224146358493faab0295ce364cd386223b9"; // 2 behind LSIF
  ...
  before(async function() {
    ...
    ({
      driver,
      gqlClient,
      resourceManager: outerResourceManager
  ...
  describe("Precise code intelligence regression test suite", () => {
    ...
    before(async function() {
      ...
      const repoCommits = [
        ...
        {
          repository: "prometheus-client-golang",
          commit: prometheusClientHeadCommit
        }
      ];
      ...
      for (const { repository } of repoCommits) {
        ...
        await clearUploads(
          gqlClient,
          `github.com/sourcegraph-testing/${repository}`
        );
      }
      ...
      for (const { repository, commit } of repoCommits) {
        ...
        uploadUrls.push(
          await performUpload(config, {
            ...
            commit,
            root: "/",
            filename: `lsif-data/github.com/sourcegraph-testing/${repository}@${commit.slice(
              0,
              12
            )}.lsif`
          })
      ...
      await clearUploads(
        gqlClient,
        "github.com/sourcegraph-testing/prometheus-redefinitions"
      );
      innerResourceManager.add(
        "Global setting",
        "codeIntel.lsif",
        await setGlobalLSIFSetting(gqlClient, true)
      );
    });
    after(async () => {
      if (!config.noCleanup) {
    ...
    const makeTestCase = (
      ...
      const prometheusCommonLocations = prometheusCommonSamplePairLocations.map(
        ({ path, line, character }) => ({
      ...
      const prometheusClientLocations = prometheusClientSamplePairLocations.map(
        ({ path, line, character }) => ({
...
function collectVisibleLinks(driver: Driver): Promise<TestLocation[]> {
  return driver.page.evaluate(() =>
    [
      ...
      url:
        a
          .querySelector(".test-file-match-children-item")
          ?.getAttribute("href") || "",
...
async function clickOnEmptyPartOfCodeView(driver: Driver): Promise<void> {
  ...
  await driver.page.waitForFunction(
    () =>
      document.querySelectorAll(".test-tooltip-go-to-definition").length === 0
  );
}
...
async function waitForHover(
  ...
  expect(normalizeWhitespace(await getTooltip(driver))).toContain(
    normalizeWhitespace(expectedHoverContains)
  );

  if (precise !== undefined) {
    expect(
      await driver.page.evaluate(
        () => document.querySelectorAll<HTMLElement>(".test-hover-badge").length
      )
    ).toEqual(precise ? 0 : 1);
  }
}
...
async function getTooltip(driver: Driver): Promise<string> {
  return driver.page.evaluate(
    () =>
      (document.querySelector(".test-tooltip-content") as HTMLElement)
        .textContent || ""
  );
}
...
async function enableBadgeAttachments(
...
async function setIncludeForks(
  gqlClient: GraphQLClient,
  enabled: boolean
): Promise<() => Promise<void>> {
...
async function clearUploads(
    ...
    .pipe(
      ...
      map(({ repository }) =>
        repository === null
          ...
          : {
              nodes: repository.lsifUploads.nodes,
              hasNextPage: repository.lsifUploads.pageInfo.hasNextPage
            }
      )
    )
    .toPromise();
  ...
  await gqlClient
    .mutateGraphQL(
      gql`
                mutation(${indices
                  .map(index => `$upload${index}: ID!`)
                  .join(", ")}) {
                    ${indices
                      .map(
                        index =>
                          gql`delete${index}: deleteLSIFUpload(id: $upload${index}) { alwaysNil }`
                      )
                      .join("\n")}
                }
            `,
...
async function performUpload(
  ...
  try {
    ...
    throw new Error(
      `Failed to upload LSIF data: ${(error.stderr as string) ||
...
async function ensureUpload(driver: Driver, uploadUrl: string): Promise<void> {
  ...
  await driver.page.waitFor(
    () =>
      document.querySelector(".test-upload-state")?.textContent ===
      "Upload processed successfully."
  );

  const isLatestForRepoText = await (
    await driver.page.waitFor(".test-is-latest-for-repo")
