...
export async function ensureLoggedInOrCreateTestUser(
  ...
  if (!username.startsWith("test-")) {
    throw new Error(
      `Test username must start with "test-" (was ${JSON.stringify(username)})`
    );
  }
  ...
  if (deleteIfExists) {
    ...
    try {
      ...
      console.log(
        ...
        }), will attempt to create user ${JSON.stringify(username)}`
      );
    }
  }
...
async function createTestUser(
  ...
  gqlClient: GraphQLClient,
  {
    username,
    testUserPassword
  }: { username: string } & Pick<Config, "testUserPassword">
): Promise<void> {
  ...
  await driver.page.waitForFunction(() =>
    document.body.textContent!.includes("Your password was reset")
  );
}
...
export async function createAuthProvider(
  ...
  const siteConfigParsed: SiteConfiguration = jsonc.parse(
    siteConfig.configuration.effectiveContents
  );
  ...
  if (
    authProviders?.some(
      provider =>
        ...
        (provider as any).displayName === authProvider.displayName
    )
  ) {
...
export async function ensureNewOrganization(
  ...
  const matchingOrgs = (
    await fetchAllOrganizations({ requestGraphQL }, { first: 1000 }).toPromise()
  ).nodes.filter(org => org.name === variables.name);
  if (matchingOrgs.length > 1) {
    throw new Error(
      `More than one organization name exists with name ${variables.name}`
    );
  }
  if (matchingOrgs.length === 1) {
...
export async function getGlobalSettings(
  ...
  const globalSettingsSubject = first(
    settings.subjects.filter(subject => subject.__typename === "Site")
  );
...
export async function login(
  ...
  await retry(async () => {
    ...
    await driver.findElementWithText(
      "Sign in with " + authProviderDisplayName,
      {
        action: "click",
        selector: "a",
        wait: { timeout: 5000 }
...
export async function loginToOkta(
  driver: Driver,
  username: string,
  password: string
): Promise<void> {
...
export async function loginToGitLab(
  driver: Driver,
  username: string,
  password: string
): Promise<void> {
  await driver.page.waitForSelector('input[name="user[login]"]', {
    timeout: 10000
  });
  await driver.replaceText({
    selector: "#user_login",
