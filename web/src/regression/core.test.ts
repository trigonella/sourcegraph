...
describe("Core functionality regression test suite", () => {
  ...
  test("2.2.1 User settings are saved and applied", async () => {
    ...
    await driver.page.goto(
      config.sourcegraphBaseUrl + `/users/${testUsername}/settings`
    );
    ...
    if (currentSettings !== previousSettings) {
      throw new Error(
        `Settings ${JSON.stringify(
          currentSettings
        )} did not match (old) saved settings ${JSON.stringify(
          previousSettings
        )}`
      );
    }
    ...
    if (JSON.stringify(currentSettings2) !== JSON.stringify(newSettings)) {
      throw new Error(
        `Settings ${JSON.stringify(
          currentSettings2
        )} did not match (new) saved settings ${JSON.stringify(newSettings)}`
      );
    }
  ...
  test("2.2.2 User profile page", async () => {
    ...
    await driver.page.goto(
      driver.sourcegraphBaseUrl + `/users/${testUsername}/settings/profile`
    );
    await driver.replaceText({
    ...
    await driver.page.waitForFunction(
      displayName => {
        const element = document.querySelector(
          ".test-user-area-header__display-name"
        );
        return (
          element?.textContent && element.textContent.trim() === displayName
        );
      },
      undefined,
      displayName
  ...
  test("2.2.3. User emails page", async () => {
    ...
    await driver.page.goto(
      driver.sourcegraphBaseUrl + `/users/${testUsername}/settings/emails`
    );
    await driver.replaceText({
  ...
  test('2.2.4 Access tokens work and invalid access tokens return "401 Unauthorized"', async () => {
    await driver.page.goto(
      config.sourcegraphBaseUrl + `/users/${testUsername}/settings/tokens`
    );
    await driver.findElementWithText("Generate new token", {
      action: "click",
      wait: { timeout: 5000 }
    });
    await driver.findElementWithText("New access token", {
      wait: { timeout: 1000 }
    });
    await driver.replaceText({
    ...
    await driver.findElementWithText("Generate token", {
      action: "click",
      wait: { timeout: 1000 }
    });
  ...
  test("2.5 Quicklinks: add a quicklink, test that it appears on the front page and works.", async () => {
    ...
    if (parsedOldContents?.quicklinks) {
      throw new Error(
        "Global setting quicklinks already exists, aborting test"
      );
    }
