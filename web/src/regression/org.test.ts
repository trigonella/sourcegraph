...
async function deleteOrganizationByName(
  ...
  await Promise.all(
    matches.map(org =>
      deleteOrganization({ requestGraphQL }, org.id).toPromise()
    )
  );
}
  ...
  describe("Organizations GUI", () => {
    ...
    test("Create an organization and test settings cascade", async () => {
      const expectedQuicklink = {
        name: "Test Org 1 Quicklink",
        url: "http://test-org-1-link.local"
      };
      ...
      await driver.page.goto(
        config.sourcegraphBaseUrl + "/site-admin/organizations"
      );
      await driver.findElementWithText("Create organization", {
        action: "click",
        wait: { timeout: 2000 }
      });
      ...
      resourceManager.add("Organization", testOrg.name, () =>
        deleteOrganizationByName(gqlClient, testOrg.name)
      );
...

      {
        ...
        if (
          !quicklinks.some(
            quicklink =>
              quicklink.name === expectedQuicklink.name &&
              quicklink.url === expectedQuicklink.url
          )
        ) {
          throw new Error(
      ...
      await driver.findElementWithText("Leave organization", {
        action: "click",
        wait: { timeout: 1000 }
      });
      ...
      await driver.page.waitForFunction(
        () =>
          ...
          !document.body.textContent.includes("Leave organization")
      );

      {
        ...
        if (
          ...
          throw new Error(
            `Found quicklink ${JSON.stringify(
              expectedQuicklink
            )} in quicklinks: ${JSON.stringify(quicklinks)}`
          );
        }
      ...
      await retry(
        async () => {
          ...
          // Check that user is not part of org
          {
            ...
            if (
              user.organizations.nodes.some(org => org.id === createdOrg.id)
            ) {
              throw new Error(
                `user ${testUser1.username} should not be part of org ${testOrg.name}`
              );
            }
          }
        },
      ...
      await retry(
        async () => {
          ...
          // Check that user is part of organization
          {
            ...
            if (
              !user.organizations.nodes.some(
                org => org.id === lastCreatedOrg.id
              )
            ) {
              throw new Error(
                `user ${testUser1.username} should be part of org ${testOrg.name}`
              );
            }
          }
        },
