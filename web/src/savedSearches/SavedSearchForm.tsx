...
export class SavedSearchForm extends React.Component<Props, State> {
    ...
    return (
      <div className="saved-search-form">
        <div className="saved-search-form__header">
          ...
          <div>
            Get notifications when there are new results for specific search
            queries
          </div>
        </div>
        <Form onSubmit={this.handleSubmit}>
          ...
          <div className="saved-search-form__input">
            <label className="saved-search-form__label">
              Email notifications:
            </label>
            <div>
              <label>
          ...
          {notifySlack && slackWebhookURL && (
            <div className="saved-search-form__input">
              ...
              <label className="small">
                Slack webhooks are deprecated and will be removed in a future
                Sourcegraph version.
              </label>
            </div>
          )}
          {this.isUnsupportedNotifyQuery(this.state.values) && (
            <div className="alert alert-warning mb-3">
              <strong>Warning:</strong> non-commit searches do not currently
              support notifications. Consider adding <code>type:diff</code> or{" "}
              <code>type:commit</code> to your query.
            </div>
          )}
          {notify &&
            ...
            !this.isUnsupportedNotifyQuery(this.state.values) && (
              <div className="alert alert-warning mb-3">
                <strong>Warning:</strong> Sending emails is not currently
                configured on this Sourcegraph server.{" "}
   ...
   */
  private isUnsupportedNotifyQuery(
    savedQuery: Omit<SavedQueryFields, "id">
  ): boolean {
    const notifying = savedQuery.notify || savedQuery.notifySlack;
    return (
      notifying &&
      !savedQuery.query.includes("type:diff") &&
      !savedQuery.query.includes("type:commit")
    );
  }
}
