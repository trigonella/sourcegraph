...
import {
  NotificationType
} from '../../../../../shared/src/api/client/services/notifications'
...
import {
  getSelectionsFromHash,
  observeSelectionsFromHash
} from '../shared/util/selections'
 ...
 * - An older GHE single file code view (newer GitHub.com code views use
 * createFileLineContainerToolbarMount)
 */
export function createFileActionsToolbarMount(codeView: HTMLElement):
    HTMLElement {
  ...
  if (!fileActions) {
    throw new Error(
        'Could not find GitHub file actions with selector .file-actions')
  }

  // Add a class to the .file-actions element, so that we can reliably match it
  // in stylesheets without bleeding CSS to other code hosts (GitLab also uses
  // .file-actions elements).
  fileActions.classList.add('sg-github-file-actions')

  // Old GitHub Enterprise PR views have a "☑ show comments" text that we want
  // to insert *after*
...
const diffCodeView: Omit<CodeView, 'element'> = {
  ...
  getScrollBoundaries : codeView => {
    ...
    if (!fileHeader) {
      throw new Error(
          'Could not find .file-header element in GitHub PR code view')
    }
    return [ fileHeader ]
  },
}
 ...
 * this. See an example here
 * https://github.com/sourcegraph/browser-extensions/issues/188.
 */
const getSnippetPositionAdjuster = (requestGraphQL:
                                        PlatformContext['requestGraphQL']):
    PositionAdjuster<RepoSpec&RevisionSpec&FileSpec&ResolvedRevisionSpec> => (
        {direction, codeView, position}) => fetchBlobContentLines({
                                              ...position,
                                              requestGraphQL
                                            }).pipe(map(lines => {
      const codeElement = singleFileDOMFunctions.getCodeElementFromLineNumber(
          codeView, position.line, position.part)
      if (!codeElement) {
        throw new Error(
            '(adjustPosition) could not find code element for line provided')
      }

      const actualLine = lines[position.line - 1] const documentLine =
          codeElement.textContent || ''

      const actualLeadingWhiteSpace =
          actualLine.length - trimStart(actualLine).length
      const documentLeadingWhiteSpace =
          documentLine.length - trimStart(documentLine).length

      const modifier =
          direction === AdjustmentDirection.ActualToCodeView ? -1 : 1
      const delta =
          Math.abs(actualLeadingWhiteSpace - documentLeadingWhiteSpace) *
          modifier
...
export const createFileLineContainerToolbarMount:
    NonNullable<CodeView['getToolbarMount']> = (codeViewElement: HTMLElement):
        HTMLElement => {
          const className = 'sourcegraph-github-file-code-view-toolbar-mount'
          const existingMount =
              codeViewElement.querySelector(`.${className}`) as HTMLElement
          if (existingMount) {
            return existingMount
          }
          const mountElement = document.createElement('div')
...
export const fileLineContainerResolver: ViewResolver<CodeView> = {
  ...
  resolveView : (fileLineContainer: HTMLElement) : CodeView | null => {
    const embeddedBlobWrapper =
        fileLineContainer.closest('.blob-wrapper-embedded')
    if (embeddedBlobWrapper) {
      ...
      const element = embeddedBlobWrapper.parentElement!return {
        element, ...snippetCodeView,
      }
    }
    ...
    return {
      element: repositoryContent as HTMLElement, ...singleFileCodeView,
...
const genericCodeViewResolver: ViewResolver<CodeView> = {
  ...
  resolveView : (element: HTMLElement) : CodeView | null => {
    ...
    if (element.closest('.discussion-item-body') ||
        element.classList.contains('js-comment-container')) {
 ...
 */
export function
checkIsGitHubEnterprise():
    boolean {
      const ogSiteName = document.head.querySelector<HTMLMetaElement>(
          'meta[property="og:site_name"]')
      return (!!ogSiteName &&
              // GitHub Enterprise v2.14.11 has "GitHub" as og:site_name
              (ogSiteName.content === 'GitHub Enterprise' ||
               ogSiteName.content === 'GitHub') &&
 ...
 */
export const checkIsGitHubDotCom = (url = window.location.href): boolean =>
    /^https?:\/\/(www\.)?github\.com/.test(url)

/**
 * Returns true if the current page is either github.com or GitHub Enterprise.
 */
export const checkIsGitHub = (): boolean =>
    checkIsGitHubDotCom() || checkIsGitHubEnterprise()

const OPEN_ON_SOURCEGRAPH_ID = 'open-on-sourcegraph'

export const createOpenOnSourcegraphIfNotExists: MountGetter =
    (container: HTMLElement):
        HTMLElement|null => {
          const pageheadActions =
              querySelectorOrSelf(container, '.pagehead-actions')
          // If ran on page that isn't under a repository namespace.
          if (!pageheadActions || pageheadActions.children.length === 0) {
            return null
          }
          // Check for existing
          let mount = pageheadActions.querySelector<HTMLElement>(
              '#' + OPEN_ON_SOURCEGRAPH_ID)
          if (mount) {
            return mount
          }
          // Create new
...
export const githubCodeHost: CodeHost = {
  ...
  codeViewResolvers : [
    genericCodeViewResolver, fileLineContainerResolver,
  ...
  getContext : () => {
    const repoHeaderHasPrivateMarker =
        ...
        !!document.querySelector(
            'h1 .octicon-lock ~ [itemprop="author"] ~ [itemprop="name"]') ||
        !!(document
               .querySelector(
                   'h1 [itemprop="author"] ~ [itemprop="name"] ~ .Label')
               ?.textContent?.trim()
               .toLowerCase() === 'private')
    const parsedURL = parseURL()
    return {
      ...parsedURL,
          revision: parsedURL.pageType === 'blob' ||
                  parsedURL.pageType === 'tree'
              ? resolveFileInfo().blob.revision
              : undefined,
          privateRepository: window.location.hostname !== 'github.com' ||
              repoHeaderHasPrivateMarker,
    }
  },
  ...
  commandPaletteClassProps : {
    ...
    listClassName :
        'p-0 m-0 js-navigation-container jump-to-suggestions-results-container',
    ...
    noResultsClassName :
        'd-flex flex-auto flex-items-center jump-to-suggestions-path p-2',
    iconClassName,
  },
  codeViewToolbarClassProps : {
    ...
    actionItemClass :
        'btn btn-sm tooltipped tooltipped-s BtnGroup-item action-item--github',
  ...
  linkPreviewContentClass :
      'text-small text-gray p-1 mx-1 border rounded-1 bg-gray text-gray-dark',
  urlToFile : (sourcegraphURL, target, context) => {
    if (target.viewState) {
      // A view state means that a panel must be shown, and panels are currently
      // only supported on Sourcegraph (not code hosts).
      return toAbsoluteBlobURL(sourcegraphURL, target)
    }

    // Make sure the location is also on this github instance, return an
    ...
    // TODO to be entirely correct, this would need to compare the revision of
    // the code view with the target revision.
    ...
    if (isSameRepo && context.part !== undefined) {
      ...
      for (const container of containers) {
        const header = container.querySelector<
            HTMLElement&{dataset : {path : string; anchor : string}}>(
        ...
        if (anchorPath === target.filePath) {
          ...
          url.hash = anchorUrl
          if (target.position) {
            // GitHub uses L for the left side, R for both right side and the
            // unchanged/white parts
            url.hash +=
                `${context.part === 'base' ? 'L' : 'R'}${target.position.line}`
          }
    ...
    // Go to blob URL
    const fragment = target.position ? `#L${target.position.line}${
                                           target.position.character
                                               ? `:${target.position.character}`
                                               : ''}`
                                     : ''
    return `https://${target.rawRepoName}/blob/${revision}/${target.filePath}${
        fragment}`
  },
