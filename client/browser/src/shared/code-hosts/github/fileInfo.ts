...
export const resolveFileInfo = (): BlobInfo => {
  ...
  if (parsedURL.pageType !== "blob" && parsedURL.pageType !== "tree") {
    throw new Error(
      `Current URL does not match a blob or tree url: ${window.location.href}`
    );
  }
...
export const resolveSnippetFileInfo = (codeView: HTMLElement): BlobInfo => {
  ...
  const commitLinkElement = codeView.querySelector(
    "a.commit-tease-sha"
  ...
  if (!commitIDMatch?.[1]) {
    throw new Error(
      `Could not parse commitID from snippet commit link href: ${commitLinkElement.href}`
    );
  }
