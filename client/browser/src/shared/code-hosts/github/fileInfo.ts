...
export const resolveDiffFileInfo = (codeView: HTMLElement):
    DiffInfo => {
...
export const resolveFileInfo = ():
    BlobInfo => {
      ...
      if (parsedURL.pageType !== 'blob' && parsedURL.pageType !== 'tree') {
        throw new Error(`Current URL does not match a blob or tree url: ${
      ...
      // Don't prepend empty filePath with slash; it's the root directory of the
      // repo
      ...
      if (!revisionAndFilePath.endsWith(filePathWithLeadingSlash)) {
        throw new Error(`The file path ${
            filePathWithLeadingSlash} should always be a suffix of revAndFilePath ${
            revisionAndFilePath}, but isn't in this case.`)
      }
      return {
        blob: {
          rawRepoName,
...
export const resolveSnippetFileInfo = (codeView: HTMLElement): BlobInfo => {
  ...
  const commitLinkElement =
      codeView.querySelector('a.commit-tease-sha') as HTMLAnchorElement
  ...
  if (!commitIDMatch?.[1]) {
    throw new Error(`Could not parse commitID from snippet commit link href: ${
        commitLinkElement.href}`)
  }
  const commitID = commitIDMatch[1]

      // We then use the permalink to determine the repo name and parse the
      // filePath.
  ...
  if (parsedURL.pageType !== 'blob') {
    throw new Error(
        `Snippet URL does not match a blob url: ${snippetPermalinkURL.href}`)
  }
  const {revisionAndFilePath, rawRepoName} = parsedURL
  if (!revisionAndFilePath.startsWith(commitID)) {
    throw new Error(`Could not parse filePath: revAndFilePath ${
        revisionAndFilePath} does not start with commitID ${commitID}`)
  }
