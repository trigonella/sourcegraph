...
import { getDiffFileName, parseURL } from "./util";

describe("util", () => {
  describe("parseURL()", () => {
    const testcases: { name: string; url: string }[] = [
      {
        name: "tree page",
      ...
      },
      {
        name: "blob page",
        url:
          "https://github.com/sourcegraph/sourcegraph/blob/3.3/shared/src/hover/HoverOverlay.tsx"
      },
      {
        name: "commit page",
        url:
          "https://github.com/sourcegraph/sourcegraph/commit/fb054666c12b40f180c794db4829cbfd1a5fabae"
      },
      {
      ...
      },
      {
        ...
        url:
          "https://github.com/sourcegraph/sourcegraph/blob/master/jest.config.base.js#L5"
      },
      {
        name: "selections - range",
        url:
          "https://github.com/sourcegraph/sourcegraph/blob/master/jest.config.base.js#L5-L12"
      },
      {
...
describe("getDiffFileName()", () => {
  ...
  const testGetDeltaFilename = ({
    ...
    test("extracts the filename", async () => {
      ...
      // TODO add examples with renamed files and check that
      // getDeltaFilename() doesn't return identical headFilePath &
      // baseFilePath
  ...
  for (const gitHubVersion of ["github.com", "ghe-2.14.11"] as const) {
    describe(gitHubVersion, () => {
      for (const [gitHubDiffPage, expectedFilePath] of Object.entries(
        tests[gitHubVersion]
      )) {
