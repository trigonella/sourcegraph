...
const getDiffCodePart = (codeElement: HTMLElement):
    DiffPart => {
      const tableCell =
          codeElement.closest('td')!
      ...
      // (which may be because it is unchanged, or because the
      // .blob-code(addition|deletion) classes aren't present), call
      // `isSplitDomDiff()`, which will look at the parent code view to
      // determine whether this is a split or unified diff view.
      if (isDomSplitDiff(codeElement)) {
        // If there are more cells on the right, this is the base, otherwise the
        // head
...
/**
 * Returns the 0-based index of the cell that holds the line number for a given
 * part, depending on whether the diff is in unified or split view. Prefers
 * head.
 */
const getLineNumberElementIndex = (part: DiffPart, isSplitDiff: boolean):
    number => {
      if (part === 'base') {
...
/**
 * Gets the line number for a given code element on unified diff, split diff and
 * blob views
 */
const getLineNumberFromCodeElement = (codeElement: HTMLElement):
    number => {
      // In diff views, the code element is the `<span>` inside the cell
      // On blob views, the code element is the `<td>` itself, so `closest()`
      // will simply return it Walk all previous sibling cells until we find one
      // with the line number
      let cell = codeElement.closest('td')!.previousElementSibling as
 ...
 */
const getCodeCellFromTarget = (target: HTMLElement):
    HTMLTableCellElement|null => {
      const cell = target.closest<HTMLTableCellElement>('td.blob-code')
      // Handle rows with the [ ↕ ] button that expands collapsed unchanged
      // lines
      if (!cell ||
          cell.parentElement?.classList.contains('js-expandable-line')) {
        return null
      }
      return cell
    }
...
/**
 * Returns the `<td>` containing the code (which may contain a
 * `.blob-code-inner`)
 */
const getDiffCodeCellFromLineNumber =
    (codeView: HTMLElement, line: number, part?: DiffPart):
        HTMLTableCellElement|null => {
          ...
          const nthChild = getLineNumberElementIndex(part!, isSplitDiff) +
                           1 // nth-child() is 1-indexed
          const lineNumberCell = codeView.querySelector<HTMLTableCellElement>(
              `td:nth-child(${nthChild})[data-line-number="${line}"]`)
          if (!lineNumberCell) {
            return null
          }
          // In unified diff, the not-changed lines shall only be returned for
          // the head. Without this check they would be returned for both head
          // and base.
          if (!isSplitDiff && part === 'base' &&
 ...
 * The code element on diff pages is the `<span class="blob-code-inner">`
 * element inside the cell, because the cell also contains a button to add a
 * comment
 */
const getBlobCodeInner = (codeCell: HTMLTableCellElement): HTMLElement =>
    codeCell.classList.contains('blob-code-inner')
        ? codeCell // `<td>`'s in sections of the table that were expanded are
                   // not commentable so the `.blob-code-inner` element is the
                   // `<td>`
...
export const diffDomFunctions: DOMFunctions = {
  ...
  isFirstCharacterDiffIndicator : codeElement => {
    ...
    //
    // <span class="blob-code-inner">+	fmt.<span
    // class="pl-c1">Println</span>...
    //                               ^
    //
    ...
    // Some versions of GitHub have blob-code-marker-* classes instead of the
    // first character diff indicator.
    const blobCodeInner = codeElement.closest('.blob-code-inner')
    const hasBlobCodeMarker = blobCodeInner && [
      'deletion', 'context', 'addition'
    ].some(name => blobCodeInner.classList.contains('blob-code-marker-' + name))

    // Some versions of GitHub have data-code-marker attributes instead of the
    // first character diff indicator.
    const tableRow = codeElement.closest('tr')
    const hasDataCodeMarkerUnified =
        tableRow?.querySelector('td[data-code-marker]')
    const hasDataCodeMarkerSplit =
        blobCodeInner?.hasAttribute('data-code-marker')
...
const getSingleFileCodeElementFromLineNumber =
    (codeView: HTMLElement, line: number):
        HTMLElement|null => {
          const lineNumberCell =
              codeView.querySelector(`td[data-line-number="${line}"]`)
          // In blob views, the `<td>` is the code element
          return lineNumberCell &&
                 (lineNumberCell.nextElementSibling as HTMLTableCellElement)
        }
...
const getSearchCodeSnippetLineNumberCellFromLineNumber =
    (codeView: HTMLElement, line: number):
        HTMLElement|
    null => {
      ...
      for (const cell of lineNumberCells) {
        const a = cell.querySelector('a')!if (a.href.endsWith(`#L${line}`)) {
...
const getSearchCodeSnippetCodeElementFromLineNumber =
    (codeView: HTMLElement, line: number):
        HTMLElement|null => {
          const lineNumberCell =
              getSearchCodeSnippetLineNumberCellFromLineNumber(codeView, line)
...
export const searchCodeSnippetDOMFunctions: DOMFunctions = {
  ...
  getLineNumberFromCodeElement : (codeElement: HTMLElement) : number => {
    const cell = codeElement.closest('td')!.previousElementSibling as
                 HTMLTableCellElement
 ...
 *
 * @param element, either an element contained in a code view or the code view
 *     itself
 */
export function isDomSplitDiff(element: HTMLElement): boolean {
  ...
  const codeView =
      element.classList.contains('file') ? element : element.closest('.file')
  if (!codeView) {
  ...
  return codeViewTable.classList.contains('js-file-diff-split') ||
         codeViewTable.classList.contains('file-diff-split')
}
