...
describe('GitHub DOM functions', () => {
  ...
  type GitHubDiffPage = 'pull-request'|'pull-request-discussion'|'commit'
      ...
      interface GitHubCodeViewFixture extends
      Omit<DOMFunctionsTest, 'htmlFixturePath'> {}

  const diffFixtures:
      Record<GitHubVersion, Record<GitHubDiffPage, GitHubCodeViewFixture>> =
  {
    'ghe-2.14.11' : {
      commit : {
        url :
            'https://ghe.sgdev.org/beyang/mux/commit/1fddf523893b7475951631ed0f7e09edd9ce50d0',
        lineCases : [
          {
            diffPart : 'head',
            lineNumber : 80,
    ...
    'github.com' : {
      commit : {
        url :
            'https://github.com/sourcegraph/sourcegraph/commit/d3d0fe7fad2c909e3a2e4de2259dc6604983a092',
        lineCases : [
          ...
          for (const extension of ['vanilla', 'refined-github']) {
            describe(startCase(extension), () => {
              if (page === 'pull-request-discussion') {
                const htmlFixturePath = `${__dirname}/__fixtures__/${version}/${
                    page}/${extension}/code-view.html`
                ...
                for (const view of ['split', 'unified']) {
                  const htmlFixturePath = `${__dirname}/__fixtures__/${
                      version}/${page}/${extension}/${view}/code-view.html`
    ...
    describe(`Version ${version}`, () => {
      const views = [
        ...
        describe(`${startCase(view)} page`, () => {
          beforeEach(() => {// TODO ideally DOM functions would not look at
                            // global state like the URL.
