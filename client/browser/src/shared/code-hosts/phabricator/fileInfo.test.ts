...
const DEFAULT_CONDUIT_RESPONSES: ConduitResponseMap = {
  '/api/diffusion.repository.search' : () => of({
    data : [
      {
        fields : {
          callsign : 'MUX',
  ...
  '/api/differential.query' : () => of({
    0 : {
      repositoryPHID : '1',
    },
  }),
  '/api/differential.querydiffs' :
      (parameters: {ids: string[]; revisionIDs : string[]}) => of({
        [parameters.ids[0]] : {
          id : parameters.ids[0],
...
const DEFAULT_GRAPHQL_RESPONSES: GraphQLResponseMap = {
  ...
  ResolveStagingRev : () => of({
    data : {resolvePhabricatorDiff : {oid : 'staging-revision'}},
    errors : undefined,
  } as SuccessGraphQLResult<IMutation>),
}

function mockQueryConduit(responseMap?: ConduitResponseMap):
    QueryConduitHelper<any> {
      return (endpoint, parameters) => {
        const mock = responseMap?.[endpoint] ||
                     DEFAULT_CONDUIT_RESPONSES[endpoint] if (!mock) {
...
type Resolver =
    (codeView: HTMLElement, requestGraphQL: PlatformContext['requestGraphQL'],
     queryConduit: QueryConduitHelper<any>) => Observable<DiffOrBlobInfo>
...
describe('Phabricator file info', () => {
    ...
    describe(
        ...
        () => {test(
            'Commit view',
            async () => {
                expect(await resolveFileInfoFromFixture({
                  htmlFixture : 'commit-view.html',
                  url :
                      'https://phabricator.sgdev.org/rMUXeab9c4f3d22d907d728aa0f5918934357866249e',
    ...
    test('Differential revision - staging repo not synced',
         async () => {
             expect(await resolveFileInfoFromFixture({
               htmlFixture : 'differential-revision.html',
               url : 'https://phabricator.sgdev.org/D7',
    ...
    test('Differential revision - staging repo synced',
         async () => {
             expect(await resolveFileInfoFromFixture({
               htmlFixture : 'differential-revision.html',
               url : 'https://phabricator.sgdev.org/D7',
               codeViewSelector : '.differential-changeset',
               graphQLResponseMap : {
                 // Echoes the raw repo name, to represent the fact that the
                 // repository
                 // exists on the Sourcegraph instance.
                 ResolveRepo : (variables: any) => of({
                   data : {
                     repository : {
    ...
    test(
        'Differential revision - comparing diffs - staging repo not synced',
        async () => {
            expect(await resolveFileInfoFromFixture({
              htmlFixture : 'differential-diff-comparison.html',
              url :
                  'https://phabricator.sgdev.org/D1?vs=2&id=3&whitespace=ignore-most#toc',
              codeViewSelector : '.differential-changeset',
              graphQLResponseMap : {
                ResolveStagingRev : (variables: any) => of({
                  data : {
