 ...
 * `data-n` attribtue (recent Phabricator versions).
 */
const getLineNumberCellFromCodeElement = (
...
const getDiffCodeElementFromLineNumber = (
  ...
  const lineNumberCell = getDiffLineNumberElementFromLineNumber(
    codeView,
    line,
    part
  );
...
export const diffDomFunctions: DOMFunctions = {
  getCodeElementFromTarget: target => {
    ...
    if (
      tableCell.classList.contains("show-more") ||
...
const getDiffusionCodeElementFromLineNumber = (
  ...
  const row = codeView.querySelector<HTMLTableRowElement>(
    `tr:nth-of-type(${line})`
  );
  if (!row) {
...
export const diffusionDOMFns: DOMFunctions = {
  ...
  getLineNumberFromCodeElement: codeElement => {
    ...
    if (lineAnchor.dataset.n !== undefined) {
      ...
      if (isNaN(lineNumber)) {
        throw new TypeError(
          `Could not parse lineNumber from data-n attribute: ${lineAnchor.dataset.n}`
        );
      }
      return lineNumber;
    }
    ...
    if (isNaN(lineNumber)) {
      throw new TypeError(
        `Could not parse lineNumber from lineAnchor.textContent: ${String(
          lineAnchor.textContent
        )}`
      );
    }
    return lineNumber;
