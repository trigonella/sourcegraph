...
/**
 * Returns `true` if the element is a line number cell in a Phabricator diff
 * code views.
 *
 * Supports both `<th>` line number cells, where the line number is the
 * `textContent` (old Phabricator versions) and `<td>` line number cells with a
 * `data-n` attribtue (recent Phabricator versions).
 */
const isLineNumberCell = (element: HTMLElement): boolean => Boolean(
    (element.tagName === 'TH' && element.textContent) || element.dataset.n)

const getLineNumber = (lineNumberCell: HTMLElement): number =>
    parseInt((lineNumberCell.tagName === 'TH' ? lineNumberCell.textContent
                                              : lineNumberCell.dataset.n) ||
                 '',
             10)
    ...
    (codeView: HTMLElement, line: number, part?: DiffPart):
        HTMLElement|null => {
          const lineNumberSelector =
              codeView.querySelector('td[data-n]')
                  ? 'td[data-n]'
                  : `th:nth-of-type(${part === 'base' ? 1 : 2})`
          for (const lineNumberCell of codeView.querySelectorAll<HTMLElement>(
                   lineNumberSelector)) {
            if (getLineNumber(lineNumberCell) === line) {
              if (part === 'head' &&
                  lineNumberCell.previousElementSibling === null) {
...
const getDiffCodeElementFromLineNumber =
    (codeView: HTMLElement, line: number, part?: DiffPart):
        HTMLElement|null => {
          const lineNumberCell =
              getDiffLineNumberElementFromLineNumber(codeView, line, part)
          let codeElement: HTMLElement|null = lineNumberCell
          while (codeElement &&
                 // On unified diffs, some <th> or td.n elements can have an
                 // empty text content or no data-n attribute (for added lines
                 // that did not exist in the base, for instance), in which case
                 // isLineNumberCell returns false.
...
export const diffDomFunctions: DOMFunctions = {
  getCodeElementFromTarget : target => {
    ...
    if (tableCell.classList.contains('show-more') ||
        tableCell.classList.contains('show-context')) {
      // This element represents a collapsed part of the diff, it's not a code
      // element.
      return null
    }
    if (!getLineNumberCellFromCodeElement(tableCell)) {
      // The element has no associated line number cell: this can be the case
      // when hovering 'empty' lines in the base part of a split diff that has
      // added lines.
      return null
    }
    return tableCell
  ...
  getDiffCodePart : codeElement => {
    ...
    // In unified diffs, both <th>'s have a class telling us which side of the
    // diff the line belongs to.
...
const getDiffusionCodeElementFromLineNumber =
    ...
    null => {
      const row =
          codeView.querySelector<HTMLTableRowElement>(`tr:nth-of-type(${line})`)
      if (!row) {
...
export const diffusionDOMFns: DOMFunctions = {
  ...
  getLineNumberFromCodeElement : codeElement => {
    ...
    while (lineCell !== null && lineCell.tagName !== 'TH' &&
    ...
    if (lineAnchor.dataset.n !== undefined) {
      ...
      if (isNaN(lineNumber)) {
        throw new TypeError(
            `Could not parse lineNumber from data-n attribute: ${
                lineAnchor.dataset.n}`)
      }
      return lineNumber
    }
    ...
    if (isNaN(lineNumber)) {
      throw new TypeError(
          `Could not parse lineNumber from lineAnchor.textContent: ${
              String(lineAnchor.textContent)}`)
    }
    return lineNumber
  },
