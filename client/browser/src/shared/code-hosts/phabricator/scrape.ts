export function getFilepathFromFileForDiff(fileContainer: HTMLElement):
    {filePath: string; baseFilePath?: string} {
  ...
  for (const meta of metas) {
    let metaText = meta.textContent!if (metaText.startsWith(movedFilePrefix)) {
      metaText = metaText.slice(0, -1) // remove trailing '.'
      baseFilePath = metaText.split(movedFilePrefix)[1] break
    }
  }
...
export function getFilePathFromFileForRevision(codeView: HTMLElement): string {
  const filePathContainer =
      document.querySelector('.differential-file-icon-header')
  ...
  if (!filePathContainer.textContent) {
    throw new Error(
        '`textContent` is undefined for revision file path container')
  }
