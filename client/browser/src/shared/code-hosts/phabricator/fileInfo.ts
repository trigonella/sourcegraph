...
import {
  getFilepathFromFileForDiff,
  getFilePathFromFileForRevision
} from './scrape'
...
export const resolveRevisionFileInfo =
    (codeView: HTMLElement, requestGraphQL: PlatformContext['requestGraphQL'],
     queryConduit = queryConduitHelper): Observable<DiffInfo> =>
        getPhabricatorState(window.location, requestGraphQL, queryConduit)
            .pipe(map((state): DiffInfo => {
              if (state.mode !== PhabricatorMode.Revision) {
                throw new Error(
                    `Unexpected Phabricator state for resolveRevisionFileInfo, PhabricatorMode: ${
                        state.mode}`)
              }
        ...
        getPhabricatorState(window.location, requestGraphQL, queryConduit)
            .pipe(switchMap(state => {
              if (state.mode !== PhabricatorMode.Differential) {
                throw new Error(
                    `Unexpected PhabricatorState for resolveDiffFileInfo, PhabricatorMode: ${
                        state.mode}`)
              }
              const {filePath, baseFilePath} =
                  getFilepathFromFileForDiff(codeView)
              const resolveBaseCommitID =
                  resolveDiffRevision({
                    ...
                    useDiffForBase : Boolean(
                        state.baseDiffID), // if ?vs and base is not `on` i.e.
                                           // the initial commit)
                    ...
                    isBase : true,
                      ...
                      .pipe(
                          map(({commitID, stagingRepoName}):
                                  Pick<FileInfo, 'commitID'|'rawRepoName'> => ({
                                    commitID,
                                    rawRepoName : stagingRepoName ||
                                                      state.baseRawRepoName,
                                  })))
              const resolveHeadCommitID =
                  resolveDiffRevision({
                      ...
                      .pipe(
                          map(({commitID, stagingRepoName}):
                                  Pick<FileInfo, 'commitID'|'rawRepoName'> => ({
                                    commitID,
                                    rawRepoName : stagingRepoName ||
                                                      state.headRawRepoName,
                                  })))
              return zip(resolveBaseCommitID, resolveHeadCommitID)
                  .pipe(map(([ baseInfo, headInfo ]): DiffInfo => ({
                              base : {
...
export const resolveDiffusionFileInfo =
    (codeView: HTMLElement, requestGraphQL: PlatformContext['requestGraphQL'],
     queryConduit = queryConduitHelper): Observable<BlobInfo> =>
        getPhabricatorState(window.location, requestGraphQL, queryConduit)
            .pipe(map((state): BlobInfo => {
              if (state.mode !== PhabricatorMode.Diffusion) {
                throw new Error(
                    `Unexpected PhabricatorState for resolveDiffusionFileInfo, PhabricatorMode: ${
                        state.mode}`)
              }
