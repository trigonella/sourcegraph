...
/**
 * Gets the actual text content we care about and returns the number of
 * characters we have stripped so that we can adjust accordingly.
 */
const getTextContent = (element: HTMLElement):
    {textContent: string; adjust : number} => {
      let textContent = element.textContent || ''
      let adjust = 0

      // For some reason, phabricator adds an invisible element to the beginning
      // of lines containing the diff indicator followed by a space (ex: '+ ').
      ...
      if (element.firstElementChild?.classList.contains('aural-only')) {
        ...
        // Codeintellify handles ignoring one character for diff indicators so
        // we'll allow it to adjust for that.
      ...
      // Phabricator adds a no-width-space to the beginning of the line in some
      // cases. We need to strip that and account for it here.
...
const getPositionAdjuster = (requestGraphQL: PlatformContext['requestGraphQL']):
    PositionAdjuster<RepoSpec&RevisionSpec&FileSpec&ResolvedRevisionSpec> => (
      ...
      const actualLineContent = lines[position.line - 1]
          ...
          const convertSpaces =
              convertSpacesToTabs(actualLineContent, documentLineContent)

      // Whether the adjustment should add or subtract from the given position.
      const modifier =
          direction === AdjustmentDirection.CodeViewToActual ? -1 : 1

      return convertSpaces
                 ? adjustCharacter(
                       position, (spacesToTabsAdjustment(documentLineContent) +
...
const toolbarButtonProps = {
  ...
  getToolbarMount : (codeView: HTMLElement) : HTMLElement => {
    let mount = codeView.querySelector<HTMLElement>(
        '.sourcegraph-phabricator-code-view-toolbar-mount')
    if (mount) {
      return mount
    }
...
export const diffCodeView = {
  ...
  getToolbarMount : (codeView: HTMLElement) : HTMLElement => {
    ...
    const mountLocation =
        codeView.querySelector('.differential-changeset-buttons')
    if (!mountLocation) {
...
const diffusionSourceCodeViewResolver =
    toCodeViewResolver('.diffusion-source', {
      dom : diffusionDOMFns,
      resolveFileInfo : resolveDiffusionFileInfo,
      getToolbarMount : () => {
        const actions = document.querySelector<HTMLElement>(
            '.phui-two-column-content .phui-header-action-links')
        if (!actions) {
...
const phabSourceCodeViewResolver =
    toCodeViewResolver('.phabricator-source-code-container', {
      dom : diffusionDOMFns,
      resolveFileInfo : resolveDiffusionFileInfo,
    })

export const checkIsPhabricator = (): boolean =>
...
export const phabricatorCodeHost: CodeHost = {
  ...
  // and find a way to listen to changes (Phabricator does not emit popstate
  // events).
