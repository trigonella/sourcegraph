...
type PhabricatorVersion = '2017.09-r1'|'2019.21.0-r25'
    ...
    interface PhabricatorCodeViewFixture extends
    Pick<DOMFunctionsTest, 'lineCases'> {}

describe('Phabricator DOM functions', () => {
    describe('diffDOMFunctions', () => {
  const DIFF_FIXTURES:
      Record<PhabricatorVersion,
             Partial<Record<PhabricatorPage, (view: 'split'|'unified') =>
                                                 PhabricatorCodeViewFixture>>> =
  {
    '2017.09-r1' : {
      commit : () => ({
        lineCases : [
      ...
      differential : view => ({
        lineCases : [
          {diffPart : 'head', lineNumber : 29}, // not changed
          {
            diffPart : 'head',
            lineNumber : 64,
            firstCharacterIsDiffIndicator : view === 'unified'
          }, // added
          {
            diffPart : 'base',
            lineNumber : 34,
    ...
    for (const [page, testCase] of Object.entries(testCases)) {
      ...
      describe(`${startCase(page)} Page`, () => {
        for (const view of ['split', 'unified'] as const) {
          const htmlFixturePath = `${__dirname}/__fixtures__/code-views/${
              version}/${page}/${view}.html`
