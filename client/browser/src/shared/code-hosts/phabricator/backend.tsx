...
function createConduitRequestForm(): FormData {
  ...
  form.set(
    "__csrf__",
    searchForm.querySelector<HTMLInputElement>("input[name=__csrf__]")!.value
  );
  form.set(
    "__form__",
    searchForm.querySelector<HTMLInputElement>("input[name=__form__]")!.value
  );
  return form;
}
 ...
 */
export async function getPhabricatorCSS(
  sourcegraphURL: string
): Promise<string> {
  const bundleUID = process.env.BUNDLE_UID!;
  const response = await fetch(
    sourcegraphURL + `/.assets/extension/css/style.bundle.css?v=${bundleUID}`,
    {
      method: "GET",
      credentials: "include",
...
export type QueryConduitHelper<T> = (
  endpoint: string,
  parameters: {}
) => Observable<T>;
 ...
 */
export function queryConduitHelper<T>(
  endpoint: string,
  parameters: {}
): Observable<T> {
...
const getRepoPHIDForRevisionID = memoizeObservable(
  ({
    ...
    queryConduit<ConduitDifferentialQueryResponse>("/api/differential.query", {
      ids: [revisionID]
    }).pipe(
      map(result => {
...
interface CreatePhabricatorRepoOptions
  extends Pick<PlatformContext, "requestGraphQL"> {
  callsign: string;
  repoName: string;
...
const createPhabricatorRepo = memoizeObservable(
  ({
    requestGraphQL,
    ...variables
  }: CreatePhabricatorRepoOptions): Observable<void> =>
  ...
  return queryConduit("/api/diffusion.repository.search", {
    ...
    switchMap(({ data }) => {
      ...
      if (!repo.attachments || !repo.attachments.uris) {
        throw new Error(
          `could not locate git uri for repo with callsign ${callsign}`
        );
      }
...
export function getSourcegraphURLFromConduit(): Promise<string> {
  return queryConduitHelper<{ url: string }>(
    "/api/sourcegraph.configuration",
    {}
  )
    .pipe(map(({ url }) => url))
    .toPromise();
  ...
  ({
    ...
    queryConduit<ConduitReposResponse>("/api/diffusion.repository.search", {
      ...
      switchMap(({ data }) => {
        ...
        if (!repo.attachments || !repo.attachments.uris) {
          throw new Error(
            `could not locate git uri for repo with phid ${phid}`
          );
        }
...
export function getRepoDetailsFromRevisionID(
  ...
  return getRepoPHIDForRevisionID({ revisionID, queryConduit }).pipe(
    switchMap(phid =>
      getRepoDetailsFromRepoPHID({ phid, requestGraphQL, queryConduit })
    )
  );
}

async function convertConduitRepoToRepoDetails(
  repo: ConduitRepo
): Promise<PhabricatorRepoDetails | null> {
  if (isExtension) {
  ...
  const mappingsString = window.localStorage.getItem(
...
function convertToDetails(repo: ConduitRepo): PhabricatorRepoDetails | null {
  const enabledURIs = repo.attachments.uris.uris
    ...
    .map(({ fields }) => ({
      isExternalURI: !fields.uri.normalized
        .replace("\\", "")
        .startsWith(window.location.host + "/"),
  ...
  const { rawURI } =
    enabledURIs.find(({ isExternalURI }) => isExternalURI) ?? enabledURIs[0];
  ...
  return getDiffDetailsFromConduit({ ...props, queryConduit }).pipe(
    switchMap(diffDetails => {
      if (
        props.isBase ||
        !props.baseDiffID ||
        hasThisFileChanged(props.filePath, diffDetails.changes)
      ) {
...
function getStagingDetails(
  ...
  | { repoName: string; ref: ConduitReference; unconfigured: boolean }
  | undefined {
  ...
  for (const reference of propsWithInfo.diffDetails.properties["arc.staging"]
    .refs) {
  ...
  return getPropsWithDiffDetails(props, queryConduit).pipe(
    switchMap(({ diffDetails, ...props }) => {
      ...
      if (!stagingDetails || stagingDetails.unconfigured) {
        ...
        return getRawDiffFromConduit({
          diffID: props.diffID,
          queryConduit
        }).pipe(
          switchMap(patch =>
            resolveStagingRevision({ ...conduitProps, patch, requestGraphQL })
          )
        );
      }
      ...
      return resolveRepo({
        rawRepoName: stagingDetails.repoName,
        requestGraphQL
      }).pipe(
