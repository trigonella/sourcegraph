...
import {
  ...
  SourcegraphIconButtonProps
} from "../../components/SourcegraphIconButton";
...
export const ViewOnSourcegraphButton: React.FunctionComponent<ViewOnSourcegraphButtonProps> = ({
  ...
  const mutedIconClassName = classNames(
    "open-on-sourcegraph__icon--muted",
    iconClassName
  );
  ...
  if (isErrorLike(repoExistsOrError)) {
    ...
    const commonErrorCaseProps: Partial<SourcegraphIconButtonProps> = {
      ...
      href: new URL(
        snakeCase(codeHostType),
        "https://docs.sourcegraph.com/integration/"
      ).href,
