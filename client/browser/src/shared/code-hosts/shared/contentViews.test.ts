...
describe('contentViews', () => {
  ...
  return element
        ...
        subscriptions.add(handleContentViews(
                  ...
                  linkPreviews : {
                    provideLinkPreview : url => {
                      ...
                      return concat(
                          ...
                          // Support checking that the provider's observable was
                          // unsubscribed.
            ...
            },
            {
              contentViewResolvers :
                  [ {selector : 'div', resolveView : () => ({element})} ],
              setElementTooltip : (element, text) =>
                  text !== null ? (element.dataset.tooltip = text)
                                : element.removeAttribute('data-tooltip'),
            }))

        // Add content view.
        mutations
            .next([ {addedNodes : [ document.body ], removedNodes : []} ])
                await wait.pipe(first())
            .toPromise()
        ...
        // Mutate content view.
        element.innerHTML = '4 <a href=#zip>zip</a> 5' await Promise.all([
          unsubscribed.pipe(first()).toPromise(), wait.pipe(first()).toPromise()
        ])
        expect(element.innerHTML)
            .toBe(
                ...
                services : {
                  linkPreviews : {
                    provideLinkPreview : url => {
                      ...
                      return url.includes('bar') ? of(null)
                                                 : fooLinkPreviewValues
                    },
                  },
                },
              },
        ...
        expect(element.innerHTML)
            .toBe(
                '0 <a href="#foo" data-tooltip="foo2">foo</a> 1 <a href="#bar">bar</a> 2')
