...
} from "../../../../../shared/src/api/client/services/viewerService";
import {
...
import {
  SourcegraphIntegrationURLs,
  BrowserPlatformContext
} from "../../platform/context";
import {
  toTextDocumentIdentifier,
  toTextDocumentPositionParameters
} from "../../backend/extension-api-conversion";
import {
  CodeViewToolbar,
  CodeViewToolbarClassProps
} from "../../components/CodeViewToolbar";
import {
  resolveRevision,
...
import {
  EventLogger,
  ConditionalTelemetryService
} from "../../tracking/eventLogger";
 ...
 */
export type CodeHostContext = RawRepoSpec &
  Partial<RevisionSpec> & { privateRepository: boolean };
...
export interface CodeHost extends ApplyLinkPreviewOptions {
   ...
   */
  commandPaletteClassProps?: CommandListPopoverButtonClassProps &
    CommandListClassProps;
...
export const createOverlayMount = (
  codeHostName: string,
  ...
  mount.classList.add(
    "hover-overlay-mount",
    `hover-overlay-mount__${codeHostName}`,
    "theme-light"
  );
...
> & {
  ...
  >({
    ...
    getHover: ({ line, character, part, ...rest }) =>
      concat(
        ...
        combineLatest([
          ...
          map(
            ([
              { isLoading, result: hoverMerged },
              alerts
            ]): MaybeLoadingResult<HoverMerged | null> => ({
              isLoading,
              result:
                hoverMerged || alerts?.length
                  ? { contents: [], ...hoverMerged, alerts }
                  : null
            })
          )
        )
    ...
    getActions: context =>
      getHoverActions({ extensionsController, platformContext }, context),
    pinningEnabled: true,
  ...
  > {
    ...
    private nextOverlayElement = hoverOverlayElements.next.bind(
      hoverOverlayElements
    );
    private nextCloseButtonClick = closeButtonClicks.next.bind(
      closeButtonClicks
    );
  ...
  if (!getHoverOverlayMountLocation) {
    ...
    subscription.add(
      observeHoverOverlayMountLocation(
        getHoverOverlayMountLocation,
        mutations
      ).subscribe(mountLocation => {
...
export function observeHoverOverlayMountLocation(
  getMountLocationSelector: NonNullable<
    CodeHost["getHoverOverlayMountLocation"]
  >,
  mutations: Observable<MutationRecordLike[]>
  ...
  return mutations.pipe(
    ...
    map(({ addedNodes, removedNodes }): HTMLElement | null => {
      ...
      for (const addedNode of addedNodes) {
        ...
        const mountLocation = querySelectorOrSelf<HTMLElement>(
          addedNode,
          selector
        );
        if (mountLocation) {
...
export interface HandleCodeHostOptions extends CodeIntelligenceProps {
  ...
  background: Pick<
    BackgroundPageApi,
    "notifyPrivateRepository" | "openOptionsPage"
  >;
}
...
export function handleCodeHost({
  ...
  const isPrivateRepo =
    !codeHost.getContext || codeHost.getContext().privateRepository;
  background.notifyPrivateRepository(isPrivateRepo).catch(error => {
    console.error(
      "Error notifying background page of private repository:",
      error
    );
  });
  ...
  if (codeHost.getCommandPaletteMount && !minimalUI) {
    subscriptions.add(
      addedElements
        .pipe(map(codeHost.getCommandPaletteMount), filter(isDefined))
        .subscribe(
          renderCommandPalette({
            extensionsController,
            history,
        ...
        render(
          <ViewOnSourcegraphButton
            ...
            onConfigureSourcegraphClick={
              isInPage ? undefined : onConfigureSourcegraphClick
            }
          />,
          mount
        );
  ...
  const codeViews = mutations.pipe(
    ...
    mergeMap(codeViewEvent =>
      asObservable(() =>
        codeViewEvent.resolveFileInfo(
          codeViewEvent.element,
        ...
        tap({
          error: error => {
            if (codeViewEvent.getToolbarMount) {
              const mount = codeViewEvent.getToolbarMount(
                codeViewEvent.element
              );
              render(
                <CodeViewToolbar
  ...
  const deleteRootReference = (uri: string): void => {
    ...
    if (updatedReferenceCount === 0) {
      extensionsController.services.workspace.roots.next(
        extensionsController.services.workspace.roots.value.filter(
          root => root.uri !== uri
        )
      );
    } else {
  ...
  subscriptions.add(
    codeViews.subscribe(codeViewEvent => {
      ...
      const initializeModelAndViewerForFileInfo = (
        ...
        const editorId = extensionsController.services.viewer.addViewer(
          editorData
        );
      ...
      const initializeModelAndViewerForDiffOrFileInfo = (
        ...
        if (diffOrFileInfo.head && diffOrFileInfo.base) {
          ...
          const editor = initializeModelAndViewerForFileInfo(
            diffOrFileInfo.head
          );
      ...
      const codeEditorWithPartialModel = initializeModelAndViewerForDiffOrFileInfo(
        diffOrBlobInfo
      );
      ...
      const applyDecorationsForFileInfo = (
        ...
        const update = (
          ...
          try {
            ...
            console.error(
              "Could not apply decorations to code view",
              codeViewEvent.element,
              error
            );
          }
      ...
      const resolveContext: ContextResolver<RepoSpec &
        RevisionSpec &
        FileSpec &
        ResolvedRevisionSpec> = ({ part }) => {
        if ("blob" in diffOrBlobInfo) {
        ...
        throw new Error(
          `Could not resolve context for diff part ${JSON.stringify(part)}`
        );
      };
...
export const determineCodeHost = (): CodeHost | undefined =>
  CODE_HOSTS.find(codeHost => codeHost.check());
...
export function injectCodeIntelligenceToCodeHost(
  ...
  const overrideSendTelemetry = observeSourcegraphURL(isExtension).pipe(
    map(sourcegraphUrl =>
      shouldOverrideSendTelemetry(isFirefox(), isExtension, sourcegraphUrl)
    )
  );

  const observeSendTelemetry = combineLatest([
    overrideSendTelemetry,
    observeOptionFlag("sendTelemetry")
  ]).pipe(
  ...
  const telemetryService = new ConditionalTelemetryService(
    innerTelemetryService,
  ...
  const minimalUI =
    minimalUIStorageFlag !== null
      ? minimalUIStorageFlag === "true"
      : codeHost.type === "gitlab" && !isExtension;
  subscriptions.add(
    extensionDisabled.subscribe(disableExtension => {
      if (disableExtension) {
        ...
        console.log(
          `${
            isExtension ? "Browser extension" : "Native integration"
          } is enabled`
        );
      }
    })
