...
describe("codeHost", () => {
  ...
  describe("createOverlayMount()", () => {
    it("should create the overlay mount", () => {
      ...
      expect(mount!.className).toBe(
        "hover-overlay-mount hover-overlay-mount__some-code-host theme-light"
      );
    });
  });
  ...
  describe("handleCodeHost()", () => {
    ...
    test("renders the hover overlay mount", async () => {
      ...
      expect(overlayMount!.className).toBe(
        "hover-overlay-mount hover-overlay-mount__github theme-light"
      );
    ...
    test("renders the command palette if codeHost.getCommandPaletteMount is defined", async () => {
      ...
      const renderedCommandPalette = elementRenderedAtMount(
        commandPaletteMount
      );
    ...
    test("detects code views based on selectors", async () => {
      ...
      await from(services.viewer.viewerUpdates)
        .pipe(first())
        .toPromise();
    ...
    describe("Decorations", () => {
      it("decorates a code view", async () => {
        ...
        if (activeEditor.type !== "CodeEditor") {
          throw new Error(
            `Expected active editor to be CodeEditor, got ${activeEditor.type}`
          );
        }
        ...
        const decorated = (): Promise<TextDocumentDecoration[] | null> =>
          services.textDocumentDecoration
            ...
            .pipe(
              filter(decorations =>
                Boolean(decorations && decorations.length > 0)
              ),
              take(1)
            )
            .toPromise();
        ...
        await decorated();
        expect(
          line.querySelectorAll(".line-decoration-attachment")
        ).toHaveLength(1);
        expect(
          line.querySelector(".line-decoration-attachment")!.textContent
        ).toEqual("test decoration");
        ...
        await services.textDocumentDecoration
          ...
          .toPromise();
        expect(
          line.querySelectorAll(".line-decoration-attachment").length
        ).toBe(1);
        expect(
          line.querySelector(".line-decoration-attachment")!.textContent
        ).toEqual("test decoration 2");
      });
      ...
      it("decorates a diff code view", async () => {
        ...
        const dom = {
          getCodeElementFromTarget: (target: HTMLElement) =>
            target.closest(".code-element") as HTMLElement,
          getCodeElementFromLineNumber: (
            codeView: HTMLElement,
            line: number,
            part?: DiffPart
          ) =>
            codeView.querySelector<HTMLElement>(
              `[line="${line}"][part="${String(part)}"] > .code-element`
            ),
        ...
        const decorated = (
          commit: string
        ): Promise<TextDocumentDecoration[] | null> =>
          services.textDocumentDecoration
        ...
        handleCodeHost({
          ...
          codeHost: {
            ...
            nativeTooltipResolvers: [
              { selector: ".native", resolveView: element => ({ element }) }
            ],
            codeViewResolvers: [
              toCodeViewResolver("#code", {
                dom,
    ...
    test("Hides native tooltips if they are disabled from settings", async () => {
      ...
      codeView.dispatchEvent(new MouseEvent("mouseover"));
      sinon.assert.called(dom.getCodeElementFromTarget);
      expect(nativeTooltip.classList.contains("native-tooltip--hidden")).toBe(
        true
      );
    });
        ...
        handleCodeHost({
          ...
          platformContext: createMockPlatformContext({
            ...
            requestGraphQL: mockRequestGraphQL({
              ...
              BlobContent: () =>
                throwError(
                  new PrivateRepoPublicSourcegraphComError("BlobContent")
                ),
              ResolveRepo: () =>
                throwError(
                  new PrivateRepoPublicSourcegraphComError("ResolveRepo")
                ),
              ResolveRev: () =>
                throwError(
                  new PrivateRepoPublicSourcegraphComError("ResolveRev")
                )
            })
          })
        })
