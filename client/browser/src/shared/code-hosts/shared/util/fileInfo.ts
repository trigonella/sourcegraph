 ...
 * was not available.
 */
const useRawRepoNameAsFallback = (
  fileInfo: FileInfo
 ...
 * value is present.
 */
export const defaultRevisionToCommitID = <T extends FileInfo>(
  fileInfo: T
): T & { revision: string } => ({
  ...fileInfo,
...
export const ensureRevisionIsClonedForFileInfo = (
  ...
  return resolveRevision({ repoName, revision: commitID, requestGraphQL }).pipe(
    retryWhenCloneInProgressError()
  );
};
