...
const resolveRepoNameForFileInfo =
    (fileInfo: FileInfo, requestGraphQL: PlatformContext['requestGraphQL']):
        Observable<FileInfoWithRepoName> =>
            resolveRepo({rawRepoName : fileInfo.rawRepoName, requestGraphQL})
                .pipe(map(repoName => ({...fileInfo, repoName})),
                      catchError(error => {
                        // ERPRIVATEREPOPUBLICSOURCEGRAPHCOM likely means that
                        // the user is viewing private code without having
                        // pointed his browser extension to a self-hosted
                        // Sourcegraph instance that has access to that code. In
                        // that case, it's impossible to resolve the repo names,
                        // so we keep the repo names inferred from the code
                        // host's DOM.
 ...
 * value is present.
 */
export const defaultRevisionToCommitID =
    <T extends FileInfo>(fileInfo: T): T&{revision : string} => ({
      ...fileInfo,
     ...
     requestGraphQL: PlatformContext['requestGraphQL']):
        Observable<string> => {
          ...
          return resolveRevision(
                     {repoName, revision : commitID, requestGraphQL})
     ...
     requestGraphQL: PlatformContext['requestGraphQL']):
        Observable<DiffOrBlobInfo<FileInfoWithRepoName>> => {
          if ('blob' in diffOrFileInfo) {
            return resolveRepoNameForFileInfo(diffOrFileInfo.blob,
                                              requestGraphQL)
                .pipe(map(fileInfo =>
                              ({blob : {...diffOrFileInfo.blob, ...fileInfo}})))
          }
          if (diffOrFileInfo.head && diffOrFileInfo.base) {
            const resolvingHeadWithRepoName =
                resolveRepoNameForFileInfo(diffOrFileInfo.head, requestGraphQL)
            const resolvingBaseWithRepoName =
                resolveRepoNameForFileInfo(diffOrFileInfo.base, requestGraphQL)

            return zip(resolvingHeadWithRepoName, resolvingBaseWithRepoName)
                .pipe(map(([ head, base ]) => ({head, base})))
          }
          if (diffOrFileInfo.head) {
            return resolveRepoNameForFileInfo(diffOrFileInfo.head,
                                              requestGraphQL)
                .pipe(map(head => ({head})))
          }
          return resolveRepoNameForFileInfo(diffOrFileInfo.base, requestGraphQL)
              .pipe(map(base => ({base})))
        }
