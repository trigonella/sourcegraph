...
const mountGetterKeys =
    [ 'getCommandPaletteMount', 'getViewContextOnSourcegraphMount' ] as
    const type MountGetterKey = typeof mountGetterKeys[number]

    /**
     * @param containerHtmlFixturePaths Paths to full-document fixtures keyed by
     *     the mount getter function name
     */
    export function
    testCodeHostMountGetters<C extends CodeHost>(
        codeHost: C, containerHtmlFixturePaths: string|
        Record<SetIntersection<MountGetterKey, keyof C>, string>): void {
  for (const mountGetterKey of mountGetterKeys) {
    const getMount = codeHost[mountGetterKey] if (!getMount) { continue }
...
export function testToolbarMountGetter(
    ...
    getToolbarMount: NonNullable<CodeView['getToolbarMount']>): void {
...
export async function getFixtureBody({
    ...
    htmlFixturePath: string
      ...
      // Do not append to global document to test that the mount getter only
      ...
      if (template.content.children.length !== 1) {
        throw new Error(`Fixture must have exactly one element, has ${
            template.content.children.length}: ${htmlFixturePath}`)
      }
    ...
    /**
     * @param isFullDocument Whether the fixture HTML is a document fragment or
     *     a full document (does it contain `<html>`?)
     * @param mayReturnNull Whether the mount getter might be called with
     *     containers where the mount does not belong.
     * It will be tested to return `null` in that case.
     */
    export function testMountGetter(
        htmlFixturePath: string, getMount: MountGetter, isFullDocument: boolean,
        mayReturnNull: boolean): void {
      it('creates and returns a new mount in the container', async () => {
        const container =
            await getFixtureBody({isFullDocument, htmlFixturePath})
    ...
    export interface DOMFunctionsTest {
      ...
      url?: string // TODO DOM functions should not rely on global state like
                   // the URL
    }
    ...
    export function testDOMFunctions(
      ...
      beforeEach(async () => {
        ...
        codeViewElement =
            await getFixtureBody({htmlFixturePath, isFullDocument : false})
      })
      for (const {diffPart,
                  lineNumber,
                  firstCharacterIsDiffIndicator} of codeElements) {
        describe(`line number ${lineNumber}` +
                     (diffPart !== undefined
                          ? ` in ${String(diffPart)} diff part`
                          : ''),
                 () => {
        ...
        codeElement = domFunctions.getCodeElementFromLineNumber(
            codeViewElement, lineNumber, diffPart)!if (!codeElement) {
          throw new Error(
              'Test depends on test for getCodeElementFromLineNumber() passing')
        }
                }
        ...
        // Default is false
        const actualFirstCharacterIsDiffIndicator =
            Boolean(domFunctions.isFirstCharacterDiffIndicator?.(codeElement))
        expect(actualFirstCharacterIsDiffIndicator)
            .toBe(Boolean(firstCharacterIsDiffIndicator))
        if (actualFirstCharacterIsDiffIndicator) {
          ...
          expect(actualFirstCharacterIsDiffIndicator)
              .toBe(diffIndicators.has(codeElement.textContent![0]))
        }
                    })
                })
                 })
