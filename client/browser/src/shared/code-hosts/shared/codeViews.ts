...
export const trackCodeViews = ({
  codeViewResolvers
}: Pick<CodeHost, "codeViewResolvers">): OperatorFunction<
  MutationRecordLike[],
  ViewWithSubscriptions<CodeView>
...
export const fetchFileContentForDiffOrFileInfo = (
  ...
  if ("blob" in diffOrBlobInfo) {
    return fetchFileContentForFileInfo(
      diffOrBlobInfo.blob,
      requestGraphQL
  ...
  if (diffOrBlobInfo.head && diffOrBlobInfo.base) {
    ...
    return zip(fetchingBaseFile, fetchingHeadFile).pipe(
      map(
        ([base, head]): DiffOrBlobInfo<FileInfoWithContent> => ({ head, base })
      )
    );
  }
  if (diffOrBlobInfo.head) {
    return fetchFileContentForFileInfo(
      diffOrBlobInfo.head,
      requestGraphQL
    ).pipe(map((head): DiffOrBlobInfo<FileInfoWithContent> => ({ head })));
  }
