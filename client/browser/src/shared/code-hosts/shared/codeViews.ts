...
export interface DOMFunctions extends CodeIntellifyDOMFuncions {
   ...
   */
  getLineElementFromLineNumber: (codeView: HTMLElement, line: number,
                                 part?: DiffPart) => HTMLElement | null
}

/**
 * Defines a code view that is present on a page.
 * Exposes operations for manipulating it, and CSS classes to be applied to
...
export interface CodeView {
   ...
   */
  resolveFileInfo: (codeView: HTMLElement,
   ...
   */
  getPositionAdjuster?: (requestGraphQL: PlatformContext['requestGraphQL']) =>
 ...
 */
export const toCodeViewResolver =
    (selector: string, spec: Omit<CodeView, 'element'>):
...
/**
 * Find all the code views on a page using both the code view specs and the code
 * view spec resolvers, calling down to {@link trackViews}.
 */
export const trackCodeViews = ({
  codeViewResolvers,
}: Pick<CodeHost, 'codeViewResolvers'>):
    OperatorFunction<MutationRecordLike[], ViewWithSubscriptions<CodeView>> =>
    ...
    (fileInfo: FileInfoWithRepoName,
     requestGraphQL: PlatformContext['requestGraphQL']):
        Observable<FileInfoWithContent> =>
            ensureRevisionIsClonedForFileInfo(fileInfo, requestGraphQL)
                .pipe(switchMap(() => fetchBlobContentLines({
     ...
     requestGraphQL: PlatformContext['requestGraphQL']):
        Observable<DiffOrBlobInfo<FileInfoWithContent>> => {
          if ('blob' in diffOrBlobInfo) {
            return fetchFileContentForFileInfo(diffOrBlobInfo.blob,
                                               requestGraphQL)
                .pipe(map(fileInfo => ({blob : fileInfo})))
          }
          if (diffOrBlobInfo.head && diffOrBlobInfo.base) {
            const fetchingBaseFile =
                fetchFileContentForFileInfo(diffOrBlobInfo.base, requestGraphQL)
            const fetchingHeadFile =
                fetchFileContentForFileInfo(diffOrBlobInfo.head, requestGraphQL)

            return zip(fetchingBaseFile, fetchingHeadFile)
                .pipe(
                    map(([ base, head ]): DiffOrBlobInfo<FileInfoWithContent> =>
                            ({head, base})))
          }
          if (diffOrBlobInfo.head) {
            return fetchFileContentForFileInfo(diffOrBlobInfo.head,
                                               requestGraphQL)
                .pipe(map((head): DiffOrBlobInfo<FileInfoWithContent> =>
                              ({head})))
          }
          return fetchFileContentForFileInfo(diffOrBlobInfo.base,
                                             requestGraphQL)
              .pipe(map(base => ({base})))
        }
