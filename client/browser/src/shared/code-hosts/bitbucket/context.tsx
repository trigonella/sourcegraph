...
function getRawRepoSpecFromLocation(
...
function getRevisionSpecFromRevisionSelector(): RevisionSpec {
  const branchNameElement = document.querySelector(
    "#repository-layout-revision-selector .name[data-revision-ref]"
  );
  if (!branchNameElement) {
    throw new Error("branchNameElement not found");
  }
  const revisionReferenceString = branchNameElement.getAttribute(
    "data-revision-ref"
  );
  ...
  if (revisionReferenceString) {
    try {
      ...
      throw new Error(
        `Could not parse revisionRefStr: ${revisionReferenceString}`
      );
    }
  }
  ...
  throw new Error(
    `revisionRefInfo is empty or has no latestCommit (revisionRefStr: ${String(
      revisionReferenceString
    )})`
  );
}
