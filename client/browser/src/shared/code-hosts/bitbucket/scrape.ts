...
const LINK_SELECTORS =
    [ 'a.raw-view-link', 'a.source-view-link', 'a.mode-source' ]

    const bitbucketToSourcegraphRepoName =
        ({repoSlug, project}: BitbucketRepoInfo):
            string => [window.location.hostname, project, repoSlug].join('/')
 ...
 * Depending on the configuration of the page, this can be a link to the raw
 * file, or to the original source view, so a few different selectors are tried.
 *
 ...
 */
const getFileInfoFromLinkInSingleFileView = (codeView: HTMLElement):
    Pick<FileInfo, 'rawRepoName'|'filePath'|'revision'>&BitbucketRepoInfo => {
      const errors: Error[] = [] for (const selector of LINK_SELECTORS) {
        try {
          ...
          // Looks like /projects/<project>/repos/<repo>/(browse|raw)/<file
          // path>?at=<revision>
          const pathMatch = path.match(
              /\/projects\/(.*?)\/repos\/(.*?)\/(?:browse|raw)\/(.*)$/)
          if (!pathMatch) {
            throw new Error(`Path of link matching selector ${
                selector} did not match path regex: ${path}`)
          }
          ...
          if (!atParameter) {
            throw new Error(`href of link matching selector ${
                selector} did not have 'at' search param: ${url.href}`)
          }
          ...
          return {
            rawRepoName: bitbucketToSourcegraphRepoName({repoSlug, project}),
                filePath: decodeURIComponent(filePath), revision, project,
                repoSlug,
          }
        } catch (error) {
          errors.push(error)
 ...
 * found on single file views (both source and "diff to previous" views) and
 * commit pages.
 */
export const getCommitIDFromLink = (selector = 'a.commitid'):
    string => {
...
const getCommitIDFromRevisionSelector = ():
    string => {
      const revisionSelectorSpan =
          document.querySelector<HTMLElement>('span[data-revision-ref]')
      if (!revisionSelectorSpan) {
        throw new Error('Could not find span[data-revision-ref] element')
      }
      try {
        const {latestCommit}: {latestCommit: string} =
            JSON.parse(revisionSelectorSpan.dataset.revisionRef!)
        return latestCommit
      } catch {
...
export const getFileInfoFromSingleFileSourceCodeView =
    (codeViewElement: HTMLElement):
        BitbucketRepoInfo&FileInfo => {
          const {rawRepoName, filePath, revision, project, repoSlug} =
              getFileInfoFromLinkInSingleFileView(codeViewElement)
          const commitID = getCommitIDFromRevisionSelector()
          return {
            rawRepoName, filePath, revision, commitID, project, repoSlug,
          }
        }
 ...
 */
export const isCompareView = (): boolean =>
    !!document.querySelector('#branch-compare')

/**
 * Returns true if the active page is a commit view.
 */
export const isCommitsView =
    ({pathname}: Pick<Location, 'pathname'>): boolean =>
...
/**
 * Returns true if the given code view is a single file source or "diff to
 * previous" view. These views have a toggle to toggle between "source" and
 * "diff to previous".
 */
 ...
 * Returns `null` if there is no badge on the page (this is expected on
 * single-file diff pages if the file was _modified_).
 */
const getChangeTypeElement =
    ({codeViewElement}: {codeViewElement: HTMLElement}): HTMLElement|null =>
...
const getChangeType = ({ changeTypeElement }: { changeTypeElement: HTMLElement | null }): ChangeType => {
  ...
  const className = [...changeTypeElement.classList ].find(
      className => /^change-type-[A-Z]+/.test(className))
  if (!className) {
...
changeType: ChangeType
  ...
  if (changeType === 'MOVE' || changeType === 'RENAME' ||
    ...
    if (!changeTypeElement) {
      throw new Error(
          `Change type is ${changeType} but no change type indicator found`)
    }
    // Need to read previous file path from change type indicator
    // Contains HTML content, example:
    // <span class="deleted">.github</span>/stale.yml &rarr;<br><span
    // class="added">test-dir</span>/stale.yml
    const tooltip = changeTypeElement.getAttribute('original-title')
    if (!tooltip) {
      throw new Error(
          'Moved change type badge did not have original-title attribute')
    }
    const span = document.createElement('span')
    span.innerHTML = tooltip
    const tooltipText =
        span.textContent!if (changeType === 'MOVE' || changeType === 'COPY') {
      const from = tooltipText.split('→')[0].trim()
      if (!from) {
        throw new Error(
            `Unexpected move change type badge content "${tooltipText}"`)
      }
      return from
    }
    if (changeType === 'RENAME') {
      ...
      if (!match) {
        throw new Error(`Rename change type badge content did not match ${
            renameRegexp.toString()}: "${tooltipText}"`)
      }
...
export const getFileInfoFromSingleFileDiffCodeView = (
    ...
    => {
      const {rawRepoName, project, repoSlug, filePath} =
          getFileInfoFromLinkInSingleFileView(codeViewElement)
      ...
      const baseFilePath = getBaseFilePathForDiffCodeView(
                               {changeTypeElement, changeType, filePath}) ||
                           filePath
      return {
        changeType, rawRepoName, filePath, baseFilePath, commitID, project,
            repoSlug,
      }
    }

    /**
     * Gets most of the file info from the DOM of a multi-file diff code view
     * (PR, compare or commit page).
     *
     ...
     * or taken from links on the page for compare and commit pages {@link
  ...
  if (!breadcrumbsElement) {
    throw new Error(
        'Could not find diff code view breadcrumbs element through selector .breadcrumbs')
  }
  ...
  return { changeType, rawRepoName, filePath, baseFilePath, project, repoSlug, }
}
...
export const getFileInfoFromCommitDiffCodeView = (codeViewElement: HTMLElement): DiffInfo => {
  ...
  return {
    head: {rawRepoName, filePath, commitID}, base: {
      rawRepoName,
...
export function getPRIDFromPathName(): number {
  const prIDMatch = window.location.pathname.match(
      /pull-requests\/(\d*?)\/(diff|overview|commits)/)
  if (!prIDMatch) {
    throw new Error(
        `Could not parse PR ID from pathname: ${window.location.pathname}`)
  }
...
export function getCommitInfoFromComparePage(): DiffResolvedRevisionSpec {
  const headCommitElement = document.querySelector(
      '#branch-compare .source-selector a.commitid[data-commitid]')
  const baseCommitElement = document.querySelector(
      '#branch-compare .target-selector a.commitid[data-commitid]')
