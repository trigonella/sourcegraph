...
/**
 * Resolves file information for a page with a single file in source (not diff)
 * view.
 */
export const resolveFileInfoForSingleFileSourceView = (codeView: HTMLElement):
    BlobInfo => {
 ...
 */
export const resolvePullRequestFileInfo = (codeView: HTMLElement):
    Observable<DiffInfo> => {
      ...
      return getCommitsForPR({...partialFileInfo, prID})
          .pipe(map(({headCommitID, baseCommitID}): DiffInfo => ({
                      head : {
                        rawRepoName,
                        filePath,
 ...
 */
export const resolveSingleFileDiffFileInfo = (codeView: HTMLElement):
    Observable<DiffInfo> => {
      const {
        changeType,
        rawRepoName,
      ...
      return getBaseCommit({commitID, ...bitbucketInfo})
          .pipe(map((baseCommitID): DiffInfo => {
            ...
            return {
              base:
                  {rawRepoName, filePath: baseFilePath, commitID: baseCommitID},
