...
const getSingleFileLineElementFromLineNumber = (codeView: HTMLElement,
                                                line: number):
    HTMLElement => {
      const lineNumberElement =
          codeView.querySelector<HTMLElement>(`[data-line-number="${line}"]`)
...
export const singleFileDOMFunctions: DOMFunctions = {
  getCodeElementFromTarget : target => {
    ...
    return container ? container.querySelector<HTMLElement>(
  ...
  getCodeElementFromLineNumber : (codeView, line) =>
      getSingleFileLineElementFromLineNumber(codeView, line)
          .querySelector<HTMLElement>(
              '.CodeMirror-line span[role="presentation"]'),
}

const getDiffLineElementFromLineNumber = (codeView: HTMLElement, line: number,
                                          part?: DiffPart):
    HTMLElement => {
      for (const lineNumberElement of codeView.querySelectorAll(
               `.line-number-${part === 'head' ? 'to' : 'from'}`)) {
        const lineNumber =
            parseInt((lineNumberElement.textContent || '').trim(), 10)
      ...
      throw new Error(`Could not locate line number element for line ${
          line}, part: ${String(part)}`)
    }
...
export const diffDOMFunctions: DOMFunctions = {
  ...
  getCodeElementFromLineNumber : (codeView, line, part) =>
      getDiffLineElementFromLineNumber(codeView, line, part)
          .querySelector<HTMLElement>(
              '.CodeMirror-line span[role="presentation"]'),
  getDiffCodePart : codeElement => {
    if (!document.querySelector('.side-by-side-diff')) {
      return codeElement.closest('.line')!.classList.contains('removed')
                 ? 'base'
                 : 'head'
    }
    ...
    const diffSide =
        codeElement
            ...
            return diffSide.previousElementSibling?.classList.contains(
