...
import {
  resolveCommitFileInfo,
  resolveDiffFileInfo,
  resolveFileInfo
} from './fileInfo'
import {getFilePathsFromCodeView, getPageInfo, GitLabPageKind} from './scrape'
...
export function
checkIsGitlab():
    boolean { return !!document.head.querySelector('meta[content="GitLab"]') }

const adjustOverlayPosition: CodeHost['adjustOverlayPosition'] =
    ({top, left}) => {
...
export const getToolbarMount = (codeView: HTMLElement):
    HTMLElement => {
      const existingMount: HTMLElement|null =
          codeView.querySelector('.sg-toolbar-mount-gitlab')
      if (existingMount) {
...
const getFileTitle = (codeView: HTMLElement):
    HTMLElement[] => {
...
const resolveView: ViewResolver<CodeView>[ 'resolveView' ] =
    (element: HTMLElement):
        CodeView|null => {
          if (element.classList.contains('discussion-wrapper')) {
            // This is a commented snippet in a merge request discussion
            // timeline (a snippet where somebody added a review comment on a
            // piece of code in the MR), we don't support adding code
          ...
          if (pageKind === GitLabPageKind.MergeRequest) {
            if (!element.querySelector('.file-actions')) {
              // If the code view has no file actions, we cannot resolve its
              // head commit ID. This can be the case for code views
              // representing added git submodules.
              return null
            }
...
export const gitlabCodeHost = subtypeOf<CodeHost>()({
  ...
  urlToFile : (sourcegraphURL, target, context) : string => {
    ...
    // only supported on Sourcegraph (not code hosts). Make sure the location is
    // also on this Gitlab instance, return an absolute URL otherwise.
    if (target.viewState ||
        !target.rawRepoName.startsWith(window.location.hostname)) {
    ...
    if (currentPage.rawRepoName === target.rawRepoName &&
        context.part !== undefined) {
      const codeViews =
          document.querySelectorAll<HTMLElement>(codeViewResolver.selector)
      for (const codeView of codeViews) {
        const {headFilePath, baseFilePath} = getFilePathsFromCodeView(codeView)
        if (headFilePath !== target.filePath &&
            baseFilePath !== target.filePath) {
          continue
        }
        if (!target.position) {
        ...
        const partSelector =
            context.part !== null
                ? {head : '.new_line', base : '.old_line'}[context.part]
                : ''
    ...
    const url = new URL(`https://${target.rawRepoName}/blob/${
        target.revision}/${target.filePath}`)
    if (target.position) {
  ...
  getHoverOverlayMountLocation : () : string | null => {
    const {pageKind} = getPageInfo()
    // On merge request pages only, mount the hover overlay to the diffs tab
    // container.
    if (pageKind === GitLabPageKind.MergeRequest) {
