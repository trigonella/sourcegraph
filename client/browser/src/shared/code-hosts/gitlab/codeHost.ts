...
export const getToolbarMount = (codeView: HTMLElement): HTMLElement => {
  const existingMount: HTMLElement | null = codeView.querySelector(
    ".sg-toolbar-mount-gitlab"
  );
  if (existingMount) {
...
export const gitlabCodeHost = subtypeOf<CodeHost>()({
  ...
  urlToFile: (sourcegraphURL, target, context): string => {
    ...
    if (
      currentPage.rawRepoName === target.rawRepoName &&
      context.part !== undefined
    ) {
      const codeViews = document.querySelectorAll<HTMLElement>(
        codeViewResolver.selector
      );
      for (const codeView of codeViews) {
        const { headFilePath, baseFilePath } = getFilePathsFromCodeView(
          codeView
        );
        if (
          headFilePath !== target.filePath &&
    ...
    const url = new URL(
      `https://${target.rawRepoName}/blob/${target.revision}/${target.filePath}`
    );
    if (target.position) {
