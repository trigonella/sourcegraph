...
/**
 * Resolves file information for a page with a single file, not including diffs
 * with only one file.
 */
  ...
  if (!filePath) {
    throw new Error(
        `Unable to determine the file path of the current file because the current URL (window.location ${
            window.location.href}) does not have a file path.`)
  }
...
const {headFilePath, baseFilePath} = getFilePathsFromCodeView(codeView)
                ...
                return {
  head: {rawRepoName, filePath: headFilePath, commitID}, base: {
    rawRepoName: baseRawRepoName,
