...
const getSingleFileCodeElementFromLineNumber =
    (codeView: HTMLElement, line: number, part?: DiffPart): HTMLElement|null =>
        codeView.querySelector<HTMLElement>(`#LC${line}`)
export const singleFileDOMFunctions: DOMFunctions = {
  getCodeElementFromTarget : target =>
      target.closest('span.line') as HTMLElement | null,
...
const getDiffCodePart: DOMFunctions['getDiffCodePart'] =
    codeElement => {
      let selector = 'old'
...
const getDiffCodeElementFromLineNumber =
    (codeView: HTMLElement, line: number, part?: DiffPart):
        HTMLElement|null => {
          ...
          if (row.classList.contains('parallel')) {
            selector =
                `.${part === 'base' ? 'left-side' : 'right-side'} ${selector}`
          }
...
export const diffDOMFunctions: DOMFunctions = {
  ...
  getLineNumberFromCodeElement : codeElement => {
    ...
    while (cell &&
           !cell.matches(
               `.diff-line-num.${part === 'base' ? 'old_line' : 'new_line'}`) &&
