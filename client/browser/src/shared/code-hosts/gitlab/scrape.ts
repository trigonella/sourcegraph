...
/**
 * General information that can be found on any GitLab page that we care about.
 * (i.e. has code)
 */
...
export const getPageKindFromPathName =
    (owner: string, projectName: string, pathname: string):
        GitLabPageKind => {
          const pageKindMatch = pathname.match(new RegExp(
              `^/${owner}/${projectName}(/-)?/(commit|merge_requests|blob)/`))
          if (!pageKindMatch) {
          ...
          case 'commit':
            ...
            case 'merge_requests': return GitLabPageKind.MergeRequest
            case 'blob': return GitLabPageKind
                .File default: return GitLabPageKind.Other
          }
        }
...
getPageInfo():
    GitLabInfo {
      ...
      const projectName = last(parts)!
          ...
          const pageKind = getPageKindFromPathName(owner, projectName,
                                                   window.location.pathname)
      const hostname = isExtension ? window.location.hostname
                                   : new URL(gon.gitlab_url).hostname

      return {
        owner, projectName,
            rawRepoName: [ hostname, owner, projectName ].join('/'), pageKind,
      }
    }
...
getFilePageInfo():
    GitLabFileInfo {
      ...
      return { rawRepoName, filePath, revision, }
    }

/**
 * Finds the merge request ID from the URL.
 */
export const getMergeRequestID = ():
    string => {
      const matches =
          window.location.pathname.match(/merge_requests\/(.*?)\/diffs/)
      if (!matches) {
...
const getFilePathFromElement = (element: HTMLElement):
    string => {
      const filePath = element.dataset.originalTitle || element.dataset.title ||
                       element.title
      if (!filePath) {
        throw new Error(
            'Unable to get file paths from code view: no file title')
      }
...
/**
 * Finds the file paths from the code view. If the name has changed, it'll
 * return the base and head file paths.
 */
export function
getFilePathsFromCodeView(codeView: HTMLElement):
    {headFilePath: string; baseFilePath : string} {
      const filePathElements =
          codeView.querySelectorAll<HTMLElement>('.file-title-name')
      if (filePathElements.length === 0) {
        throw new Error(
            'Unable to get file paths from code view: no .file-title.name')
      }

      const filePathDidChange = filePathElements.length > 1
      const filePath = getFilePathFromElement(
          filePathElements.item(filePathDidChange ? 1 : 0))

      return {
        headFilePath: filePath,
            baseFilePath: filePathDidChange
                ? getFilePathFromElement(filePathElements.item(0))
                : filePath,
      }
    }

interface GitLabCommitPageInfo extends RawRepoSpec,
             Pick<GitLabInfo, 'owner'|'projectName'> {
