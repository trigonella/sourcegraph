...
export function assignOptionFlagValues(
  values: OptionFlagValues
): OptionFlagWithValue[] {
  return optionFlagDefinitions.map(flag => ({
    ...flag,
 ...
 */
export function applyOptionFlagDefaults(
  values: Partial<OptionFlagValues> | undefined
): OptionFlagValues {
...
export const observeOptionFlags = (): Observable<OptionFlagValues> =>
  observeStorageKey("sync", OPTION_FLAGS_SYNC_STORAGE_KEY).pipe(
    map(applyOptionFlagDefaults)
  );
