...
export type OptionFlagKey =|'sendTelemetry'|'allowErrorReporting'|
    'experimentalLinkPreviews'|'experimentalTextFieldCompletion'
...
export function assignOptionFlagValues(values: OptionFlagValues):
    OptionFlagWithValue[] {
      return optionFlagDefinitions.map(
          flag => ({...flag, value : values[flag.key]}))
    }
 ...
 */
export function applyOptionFlagDefaults(values: Partial<OptionFlagValues>|
                                        undefined):
    OptionFlagValues {
...
export const observeOptionFlags = (): Observable<OptionFlagValues> =>
    observeStorageKey('sync', OPTION_FLAGS_SYNC_STORAGE_KEY)
        .pipe(map(applyOptionFlagDefaults))

/**
 * Observe an option flag value, with default value already applied.
 */
export function observeOptionFlag(key: OptionFlagKey):
    Observable<boolean> {
      return observeOptionFlags().pipe(map(value => value[key]),
...
export function shouldOverrideSendTelemetry(
    isFirefox: boolean, isExtension: boolean, sourcegraphUrl: string): boolean {
