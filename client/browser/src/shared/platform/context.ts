...
import {
  ModelService
} from '../../../../shared/src/api/client/services/modelService'
import {
...
import {
  LocalStorageSubject
} from '../../../../shared/src/util/LocalStorageSubject'
import {toPrettyBlobURL} from '../../../../shared/src/util/url'
import {
  ExtensionStorageSubject
} from '../../browser-extension/web-extension-api/ExtensionStorageSubject'
...
import {
  InlineExtensionsService,
  shouldUseInlineExtensions
} from './inlineExtensionsService'
import {
  editClientSettings,
  fetchViewerSettings,
  mergeCascades,
...
export interface SourcegraphIntegrationURLs {
  /**
   * The URL of the configured Sourcegraph instance. Used for extensions,
   * find-references, ...
   */
...
/**
 * The PlatformContext provided in the browser (for browser extensions and
 * native integrations).
 */
...
/**
 * Creates the {@link PlatformContext} for the browser (for browser extensions
...
export function createPlatformContext(
    ...
    isExtension: boolean): BrowserPlatformContext {
  const updatedViewerSettings =
      new ReplaySubject<Pick<GQL.ISettingsCascade, 'subjects'|'final'>>(1)
    ...
    if (mightContainPrivateInfo && sourcegraphURL === DEFAULT_SOURCEGRAPH_URL) {
      // If we can't determine the code host context, assume the current
      // repository is private.
      const privateRepository =
          getContext ? getContext().privateRepository : true
    ...
    return requestGraphQlHelper(isExtension,
                                sourcegraphURL)<T, V>({request, variables})
            })
        )
        ...
        const context: BrowserPlatformContext = {
           ...
           *
           * - For unauthenticated users, this is the GraphQL settings plus
           * client settings (which are stored locally in the browser
           * extension).
           * - For authenticated users, this is just the GraphQL settings
           * (client settings are ignored to simplify the UX).
           */
          settings :
              combineLatest([ updatedViewerSettings, storageSettingsCascade ])
                  .pipe(map(([ gqlCascade, storageCascade ]) =>
                                gqlCascade
                                    ? mergeCascades(
                                          gqlToCascade(gqlCascade),
                                          gqlCascade.subjects.some(
                                              subject =>
          ...
          refreshSettings : async () => {
            try {
              const settings =
                  await fetchViewerSettings(requestGraphQL).toPromise()
              updatedViewerSettings.next(settings)
            } catch (error) {
              if (isHTTPAuthError(error)) {
                // User is not signed in
                console.warn(`Could not fetch Sourcegraph settings from ${
                    sourcegraphURL} because user is not signed into Sourcegraph`)
          ...
          updateSettings : async (subject, edit) => {
            if (subject === 'Client') {
              // Support storing settings on the client (in the browser
              // extension) so that unauthenticated Sourcegraph viewers can
              // update settings.
              await updateSettings(context, subject, edit,
                                   () => editClientSettings(edit))
              return
            }
            ...
            // TODO: We shouldn't need to make another HTTP request to get the
            // latest state
            await context.refreshSettings()
          },
          requestGraphQL,
          ...
          getScriptURLForExtension : async bundleURL => {
            ...
            // We need to import the extension's JavaScript file (in
            // importScripts in the Web Worker) from a blob: URI, not its
            // original http:/https: URL, because Chrome extensions are not
            // allowed to be published with a CSP that allowlists https://* in
            // script-src (see
            // https://developer.chrome.com/extensions/contentSecurityPolicy#relaxing-remote-script).
            // (Firefox add-ons have an even stricter restriction.)
            const blobURL = await background.createBlobURL(bundleURL)
            return blobURL
          },
          urlToFile : ({rawRepoName, ...target}, context) => {
            // We don't always resolve the rawRepoName, e.g. if there are
            // multiple definitions. Construct URL to file on code host, if
            // possible.
            if (rawRepoName && urlToFile) {
              return urlToFile(sourcegraphURL, {rawRepoName, ...target},
                               context)
            }
            // Otherwise fall back to linking to Sourcegraph (with an absolute
            // URL).
          ...
          sideloadedExtensionURL :
              isInPage
                  ? new LocalStorageSubject<string|null>(
