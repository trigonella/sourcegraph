...
/**
 * Returns a `MessagePort` that was connected to the given
 ...
 * @param prefix A prefix unique to this call of `browserPortToMessagePort` (but
 *     the same on both sides) to prefix Port names. Incoming ports not matching
 *     this prefix will be ignored.
 * @param connect A callback that is called to create a new connection to the
 *     other side of `browserPort` (e.g. through `browser.runtime.connect()` or
 *     `browser.tabs.connect()`).
 */
export function browserPortToMessagePort(
    browserPort: browser.runtime.Port, prefix: string,
    connect: (name: string) => browser.runtime.Port): {messagePort: MessagePort
  ...
  /**
   * Callbacks from messages that referenced a port ID, waiting for that port
   * to connect
   */
  const waitingForPorts =
      new Map<string, (port: browser.runtime.Port) => void>()
  ...
  const connectListener =
      (incomingPort: browser.runtime.PortWithSender): void => {
  ...
  const whenConnected =
      (id: string, callback: (port: browser.runtime.Port) => void):
          void => {
            const browserPort = connectedBrowserPorts.get(id)
            if (browserPort) {
              // We can delete the port from the Map after we set up a
              // connection. There can never be the same ID twice and it is
              // impossible to transfer the same MessagePort twice.
  ...
  /**
   * Set up bi-directional listeners between the two ports that open new
   * `browser.runtime.Port`s for transferred `MessagePort`s.
   *
   * @param browserPort The browser port used for communication with the other
   *     thread.
   * @param adapterPort The `MessagePort` used to communicate with comlink.
   */
  function link(browserPort: browser.runtime.Port, adapterPort: MessagePort):
      void {
        ...
        adapterPort.close()
            ...
            const adapterListener = (event: MessageEvent): void => {
              ...
              // Message from comlink needing to be forwarded to browser port
              // with MessagePorts removed
              const portReferences: PortReference[] = []
                  // Find message port references and connect to the other side
                  // with the found IDs.
                  for (const {value,
                              path,
                              key,
                              parent} of iteratePropertiesDeep(data)) {
              ...
              const browserPortMessage: BrowserPortMessage =
                  {message : data, portRefs : portReferences}
            ...
            subscription.add(() => adapterPort.removeEventListener(
                                 'message', adapterListener))

            const browserPortListener =
                ({message, portRefs}: BrowserPortMessage): void => {
                  const transfer: MessagePort[] =
                      [] for (const portReference of portRefs) {
                    const {
                      port1 : comlinkMessagePort,
                      port2 : intermediateMessagePort
                    } = new MessageChannel()

                    // Replace the port reference at the given path with a
                    // MessagePort that will be transferred.
                    replaceValueAtPath(message, portReference.path,
                                       comlinkMessagePort)
                    transfer.push(comlinkMessagePort)

                    // Once the port with the mentioned ID is connected, link it
                    // up
                    whenConnected(portReference.id,
                                  browserPort => link(browserPort,
                                                      intermediateMessagePort))
                  }

                  // Forward message, with MessagePorts
                  adapterPort.postMessage(message, transfer)

                  // Handle release messages (sent before the other end is
                  // closed) by cleaning up all Ports we control
            ...
            subscription.add(() => browserPort.onDisconnect.removeListener(
                                 disconnectListener))

            adapterPort.start()
      }

  const {port1 : returnedMessagePort, port2 : adapterPort} =
      new MessageChannel()
  link(browserPort, adapterPort)

  return { messagePort: returnedMessagePort, subscription: rootSubscription, }
}
...
interface PortReference {
    ...
    interface BrowserPortMessage {
      /**
       * The original comlink message, but with `MessagePort`s replaced with
       * `null`.
       */
      message: Message

      /**
       * Where in the message `MessagePort`s were referenced and the ID of the
       * `browser.runtime.Port` created for each.
       */
    ...
    /**
     * The key of the current value in the parent object. Equivalent to
     * `path[path.length - 1]`. `null` if the root value.
     */
    key: Key|null
...
function replaceValueAtPath(value: any, path: Path, newValue: unknown): unknown {
      const lastProperty =
          path[path.length - 1] for(const property of path.slice(0, -1)) {
    ...
    /**
     * Iterate all properties of a given value or object recursively, starting
     * with the value itself.
     */
    export function*
        iteratePropertiesDeep<T>(
            value: T, key: Key|null = null, parent: any = null, path: Path = [],
            visited = new WeakSet()): Iterable<PropertyIteratorEntry> {
      ...
      const keys =
          Object.keys(value) as (keyof typeof value)[] for (const key of keys) {
        yield* iteratePropertiesDeep(value[key], key, value, [...path, key ],
                                     visited)
      }
    }
     ...
     */
    export function*
        findMessagePorts(message: unknown): Iterable<Transferable> {
