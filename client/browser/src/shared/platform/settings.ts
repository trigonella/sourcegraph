          ...
          SettingsSubject = {
            ...
            viewerCanAdminister : true,
                            ...
                            return storageSubject.pipe(
                                map(clientSettingsString =>
                                        parseJSONC(clientSettingsString || '')),
                                map(clientSettings => ({
                                      subjects : [
                                        {
                                          subject,
...
/**
 * Merge two settings cascades (used to merge viewer settings and client
 * settings).
 */
export function mergeCascades(cascadeOrError: SettingsCascadeOrError,
                              cascade: SettingsCascade):
    SettingsCascadeOrError {
      return {
        subjects: cascadeOrError.subjects === null
            ? cascade.subjects
            : isErrorLike(cascadeOrError.subjects)
                  ? cascadeOrError.subjects
                  : [...cascadeOrError.subjects, ...cascade.subjects ],
            final: cascadeOrError.final === null
                ? cascade.final
...
// This is a fragment on the DEPRECATED GraphQL API type ConfigurationCascade
// (not SettingsCascade) for backcompat.
 ...
 *
 * TODO(sqs): This uses the DEPRECATED GraphQL Query.viewerConfiguration and
 * ConfigurationCascade for backcompat.
 */
export function fetchViewerSettings(
    requestGraphQL: PlatformContext['requestGraphQL']):
    Observable<Pick<GQL.ISettingsCascade, 'subjects'|'final'>> {
      return from(requestGraphQL<GQL.IQuery>({
               request : gql`
          ...
          .pipe(map(dataOrThrowErrors), map(({viewerConfiguration}) => {
                  if (!viewerConfiguration) {
                    throw new Error(
                        'fetchViewerSettings: empty viewerConfiguration')
                  }
 ...
 */
export async function editClientSettings(edit: SettingsEdit|
                                         string): Promise<void> {
