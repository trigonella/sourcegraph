...
/**
 * Returns a promise of a communication channel to an extension host and a
 * Subscription to cleanup
 *
 ...
 * Subscription to cleanup.
 *
 * When executing in the browser extension, we create pair of
 * browser.runtime.Port objects, named 'expose-{uuid}' and 'proxy-{uuid}', and
 * return the ports wrapped using ${@link endpointFromPort}.
 *
 * The background script will listen to newly created ports, create an extension
 * host worker per pair of ports, and forward messages between the port objects
