...
describe("Sourcegraph browser extension on github.com", function() {
  ...
  const { browser, sourcegraphBaseUrl, ...restConfig } = getConfig(
    "browser",
    "sourcegraphBaseUrl"
  );
        ...
        for (const side of ["base", "head"] as const) {
          ...
          it(`provides hover tooltips on token "${token}" in the ${side} part`, async () => {
            await driver.page.goto(
              `https://github.com/gorilla/mux/pull/117/files?diff=${diffType}`
            );
            ...
            const tokenElement = await retry(async () => {
              const lineNumberElement = await driver.page.waitForSelector(
                `#${lineId}`,
                {
                  timeout: 10000
                }
