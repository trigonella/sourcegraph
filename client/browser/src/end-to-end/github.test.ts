...
describe('Sourcegraph browser extension on github.com', function() {
  ...
  const {browser, sourcegraphBaseUrl, ...restConfig} =
  ...
  before('Open browser', async function() {
    ...
    driver = await createDriverForTest(
        {loadExtension : true, browser, sourcegraphBaseUrl, ...restConfig})
  ...
  testSingleFilePage({
    ...
    sourcegraphBaseUrl,
    // Not using '.js-file-line' because it breaks the reliance on :nth-child()
    // in testSingleFilePage()
    ...
    for (const diffType of ['unified', 'split']) {
      describe(`${startCase(diffType)} view`, () => {
        for (const side of ['base', 'head'] as const) {
          ...
          it(`provides hover tooltips on token "${token}" in the ${side} part`,
             async () => {
               await driver.page
                   .goto(`https://github.com/gorilla/mux/pull/117/files?diff=${
                       diffType}`)
                   // The browser extension takes a bit to initialize and
                   // register all event listeners. Waiting here saves one retry
                   // cycle below in the common case. If it's not enough, the
                   // retry will catch it.
                   await driver.page.waitFor(1500)
                        ...
                        await retry(async () => {
                            ...
                            if (new URL(goToDefinitionURL).hostname !==
                                'github.com') {
                              ;
                              [page] = await Promise.all([
                                fromEvent<Target>(driver.browser,
                                                  'targetcreated')
                                    .pipe(timeout(5000),
                                  ...
                                  '.test-tooltip-go-to-definition'),
                                                ...
                                                assert.strictEqual(
                                                    await page.evaluate(
                                                        () => location.href),
                                                    goToDefinitionURL)})
                            // If an additional page was opened, close it so we
                            // return to the original `driver.page`.
                            if (page !== driver.page) {
                              await page.close()
                            }
             })
