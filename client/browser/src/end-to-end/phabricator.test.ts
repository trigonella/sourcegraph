...
// By default, these tests run against a local Phabricator instance and a local
// Sourcegraph instance. To run them against phabricator.sgdev.org and
// umami.sgdev.org, set the below env vars in addition to SOURCEGRAPH_BASE_URL.

const PHABRICATOR_BASE_URL =
    process.env.PHABRICATOR_BASE_URL || 'http://127.0.0.1'
...
const TEST_NATIVE_INTEGRATION =
    Boolean(process.env.TEST_NATIVE_INTEGRATION &&
            JSON.parse(process.env.TEST_NATIVE_INTEGRATION))
const {gitHubToken, sourcegraphBaseUrl, ...restConfig} =
...
async function addPhabricatorRepo(driver: Driver): Promise<void> {
  // These steps are idempotent as they will error if the same repo already
  // exists

  // Add new repo to Diffusion
  await driver.page
      .goto(PHABRICATOR_BASE_URL + '/diffusion/edit/?vcs=git') await driver.page
      .waitForSelector('input[name=shortName]') await driver.page
      .type('input[name=name]', 'sourcegraph/jsonrpc2')
          await driver.page.type('input[name=callsign]', 'JRPC') await driver
      .page.type('input[name=shortName]', 'jrpc') await driver.page
      .click('button[type=submit]')

      // Configure it to clone github.com/sourcegraph/jsonrpc2
      await driver.page
      .goto(PHABRICATOR_BASE_URL + '/source/jrpc/uri/edit/')
          await driver.page.waitForSelector('input[name=uri]') await driver.page
      .type('input[name=uri]', 'https://github.com/sourcegraph/jsonrpc2.git')
          await driver.page.select('select[name=io]', 'observe') await driver
      .page.select('select[name=display]', 'always') await driver.page
      .click('button[type="submit"][name="__submit__"]')
  ...
  const buttonLabel =
      ((await (await activateButton.getProperty('textContent')).jsonValue()) as
       string)
          .trim()
  // Don't click if it says "Deactivate Repository"
  if (buttonLabel === 'Activate Repository') {
    await activateButton.click() await driver.page
        .waitForSelector('form[action="/source/jrpc/edit/activate/"]') await (
            await driver.page.$x('//button[text()="Activate Repository"]'))[0]
        .click() await driver.page.waitForNavigation()
...
async function configureSourcegraphIntegration(driver: Driver): Promise<void> {
  ...
  try {
    await driver.page.waitForSelector('a[href="/config/group/sourcegraph/"]',
                                      {timeout : 2000})
  } catch {
    throw new Error(`The Sourcegraph native integration is not installed on ${
        PHABRICATOR_BASE_URL}. Please see https://docs.sourcegraph.com/dev/phabricator_gitolite#install-the-sourcegraph-phabricator-extension`)
  }

  // Configure the Sourcegraph URL
  await driver.page
      .goto(PHABRICATOR_BASE_URL + '/config/edit/sourcegraph.url/') await driver
      .replaceText({
        selector : '[name="value"]',
      ...
      await driver.page.goto(PHABRICATOR_BASE_URL +
                             '/config/edit/sourcegraph.callsignMappings/')
  const callSignConfigString = await driver.page.evaluate(
      () =>
          document.querySelector<HTMLTextAreaElement>(
                      'textarea[name="value"]')!.value.trim())
  const callSignConfig: PhabricatorMapping[] =
      (callSignConfigString && JSON.parse(callSignConfigString)) ||
      [] const jsonRpc2Mapping: PhabricatorMapping = {
        path : 'github.com/sourcegraph/jsonrpc2',
        callsign : 'JRPC',
      } if (!callSignConfig.some(mapping =>
                                     isEqual(jsonRpc2Mapping, mapping))) {
    await driver
        .replaceText({
  ...
  await driver.page
      .goto(PHABRICATOR_BASE_URL + '/config/edit/sourcegraph.enabled/')
  ...
  await driver
      .ensureHasExternalService({
        ...
        ensureRepos : [ 'github.com/sourcegraph/jsonrpc2' ],
          ...
          await driver.ensureHasCORSOrigin(
              {corsOriginURL : PHABRICATOR_BASE_URL})
              await phabricatorLogin(driver) await addPhabricatorRepo(driver)
...
describe('Sourcegraph Phabricator extension', () => {
    let driver: Driver
            ...
            before(async function() {
              ...
              driver = await createDriverForTest({
                loadExtension : !TEST_NATIVE_INTEGRATION,
                sourcegraphBaseUrl
              }) await init(driver)
            })
    ...
    expect(await driver.page.$$('.code-view-toolbar .open-on-sourcegraph'))
        .toHaveLength(1) await Promise.all([
          driver.page.waitForNavigation(),
    ...
    // Once the line is tokenized, we can click on the individual token we want
    // a hover for.
