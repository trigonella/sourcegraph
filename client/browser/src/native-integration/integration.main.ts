...
function init(): void {
  ...
  if (document.querySelector(`#${EXTENSION_MARKER_ID}`) !== null) {
    // If the extension marker already exists, it means the browser extension is
    // currently executing. Dispatch a custom event to signal that browser
    // extension resources should be cleaned up.
