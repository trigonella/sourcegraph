/**
 * This hacks javelin Stratcom to ignore command + click actions on sg-clickable
 * tokens. Without this, two windows open when a user command + clicks on a
 * token.
 *
...
export function metaClickOverride(): void {
  ...
  javelin.Stratcom._dispatchProxyPreMeta = javelin.Stratcom._dispatchProxy
    ...
    if (proxyEvent.__auto__type === 'click' &&
