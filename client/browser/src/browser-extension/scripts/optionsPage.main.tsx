...
const fetchCurrentTabStatus = async (): Promise<TabStatus> => {
  ...
  if (tabs.length > 1) {
    throw new Error(
      "Querying for the currently active tab returned more than one result"
    );
  }
  const { url, id } = tabs[0];
  if (!url) {
...
const validateSourcegraphUrl = (url: string): Observable<string | undefined> =>
  fetchSite(options =>
    requestGraphQL({ ...options, sourcegraphURL: url })
  ).pipe(
    mapTo(undefined),
    catchError(error => {
...
const observeOptionFlagsWithValues = (): Observable<OptionFlagWithValue[]> => {
  const overrideSendTelemetry: Observable<boolean> = observeSourcegraphURL(
    IS_EXTENSION
  ).pipe(
    map(sourcegraphUrl =>
...
const observingIsActivated = observeStorageKey("sync", "disableExtension").pipe(
...
const Options: React.FunctionComponent = () => {
  ...
  const optionFlagsWithValues =
    useObservable(observingOptionFlagsWithValues) || [];
  ...
  useEffect(() => {
    fetchCurrentTabStatus().then(tabStatus => {
      setCurrentTabStatus({
        status: tabStatus,
        handler: buildRequestPermissionsHandler(tabStatus)
      });
    }, noop);
  }, []);

  const showSourcegraphCloudAlert = currentTabStatus?.status.host.endsWith(
    "sourcegraph.com"
  );
  ...
  if (
    ...
    const knownCodeHost = knownCodeHosts.find(
      ({ host }) => host === currentTabStatus.status.host
    );
    if (knownCodeHost) {
    ...
    <ThemeWrapper>
      <OptionsPage
        ...
        showPrivateRepositoryAlert={
          currentTabStatus?.status.isPrivateRepository &&
