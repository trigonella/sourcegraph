...
async function main(): Promise<void> {
  ...
  if (document.readyState !== 'complete' &&
    ...
    await new Promise<Event>(resolve => document.addEventListener(
  ...
  // If the extension marker isn't present, inject it and listen for a custom
  // event sent by the native integration to signal its activation.
  injectExtensionMarker()
  const nativeIntegrationActivationEventReceived =
      fromEvent(document, NATIVE_INTEGRATION_ACTIVATED)
          .pipe(first())
          .toPromise()
  ...
  // Add style sheet and wait for it to load to avoid rendering unstyled
  // elements (which causes an annoying flash/jitter when the stylesheet loads
  // shortly thereafter).
  const styleSheet = (() => {
  ...
  subscriptions.add(injectCodeIntelligence(
