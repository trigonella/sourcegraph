...
import {
  createExtensionHostWorker
} from '../../../../shared/src/api/extension/worker'
import {
  GraphQLResult,
...
import {
  BrowserActionIconState,
  setBrowserActionIconState
} from '../browser-action-icon'
...
import {
  BackgroundPageApi,
  BackgroundPageApiHandlers
} from '../web-extension-api/types'

const IS_EXTENSION = true

// Interval to check if the Sourcegraph URL is valid
// This polling allows to detect if Sourcegraph instance is invalid or needs
// authentication.
...
// jsContentScriptOrigins are the required URLs inside of the manifest. When
// checking for permissions to inject the content script on optional pages
// (inside browser.tabs.onUpdated) we need to skip manual injection of the
// script since the browser extension will automatically inject it.
const jsContentScriptOrigins: string[] = [] if (contentScripts) {
...
variables: V
    ...
    async function main(): Promise<void> {
       ...
       * repository. The content script notifies the background page if it's on
       * a private repository by `notifyPrivateRepository` message.
       */
      ...
      browser.runtime.onInstalled.addListener(event => {
        if (event.reason === 'install') {
          browser.tabs
              .create({url : browser.extension.getURL('after_install.html')})
              .catch(error => {console.error(
                         'Error opening after-install page:', error)})
        }
      })

      // Mirror the managed sourcegraphURL to sync storage
      subscriptions.add(observeStorageKey('managed', 'sourcegraphURL')
                            .pipe(filter(isDefined),
                                  concatMap(sourcegraphURL => storage.sync.set(
                                                {sourcegraphURL})))
                            .subscribe())
      // Configure the omnibox when the sourcegraphURL changes.
      subscriptions.add(
          observeSourcegraphURL(IS_EXTENSION)
              .subscribe(sourcegraphURL => {configureOmnibox(sourcegraphURL)}))

      // Update the browserAction icon based on the state of the extension
      subscriptions.add(observeBrowserActionState().subscribe(
          state => {setBrowserActionIconState(state)}))

      const permissions = await browser.permissions.getAll()
      if (!permissions.origins) {
        customServerOrigins = [] return
      }
      customServerOrigins =
          without(permissions.origins, ...jsContentScriptOrigins)
      ...
      if (browser.permissions.onRemoved) {
        browser.permissions.onRemoved.addListener(permissions => {
          ...
          customServerOrigins =
              without(customServerOrigins, ...permissions.origins)
      ...
      browser.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
        if (changeInfo.status === 'loading') {
          // A new URL is loading in the tab, so clear the cached private
          // repository flag.
        ...
        if (changeInfo.status === 'complete' &&
            customServerOrigins.some(
                origin => origin === '<all_urls>' ||
                          (!!tab.url &&
                           tab.url.startsWith(origin.replace('/*', ''))))) {
          // Inject content script whenever a new tab was opened with a URL for
          // which we have permission
          await browser.tabs.executeScript(
              tabId, {file : 'js/inject.bundle.js', runAt : 'document_end'})
        }
      })
    ...
    return (handlers[method] as
                (payload: any, sender?: browser.runtime.MessageSender) =>
                ReturnType<BackgroundPageApi[typeof method]>)(message.payload,
                                                              sender)
    })

    await browser.runtime.setUninstallURL('https://about.sourcegraph.com/uninstall/')

            // The `popup=true` param is used by the options page to determine
            // if it's loaded in the popup or in th standalone options page.
            ...
            const portKind = (port: browser.runtime.Port):
                string|undefined => {
            ...
            /**
             * A stream of EndpointPair created from Port objects emitted by
             * browser.runtime.onConnect.
             *
             * On initialization, the content script creates a pair of
             * browser.runtime.Port objects using browser.runtime.connect(). The
             * two ports are named 'proxy-{uuid}' and 'expose-{uuid}', and
             * wrapped using {@link endpointFromPort} to behave like comlink
             * endpoints on the content script side.
             *
             * This listens to events on browser.runtime.onConnect, pairs
             * emitted ports using their naming pattern, and emits pairs. Each
             * pair of ports represents a connection with an instance of the
             * content script.
             */
            const browserPortPairs: Observable<
                Record<keyof EndpointPair, browser.runtime.PortWithSender>> =
                fromBrowserEvent(browser.runtime.onConnect)
                    .pipe(map(([ port ]) => port),
                          groupBy(port => (port.name || 'other')
                                              .replace(ENDPOINT_KIND_REGEX, ''),
                                  port => port,
                                  group => group.pipe(bufferCount(2))),
                          filter(group => group.key !== 'other'),
                          mergeMap(group => group.pipe(
                                       bufferCount(2), map(ports => {
                                         const proxyPort = ports.find(
                                             port => portKind(port) === 'proxy')
                                         if (!proxyPort) {
                                           throw new Error('No proxy port')
                                         }
                                         const exposePort = ports.find(
                                             port =>
                                                 portKind(port) === 'expose')
                                         if (!exposePort) {
                                           throw new Error('No expose port')
                                         }
                                         return {
                                           proxy: proxyPort, expose: exposePort,
                                         }
                                       }))))

            // Extension Host Connection
            // When an Port pair is emitted, create an extension host worker.
            // Messages from the ports are forwarded to the endpoints returned
            // by {@link createExtensionHostWorker}, and vice-versa. The
            // lifetime of the extension host worker is tied to that of the
            // content script instance: when a port disconnects, the worker is
            // terminated. This means there should always be exactly one
            // extension host worker per active instance of the content script.
            subscriptions.add(browserPortPairs.subscribe({
              next : browserPortPair => {
                subscriptions.add(handleBrowserPortPair(browserPortPair))
              },
              error : error => {
                console.error('Error handling extension host client connection',
                              error)
              },
            }))
    ...
    const workerBundleURL =
        browser.runtime.getURL('js/extensionHostWorker.bundle.js')
    ...
    function handleBrowserPortPair(
        browserPortPair:
            Record<keyof EndpointPair, browser.runtime.PortWithSender>):
        Subscription{/** Subscriptions for this browser port pair */
                     const subscriptions = new Subscription()
    ...
    const forwardEndpoint = (from: Endpoint, to: Endpoint):
        void => {
          ...
          subscriptions
              .add(() => from.removeEventListener('message', messageListener))

              // False positive https://github.com/eslint/eslint/issues/12822
              from.start?.()
        }

    const linkPortAndEndpoint = (role: keyof EndpointPair):
        void => {
          const browserPort = browserPortPair[role] const endpoint =
              clientEndpoints[role] const tabId = browserPort.sender.tab?.id
          if (!tabId) {
            throw new Error('Expected Port to come from tab')
          }
          const link = browserPortToMessagePort(
              browserPort, `comlink-${role}-`,
              name => browser.tabs.connect(tabId, {name}))
          ...
          const disconnectListener =
              subscriptions.unsubscribe.bind(subscriptions)
          browserPort.onDisconnect.addListener(disconnectListener)
          subscriptions.add(
              () => browserPort.onDisconnect.removeListener(disconnectListener))
        }
...
main()
    ...
    function validateSite():
        Observable<boolean>{return fetchSite(requestGraphQL)
                                .pipe(mapTo(true), catchError(() => [false]))}

    function
    observeSourcegraphUrlValidation():
        Observable<boolean> {
          return merge(
              // Whenever the URL was persisted to storage, we can assume it was
              // validated before-hand
              observeStorageKey('sync', 'sourcegraphURL').pipe(mapTo(true)),
              timer(0, INTERVAL_FOR_SOURCEGRPAH_URL_CHECK)
                  .pipe(mergeMap(() => validateSite())))
        }

    function
    observeBrowserActionState(): Observable<BrowserActionIconState> {
      return combineLatest([
               observeStorageKey('sync', 'disableExtension'),
               observeSourcegraphUrlValidation()
             ])
          .pipe(map(([ isDisabled, isSourcegraphUrlValid ]) => {
            if (isDisabled) {
              return 'inactive'
            }
