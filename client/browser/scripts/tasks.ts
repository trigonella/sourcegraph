 ...
 * This is needed for e2e and integration tests because it is not possible to
 * accept the permission prompt with puppeteer.
 */
const EXTENSION_PERMISSIONS_ALL_URLS =
    Boolean(process.env.EXTENSION_PERMISSIONS_ALL_URLS &&
            JSON.parse(process.env.EXTENSION_PERMISSIONS_ALL_URLS))
...
function copyExtensionAssets(toDirectory: string):
    void {
      shelljs.mkdir('-p', `${toDirectory}/js`, `${toDirectory}/css`,
                    `${toDirectory}/img`)
...
/**
 * When building with inline (bundled) Sourcegraph extensions, copy the built
 * Sourcegraph extensions into the output. They will be available as
 * `web_accessible_resources`.
 *
 * The pre-requisite step is to first clone, build, and copy into
 * `build/extensions`.
 */
function copyInlineExtensions(toDirectory: string):
    void { shelljs.cp('-R', 'build/extensions', toDirectory)}
...
copyIntegrationAssets():
    void {
      ...
      shelljs.cp('build/dist/js/phabricator.bundle.js',
                 'build/integration/scripts')
      shelljs.cp('build/dist/js/integration.bundle.js',
                 'build/integration/scripts')
      shelljs.cp('build/dist/js/extensionHostWorker.bundle.js',
                 'build/integration/scripts')
      shelljs.cp('build/dist/css/style.bundle.css', 'build/integration/css')
      shelljs.cp('src/native-integration/extensionHostFrame.html',
                 'build/integration')
...
function writeSchema(environment: BuildEnvironment, browser: Browser,
                     writeDirectory: string):
    void {
      fs.writeFileSync(`${writeDirectory}/schema.json`,
                       JSON.stringify(schema, null, 4))
    }

const version = process.env.BROWSER_EXTENSION_VERSION || utcVersion()

const shouldBuildWithInlineExtensions = (browser: Browser): boolean =>
    browser === 'firefox'

function writeManifest(environment: BuildEnvironment, browser: Browser,
                       writeDirectory: string): void {
  const manifest = {
  ...
  if (shouldBuildWithInlineExtensions(browser)) {
    ...
    manifest.web_accessible_resources
        ...
        // Revert the CSP to default, in order to remove the `blob` policy
        // exception.
  ...
  fs.writeFileSync(`${writeDirectory}/manifest.json`,
                   JSON.stringify(manifest, null, 4))
}

function buildForBrowser(browser: Browser): (environment: BuildEnvironment) =>
    () => void {
  ensurePaths()
  return environment => {
    const title = BROWSER_TITLES[browser]
        ...
        const buildDirectory =
            path.resolve(process.cwd(), `${BUILDS_DIR}/${browser}`)
    ...
    return () => {
      ...
      const zipDestination =
          path.resolve(process.cwd(),
                       `${BUILDS_DIR}/bundles/${BROWSER_BUNDLE_ZIPS[browser]}`)
      if (zipDestination) {
