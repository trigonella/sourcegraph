...
const extensionNames =
    require('../bundled-code-intel-extensions.json').extensions ||
...
const codeIntelExtensionsDirectoryExists =
    shelljs.test('-d', temporarySourceDirectory)
if (codeIntelExtensionsDirectoryExists) {
  console.log('Found existing code-intel-extensions.')
}
else {console.log(
    'Did not find an existing code-intel-extensions. Running fetch-code-intel-extensions')
...
shelljs.exec('yarn --cwd code-intel-extensions install')
  ...
  for (const extensionName of extensionNames) {
    const extensionDirectory =
        `${temporarySourceDirectory}/extensions/${extensionName}`
    if (!shelljs.test('-d', extensionDirectory)) {
      console.error(
          `Code intel extension "${extensionName}" was not found for bundling.`)
      console.error(`Expected extension directory: ${extensionDirectory}`)
      process.exit(1)
    }
    shelljs.exec(`yarn --cwd ${temporarySourceDirectory}/extensions/${
        extensionName} run build`)
  }
  ...
  for (const extensionName of extensionNames) {
    ...
    shelljs.cp(`${temporarySourceDirectory}/extensions/${
                   extensionName}/dist/extension.js`,
