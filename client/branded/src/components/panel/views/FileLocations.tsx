...
import {
  ...
  IFileMatch,
  ILineMatch
} from "../../../../../shared/src/components/FileMatch";
...
export const FileLocationsError: React.FunctionComponent<{
  error: ErrorLike;
}> = ({ error }) => (
  <div className="file-locations__error alert alert-danger m-2">
    <AlertCircleIcon className="icon-inline" /> Error getting locations:{" "}
...
export class FileLocations extends React.PureComponent<Props, State> {
  ...
  public componentDidMount(): void {
    ...
    this.subscriptions.add(
      locationsChanges
        .pipe(
          switchMap(query =>
            query.pipe(catchError(error => [asError(error) as ErrorLike]))
          ),
          startWith(LOADING),
  ...
  public render(): JSX.Element | null {
    ...
    if (
      this.state.locationsOrError === null ||
      this.state.locationsOrError.length === 0
    ) {
...
function referencesToFileMatch(
  uri: string,
  references: Badged<Location>[]
): IFileMatch {
      ...
      (reference): ILineMatch => ({
        ...
        offsetAndLengths: [
          [
            ...
            reference.range.end.character - reference.range.start.character
          ]
        ],
