 ...
 *
 * There are multiple levels of grouping. For example, the first level might be
 * the repository, and the second level might be the file. Typically the levels
 * are displayed as columns, with
 *
...
export function groupLocations<L = Location, G = string>(
    locations: L[], selectedGroups: G[]|null,
    groupKeys: ((location: L) => G | undefined)[],
    locationForDefaultSelection: L): GroupedLocations<L, G> {
  ...
  const groups: GroupedLocations<L, G>[ 'groups' ] = []
    ...
    selectedGroups = [] for (const groupKey of groupKeys) {
  ...
  const visibleLocations: L[] = [] for (const location of locations) {
    for (const [index, groupKey] of groupKeys.entries()) {
      ...
      const groupEntry =
          groups[index].find(groupEntry => groupEntry.key === group)
      if (groupEntry) {
        groupEntry.count++
      }
      else {
      ...
      if (selectedGroups[index] !== group) {
        // This location won't be visible and won't contribute to any more
        // groups, so stop processing it.
        break
      }
  ...
  return { groups, selectedGroups, visibleLocations, }
}
