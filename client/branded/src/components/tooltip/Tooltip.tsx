...
export class Tooltip extends React.PureComponent<Props, State> {
  ...
  public static forceUpdate(): void {
    ...
    if (instance) {
      instance.setState(previousState => {
        const subject =
          previousState.lastEventTarget &&
          instance.getSubject(previousState.lastEventTarget);
        return {
          subject,
  ...
  private handleEvent = (event: Event): void => {
    ...
    this.setState(previousState => ({
      subject,
      subjectSeq:
        previousState.subject === subject
          ? previousState.subjectSeq
          : previousState.subjectSeq + 1,
 ...
 * tooltip).
 */
export function setElementTooltip(
  element: HTMLElement,
  tooltip: string | null
): void {
  if (tooltip) {
