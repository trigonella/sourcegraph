 ...
 * @template T The type that describes a tab.
 */
class TabBar<ID extends string, T extends Tab<ID>> extends React.PureComponent<
  TabBarProps<ID, T>
> {
    ...
    return (
      <div id={this.props.id} className={`tabs ${this.props.className || ""}`}>
        ...
        {this.props.toolbarFragment && (
          <div className="tabs__toolbar small">
            {this.props.toolbarFragment}
          </div>
        )}
 ...
 */
export class TabsWithLocalStorageViewStatePersistence<
  ID extends string,
  T extends Tab<ID>
...
> {
  ...
  private static saveToLocalStorage<ID extends string>(
    storageKey: string,
    lastTabID: ID
  ): void {
  ...
  private onSelectTab = (tab: ID): void => {
    ...
    this.setState({ activeTab: tab }, () =>
      TabsWithLocalStorageViewStatePersistence.saveToLocalStorage(
        this.props.storageKey,
        tab
      )
    );
...
interface TabsWithURLViewStatePersistenceProps<
  ID extends string,
  T extends Tab<ID>
> extends TabsProps<ID, T> {
  location: H.Location;
}
 ...
 */
export class TabsWithURLViewStatePersistence<
  ID extends string,
  T extends Tab<ID>
> extends React.PureComponent<
...
> {
  constructor(props: TabsWithURLViewStatePersistenceProps<ID, T>) {
    super(props);
    this.state = {
      activeTab: TabsWithURLViewStatePersistence.readFromURL(
        props.location,
        props.tabs
      )
    };
  ...
  public static readFromURL<ID extends string, T extends Tab<ID>>(
    location: H.Location,
    tabs: T[]
  ): ID | undefined {
  ...
  public componentDidUpdate(
    previousProps: TabsWithURLViewStatePersistenceProps<ID, T>
  ): void {
    if (
      previousProps.location !== this.props.location ||
      previousProps.tabs !== this.props.tabs
    ) {
  ...
  private renderTab = ({
    ...
    <Link
      className={className}
      to={TabsWithURLViewStatePersistence.urlForTabID(
        this.props.location,
        tab.id
      )}
      onClick={() => {
