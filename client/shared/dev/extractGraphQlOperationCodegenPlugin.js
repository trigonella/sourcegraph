 ...
 * @param {import('@graphql-codegen/plugin-helpers').Types.DocumentFile[]}
 *     documents
...
const plugin =
    (schema, documents, config) => {
                ...
                if (node.name && node.name.value) {
                  allOperations.push({
                    ...
                    location : item.location &&
                                   path.relative(ROOT_FOLDER, item.location),
                  })
                }
              },
            },
      ...
      if (interfaceFields.length === 0) {
        logger.warn('No operations found to generate interface ' +
                    interfaceNameForOperations)
      }
      return ('\n' + //
