...
describe('validateFilter()', () => {
  interface TestCase {
    description: string
    filterType: string
    expected: ReturnType<typeof validateFilter>token: Literal|Quoted
  }
  const TESTCASES: TestCase[] =
      [
        ...
        },
        {
          ...
          expected : {
            valid : false,
            reason : 'Invalid filter value, expected one of: yes, no.'
          },
    ...
    test(description,
         () => {expect(validateFilter(filterType,
                                      {token, range : {start : 0, end : 1}}))
