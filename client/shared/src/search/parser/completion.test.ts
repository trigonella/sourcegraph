...
test(
    "returns static filter type completions along with dynamically fetched completions when the token doesn't match a filter",
    async () => {
        expect(
            (await getCompletionItems(
            ...
            .toStrictEqual([
              ...
              '-message',    'patterntype',
              'repo',        '-repo',
              'repogroup',   'repohascommitafter',
              'repohasfile', '-repohasfile',
              'rev',         'stable',
              'timeout',     'type',
              'visibility',  'github.com/sourcegraph/jsonrpc2',
              'RepoRoutes',
            ])})

test(
    'returns suggestions for an empty query',
    async () => {
        expect((await getCompletionItems(
                    (parseSearchQuery('') as ParseSuccess<Sequence>).token,
                    {column : 1}, NEVER, false))
                   ?.suggestions.map(({label}) => label))
            .toStrictEqual([
              'after',       'archived',     'author',     '-author',
              'before',      'case',         'committer',  '-committer',
              'content',     '-content',     'count',      'file',
              '-file',       'fork',         'index',      'lang',
              '-lang',       'message',      '-message',   'patterntype',
              'repo',        '-repo',        'repogroup',  'repohascommitafter',
              'repohasfile', '-repohasfile', 'rev',        'stable',
              'timeout',     'type',         'visibility',
            ])})

test('returns suggestions on whitespace',
     async () => {
         expect((await getCompletionItems(
...
test(
    'returns static filter type completions for case-insensitive query',
    async () => {
        expect((await getCompletionItems(
                    (parseSearchQuery('rE') as ParseSuccess<Sequence>).token,
                    {column : 3}, of([]), false))
...
test('returns completions for filters with discrete values',
     async () => {
         expect(
             (await getCompletionItems(
                  (parseSearchQuery('case:y') as ParseSuccess<Sequence>).token,
                  {column : 7}, NEVER, false))
                 ?.suggestions.map(({label}) => label))
             .toStrictEqual([ 'yes', 'no' ])})

test(
    'returns completions for filters with static suggestions',
    async () => {
        expect(
            (await getCompletionItems(
...
test('returns dynamically fetched completions',
     async () => {
         expect(
             (await getCompletionItems(
                  ...
                  false))
                 ?.suggestions.map(({label, insertText}) =>
                                       ({label, insertText})))
             .toStrictEqual(
                 [ {label : 'connect.go', insertText : '^connect\\.go$ '} ])})

test(
    'inserts valid filters when selecting a file or repository suggestion',
    async () => {
        expect(
            (await getCompletionItems(
            ...
            .toStrictEqual([
              'file:^jsonrpc2\\.go$ ',
              'repo:^github\\.com/sourcegraph/jsonrpc2\\.go$ '
            ])})

test('sets current filter value as filterText',
     async () => {
         expect((await getCompletionItems(
                     (parseSearchQuery('f:^jsonrpc') as ParseSuccess<Sequence>)
                         .token,
                     {column : 11}, of([
                       {
                         __typename : 'File',
...
test('includes file path in insertText with fuzzy completions',
     async () => {
         expect(
             (await getCompletionItems(
