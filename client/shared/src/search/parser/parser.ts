...
/**
 * Converts a zero-indexed, single-line {@link CharacterRange} to a
 * Monaco {@link IRange}.
 */
...
export interface Filter {
  type: 'filter'
  filterType: Pick<ParseSuccess<Literal>, 'range'|'token'>filterValue:
      Pick<ParseSuccess<Literal|Quoted>, 'range'|'token'>|undefined
}
...
export type Token =
    |{type : 'whitespace'}|{type : 'openingParen'}|{type : 'closingParen'}|
    {type : 'operator'}|Comment|Literal|Filter|Sequence|Quoted

        /**
         * Represents the failed result of running a {@link Parser} on a search
         * query.
         */
...
/**
 * Represents the successful result of running a {@link Parser} on a search
 * query.
 */
...
export type ParserResult<T = Token> = ParseError|ParseSuccess<T>
     ...
     */
    const zeroOrMore = (parseToken: Parser):
        Parser<Sequence> => (input, start) => {
          const members:
              Pick<ParseSuccess<Exclude<Token, Sequence>>, 'range'|'token'>[] =
...
const oneOf = <T>(...parsers: Parser<T>[]):
    Parser<T> => (input, start) => {
      const expected: string[] = [] for (const parser of parsers) {
      ...
      return {
        type: 'error', expected: `One of: ${expected.join(', ')}`, at: start,
      }
    }
...
const quoted: Parser<Quoted> =
    (input, start) => {
      ...
      return {
        type: 'success',
            // end + 1 as `end` is currently the index of the quote in the
            // string.
...
const pattern = <T extends Token = Literal>(
    regexp: RegExp, output?: T|((input: string, range: CharacterRange) => T),
    expected?: string):
    Parser<T> => {
      ...
      return (input, start) => {
        ...
        if (!matchTarget) {
          return {
            type: 'error', expected: expected || `/${regexp.source}/`, at: start
          }
        }
        ...
        const range = {start, end : start + match[0].length} return {
          type: 'success', range,
              token: output
                  ? typeof output === 'function' ? output(input, range) : output
...
const operator =
    pattern(/(and|AND|or|OR|not|NOT)/,
            (input, {start, end}): Operator =>
                ({type : 'operator', value : input.slice(start, end)}))

const comment =
    pattern(/\/\/.*/, (input, {start, end}): Comment =>
                          ({type : 'comment', value : input.slice(start, end)}))

const filterKeyword =
    pattern(new RegExp(`-?(${filterTypeKeysWithAliases.join('|')})+(?=:)`, 'i'))
 ...
 */
const followedBy = (parseToken: Parser<Exclude<Token, Sequence>>,
    ...
    Parser<Sequence> => (input, start) => {
      ...
      if (input[end] !== undefined) {
        ...
        members.push(
            {token : separatorResult.token, range : separatorResult.range})
...
const filter: Parser<Filter> =
    (input, start) => {
      ...
      const parsedValue = input[parsedDelimiter.range.end] === undefined
                              ? undefined
                              : filterValue(input, parsedDelimiter.range.end)
      ...
      return {
        type: 'success', range: {
          start,
          end: parsedValue ? parsedValue.range.end : parsedDelimiter.range.end
        },
            token: {
              type: 'filter',
...
const baseTerms = [ operator, filter, quoted, literal ]
    ...
    const createParser = (terms: Parser<Exclude<Token, Sequence>>[]):
        Parser<Sequence> => zeroOrMore(oneOf<Token>(
            whitespace, openingParen, closingParen,
            ...terms.map(
                token => followedBy(
                    token, oneOf<{type : 'whitespace'}|{type : 'closingParen'}>(
                               whitespace, closingParen)))))
...
export const parseSearchQuery =
    (query: string, interpretComments?: boolean): ParserResult<Sequence> =>
        interpretComments ? searchQueryWithComments(query, 0)
                          : searchQuery(query, 0)
