...
suggestions?: SearchSuggestion['__typename']|string[] default?: string
...
export type FilterDefinition = BaseFilterDefinition | NegatableFilterDefinition
    ...
    export const LANGUAGES: string[] =
        [
          'c',        'cpp',  'csharp',     'css',        'go',   'graphql',
          'haskell',  'html', 'java',       'javascript', 'json', 'lua',
          'markdown', 'php',  'powershell', 'python',     'r',    'ruby',
          'rust',     'sass', 'swift',      'typescript',
        ]
        ...
        Record<Exclude<FilterType, NegatableFilter>, BaseFilterDefinition> = {
          ...
          [FilterType.author] : {
            ...
            description : negated => `${
                negated ? 'Exclude'
                        : 'Include only'} commits or diffs authored by a user.`,
          },
          [FilterType.before] : {
          ...
          [FilterType.committer] : {
            description : (negated: boolean) : string => `${
                negated
                    ? 'Exclude'
                    : 'Include only'} commits and diffs committed by a user.`,
            negatable : true,
            singular : true,
          },
          [FilterType.content] : {
            description : (negated: boolean) : string => `${
                negated
                    ? 'Exclude'
                    : 'Include only'} results from files if their content matches the search pattern.`,
            negatable : true,
            singular : true,
          },
          [FilterType.count] : {
          ...
          [FilterType.file] : {
            ...
            description : negated => `${
                negated
                    ? 'Exclude'
                    : 'Include only'} results from files matching the given search pattern.`,
            suggestions : 'File',
          },
          [FilterType.fork] : {
          ...
          [FilterType.lang] : {
            ...
            description : negated => `${
                negated ? 'Exclude'
                        : 'Include only'} results from the given language`,
          ...
          [FilterType.message] : {
            ...
            description : negated => `${
                negated
                    ? 'Exclude'
                    : 'Include only'} Commits with messages matching a certain string`,
          },
          [FilterType.patterntype] : {
            discreteValues : [ 'regexp', 'literal', 'structural' ],
            description :
                'The pattern type (regexp, literal, structural) in use',
            singular : true,
          },
          [FilterType.repo] : {
            alias : 'r',
            ...
            description : negated => `${
                negated
                    ? 'Exclude'
                    : 'Include only'} results from repositories matching the given search pattern.`,
          ...
          [FilterType.repohascommitafter] : {
            description :
                '"string specifying time frame" (filter out stale repositories without recent commits)',
            singular : true,
          },
          [FilterType.repohasfile] : {
            ...
            description : negated => `${
                negated
                    ? 'Exclude'
                    : 'Include only'} results from repos that contain a matching file`,
          },
          [FilterType.rev] : {
            description :
                'Search a revision (branch, commit hash, or tag) instead of the default branch.',
            singular : true,
          },
          [FilterType.stable] : {
            discreteValues : [ 'yes', 'no' ],
            default : 'no',
            description :
                'Forces search to return a stable result ordering (currently limited to file content matches).',
            singular : true,
          },
          [FilterType.timeout] : {
          ...
          [FilterType.type] : {
            ...
            discreteValues :
                [ 'diff', 'commit', 'symbol', 'repo', 'path', 'file' ],
          },
          [FilterType.visibility] : {
            discreteValues : [ 'any', 'private', 'public' ],
            description :
                'Include results from repositories with the matching visibility (private, public, any).',
            singular : true,
          },
        }
    ...
    export const discreteValueAliases: {[key: string]: string[]} = {
      yes : [
        'yes', 'y', 'Y', 'YES', 'Yes', '1', 't', 'T', 'true', 'TRUE', 'True'
      ],
      no : [
        'n', 'N', 'no', 'NO', 'No', '0', 'f', 'F', 'false', 'FALSE', 'False'
      ],
      only : [ 'o', 'only', 'ONLY', 'Only' ],
    }

    /**
     * Returns the {@link FilterDefinition} for the given filterType if it
     * exists, or `undefined` otherwise.
     */
    export const resolveFilter = (filterType: string):|{
    ...
    |undefined => {
      ...
      if (isNegatedFilter(filterType)) {
        ...
        return { type, definition: FILTERS[type], negated: true, }
      }
      if (isFilterType(filterType)) {
        if (isNegatableFilter(filterType)) {
          return {
            type: filterType, definition: FILTERS[filterType], negated: false,
          }
        }
      ...
      for (const [type, definition] of Object.entries(
               FILTERS as Record<FilterType, FilterDefinition>)) {
    ...
    /**
     * Checks whether a discrete value is valid for a given filter, accounting
     * for valid aliases.
     */
         ...
         value: string):
            boolean => {
              if (!definition.discreteValues ||
                  definition.discreteValues.includes(value)) {
                return true
              }

              const validDiscreteValuesForDefinition =
                  Object.keys(discreteValueAliases)
                      .filter(key => definition.discreteValues?.includes(key))
    ...
    export const validateFilter =
        (filterType: string, filterValue: Filter['filterValue']): {valid: true}|
    {
      valid: false;
      reason: string
    }
    => {
      ...
      if (definition.discreteValues &&
          (!filterValue ||
           ...
           (filterValue.token.type === 'literal' &&
            !isValidDiscreteValue(definition, filterValue.token.value)) ||
           (filterValue.token.type === 'quoted' &&
            !isValidDiscreteValue(definition,
                                  filterValue.token.quotedValue)))) {
        return {
          valid: false,
              reason: `Invalid filter value, expected one of: ${
                  definition.discreteValues.join(', ')}.`,
        }
      }
      return { valid: true }
    }
