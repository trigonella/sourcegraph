...
/**
 * Returns the diagnostics for a parsed search query to be displayed in the
 * Monaco query input.
 */
export function getDiagnostics({members}: Pick<Sequence, 'members'>,
  ...
  const diagnostics: Monaco.editor.IMarkerData[] =
    ...
    if (token.type === 'filter') {
      ...
      const validationResult =
          validateFilter(filterType.token.value, filterValue)
      ...
      if (patternType === SearchPatternType.regexp &&
