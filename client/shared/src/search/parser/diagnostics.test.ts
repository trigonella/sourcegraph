...
describe("getDiagnostics()", () => {
  test("do not raise invalid filter type", () => {
    expect(
      getDiagnostics(
        (parseSearchQuery("repos:^github.com/sourcegraph") as ParseSuccess<
          Sequence
        >).token,
  ...
  test("invalid filter value", () => {
    expect(
      getDiagnostics(
        (parseSearchQuery("case:maybe") as ParseSuccess<Sequence>).token,
        SearchPatternType.literal
      )
    ).toStrictEqual([
      {
  ...
  test("search query containing colon, regexp pattern type, suggest quotes", () => {
    expect(
      getDiagnostics(
        (parseSearchQuery("Configuration::doStuff(...)") as ParseSuccess<
          Sequence
        >).token,
  ...
  test("search query containing parenthesized parameterss", () => {
    expect(
      getDiagnostics(
        (parseSearchQuery("repo:a (file:b and c)") as ParseSuccess<Sequence>)
          .token,
        SearchPatternType.regexp
      )
    ).toStrictEqual([]);
