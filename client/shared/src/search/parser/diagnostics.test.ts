...
test('invalid filter value',
     () => {expect(getDiagnostics((parseSearchQuery('case:maybe') as
                                   ParseSuccess<Sequence>)
                                      .token,
                                  SearchPatternType.literal))
                .toStrictEqual([
                  {
...
test('search query containing colon, regexp pattern type, suggest quotes',
     () => {
         expect(
             getDiagnostics((parseSearchQuery('Configuration::doStuff(...)') as
                             ParseSuccess<Sequence>)
                                .token,
             ...
             .toStrictEqual([
               {
                 ...
                 endLineNumber : 1,
                 message :
                     'Quoting the query may help if you want a literal match.',
                 severity : 4,
                 startColumn : 1,
                 startLineNumber : 1,
               },
...
test('search query containing colon, literal pattern type, do not raise error',
     () => {expect(getDiagnostics(
                       (parseSearchQuery('Configuration::doStuff(...)') as
                        ParseSuccess<Sequence>)
                           .token,
                       SearchPatternType.literal))
                .toStrictEqual([])})
...
test('search query containing parenthesized parameterss',
     () => {expect(getDiagnostics((parseSearchQuery('repo:a (file:b and c)') as
                                   ParseSuccess<Sequence>)
                                      .token,
                                  SearchPatternType.regexp))
                .toStrictEqual([])})
