...
test('whitespace', () => expect(parseSearchQuery('  ')).toMatchObject({
  range : {
    start : 0,
    end : 2,
  },
...
test('literal', () => expect(parseSearchQuery('a')).toMatchObject({
  range : {
    start : 0,
    end : 1,
  },
...
test('triple quotes', () => {expect(parseSearchQuery('"""')).toMatchObject({
                        range : {
                          end : 3,
                          start : 0,
                        },
...
test('filter', () => expect(parseSearchQuery('f:b')).toMatchObject({
  range : {
    end : 3,
    start : 0,
  },
...
test('negated filter', () => expect(parseSearchQuery('-f:b')).toMatchObject({
  range : {
    end : 4,
    start : 0,
  },
...
test('quoted', () => expect(parseSearchQuery('"a:b"')).toMatchObject({
  range : {
    end : 5,
    start : 0,
  },
...
test('complex query',
     () =>
         expect(
             parseSearchQuery(
                 'repo:^github\\.com/gorilla/mux$ lang:go -file:mux.go Router'))
             .toMatchObject({
               range : {
                 end : 58,
                 start : 0,
