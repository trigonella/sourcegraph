...
export const repositoryCompletionItemKind =
...
type PartialCompletionItem =
    ...
    /**
     * COMPLETION_ITEM_SELECTED is a custom Monaco command that we fire after
     * the user selects an autocomplete suggestion. This allows us to be
     * notified and run custom code when a user selects a suggestion.
     */
...
const FILTER_TYPE_COMPLETIONS:
    Omit<Monaco.languages.CompletionItem, 'range'>[] =
        Object.keys(FILTERS)
            .flatMap(label => {
              const filterType = label as FilterType
              const completionItem:
                  Omit<Monaco.languages.CompletionItem, 'range'|'detail'> = {
                    label,
    ...
    ({name}: IRepository,
     options: {isFilterValue: boolean; globbing : boolean}):
        PartialCompletionItem => {
          ...
          insertText =
              (options.isFilterValue ? insertText
                                     : `${FilterType.repo}:${insertText}`) +
              ' '
          return {
            label: name, kind: repositoryCompletionItemKind, insertText,
                filterText: name,
    ...
    ({name, path, repository, isDirectory}: IFile,
     options: {isFilterValue: boolean; globbing : boolean}):
        PartialCompletionItem => {
          ...
          insertText =
              (options.isFilterValue ? insertText
                                     : `${FilterType.file}:${insertText}`) +
              ' '
          return {
            label: name,
                kind: isDirectory ? Monaco.languages.CompletionItemKind.Folder
                                  : Monaco.languages.CompletionItemKind.File,
                insertText, filterText: name,
 ...
 */
const symbolKindToCompletionItemKind:
    Record<SymbolKind, Monaco.languages.CompletionItemKind> = {
...
const symbolToCompletion = ({name, kind, location}:
                                ISymbol): PartialCompletionItem => ({
  label : name,
  kind : symbolKindToCompletionItemKind[kind],
  insertText : name + ' ',
  filterText : name,
  detail :
      `${startCase(kind.toLowerCase())} - ${location.resource.repository.name}`,
})

const languageToCompletion =
    ({name}: ILanguage): PartialCompletionItem|undefined => name ? {
      label : name,
...
const suggestionToCompletionItem =
    ...
    undefined => {
      ...
      case 'File':
        ...
        case 'Repository': return repositoryToCompletion(suggestion, options)
        case 'Symbol': return symbolToCompletion(suggestion)
        case 'Language': return languageToCompletion(suggestion)
        case 'RepoGroup': return repoGroupToCompletion(suggestion)
      }
    }

                 /**
                  * An internal Monaco command causing completion providers to
                  * be invoked, and the suggestions widget to be shown.
                  *
                  * Useful to show the suggestions widget right after selecting
                  * a filter type completion, to offer filter values
                  * completions.
                  */
...
/**
 * Returns the completion items for a search query being typed in the Monaco
 * query input, including both static and dynamically fetched suggestions.
 */
export async function getCompletionItems(
    {members}: Pick<Sequence, 'members'>,
    {column}: Pick<Monaco.Position, 'column'>,
    dynamicSuggestions: Observable<SearchSuggestion[]>,
    globbing: boolean): Promise<Monaco.languages.CompletionList|null> {
  const defaultRange = {
    startLineNumber : 1,
    endLineNumber : 1,
    startColumn : column,
  ...
  const tokenAtColumn = members.find(({range}) => range.start + 1 <= column &&
                                                  range.end + 1 >= column)
  if (!tokenAtColumn) {
  ...
  if (token.type === 'literal' || token.type === 'whitespace') {
    ...
    if (token.type === 'literal' &&
        staticSuggestions.some(
            ({label}) => label.startsWith(token.value.toLowerCase()))) {
    ...
    return {
      suggestions: [
        ...
        ...(await dynamicSuggestions.pipe(first()).toPromise())
            .map(suggestion => suggestionToCompletionItem(
                     suggestion, {isFilterValue : false, globbing}))
            .filter(isDefined)
            .map(completionItem => ({
                   ...completionItem,
                   range : toMonacoRange(range),
  ...
  if (token.type === 'filter') {
    ...
    const completingValue =
        !filterValue || filterValue.range.start + 1 <= column
    if (!completingValue) {
      return null
    }
    ...
    if (resolvedFilter.definition.suggestions) {
      if (Array.isArray(resolvedFilter.definition.suggestions)) {
        return {
          suggestions: resolvedFilter.definition.suggestions.map(
              label => ({
                label,
                kind : Monaco.languages.CompletionItemKind.Text,
                insertText : label,
                range : filterValue ? toMonacoRange(filterValue.range)
                                    : defaultRange,
                command : COMPLETION_ITEM_SELECTED,
              })),
        }
      ...
      return {
        suggestions: suggestions
            .filter(({__typename}) =>
                        __typename === resolvedFilter.definition.suggestions)
            .map(suggestion => suggestionToCompletionItem(
                     suggestion, {isFilterValue : true, globbing}))
            .filter(isDefined)
            .map(partialCompletionItem => ({
                   ...partialCompletionItem,
                   // Set the current value as filterText, so that all dynamic
                   // suggestions returned by the server are displayed.
                   // otherwise, if the current filter value is a regex pattern,
                   // Monaco's filtering might hide some suggestions.
                   filterText :
                       filterValue && (filterValue?.token.type === 'literal'
