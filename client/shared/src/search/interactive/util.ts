...
export interface FiltersToTypeAndValue {
   ...
   * */
  [key: string]: {// `type` is the field type of the filter (repo, file, etc.)
                  type: FilterType
  // `value` is the current value for that particular filter,
  value: string
  // `editable` is whether the corresponding filter input is currently editable
  // in the UI.
  editable: boolean
  // `negated` is whether the filter is negated. Optional because some filters
  // are non-negatable.
        negated?: boolean
    }
}
...
export const isFilterType = (filter: string): filter is FilterType =>
    filter in FilterType
export const isAliasedFilterType = (filter: string): boolean =>
    filter in AliasedFilterType

export const filterTypeKeys: FilterType[] =
    Object.keys(FilterType) as
    FilterType[] export const filterTypeKeysWithAliases:
        (FilterType|AliasedFilterType)[] =
        [
          ...filterTypeKeys,
...
export type NegatableFilter =|FilterType.repo|FilterType.file|
                             FilterType.repohasfile|FilterType.lang|
                             FilterType.content|FilterType.committer|
                             FilterType.author|FilterType.message

export const isNegatableFilter =
    (filter: FilterType): filter is NegatableFilter =>
        Object.keys(NegatedFilters).includes(filter)

/**
 * The list of all negated filters. i.e. all valid filters that have `-` as a
 * suffix.
 */
...
export const resolveNegatedFilter = (filter: NegatedFilters): NegatableFilter =>
    negatedFilterToNegatableFilter[filter]
