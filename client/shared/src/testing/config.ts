...
interface Field<T = string>{envVar : string
description?: string
    defaultValue?: T
}
...
type ConfigFields = {
      [K in keyof Config]: Field<Config[K]>&
          (Config[K] extends string
           ? Partial<FieldParser>: FieldParser<Config[K]>)
    }

    const parseBool = (string: string):
        boolean => {
          if ([ '1', 't', 'true' ].some(trueString => trueString ===
                                                      string.toLowerCase())) {
            return true
          }
          if ([ '0', 'f', 'false' ].some(falseString => falseString ===
                                                        string.toLowerCase())) {
            return false
          }
          throw new Error(
              `could not parse string ${JSON.stringify(string)} to boolean`)
        }
    ...
    const configFields: ConfigFields = {
      ...
      sudoUsername : {
        ...
        description :
            'The site-admin-level username that will be impersonated with the sudo access token.',
      },
      gitHubClientID : {
        ...
        defaultValue :
            'cf9491b706c4c3b1f956', // "Local dev sign-in via GitHub" OAuth app
                                    // from github.com/sourcegraph
      },
      gitHubClientSecret : {
      ...
      gitHubUserAmyPassword : {
        ...
        description :
            'Password of the GitHub user sg-e2e-regression-test-amy, used to log in to Sourcegraph.',
      },
      gitHubUserBobPassword : {
        envVar : 'GITHUB_USER_BOB_PASSWORD',
        description :
            'Password of the GitHub user sg-e2e-regression-test-bob, used to log in to Sourcegraph.',
      },
      gitHubUserBobToken : {
      ...
      gitLabUserAmyPassword : {
        ...
        description :
            'Password of the GitLab user sg-e2e-regression-test-amy, used to log in to Sourcegraph.',
      },
      oktaMetadataUrl : {
      ...
      oktaUserAmyPassword : {
        ...
        description :
            'Password of the Okta user, beyang+sg-e2e-regression-test-amy@sourcegraph.com',
      },
      sourcegraphBaseUrl : {
      ...
      init : {
        ...
        defaultValue : false,
        description :
            'If true, run the test for initializing a brand new instance of Sourcegraph',
      },
      testUserPassword : {
      ...
      keepBrowser : {
        ...
        parser : parseBool,
        description :
            'If true, browser window will remain open after tests run',
        defaultValue : false,
      },
      logBrowserConsole : {
      ...
      slowMo : {
        ...
        parser : parseInt,
        description :
            'Slow down puppeteer by the specified number of milliseconds',
        defaultValue : 0,
      },
      devtools : {
        envVar : 'DEVTOOLS',
    ...
    /**
     * Reads e2e config from environment variables. The caller should specify
     * the config fields that it depends on. This should NOT be called from
     * helper packages. Instead, call it near the start of "test main" function
     * (i.e., Jest `test` blocks). Doing this ensures that all the necessary
     * environment variables necessary for a test are presented to the user in
     * one go.
     */
    export function
    getConfig<T extends keyof Config>(...required: T[]): Partial<Config>&
        Pick<Config, T> {
      // Read config
      const config: any = {} for (const fieldName of Object.keys(
                                      configFields) as (keyof Config)[]) {
        const field =
            configFields[fieldName] if (field.defaultValue !== undefined) {
          config[fieldName] = field.defaultValue
        }
        const environmentValue =
            process.env[field.envVar] if (environmentValue) {
          config[fieldName] =
              field.parser ? field.parser(environmentValue) : environmentValue
        }
      }
      ...
      if (missingKeys.length > 0) {
        const fieldInfo = (key: T): string => {
          const field = configFields[key] if (!field) { return '' }
          const info = [ field.envVar ] if (field.defaultValue) {
