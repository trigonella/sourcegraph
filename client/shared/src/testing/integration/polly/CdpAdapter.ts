...
function toBase64(input: string):
    string{return Buffer.from(input).toString('base64')}

function
fromBase64(input: string):
    string { return Buffer.from(input, 'base64').toString() }
...
interface PollyPromise extends Promise<PollyResponse> {
  resolve(response: PollyResponse): void reject(error: any): void
}
...
export class CdpAdapter extends PollyAdapter {
   ...
   */
  public static get id(): string { return 'cdp' }
  ...
  public options!: CdpAdapterOptions
      ...
      /**
       * Event that can be subscribed to handle errors that occurred in request
       * handlers.
       */
      public readonly errors: Observable<unknown> =
          this._errors
              .asObservable()
          ...
          /**
           * A map of all intercepted requests to their respond function, which
           * will be called by the 'response' event listener, causing Polly to
           * record the response content.
           */
          private pendingRequests =
              new Map<string, PollyPromise>()

              /**
               * The CDP session used to control request interception in the
               * browser.
               */
              private cdpSession?: Puppeteer.CDPSession

              /**
               * A map of all intercepted requests to their passthrough
               * callbacks function, which will be called by the
               * onResponseReceived event listener, causing Polly to record the
               * response content.
               */
              private passthroughPromises = new Map<string, {
  ...
  constructor(polly: Polly) {
    ...
    // @ts-ignore
    super(polly) this.page = this.options.page
  }
    ...
    const fetchEnableRequest: Protocol.Fetch.EnableRequest =
        {
          patterns :
            ...
            .send('Fetch.enable', fetchEnableRequest)

                this.cdpSession.on(
                    'Fetch.requestPaused',
                    (event: Protocol.Fetch.RequestPausedEvent): void => {
  ...
  /**
   * Given a request that should be allowed to pass through (not be
   * intercepted), return a Promise of the Response for that request, which will
   * be passed to request.respond().
   */
  public passthroughRequest(pollyRequest: PollyRequest&
                            PollyRequestArguments): Promise<PollyResponse> {
    const {
      requestArguments : {requestId},
    } = pollyRequest

    return new Promise<PollyResponse>(
        (resolve, reject) => {
            this.passthroughPromises.set(requestId, {resolve, reject}) this
                .continuePausedRequest({requestId})
                .catch(console.error)})
  }
   ...
   */
  public async respondToRequest(pollyRequest: PollyRequest&
  ...
  /**
   * Called when a request is intercepted, for all requests (passthrough or
   * stubbed).
   *
   * Adds an entry to pendingRequests, that will call the provided
   * promise.resolve function when a response for this request is received.
   */
  public onRequest(pollyRequest: PollyRequest&PollyRequestArguments&
                   {promise : PollyPromise}): void {
    const {requestArguments, promise} = pollyRequest

    const {requestId} =
        requestArguments this.pendingRequests.set(requestId, promise)
  }

  private async fulfillPausedRequest(
      request: Protocol.Fetch.FulfillRequestRequest): Promise<void> {
    await this.trySendCdpRequest('Fetch.fulfillRequest', request)
  }

  private async continuePausedRequest(
      request: Protocol.Fetch.ContinueRequestRequest): Promise<void> {
   ...
   */
  private async trySendCdpRequest(cdpRequestName: string,
                                  request?: object): Promise<void> {
    try {
      ...
      if (isErrorLike(error) &&
          (error.message.endsWith(
           ...
           // Invalid interceptionId probably means the request has been
           // aborted.
  ...
  private async abortPausedRequest(request: Protocol.Fetch.FailRequestRequest):
      Promise<void> {
    await this.cdpSession?.send('Fetch.failRequest', request)
  }

  private async getResponseBody(event: Protocol.Fetch.RequestPausedEvent):
      Promise<string> {
    if (getLocationHeader(event)) {
      return '' // CDP doesn't let us obtain the body of redirect requests, so
                // we don't attempt it.
    }
    if (!this.cdpSession) {
  ...
  private handlePausedRequestInResponseStage(
    ...
    if (event.responseErrorReason) {
      const error = new Error(event.responseErrorReason)
                    ...
                    // Reject passthrough
                    this.passthroughPromises.get(requestId)
                        ?.reject(error) this.passthroughPromises
                        .delete(requestId)

                    /// Reject pending request
                    this.pendingRequests.get(requestId)
                        ?.reject(error) this.pendingRequests.delete(requestId)
      return
    }
    ...
    this.getResponseBody(event)
        .then(body => {
          ...
          // The requestId may or may not have an associated passthrough
          // callback. If it does, call it and delete it.
          this.passthroughPromises.get(requestId)
              ?.resolve(pollyResponse) this.passthroughPromises
              .delete(requestId)

              // Each pending request has an associated promise. Because at this
              // point the request is done (given that a response has been
              // received), we can resolve the pending request promise.
              this.pendingRequests.get(requestId)
              ?.resolve(pollyResponse) this.pendingRequests.delete(requestId)
        })
        .catch(console.error)
  }
}
 ...
 */
function eventIsInResponseStage(event: Protocol.Fetch.RequestPausedEvent):
    boolean{return event.responseStatusCode !== undefined ||
                   event.responseErrorReason !== undefined}

/**
 * Get the value of the "Location" response header.
 **/
function getLocationHeader(event: Protocol.Fetch.RequestPausedEvent):
    string|undefined {
      const {responseHeaders = []} = event
      const foundLocationHeader =
          responseHeaders.find(header => header.name === 'location')
 ...
 */
function headerEntriesToHeaderObject(
 ...
 */
function headerObjectToHeaderEntries(headers: Record<string, string>):
    Protocol.Fetch.HeaderEntry[] {
      return Object.entries(headers).map(([ name, value ]) => ({name, value}))
    }

/**
 * Get the body data as a string, from the response of a `Fetch.getResponseBody`
 * call.
 */
function getBodyStringFromCdpBody(body: Protocol.Fetch.GetResponseBodyResponse):
    string {
      if (!body) {
        return ''
      }
