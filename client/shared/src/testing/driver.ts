...
import {appendFile, mkdir, readFile} from 'mz/fs'
import * as os from 'os'
import * as path from 'path'
import puppeteer,
{ConsoleMessage, LaunchOptions, Page, PageEventObj, PageFnOptions, Serializable,
 Target,} from 'puppeteer'
...
/**
 * Returns a Promise for the next emission of the given event on the given
 * Puppeteer page.
 */
export const oncePageEvent = <E extends keyof PageEventObj>(
    page: Page, eventName: E): Promise<PageEventObj[E]> =>
    new Promise(resolve => page.once(eventName, resolve))

export const percySnapshot =
    readEnvironmentBoolean({variable : 'PERCY_ON', defaultValue : false})
        ? realPercySnapshot
        : () => Promise.resolve()
 ...
 *
 * - Meta+A doesn't work in input boxes
...
/**
 * Specifies how to enter text. Typing is preferred in cases where it's
 * important to test the process of manually typing out the text to enter.
 * Pasting is preferred in cases where typing would be too slow or we explicitly
 * want to test paste behavior.
 */
...
function findElementRegexpStrings(
    text: string, {fuzziness = 'space-prefix'}:
                      Pick<FindElementOptions, 'fuzziness'>): string[] {
  //  Escape regexp special chars. Copied from
  //  https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_Expressions
  const escapedText = escapeRegExp(text)
  const regexps = [ `^${escapedText}$` ] if (fuzziness === 'exact') {
    return regexps
  }
...
function findElementMatchingRegexps(tag: string,
                                    regexps: string[]): HTMLElement|null {
...
function getDebugExpressionFromRegexp(tag: string, regexp: string): string {
  return `Array.from(document.querySelectorAll(${
      JSON.stringify(
          tag)})).filter(e => e.innerText && e.innerText.match(/${regexp}/))`
}
...
export class Driver {
  ...
  public visitedPages: Readonly<URL>[] = []
      ...
      public sourcegraphBaseUrl: string private keepBrowser:
          boolean private subscriptions = new Subscription()

  constructor(public browser: puppeteer.Browser, public page: puppeteer.Page,
              options: DriverOptions) {
    this.sourcegraphBaseUrl = options.sourcegraphBaseUrl this.keepBrowser =
        !!options
              .keepBrowser

          // Record visited pages
          this.subscriptions.add(
              merge(fromEvent<Target>(browser, 'targetchanged'),
                    fromEvent<Target>(browser, 'targetcreated'))
                  .pipe(filter(target => target.type() === 'page'))
                  .subscribe(target => {this.visitedPages.push(
                                 new URL(target.url()))}))

    // Log browser console
    if (options.logBrowserConsole) {
      this.subscriptions.add(
          merge(from(browser.pages()).pipe(mergeAll()),
                fromEvent<Target>(browser, 'targetcreated')
                    .pipe(mergeMap(target => target.page()), filter(isDefined)))
              .pipe(mergeMap(
                  page =>
                          ...
                          .pipe(
                              filter(
                                  message =>
                                      ...
                                      !message.text().includes(
                                          'React-Hot-Loader') &&
                                      // These requests are expected to fail, we
                                      // use them to check if the browser
                                      // extension is installed.
                                      message.location().url !==
                                          'chrome-extension://invalid/'),
                              // Immediately format remote handles to strings,
                              // but maintain order.
                              map(message => formatPuppeteerConsoleMessage(
                                      page, message)),
                              concatAll(),
    ...
    password: string
        ...
        localStorage.setItem('has-dismissed-integrations-toast', 'true')
            ...
            if (url.pathname === '/site-admin/init') {
              ...
              await this.page.type('input[name=username]', username) await this
                  .page.type('input[name=password]', password) await this.page
                  .waitForSelector('button[type=submit]:not(:disabled)')
                  // TODO(uwedeportivo): investigate race condition between
                  // puppeteer clicking this very fast and background gql client
                  // request fetching ViewerSettings. this race condition
                  // results in the gql request "winning" sometimes without
                  // proper credentials which confuses the login state machine
                  // and it navigates you back to the login page
                  await delay(1000)
              ...
              await this.page.waitForSelector('.test-signin-form')
                  await this.page.type('input', username) await this.page
        ...
        /**
         * Navigates to the Sourcegraph browser extension options page and sets
         * the sourcegraph URL.
         */
        ...
        public async selectAll(method: SelectTextMethod = 'selectall'):
            Promise<void> {
          switch (method) {
          case 'selectall': {
            await this.page.evaluate(
          ...
          case 'keyboard': {
            const modifier =
                os.platform() === 'darwin'
                    ? Key.Meta
                    : Key.Control await this.page.keyboard
                          .down(modifier)
        ...
        public async enterText(method: EnterTextMethod = 'type',
                               text: string): Promise<void> {
          // Pasting does not work on macOS. See:
          // https://github.com/GoogleChrome/puppeteer/issues/1313
          method = os.platform() === 'darwin' ? 'type' : method
          switch (method) {
          case 'type':
            await this.page.keyboard.type(text)
            break case 'paste': await this.paste(text)
            break
          }
        }
    ...
    selectMethod?: SelectTextMethod
        enterTextMethod?: EnterTextMethod
          ...
          await retry(
              async () => {await this.page.waitForSelector(selector)
        ...
        public async acceptNextDialog(): Promise<void> {
          const dialog =
              await oncePageEvent(this.page, 'dialog') await dialog.accept()
        }
    ...
    ensureRepos?: string[]
        alwaysCloning?: string[]
          ...
          const {externalServices} =
              dataOrThrowErrors(await this.makeGraphQLRequest<IQuery>({
                request : gql`
          ...
          if (externalServices.totalCount !== 0) {
            await this.page
                .goto(this.sourcegraphBaseUrl + '/site-admin/external-services')
                    await this.page
                .waitFor('.test-filtered-connection')
                    await this.page.waitForSelector(
                        '.test-filtered-connection__loader', {hidden : true})

            // Matches buttons for deleting external services named
            // ${displayName}.
            const deleteButtonSelector = `[data-test-external-service-name="${
                displayName}"] .test-delete-external-service-button`
            if (await this.page.$(deleteButtonSelector)) {
              await Promise.all([
                this.acceptNextDialog(), this.page.click(deleteButtonSelector)
              ])
            }
          }

          // Navigate to the add external service page.
          console.log('Adding external service of kind', kind) await this.page
              .goto(this.sourcegraphBaseUrl +
                    '/site-admin/external-services/new') await this.page
              .waitForSelector(`[data-test-external-service-card-link="${
                                   kind.toUpperCase()}"]`,
                               {
                                 visible : true,
                               }) await this.page
              .evaluate(
                  (selector: string) => {
                    ...
                    if (!element) {
                      throw new Error(
                          `Could not find element to click on for selector ${
                              selector}`)
                    }
                    element.click()
                  },
                  `[data-test-external-service-card-link="${
                      kind.toUpperCase()}"]`) await this
              .replaceText({
                ...
                newText : displayName,
                  ...
                  await this.page
              .waitForSelector('.test-external-service-editor .monaco-editor')
              // Type in a new external service configuration.
              await this
              .replaceText({
                selector :
                    '.test-external-service-editor .monaco-editor .view-line',
                newText : config,
                selectMethod : 'keyboard',
              })
                  await Promise.all([
                    this.page.waitForNavigation(),
                    this.page.click('.test-add-external-service-button')
                  ])
          ...
          if (ensureRepos) {
            ...
            for (const slug of ensureRepos) {
              await this.page
                  .goto(this.sourcegraphBaseUrl +
                        `/site-admin/repositories?filter=cloned&query=${
                            encodeURIComponent(slug)}`) await this.page
                  .waitForSelector(
                      `.repository-node[data-test-repository='${slug}']`,
                      {visible : true})
                  // Workaround for
                  // https://github.com/sourcegraph/sourcegraph/issues/5286
          ...
          if (alwaysCloning) {
            for (const slug of alwaysCloning) {
              await this.page
                  .goto(this.sourcegraphBaseUrl +
                        `/site-admin/repositories?filter=cloning&query=${
                            encodeURIComponent(slug)}`) await this.page
                  .waitForSelector(
                      `.repository-node[data-test-repository='${slug}']`,
                      {visible : true})
        ...
        public async assertWindowLocation(location: string,
                                          isAbsolute = false): Promise<any> {
          const url =
              isAbsolute
                  ? location
                  : this.sourcegraphBaseUrl +
                        location await retry(
                            async () => {expect(await this.page.evaluate(
                                                    () => window.location.href))
                                             .toEqual(url)})
        }

        public async assertWindowLocationPrefix(locationPrefix: string,
                                                isAbsolute = false):
            Promise<any> {
        ...
        public async assertStickyHighlightedToken(label: string):
            Promise<void> {
          await this.page.waitForSelector(
              '.selection-highlight-sticky',
              {visible : true}) // make sure matched token is highlighted
              await retry(
                  async () =>
        ...
        public async assertAllHighlightedTokens(label: string): Promise<void> {
          const highlightedTokens = await this.page.evaluate(
              () => [...document.querySelectorAll('.selection-highlight')].map(
        ...
        public async assertNonemptyExternalRefs(): Promise<void> {
          ...
          await this.page
              ...
              // verify there are some references
              await this.page.waitForSelector(
                  '.panel__tabs-content .hierarchical-locations-view__item',
                  {visible : true})
        }

        private async makeRequest<T = void>({url, init}: {
          url: string; init : RequestInit & Serializable
        }): Promise<T> {
    ...
    private async makeGraphQLRequest<T, V = object>({
        ...
        variables: V
          ...
          const xhrHeaders =
              (await this.page.evaluate(
                  sourcegraphBaseUrl =>
                      location.href.startsWith(sourcegraphBaseUrl) &&
                      (window as any).context.xhrHeaders,
                  this.sourcegraphBaseUrl)) ||
          {}
          const response = await this.makeRequest<GraphQLResult<T>>({
            url : `${this.sourcegraphBaseUrl}/.api/graphql${
        ...
        public async getRepository(name: string):
            Promise<Pick<IRepository, 'id'>> {
        ...
        public async setConfig(path: jsonc.JSONPath,
                               editFunction: (oldValue: jsonc.Node|
                                              undefined) => any):
            Promise<void> {
          ...
          const updateConfigResponse =
              await this.makeGraphQLRequest<IMutation>({
                request : gql`
        ...
        public async ensureHasCORSOrigin({corsOriginURL}: {
          ...
          await this.setConfig(
              ...
              oldCorsOrigin => {
                  const urls =
                      oldCorsOrigin
                          ? (oldCorsOrigin.value as string).split(' ')
                          : [] return (urls.includes(corsOriginURL) ? urls : [
                              ...urls, corsOriginURL
                            ]).join(' ')})
        }
        ...
        public async setUserSettings<S extends Settings>(settings: S):
            Promise<void> {
          ...
          const updateConfigResponse =
            ...
            `,
                variables : {
                  ...
                  lastID : currentUser.latestSettings
                               ? currentUser.latestSettings.id
                               : null,
                },
              })
          dataOrThrowErrors(updateConfigResponse)
        ...
        /**
         * Finds the first HTML element matching the text using the regular
         * expressions returned in
         * {@link findElementRegexpStrings}.
         *
         * @param options specifies additional parameters for finding the
         *     element. If you want to wait
         * until the element appears, specify the `wait` field (which can
         * contain additional inner options for how long to wait).
         */
        public async findElementWithText(text: string,
          ...
          const notFoundError = (underlying?: Error):
              Error => {
                const debuggingExpressions = regexps.map(
                    regexp => getDebugExpressionFromRegexp(tag, regexp))
                return new Error(
                    `Could not find element with text ${
                        JSON.stringify(
                            text)}, options: ${JSON.stringify(options)}` +
                    (underlying ? `. Underlying error was: ${
                                      JSON.stringify(underlying.message)}.`
                                : '') +
          ...
          return retry(async () => {
            const handlePromise =
                wait
                    ? this.page
                          .waitForFunction(findElementMatchingRegexps,
                                           typeof wait === 'object' ? wait : {},
                                           tag, regexps)
                          .catch(error => {throw notFoundError(error)})
                    : this.page.evaluateHandle(findElementMatchingRegexps, tag,
                                               regexps)
        ...
        public async waitUntilURL(url: string,
                                  options: PageFnOptions = {}): Promise<void> {
          await this.page.waitForFunction(url => document.location.href === url,
                                          options, url)
        }
}

export function modifyJSONC(text: string, path: jsonc.JSONPath,
                            editFunction: (oldValue: jsonc.Node|undefined) =>
                                any):
    any{const old = jsonc.findNodeAtLocation(jsonc.parseTree(text), path)
...
interface DriverOptions extends LaunchOptions {
    ...
    export async function createDriverForTest(options?: DriverOptions):
        Promise<Driver> {
      // Apply defaults
      options = {
        ...getConfig('sourcegraphBaseUrl', 'headless', 'slowMo', 'keepBrowser',
                     'browser', 'devtools'),
        ...options,
      }

      const {loadExtension} = options
      const args: string[] = [] const launchOptions: puppeteer.LaunchOptions = {
      ...
      if (options.browser === 'firefox') {
        ...
        if (!(await readFile(cfgPath, 'utf-8'))
                 .includes(disableCspPreference)) {
          await appendFile(cfgPath, disableCspPreference)
        }
        if (loadExtension) {
          const cdpPort = await getFreePort()
          const firefoxExtensionPath = path.resolve(
              __dirname, '..', '..', '..', 'browser', 'build', 'firefox')
        ...
        if (loadExtension) {
          const chromeExtensionPath = path.resolve(__dirname, '..', '..', '..',
                                                   'browser', 'build', 'chrome')
          const manifest =
              JSON.parse(await readFile(
                  path.resolve(chromeExtensionPath, 'manifest.json'),
                  'utf-8')) as {
            permissions: string[]
          }
          ...
          args.push(`--disable-extensions-except=${chromeExtensionPath}`,
