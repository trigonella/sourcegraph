...
export async function formatPuppeteerConsoleMessage(
    page: Page, message: ConsoleMessage): Promise<string> {
  const color = colors[message.type()] ?? identity
  const icon = icons[message.type()] ?? ''
  const formattedLocation = 'at ' + chalk.underline([
  ...
  const locationLine = chalk.dim(
      ...
      '\n' +
          (!process.env.CI
               ? ' '.repeat(Math.max(0, terminalSize().columns -
                                            stringWidth(formattedLocation))) +
                     formattedLocation
               : '\t' + formattedLocation))
  return [
    chalk.bold('🖥  Browser console:'),
    color(...[message.type() !== 'log' ? chalk.bold(icon, message.type()) : '',
              message.args().length === 0 ? message.text() : '',
              ...(await Promise.all(message.args().map(async argumentHandle => {
                try {
                  const json = (await (await argumentHandle.evaluateHandle(
                                           value => JSON.stringify(
                                               value,
                                               (key, value: unknown) => {
                                                 // Check if value is error,
                                                 // because Errors are not
                                                 // serializable but commonly
                                                 // logged
                  ...
                  return chalk.italic(
                      `[Could not serialize: ${asError(error).message}]`)
                }
              })))
                  .map(value =>
