 ...
 */
export function afterEachSaveScreenshotIfFailed(
...
async function takeScreenshot({
  ...
  if (process.env.CI) {
    ...
    console.log(
      `\u001B]1338;url="artifact://${path.relative(
        repoRootDir,
        filePath
      )}";alt="Screenshot"\u0007`
    );
    ...
    console.log(
      `\u001B]1337;File=name=${nameBase64};inline=1;width=500px:${screenshot.toString(
        "base64"
      )}\u0007`
    );
  } else {
    console.log(
      `📸  Saved screenshot of failure to ${path.relative(
        process.cwd(),
        filePath
      )}`
    );
