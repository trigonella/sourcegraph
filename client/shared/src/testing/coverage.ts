...
declare global{interface FileCoverage{/** Absolute path. */
                                      path : string
        hash: string
...
let warnedNoCoverage = false
         ...
         * after each test.
         */
        export function afterEachRecordCoverage(getDriver: () => Driver):
            void {
              afterEach('Record coverage',
                        () => recordCoverage(getDriver().browser))
            }
          ...
          const targets =
              await browser
                  .targets()
                      ...
                      await Promise
                  .all(targets.map(async target => {
                    if (target.url() === 'about:blank') {
                      return
                    }
                    const executionContext =
                        (await target.worker()) ?? (await target.page())
                    if (!executionContext) {
                      return
                    }
                    const coverage: typeof __coverage__ = await pTimeout(
                        ...
                        2000,
                        new Error(
                            `Timeout getting coverage from ${target.url()}`))
                    if (!coverage) {
                      if (!warnedNoCoverage) {
                        console.error(
                    ...
                    await Promise.all(Object.values(coverage).map(
                        async fileCoverage => {await writeFile(
