...
const MENU_ITEMS_PROP_SORT_ORDER = [ 'group', 'id' ]
    ...
    export function getContributedActionItems(
        contributions: Evaluated<Contributions>, menu: ContributableMenu):
        ActionItemAction[] {
          if (!contributions.actions) {
            return []
          }

          const allItems: ActionItemAction[] = [] const menuItems =
              contributions.menus?.[menu] if (menuItems) {
            for (const {action : actionID, alt : altActionID} of sortBy(
                     menuItems, MENU_ITEMS_PROP_SORT_ORDER)) {
              const action = contributions.actions.find(a => a.id === actionID)
              const altAction =
                  contributions.actions.find(a => a.id === altActionID)
              if (action) {
