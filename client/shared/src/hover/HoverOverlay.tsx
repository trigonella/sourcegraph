...
const transformMouseEvent = (handler: (event: MouseEvent) => void) => (
  event: React.MouseEvent<HTMLElement>
) => handler(toNativeEvent(event));

export type HoverContext = RepoSpec &
  RevisionSpec &
  FileSpec &
  ResolvedRevisionSpec;
...
const isEmptyHover = ({
  ...
  HoverOverlayProps,
  "hoveredToken" | "hoverOrError" | "actionsOrError"
>): boolean =>
  !hoveredToken ||
...
> {
    ...
    this.subscription.add(
      this.props.platformContext.settings.subscribe(settingsCascadeOrError => {
        if (
          ...
          !isErrorLike(settingsCascadeOrError.final)
        ) {
  ...
  public componentDidUpdate(previousProps: HoverOverlayProps): void {
    ...
    if (
      ...
      (!isEqual(this.props.hoveredToken, previousProps.hoveredToken) ||
        isEmptyHover(previousProps))
    ) {
      this.logTelemetryEvent();
    }
  }
      ...
      >
        <div className={classNames("hover-overlay__contents")}>
            ...
            <button
              ...
              className={classNames(
                ...
                hoverOrError === LOADING &&
                  "hover-overlay__close-button--loading"
              )}
              onClick={
                onCloseButtonClick
                  ? transformMouseEvent(onCloseButtonClick)
                  : undefined
              }
            >
          ...
          {hoverOrError === LOADING ? (
            ...
            (!hoverOrError?.contents.length && hoverOrError?.alerts?.length) ? (
            ...
            hoverOrError?.contents.map((content, index) => {
              if (content.kind === "markdown") {
                try {
                  ...
                  return (
                    <div
                      className={classNames(this.props.errorAlertClassName)}
                      key={index}
                    >
                ...
                >
                  ...
                  {summary.kind === "plaintext" ? (
                    ...
                    <span
                      ...
                      dangerouslySetInnerHTML={{
                        __html: renderMarkdown(summary.value)
                      }}
                    />
                  )}
  ...
  private onAlertDismissedCallback(
    alertType: string
