...
} from '../api/client/services/location'
import {
  createTestViewerService
} from '../api/client/services/viewerService.test'
import {
...
const FIXTURE_HOVER_CONTEXT: HoveredToken&
    HoverContext =
        {
          repoName : 'r',
          commitID : 'c',
        ...
        testWorkspaceService(
            roots: readonly WorkspaceRootWithMetadata[] =
                [ {uri : 'git://r3?c3', inputRevision : 'v3'} ],
            versionContext: string|undefined = undefined):
            WorkspaceService {
              return {
                roots: new BehaviorSubject(roots),
                    versionContext: new BehaviorSubject(versionContext)
              }
            }

        // Use toPrettyBlobURL as the urlToFile passed to these functions
        // because it results in the most readable/familiar expected test
        // output. Some tests may override this with .callsFake()
        let urlToFile!:
            sinon.SinonStub<Parameters<PlatformContext['urlToFile']>,
                            string>beforeEach(() => {
              urlToFile =
                  sinon.stub<Parameters<PlatformContext['urlToFile']>, string>()
                      .callsFake(toPrettyBlobURL)
            })
...
it('shows a loader for the definition if slow',
   () => {scheduler().run(
       ({cold, expectObservable}) =>
           expectObservable(from(getHoverActionsContext({
             extensionsController : {
               services : {
                 workspace : testWorkspaceService(),
                 textDocumentDefinition : {
                   getLocations : () => cold<MaybeLoadingResult<Location[]>>(
                       `l ${LOADER_DELAY + 100}ms r`, {
                 ...
                 textDocumentReferences : {
                   providersForDocument : () =>
                       cold<ProvideTextDocumentLocationSignature<
                           ReferenceParameters, Location>[]>(
                           'a', {a : [ () => of(null) ]}),
                 },
               },
             },
             platformContext : {urlToFile, requestGraphQL},
               ...
               .toBe(`n ${LOADER_DELAY - 1}ms (lf) 97ms g`,
                     ((): {[key: string]: HoverActionsContext} => ({
                        ...
                        f : {
                          ...
                          'goToDefinition.error' : false,
                          'findReferences.url' :
                              '/r@v/-/blob/f#L2:2&tab=references',
                          hoverPosition : FIXTURE_PARAMS,
                        },
                        // Show go to definition button, hide loader, show find
                        // references button
                        g : {
                          'goToDefinition.showLoading' : false,
       ...
       ({cold, expectObservable}) =>
           expectObservable(from(getHoverActionsContext({
             ...
             platformContext : {urlToFile, requestGraphQL},
                                                        ...
                                                        FIXTURE_HOVER_CONTEXT)))
               .toBe(`e n ${LOADER_DELAY - 2}ms (lf) 49ms g`,
                     ((): {[key: string]: HoverActionsContext} => ({
                        // Show nothing
                        e : {
                          'goToDefinition.showLoading' : false,
...
it('shows the find references button when reference providers are registered later',
   () => {scheduler().run(
       ({cold, expectObservable}) =>
           expectObservable(
               from(getHoverActionsContext({
                 extensionsController : {
                   services : {
                     workspace : testWorkspaceService(),
                     textDocumentDefinition : {
                       getLocations : () => cold<
                           MaybeLoadingResult<Location[]>>('-b', {
...
it('emits null if the locations result is empty',
   () =>
       expect(getDefinitionURL({urlToFile, requestGraphQL}, {
                workspace : testWorkspaceService(),
                textDocumentDefinition :
    ...
    const urlToFile =
        sinon
            .spy((_location: RepoSpec&Partial<RawRepoSpec>&RevisionSpec&
                  FileSpec&Partial<UIPositionSpec>&Partial<ViewStateSpec>) =>
                     '') await getDefinitionURL({urlToFile, requestGraphQL}, {
              workspace : testWorkspaceService(),
              textDocumentDefinition : {
                getLocations : () => of<MaybeLoadingResult<Location[]>>({
                  isLoading : false,
    ...
    sinon.assert.calledOnce(urlToFile)
        ...
        const urlToFile =
            sinon
                .spy() await getDefinitionURL({urlToFile, requestGraphQL}, {
                  workspace : testWorkspaceService(),
                  textDocumentDefinition : {
                    getLocations : () => of<MaybeLoadingResult<Location[]>>({
                      isLoading : false,
        ...
        sinon.assert.calledOnce(urlToFile)
            ...
            describe(
                'when the result is inside the current root',
                () => {it(
                    'emits the definition URL the user input revision (not commit SHA) of the root',
                    () => expect(getDefinitionURL({urlToFile, requestGraphQL}, {
                                   workspace : testWorkspaceService(),
                                   textDocumentDefinition : {
                                     getLocations : () =>
                              ...
                              .resolves.toEqual({
                                isLoading : false,
                                result :
                                    {url : '/r3@v3/-/blob/f', multiple : false}
                              }))})
...
let commands!: CommandRegistry
    ...
    contribution = new ContributionRegistry(createTestViewerService({}), {
      getPartialModel : () => ({languageId : 'x'}),
    },
                                            of(EMPTY_SETTINGS_CASCADE), of({}))
    ...
    locationAssign = sinon.spy((_url: string) => undefined)
        ...
        const getHoverActions =
            (context: HoverActionsContext): Promise<ActionItemAction[]> =>
                contribution.getContributions(undefined, context)
                    .pipe(first(),
                          map(contributions => getContributedActionItems(
  ...
  action: {
    ...
    commandArguments:
        [
          '{"textDocument":{"uri":"git://r?c#f"},"position":{"line":1,"character":1}}'
        ],
    id: 'goToDefinition',
  ...
  action: {
    ...
    describe('goToDefinition command', () => {
        ...
        textDocumentDefinition.getLocations = () =>
            of({isLoading : false, result : [ FIXTURE_LOCATION ]}) // mock
            await commands.executeCommand({
              command : 'goToDefinition',
              arguments : [ JSON.stringify(FIXTURE_PARAMS) ]
            })
        ...
        expect(history).toHaveLength(1)
        urlToFile.callsFake(
            toAbsoluteBlobURL.bind(null, 'https://sourcegraph.test'))
        textDocumentDefinition.getLocations = () =>
            of({
              isLoading : false,
              result : [
                FIXTURE_LOCATION, {...FIXTURE_LOCATION, uri : 'git://r3?v3#f3'}
              ]
            }) // mock
            await commands.executeCommand({
              command : 'goToDefinition',
        ...
        sinon.assert.calledWith(
            locationAssign,
            'https://sourcegraph.test/r@c/-/blob/f#L2:2&tab=def')
            expect(history).toHaveLength(1)
        })
