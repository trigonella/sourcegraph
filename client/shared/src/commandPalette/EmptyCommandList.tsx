...
export const EmptyCommandList: React.FunctionComponent<Props> = ({
  settingsCascade,
  sourcegraphURL
}) => {
  // if no settings cascade, default to 'no active extensions'
  const onlyDefault = settingsCascade
    ? onlyDefaultExtensionsAdded(settingsCascade)
    : false;
    ...
    <div className="empty-command-list">
      <p className="empty-command-list__title">
        {onlyDefault
          ...
          : "You don't have any active actions"}
      </p>
...
const PuzzleIllustration = React.memo<{ className?: string }>(
  ({ className }) => (
    <svg
      width="59"
      height="64"
