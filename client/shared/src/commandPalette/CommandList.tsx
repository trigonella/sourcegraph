...
export class CommandList extends React.PureComponent<CommandListProps, State> {
  ...
  private static readRecentActions(): string[] | null {
    ...
    try {
      ...
      if (
        ...
        recentActions.every(a => typeof a === "string")
      ) {
  ...
  public componentDidMount(): void {
    ...
    this.subscriptions.add(
      this.props.platformContext.settings.subscribe(settingsCascade =>
  ...
  public componentDidUpdate(
    _previousProps: CommandListProps,
    previousState: State
  ): void {
  ...
  public render(): JSX.Element | null {
    ...
    const allItems = getContributedActionItems(
    ...
    const items = filterAndRankItems(
      allItems,
      this.state.input,
    ...
    const selectedIndex =
      ((this.state.selectedIndex % items.length) + items.length) % items.length;
        ...
        <div className={this.props.resultsContainerClassName}>
          <ul className={this.props.listClassName}>
            {items.length > 0 ? (
              items.map((item, index) => (
                <li
                  className={classNames(
                    ...
                    index === selectedIndex &&
                      this.props.selectedListItemClassName
                  )}
                  key={item.action.id}
                >
                  <ActionItem
                    ...
                    className={classNames(
                      ...
                      index === selectedIndex &&
                        this.props.selectedActionItemClassName
                    )}
                    {...item}
                    ref={
                      index === selectedIndex ? this.setSelectedItem : undefined
                    }
                    title={
                      <HighlightedMatches
                        text={[
              ...
              <li className={this.props.noResultsClassName}>
  ...
  private onInputKeyDown: React.KeyboardEventHandler<
    HTMLInputElement
  > = event => {
    switch (event.key) {
      case Key.ArrowDown: {
  ...
  private setSelectedIndex(delta: number): void {
    this.setState(previousState => ({
      selectedIndex: previousState.selectedIndex + delta
    }));
  }
  ...
  private onActionDidExecute = (actionID: string): void => {
    ...
    this.setState(previousState => {
      ...
      return {
        recentActions: uniq([actionID, ...recentActions]).slice(
          0,
          KEEP_RECENT_ACTIONS
        )
      };
...
export const CommandListPopoverButton: React.FunctionComponent<CommandListPopoverButtonProps> = ({
  ...
  return (
    <ButtonElement
      role="button"
      className={`command-list__popover-button ${buttonClassName} ${
