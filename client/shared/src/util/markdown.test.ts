...
describe('renderMarkdown', () => {
    it('renders code blocks, with syntax highlighting', () => {
        ...
        it('renders to plain text with plainText: true',
           () => {expect(renderMarkdown('A **b**', {plainText : true}))
                      .toBe('A b\n')})
        it('sanitizes script tags',
           () => {expect(renderMarkdown('<script>evil();</script>')).toBe('')})
        it('sanitizes event handlers',
           () => {expect(renderMarkdown(
                             '<svg><rect onclick="evil()"></rect></svg>'))
                      .toBe('<p><svg><rect></rect></svg></p>\n')})
        it('sanitizes non-SVG <object> tags',
           () => {expect(renderMarkdown('<object data="something"></object>'))
                      .toBe('<p></p>\n')})
        it('allows SVG <object> tags',
           () => {
               expect(
                   renderMarkdown(
                       '<object data="something" type="image/svg+xml"></object>'))
                   .toBe(
    ...
    test('default disabled',
         () => {expect(renderMarkdown(MARKDOWN_WITH_DOWNLOAD))
                    .toBe('<p><a>D</a>\n<a>D2</a></p>\n')})
        test('enabled', () => {
