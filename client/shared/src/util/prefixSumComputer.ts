...
export class PrefixSumIndexOfResult {
  public _prefixSumIndexOfResultBrand: void
      ...
      public index: number public remainder: number

  constructor(index: number, remainder: number) {
    this.index = index this.remainder = remainder
  }
}
...
export class PrefixSumComputer {
  ...
  constructor(values: Uint32Array) {
    this.values = values this.prefixSum =
        new Uint32Array(values.length) this.prefixSumValidIndex =
            new Int32Array(1) this.prefixSumValidIndex[0] = -1
  }

  public getCount(): number { return this.values.length }
  ...
  public insertValues(insertIndex: number, insertValues: Uint32Array): boolean {
    ...
    this.values =
        new Uint32Array(oldValues.length + insertValuesLength) this.values
            .set(oldValues.subarray(0, insertIndex), 0) this.values
            .set(oldValues.subarray(insertIndex),
                 insertIndex + insertValuesLength) this.values.set(insertValues,
                                                                   insertIndex)
  ...
  public removeValues(startIndex: number, cnt: number): boolean {
    ...
    this.values = new Uint32Array(oldValues.length - cnt) this.values
                      .set(oldValues.subarray(0, startIndex), 0) this.values
                      .set(oldValues.subarray(startIndex + cnt), startIndex)
  ...
  private _getAccumulatedValue(valueIndex: number): number {
    ...
    this.prefixSumValidIndex[0] =
        Math.max(this.prefixSumValidIndex[0], valueIndex)
    return this.prefixSum[valueIndex]
  }
