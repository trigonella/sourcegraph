  ...
  return (
    ...
    concat(
      ...
      switchMap(value => {
        ...
        const syncReason = head(
          compact(synchronousValidators?.map(validator => validator(value)))
        );
        if (syncReason) {
        ...
        return zip(
          ...(asynchronousValidators?.map(validator => validator(value)) ?? [])
        ).pipe(
