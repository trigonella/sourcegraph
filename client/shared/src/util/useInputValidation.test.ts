...
let setupValidationPipelineTest: (validationOptions: ValidationOptions) =>
...
afterAll(() => {clock.restore()})
  ...
  const inputValidationStates: InputValidationState[] = []
      ...
      const validationPipeline = createValidationPipeline(
          validationOptions, inputReference,
  ...
  // Creating this type instead of a generic util because TS doesn't support
  // higher-kinded types
  type ObservableEmission<T> = T extends Observable<infer V>? V : never
  const changeEvents = new Subject<
      ObservableEmission<Parameters<typeof validationPipeline>[ 0 ]>>()
  ...
  // "Scripting" user interaction: strings are new input values, numbers are
  // delays before next input in ms.
  return function executeUserInputScript(inputScript) {
    ...
    return inputValidationStates
         ...
         *
         * Implements the `HTMLInputElement` properties needed by
         * `useInputValidation`, along with a `changeValue` method to simulate
         * input and change the internal value
         */
  ...
  return {
    get value() { return internalStrings.value }
    , get validationMessage() { return internalStrings.validationMessage }
    , checkValidity() {
    ...
    , setCustomValidity(error) { internalStrings.validationMessage = error }
    , changeValue(newValue) { internalStrings.value = newValue }
    ,
  }
    }
  ...
  return of(email === 'test@sg.co' ? 'Email is taken' : undefined)
      .pipe(delay(200))
        }
