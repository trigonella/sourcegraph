...
describe("isErrorLike", () => {
  test("reports true for Error values", () =>
    expect(isErrorLike(new Error("m"))).toBeTruthy());
  test("reports true for error-like values", () =>
    expect(isErrorLike({ message: "m" })).toBeTruthy());
  test("reports false for non-error-like values", () =>
    expect(!isErrorLike("m")).toBeTruthy());
});
