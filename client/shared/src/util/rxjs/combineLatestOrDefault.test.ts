...
describe(
    'with 1 source observable',
    () => {test(
        'waits to emit/complete until the source observable emits/completes',
        () => scheduler().run(({cold, expectObservable}) =>
                                  expectObservable(combineLatestOrDefault([
                                    cold('-b-|', {b : 1})
                                  ])).toBe('-b-|', {
                                    b : [ 1 ],
                                  })))})
