 ...
 *
 * It does not emit when the localStorage item is changed in the same window
 * (because that does not trigger the "storage" event per the Web Storage API
 * specification). To emit for changes in the same window, call
 * {@link LocalStorageSubject#next}.
 */
export class LocalStorageSubject<T>
    // eslint-disable-next-line rxjs/no-subclass
    extends Observable<T> implements NextObserver<T>,
                                     Pick<BehaviorSubject<T>, 'value'> {
  ...
  public next(value: T): void {
    ...
    // Does not set oldValue or other StorageEventInit keys because we don't
    // need them.
    window.dispatchEvent(
        new StorageEvent('storage', {key : this.key, newValue : json}))
  }
