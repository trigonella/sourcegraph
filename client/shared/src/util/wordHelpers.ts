 ...
 * Matches words.
 *
 * @todo It is convenient for this to also match tokens that begin with common
 * completion trigger characters, such as `@` (for username completion). That is
 * not useful in general. When we have a need for this to be stricter, support
 * custom regexps and make this not match `@` and the other non-standard word
 * symbols..
 *
 * Users of this value *must* reset it before using it as follows:
 ...
 * {@link WordAtPosition#endColumn}. Use this when {@link text} is a line
 * suffix, not a whole line.
 */
export function getWordAtText(column: number, text: string): WordAtPosition|
    null {
  WORD_REGEXP.lastIndex = 0
  ...
  while (true) {
    ...
    if (matchIndex <= column && WORD_REGEXP.lastIndex >= column) {
      return {
        word: match[0], startColumn: matchIndex,
