...
expect(result.current).toBe(2)
    ...
    const {result} =
        renderHook(() => useObservable(
                       useMemo(() => new Observable<number>(subscribe), [])))
    expect(result.current).toBe(undefined)
    sinon.assert.calledOnce(subscribe)
    const [subscriber] = subscribe.args[0]
    act(() => {subscriber.next(1)})
    expect(result.current).toBe(1)
        sinon.assert.calledOnce(subscribe)
    })
    ...
    const subscribe =
        sinon.spy((subscriber: Subscriber<number>) => {subscriber.error(error)})
    const {result} =
        renderHook(() => useObservable(
                       useMemo(() => new Observable<number>(subscribe), [])))
    ...
    it('should subscribe if component rerenders and observable changed', () => {
        const subscribe = sinon.spy((subscriber: Subscriber<number>) => {
            ...
            const {result, rerender} = renderHook(
                ...
                useObservable(
                    useMemo(() => new Observable<number>(subscribe), [ dep ])))
            expect(result.current).toBe(1)
            sinon.assert.calledOnce(subscribe)
            dep = 'changed'
            rerender()
    ...
    it('should not subscribe if component rerenders and observable did not change', () => {
        const subscribe = sinon.spy((subscriber: Subscriber<number>) => {
            ...
            const {result, rerender} =
                renderHook(() => useObservable(useMemo(
    ...
    it('should unsubscribe when the component unmounts', () => {
        ...
        describe(
            'useEventObservable()',
            () => {it(
                'should emit on the transform function whenever an event was triggered',
                () => {
                  const spy = sinon.spy((number: number) => number + 10)
                  const {result} = renderHook(
                      () => useEventObservable(useCallback(
                          (events: Observable<number>) => events.pipe(map(spy)),
                          [])))
