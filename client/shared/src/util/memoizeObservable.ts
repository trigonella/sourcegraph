...
/**
 * Clears all memoized data for memoizeObservable calls. All calls made to those
 * functions after clearing will result in the fetch func being called again.
 *
 * You must call this function after you've modified a memoized resource, or
 * else some components of the UI may have a stale view of the resource.
 */
export function resetAllMemoizationCaches(): void { allCachesResetSeq++}
 ...
 */
export function
memoizeObservable<P, T>(func: (parameters: P) => Observable<T>,
  ...
  return (parameters: P, force = false) => {
    ...
    const observable =
        func(parameters).pipe(publishReplay(), refCount(), tap({
