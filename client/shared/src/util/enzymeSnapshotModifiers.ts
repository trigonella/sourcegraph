// This file contains modifier functions to customize JSON serialization of
// snapshot tests using enzyme-to-json. These are used in
// share/dev/enzymeSerializer.js
...
function maskProps(props: Json['props'], key = '',
                   seenProperties: ReadonlySet<unknown> = new Set()): unknown {
  if (!props) {
    return props
  }
  ...
  if (isPlainObject(props)) {
    return Object.fromEntries(Object.entries(props).map(
        ([ key, value ]) => [key, maskProps(value, key, treeSeenProperties)]))
  }
  return props
}
...
export function replaceVerboseObjects(json: Json): Json|string {
  if (/Memo\(\w+Icon\)/.test(json.type)) {
    json.children = [] return json
  }
  if (json.props) {
    return { ...json, props: maskProps(json.props) as Json['props'], }
  }
