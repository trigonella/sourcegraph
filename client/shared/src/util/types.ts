 ...
 * @param key The key of the property to check.
 */
export const keyExistsIn = <O extends object>(
...
export function anyOf(...typeGuards: any[]): any {
  return (value: unknown) =>
    typeGuards.some((guard: (value: unknown) => boolean) => guard(value));
}
...
export function allOf(...typeGuards: any[]): any {
  return (value: unknown) =>
    typeGuards.every((guard: (value: unknown) => boolean) => guard(value));
}
...
export const check = <T>(simpleCondition: (value: T) => boolean) => (
  value: T
): value is T => simpleCondition(value);
