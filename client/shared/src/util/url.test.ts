...
describe("parseRepoURI", () => {
  ...
  test("should parse repo with commitID", () => {
    const parsed = parseRepoURI(
      "git://github.com/gorilla/mux?24fca303ac6da784b9e8269f724ddeb0b2eea5e7"
    );
    assertDeepStrictEqual(parsed, {
      repoName: "github.com/gorilla/mux",
  ...
  test("should parse repo with revision and file and position", () => {
    const parsed = parseRepoURI(
      "git://github.com/gorilla/mux?branch#mux.go:3,5"
    );
    assertDeepStrictEqual(parsed, {
      repoName: "github.com/gorilla/mux",
      revision: "branch",
  ...
  test("should parse repo with revision and file and range", () => {
    const parsed = parseRepoURI(
      "git://github.com/gorilla/mux?branch#mux.go:3,5-6,9"
    );
    assertDeepStrictEqual(parsed, {
      repoName: "github.com/gorilla/mux",
      revision: "branch",
  ...
  test("should parse a file with special characters", () => {
    const parsed = parseRepoURI(
      "git://github.com/gorilla/mux?branch#space%20here.go"
    );
    assertDeepStrictEqual(parsed, {
      repoName: "github.com/gorilla/mux",
      revision: "branch",
...
describe("util/url", () => {
  ...
  describe("parseHash", () => {
    ...
    test("parses hash with range", () => {
      ...
      expect(parseHash("L34:2-L38:3")).toEqual({
        line: 34,
        character: 2,
        endLine: 38,
        endCharacter: 3
  ...
  describe("toPrettyBlobURL", () => {
    test("formats url for empty revision", () => {
      expect(toPrettyBlobURL(context)).toBe(
    ...
    test("formats url with view state", () => {
      expect(
        toPrettyBlobURL({
          ...context,
          position: lineCharPosition,
          viewState: "references"
        })
      ).toBe("/github.com/gorilla/mux/-/blob/mux.go#L1:1&tab=references");
    });
  });
  ...
  describe("toAbsoluteBlobURL", () => {
    ...
    test("default sourcegraph URL, specified revision", () => {
      expect(
        toAbsoluteBlobURL(sourcegraphUrl, { ...target, revision: "branch" })
      ).toBe(
...
describe("buildSearchURLQuery", () => {
  it("builds the URL query for a regular expression search", () =>
    expect(
      buildSearchURLQuery("foo", SearchPatternType.regexp, false, undefined)
    ).toBe("q=foo&patternType=regexp"));
  it("builds the URL query for a literal search", () =>
    expect(
      buildSearchURLQuery("foo", SearchPatternType.literal, false, undefined)
    ).toBe("q=foo&patternType=literal"));
  it("handles an empty query", () =>
    expect(
      buildSearchURLQuery("", SearchPatternType.regexp, false, undefined)
  ...
  it("builds the URL query with a case parameter if caseSensitive is true", () =>
    expect(
      buildSearchURLQuery("foo", SearchPatternType.literal, true, undefined)
  ...
  it("builds url query with a version context", () =>
    ...
    ).toBe("q=foo+&patternType=literal&c=3.15"));
});
...
describe("appendSubtreeQueryParam", () => {
  it("appends subtree=true to urls", () => {
    expect(
      appendSubtreeQueryParameter(
        "/github.com/sourcegraph/sourcegraph/-/blob/.gitattributes#L2:24"
      )
    ).toBe(
  ...
  it("appends subtree=true to urls with other query params", () => {
    expect(
      appendSubtreeQueryParameter(
        "/github.com/sourcegraph/sourcegraph/-/blob/.gitattributes?test=test#L2:24"
      )
    ).toBe(
      "/github.com/sourcegraph/sourcegraph/-/blob/.gitattributes?test=test&subtree=true#L2:24"
    );
  });
});
