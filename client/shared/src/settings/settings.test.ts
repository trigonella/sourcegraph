...
const SETTINGS_ERROR_FOR_FIXTURE_USER = createAggregateError(
    [ new Error('parse error (code: 0, offset: 0, length: 1)') ])
...
describe('gqlToCascade', () => {
    ...
    expect(isErrorLike(value.final) && value.final.message)
...
test('merges multiple values',
     () => expect(mergeSettings<{a?: number; b?: number}&Settings>([
             {a : 1}, {b : 2}, {a : 3}
           ])).toEqual({
       a : 3,
       b : 2,
...
test('deeply merges extensions property',
     () =>
         expect(
             mergeSettings<
                 {a?: {[key: string] : boolean}; b?: {[key: string] : boolean}}&
                 Settings>([
               {
                 extensions : {
               ...
               {
                 extensions :
                     {'sourcegraph/go' : false, 'sourcegraph/typescript' : true}
               },
             ]))
             .toEqual({
               extensions : {
                 'sourcegraph/cpp' : false,
                 'sourcegraph/go' : false,
                 'sourcegraph/typescript' : true
               },
             }))
test('merges experimentalFeatures property',
     () => expect(mergeSettings<Settings&{
             experimentalFeatures : {a?: boolean; b?: boolean; c?: boolean}
           }>([
...
test('merges quicklinks property',
     () => expect(mergeSettings<{
             ...
             {
               quicklinks : [ {
                 name : 'About Sourcegraph',
                 value : 'https://docs.internal/about-sourcegraph'
               } ]
             },
             {
               quicklinks : [
                 {
                   name : 'mycorp extensions',
                   value : 'https://sourcegraph.com/extensions?query=mycorp%2F'
                 },
               ],
             },
           ])).toEqual({
...
test(
    'merges search.repositoryGroups property',
    () =>
        expect(mergeSettings<
...
describe('merge', () => {
  ...
  expect(base).toEqual(expected)
    ...
    test('merges with empty', () => {
        ...
        test('merges top-level objects deeply',
             () => assertMerged({a : 1}, {b : 2}, {a : 1, b : 2}))
        test('does not merge nested objects deeply',
             () => assertMerged({a : {b : 1}}, {a : {c : 2}}, {a : {c : 2}}))
        test('overwrites arrays',
             () => assertMerged({a : [ 1 ]}, {a : [ 2 ]}, {a : [ 2 ]}))
