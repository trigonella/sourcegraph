...
export type IFileMatch = Partial<
  Pick<GQL.IFileMatch, "revSpec" | "symbols" | "limitHit">
> & {
  file: Pick<GQL.IFile, "path" | "url"> & {
...
export type ILineMatch = Pick<
  GQL.ILineMatch,
  "preview" | "lineNumber" | "offsetAndLengths" | "limitHit"
> & {
...
export class FileMatch extends React.PureComponent<Props> {
  ...
  constructor(props: Props) {
    ...
    const subsetMatches = parseInt(
      localStorage.getItem(SUBSET_COUNT_KEY) || "",
      10
    );
  ...
  public render(): React.ReactNode {
    ...
    const items: IMatchItem[] = this.props.result.lineMatches.map(match => ({
      highlightRanges: match.offsetAndLengths.map(
        ([start, highlightLength]) => ({ start, highlightLength })
      ),
    ...
    const { repoAtRevURL, revDisplayName } =
      result.revSpec?.__typename === "GitRevSpecExpr" &&
      result.revSpec.object?.commit
        ? {
            repoAtRevURL: result.revSpec.object?.commit?.url,
            revDisplayName: result.revSpec.expr
          }
        : result.revSpec?.__typename === "GitRef"
        ? {
            repoAtRevURL: result.revSpec.url,
            revDisplayName: result.revSpec.displayName
          }
    ...
    if (this.props.showAllMatches) {
      ...
      containerProps = {
        ...
        expandedChildren,
        collapseLabel: `Hide ${length} ${pluralize(
          "match",
          length,
          "matches"
        )}`,
        expandLabel: `Show ${length} more ${pluralize(
          "match",
          length,
          "matches"
        )}`,
