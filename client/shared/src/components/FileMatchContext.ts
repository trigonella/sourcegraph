/**
 * Groups highlights that have overlapping or adjacent context. The input must
 * be sorted.
 */
export const mergeContext = <T extends {line : number}>(
    context: number, highlights: T[]): T[][] => {
  const groupsOfHighlights: T[][] = []

      for (let index = 0; index < highlights.length; index++) {
    const current = highlights[index] const previous =
        highlights[index - 1] if (!previous ||
                                  current.line - previous.line - 2 * context >
                                      1) {
