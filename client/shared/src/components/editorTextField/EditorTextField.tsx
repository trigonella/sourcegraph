...
} from "../../api/client/services/viewerService";
import {
  ModelService,
  TextModel
} from "../../api/client/services/modelService";
import {
  offsetToPosition,
  positionToOffset
...
export const EditorTextFieldUtils = {
  ...
  getEditorDataFromElement: (
    ...
    const selectionStart = isReversed
      ? element.selectionEnd
      : element.selectionStart;
    const selectionEnd = isReversed
      ? element.selectionStart
      : element.selectionEnd;
    ...
    const end =
      selectionStart === selectionEnd
        ? start
        : offsetToPosition(element.value, selectionEnd);
    return {
      text: element.value,
  ...
  updateEditorSelectionFromElement: (
    ...
    viewerService.setSelections(
      editor,
      EditorTextFieldUtils.getEditorDataFromElement(element).selections
    );
  },
  ...
  updateElementOnEditorOrModelChanges: (
    ...
    textAreaReference: React.RefObject<
      Pick<HTMLTextAreaElement, "value" | "setSelectionRange">
    >
  ): Unsubscribable => {
      ...
      changes
        ...
        .subscribe(selections => {
          ...
          if (textArea) {
            ...
            const end = isEmpty
              ? start
              : positionToOffset(textArea.value, sel.end);
...
export const EditorTextField: React.FunctionComponent<Props> = ({
  ...
  const textAreaReference =
    _textAreaReference || createRef<HTMLTextAreaElement>();
  ...
  return (
    <textarea
      ...
      value={value}
      onInput={event => {
        EditorTextFieldUtils.updateModelFromElement(
          modelService,
          modelUri,
          event.currentTarget
        );
        EditorTextFieldUtils.updateEditorSelectionFromElement(
          viewerService,
          { viewerId },
          event.currentTarget
        );
