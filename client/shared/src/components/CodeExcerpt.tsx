...
export class CodeExcerpt extends React.PureComponent<Props, State> {
  ...
  private getFirstLine(): number {
    const contextLines =
      this.props.context || this.props.context === 0 ? this.props.context : 1;
    ...
    return Math.max(
      0,
      Math.min(...this.props.highlightRanges.map(range => range.line)) -
        contextLines
    );
  }
  ...
  private getLastLine(blobLines: string[] | undefined): number {
    ...
    const highlightRangeLines = this.props.highlightRanges.map(
      range => range.line
    );
    ...
    const contextLineHasHighlight =
      lastHighlightLineNumber > this.props.lastSubsetMatchLineNumber;
    ...
    const remainingContextLinesToShow =
      lastHighlightLineNumber - this.props.lastSubsetMatchLineNumber;

    const numberOfcontextLinesToShow = contextLineHasHighlight
      ? contextLines -
        (remainingContextLinesToShow <= contextLines
          ? remainingContextLinesToShow
          : 0)
      : contextLines;
      ...
      >
        <code
          className={`code-excerpt ${this.props.className || ""}${
            isErrorLike(this.state.blobLinesOrError)
              ? " code-excerpt-error"
              : ""
          }`}
        >
  ...
  private makeTableHTML(blobLines: string[]): string {
    return (
      "<table>" +
      blobLines
        .slice(this.getFirstLine(), this.getLastLine(blobLines) + 1)
        .join("") +
      "</table>"
    );
  }
