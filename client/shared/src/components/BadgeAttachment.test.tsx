...
export const base64ImageBadge: Omit<BadgeAttachmentRenderOptions, "kind"> = {
  ...
  linkURL:
    "https://docs.sourcegraph.com/code_intelligence/explanations/basic_code_intelligence"
};
...
describe("BadgeAttachment", () => {
  ...
  it("renders an img element with a base64 icon", () => {
    const { container } = render(
      <BadgeAttachment
        attachment={base64ImageBadge as BadgeAttachmentRenderOptions}
        isLightTheme={true}
      />
    );
  ...
  it("renders an svg element with a predefined icon", () => {
    const { container } = render(
      <BadgeAttachment attachment={badgeWithKind} isLightTheme={true} />
    );
    expect(container).toMatchSnapshot();
  });
});
