...
import {
  offsetToPosition,
  positionToOffset
} from "../../api/client/types/textDocument";
...
export interface EditorCompletionWidgetProps
  extends ExtensionsControllerProps,
    Pick<
      ...
      Exclude<
        keyof CompletionWidgetProps,
        "completionListOrError" | "onSelectItem"
      >
    > {
  /**
...
export const EditorCompletionWidget: React.FunctionComponent<EditorCompletionWidgetProps> = ({
  extensionsController: {
    services: {
      ...
      model: modelService,
      completionItems: completionItemsService
    }
  },
  viewerId,
  textArea,
  ...
  useEffect(() => {
    const subscription = observeEditorAndModel(
      ...
      .pipe(
        ...
        distinctUntilChanged(
          (a, b) =>
            isEqual(a.selections, b.selections) && a.model.text === b.model.text
        ),
        switchMap(editor => {
          ...
          const result = completionItemsService
            ...
            .pipe(share());
          return merge(
            of(LOADING).pipe(delay(2000), takeUntil(result)),
            result
          );
        }),
  ...
  const onSelectItem = useCallback(
    async (item: CompletionItem): Promise<void> => {
      const editor = await observeEditorAndModel(
        { viewerId },
        viewerService,
        modelService
      )
      ...
      const word = getWordAtText(
        positionToOffset(editor.model.text, sel.active),
        editor.model.text
      );
      if (word) {
        replaceRange = {
      ...
      const beforeText = editor.model.text.slice(
        0,
        positionToOffset(editor.model.text, replaceRange.start)
      );
      const afterText = editor.model.text.slice(
        positionToOffset(editor.model.text, replaceRange.end)
      );
      const itemText =
        item.insertText !== undefined ? item.insertText : item.label;
      modelService.updateModel(
        editor.resource,
        beforeText + itemText + afterText
      );
