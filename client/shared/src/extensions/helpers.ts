...
/**
 * @returns An observable that emits the list of extensions configured in the
 *     viewer's final settings upon
...
export function viewerConfiguredExtensions({
  ...
  return from(settings).pipe(
      ...
      switchMap(extensionIDs => queryConfiguredRegistryExtensions(
                    {requestGraphQL}, extensionIDs)),
      catchError(error => throwError(asError(error))), publishReplay(),
      refCount())
}

/**
 * Query the GraphQL API for registry metadata about the extensions given
 * in {@link extensionIDs}.
 *
 * @returns An observable that emits once with the results.
 */
export function queryConfiguredRegistryExtensions(
    {requestGraphQL}: Pick<PlatformContext, 'requestGraphQL'>,
    extensionIDs: string[]): Observable<ConfiguredRegistryExtension[]> {
