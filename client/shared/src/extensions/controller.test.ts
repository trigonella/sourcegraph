...
describe(
    ...
    () => {test(
        'registers a contributions observable that emits contributions from all extensions',
        async () => {
          const registerContributions = sinon.spy(
              (entry: ContributionsEntry): ContributionUnsubscribable =>
                  ({entry, unsubscribe : noop}))
          const fakeRegistry:
              Pick<ContributionRegistry, 'registerContributions'> = {
          ...
          registerExtensionContributions(fakeRegistry, {
            activeExtensions :
                from([ take(extensions, 1), take(extensions, 2), extensions ]),
          })
          sinon.assert.calledOnce(registerContributions)
          const extensionContributions =
              registerContributions.getCalls()[0].args[0].contributions
          if (!isObservable<Contributions[]>(extensionContributions)) {
            throw new Error('Expected extensionContributions to be Observable')
          }
          expect(await extensionContributions.toPromise())
              .toEqual(extensions.map(
                  ({manifest}) =>
                      (manifest as NonNullable<ExtensionManifest>).contributes))
        })})
