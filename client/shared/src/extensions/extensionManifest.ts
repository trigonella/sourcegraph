...
import {
  ExtensionManifest as ExtensionManifestSchema
} from '../schema/extensionSchema'
import {ErrorLike, isErrorLike} from '../util/errors'
import {parseJSONCOrError} from '../util/jsonc'

/**
 * Represents an input object that is validated against a subset of properties
 * of the {@link ExtensionManifest} JSON Schema. For simplicity, only necessary
 * fields are validated and included here.
 */
export type ExtensionManifest =
    Pick<ExtensionManifestSchema,
         |'description'|'repository'|'categories'|'tags'|'readme'|'url'|'icon'|
         'activationEvents'|'contributes'|'publisher'>

    /**
     * Parses and validates the extension manifest. If parsing or validation
     * fails, an error value is returned (not thrown).
     *
     * @todo Contribution ("contributes" property) validation is incomplete.
     */
    export function parseExtensionManifestOrError(
        input: string): ExtensionManifest|ErrorLike {
      ...
      if (!isErrorLike(value)) {
        ...
        } else if (!value.activationEvents.every(event => typeof event ===
                                                          'string')) {
          problems.push(
              '"activationEvents" property must be an array of strings')
        }
