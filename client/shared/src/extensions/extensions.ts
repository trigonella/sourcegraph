 ...
 */
export function onlyDefaultExtensionsAdded(
  settings: SettingsCascadeOrError
): boolean {
  if (!isErrorLike(settings.subjects) && settings.subjects) {
    const userSettings = settings.subjects.find(
      subject => subject.subject.__typename === "User"
    );
    ...
    if (userSettings && defaultSettings) {
      const userExtensions =
        !isErrorLike(userSettings.settings) &&
        userSettings.settings?.extensions;
      const defaultExtensions =
        !isErrorLike(defaultSettings.settings) &&
