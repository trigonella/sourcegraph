...
export const isURL = (value: unknown): value is URL =>
    typeof value === 'object' && value !== null && hasProperty('href')(value) &&
    hasProperty('toString')(value) && typeof value.toString === 'function' &&
...
export const syncSubscription = (subscriptionPromise: Promise<Remote<Unsubscribable & ProxyMarked>>): Subscription =>
    ...
    new Subscription(async () => {
        ...
        /**
         * Runs f and returns a resolved promise with its value or a rejected
         * promise with its exception, regardless of whether it returns a
         * promise or not.
         */
        export const tryCatchPromise =
            async<T>(function_: () => T | Promise<T>): Promise<T> => function_()
        ...
        export const isPromiseLike =
            (value: unknown): value is PromiseLike<unknown> =>
                ...
                hasProperty('then')(value) && typeof value.then === 'function'

        /**
         * Reports whether value is a {@link sourcegraph.Subscribable}.
         */
        export const isSubscribable =
            (value: unknown): value is Subscribable<unknown> =>
                typeof value === 'object' && value !== null &&
                hasProperty('subscribe')(value) &&
                typeof value.subscribe === 'function'

        /**
         * Promisifies method calls and objects if specified, throws otherwise
         * if there is no stub provided NOTE: it does not handle ProxyMethods
         * and callbacks yet NOTE2: for testing purposes only!!
         */
        export const pretendRemote = <T>(object: Partial<T>): Remote<T> =>
            ...
            (new Proxy(object, {
               get : (a, property) => {
                 if (property === 'then') {
                   // Promise.resolve(pretendRemote(..)) checks if this is a
                   // Promise we will let it know that no, this is not a Promise
                   return undefined
                 }
                 if (property in a) {
                   ...
                   return (...args: any[]) =>
                              Promise.resolve((a as any)[property](...args))
                 }
                 throw new Error(`unspecified property in the stub: "${
                     property.toString()}"`)
               },
