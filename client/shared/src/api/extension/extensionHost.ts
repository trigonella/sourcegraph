...
/**
 * Starts the extension host, which runs extensions. It is a Web Worker or other
 * similar isolated JavaScript execution context. There is exactly 1 extension
 * host, and it has zero or more extensions activated (and running).
 *
 * It expects to receive a message containing {@link InitData} from the client
 * application as the first message.
 *
 * @param endpoints The endpoints to the client.
 * @returns An unsubscribable to terminate the extension host.
 */
export function startExtensionHost(endpoints: EndpointPair): Unsubscribable&
    {extensionAPI : Promise<typeof sourcegraph>} {
  const subscription = new Subscription()

  // Wait for "initialize" message from client application before proceeding to
  // create the extension host.
  let initialized = false
  const extensionAPI = new Promise<typeof sourcegraph>(resolve => {
    const factory : ExtensionHostAPIFactory =
              initData => {
                if (initialized) {
...
/**
 * Initializes the extension host using the {@link InitData} from the client
 * application. It is called by {@link startExtensionHost} after the {@link
 * InitData} is received.
 *
 * The extension API is made globally available to all requires/imports of the
 * "sourcegraph" module by other scripts running in the same JavaScript context.
 *
 * @param endpoints The endpoints to the client.
 * @param initData The information to initialize this extension host.
 * @returns An unsubscribable to terminate the extension host.
 */
function initializeExtensionHost(endpoints: EndpointPair, initData: InitData): {
  extensionHostAPI: ExtensionHostAPI; extensionAPI : typeof sourcegraph;
  subscription : Subscription
} {
  const subscription = new Subscription()

  const {extensionAPI, extensionHostAPI, subscription : apiSubscription} =
      createExtensionAPI(initData, endpoints)
  subscription.add(apiSubscription)

  // Make `import 'sourcegraph'` or `require('sourcegraph')` return the
  // extension API.
  globalThis.require =
      ((modulePath: string): any => {
        ...
        // All other requires/imports in the extension's code should not reach
        // here because their JS bundler should have resolved them locally.
  ...
  subscription.add(
      () => {
          globalThis.require =
              (() => {
                   // Prevent callers from attempting to access the extension
                   // API after it was
                   // unsubscribed.
                   throw new Error(
                       'require: Sourcegraph extension API was unsubscribed')}) as
              any})

  return { subscription, extensionAPI, extensionHostAPI }
}

function createExtensionAPI(initData: InitData,
  ...
  // For debugging/tests.
  const sync = async(): Promise<void> => { await proxy.ping() } const context =
      new ExtensionContext(proxy.context)
  ...
  const languageFeatures =
      new ExtensionLanguageFeatures(proxy.languageFeatures, documents)
  ...
  // "redefines" everything instead of exposing internal Ext* classes directly
  // so as to:
  // - Avoid exposing private methods to extensions
  // - Avoid exposing proxy.* to extensions, which gives access to the main
  // thread
  const extensionAPI: typeof sourcegraph&
      {// Backcompat definitions that were removed from sourcegraph.d.ts but are
       // still defined (as
       // noops with a log message), to avoid completely breaking extensions
       // that use them.
       languages : {registerTypeDefinitionProvider : any
    ...
    return {
              extensionHostAPI, extensionAPI, subscription
            }
}
