...
const isInitMessage = (value: unknown): value is InitMessage =>
    typeof value === 'object' && value !== null && hasProperty('endpoints')(value) && isEndpointPair(value.endpoints)
        ...
        /**
         * The entrypoint for the JavaScript context that runs the extension
         * host (and all extensions).
         *
         * To initialize the extension host, the parent sends it an {@link
         * InitMessage}
         */
        async function extensionHostMain(): Promise<void> {
          try {
            ...
            if (!isInitMessage(event.data)) {
              throw new Error(
                  'First message event in extension host worker was not a well-formed InitMessage')
            }
