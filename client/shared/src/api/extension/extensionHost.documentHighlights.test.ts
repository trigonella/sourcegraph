...
describe("mergeDocumentHighlightResults", () => {
  ...
  it("merges a DocumentHighlight into result", () => {
    ...
    const highlight3: DocumentHighlight = {
      range: range3
    };
    const merged: DocumentHighlight[] = [highlight1, highlight2, highlight3];
    expect(
      mergeDocumentHighlightResults([[highlight1], [highlight2, highlight3]])
    ).toEqual(merged);
  });
  ...
  it("omits non DocumentHighlight values from document highlight result", () => {
    ...
    expect(
      mergeDocumentHighlightResults([[highlight], null, LOADING, undefined])
    ).toEqual(merged);
  });
});
...
describe("getDocumentHighlights from ExtensionHost API, it aims to have more e2e feel", () => {
  ...
  it("restarts document highlights call if a provider was added or removed", () => {
    ...
    const { exposedToMain, languages } = initNewExtensionAPI(
      noopMain,
      emptySettings,
      documents
    );
    ...
    const subscription = languages.registerDocumentHighlightProvider(
      [{ pattern: "*.ts" }],
      {
    ...
    expect(results).toEqual([
      [],
      [2, 0].map(value => documentHighlight(value))
    ]);
    results = [];
