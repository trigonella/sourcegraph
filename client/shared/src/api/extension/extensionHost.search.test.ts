...
const observe =
    (onValue: (value: string) => void): Remote<Observer<string>&ProxyMarked> =>
        pretendRemote({
          next : onValue,
...
exposedToMain.transformSearchQuery('a').subscribe(
    ...
    search.registerQueryTransformer(
        {transformQuery : query => Promise.resolve(query + '!')})
    const result = await new Promise(
        resolve =>
            exposedToMain.transformSearchQuery('a').subscribe(observe(resolve)))
  ...
  const {exposedToMain, search} =
      initNewExtensionAPI(noopMain, emptySettings, noopDocuments)
  ...
  search.registerQueryTransformer({
    transformQuery : () => {
      const promise = new Promise<string>(resolve => {resolves.push(resolve)})
  ...
  search.registerQueryTransformer({
    transformQuery : query => { secondTransformerCallCount++ return query },
  })
