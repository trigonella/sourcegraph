...
export class ExtensionDocument implements sourcegraph.TextDocument {
  private _eol: string private _lines: string[] public uri:
      string public languageId: string public text: string|undefined

  constructor(private model:
                  Pick<sourcegraph.TextDocument, 'uri'|'languageId'|'text'>) {
    this._eol = getEOL(model.text || '') this._lines =
        model.text !== undefined
            ? model.text.split(this._eol)
            : [] this.uri = this.model.uri this.languageId =
                  this.model.languageId this.text = this.model.text
  }
  ...
  public update({text}: Pick<sourcegraph.TextDocument, 'text'>): void {
    this.model = {
      ...this.model,
      text,
    } this._eol = getEOL(text || '') this._lines =
        text !== undefined ? text.split(this._eol) : [] this.text = text
  }

  public offsetAt(position: sourcegraph.Position): number {
    this.throwIfNoModelText()
    position = this.validatePosition(position)
    return this.lineStarts.getAccumulatedValue(position.line - 1) +
           position.character
  }
  ...
  public positionAt(offset: number): sourcegraph.Position {
    ...
    const character =
        Math.min(out.remainder, lineLength) // ensure we return a valid position
    return new Position(out.index, character)
  }

  public validatePosition(position: sourcegraph.Position):
  ...
  public getWordRangeAtPosition(position: sourcegraph.Position):
      sourcegraph.Range|undefined {
    this.throwIfNoModelText()
    position = this.validatePosition(position)
    const wordAtText =
        getWordAtText(position.character, this._lines[position.line])
    if (wordAtText) {
      return new Range(position.line, wordAtText.startColumn, position.line,
  ...
  private _lineStarts: PrefixSumComputer|null =
      null private get lineStarts(): PrefixSumComputer {
    if (!this._lineStarts) {
  ...
  public toJSON(): any { return this.model }
}
