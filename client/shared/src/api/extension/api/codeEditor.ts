...
/**
 * Returns true if all of the objects properties are empty null, undefined,
 * empty strings or objects that are also empty.
 */
const isEmptyObjectDeep = (value: any): boolean =>
    Array.isArray(value) ? value.every(isEmptyObjectDeep)
                         : typeof value === 'object' && value !== null
                               ? Object.values(value).every(isEmptyObjectDeep)
                               : !value

const isDecorationEmpty =
    ({range, isWholeLine, ...contents}: clientType.TextDocumentDecoration):
        boolean => isEmptyObjectDeep(contents)
...
export class ExtensionCodeEditor implements sourcegraph.CodeEditor {
  ...
  constructor(data: CodeEditorData&ViewerId,
              private proxy: Remote<ClientCodeEditorAPI>,
              private documents: ExtensionDocuments) {
    this.resource = data.resource this.update(data)
  }
  ...
  public get selection(): sourcegraph.Selection|null {
    return this.selectionsChanges.value.length > 0
               ? this.selectionsChanges.value[0]
               : null
  }
  ...
  public setDecorations(decorationType: sourcegraph.TextDocumentDecorationType|
                        null,
    ...
    decorationType =
        ...
        this.proxy.$setDecorations(
            this.resource, decorationType.key,
            decorations.map(fromTextDocumentDecoration)
                .filter(decoration => !isDecorationEmpty(decoration)))
  }

  public update(data: Pick<CodeEditorData, 'selections'>): void {
    this.selectionsChanges.next(
        data.selections.map(selection => Selection.fromPlain(selection)))
  }
...
function fromTextDocumentDecoration(
    ...
    clientType.TextDocumentDecoration {
  return { ...decoration, range: (decoration.range as Range).toJSON(), }
}
