...
export class ExtensionContent {
  ...
  public registerLinkPreviewProvider(urlMatchPattern: string,
    ...
    const providerFunction: comlink.Local<
        ...
        comlink.proxy(
            (url: string) => toProxyableSubscribable(
                provider.provideLinkPreview(new URL(url)), preview => preview))
    return syncSubscription(this.proxy.$registerLinkPreviewProvider(
        urlMatchPattern, providerFunction))
  }
}
