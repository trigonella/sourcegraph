...
class DocumentNotFoundError extends Error {
  ...
  constructor(resource: string) { super(`document not found: ${resource}`) }
}
...
export class ExtensionDocuments implements ExtensionDocumentsAPI, ProxyMarked {
  ...
  /**
   * If needed, perform a sync with the client to ensure that its pending sends
   * have been received before retrieving this document.
   *
   * @todo This is necessary because hovers can be sent before the document is
   * loaded, and it will cause a "document not found" error.
   *
   * @deprecated `getSync()` makes no additional guarantees over `get()`
   *     anymore.
   */
   ...
   * @internal
   */
  public getAll(): ExtensionDocument[] { return [...this.documents.values() ] }

  public openedTextDocuments = new Subject<TextDocument>()

      public $acceptDocumentData(modelUpdates: readonly TextModelUpdate[]):
          void {
    for (const update of modelUpdates) {
      ...
      case 'added': {
        ...
        const textDocument =
            new ExtensionDocument({uri, languageId, text}) this.documents
                .set(update.uri,
                     textDocument) this.openedTextDocuments.next(textDocument)
        break
      }
      case 'updated': {
