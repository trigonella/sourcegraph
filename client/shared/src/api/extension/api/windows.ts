...
export class ExtensionWindow implements sourcegraph.Window {
  ...
  const entries = [...this.viewComponents.entries() ] return sortBy(entries, 0)
  ...
  for (const update of data) {
    ...
    case 'added': {
      ...
      case 'CodeEditor':
        viewer = new ExtensionCodeEditor({viewerId, ...viewerData},
                                         this.proxy.codeEditor, this.documents)
        break case 'DirectoryViewer': viewer = {
          type : 'DirectoryViewer',
          // Since directories don't have any state beyond the immutable URI,
          // we can set the model to a static object for now and don't need to
          // track directory models in a Map.
          directory : {
    ...
    case 'updated': {
      ...
      if (!editor) {
        throw new Error(
            `Could not perform update: viewer ${viewerId} not found`)
      }
      if (editor.type !== 'CodeEditor') {
        throw new Error(`Could not perform update: viewer ${viewerId} is type ${
            editor.type}, not CodeEditor`)
      }
  ...
  return {
    visibleViewComponents: this.visibleViewComponents,
        activeViewComponent: this.activeViewComponent
  }
    }
}
...
export class ExtensionWindows implements ExtensionWindowsAPI, ProxyMarked {
  ...
  this.activeWindow = new ExtensionWindow(this.proxy, this.documents,
                                          []) this.activeWindowChanges =
      of(this.activeWindow)
    }
