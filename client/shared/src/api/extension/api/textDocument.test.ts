...
describe('positionAt', () => {
  for (const [index, {text, ...testCase}] of OFFSET_TO_POSITION_TESTS
           .entries()) {
    test(index.toString(),
         () => expect(textDocument(text).positionAt(testCase.offset))
                   .toMatchObject(testCase.pos))
  }
})
...
describe('offsetAt', () => {
  for (const [index, {text, ...testCase}] of POSITION_TO_OFFSET_TESTS
           .entries()) {
    test(index.toString(),
         () => expect(textDocument(text).offsetAt(new Position(
                          testCase.pos.line, testCase.pos.character)))
