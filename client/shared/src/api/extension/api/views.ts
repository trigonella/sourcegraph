...
class ExtensionPanelView implements sourcegraph.PanelView {
  ...
  public set title(value: string) { this.data.title = value this.sendData() }

  public get content(): string { return this.data.content }
  public set content(value: string) {
    this.data.content = value this.sendData()
  }

  public get priority(): number { return this.data.priority }
  public set priority(value: number) {
    this.data.priority = value this.sendData()
  }
  ...
  public set component(value: sourcegraph.PanelView['component']) {
    this.data.component = value this.sendData()
  }
...
export class ExtensionViewsApi implements comlink.ProxyMarked {
  ...
  public registerViewProvider(id: string, provider: sourcegraph.ViewProvider):
      Unsubscribable {
    switch (provider.where) {
    case ContributableViewContainer.Directory: {
      return syncSubscription(this.proxy.$registerDirectoryViewProvider(
          id,
          comlink.proxy(
              (context:
                   ViewContexts[typeof ContributableViewContainer.Directory]) =>
                  toProxyableSubscribable(provider.provideView({
                    viewer : {
                      ...context.viewer,
                      directory : {
    ...
    case ContributableViewContainer.Homepage: {
      return syncSubscription(this.proxy.$registerHomepageViewProvider(
          id,
          comlink.proxy(
              (context:
                   ViewContexts[typeof ContributableViewContainer.Homepage]) =>
                  toProxyableSubscribable(provider.provideView(context),
                                          result => result || null))))
    }
    case ContributableViewContainer.InsightsPage: {
      return syncSubscription(this.proxy.$registerInsightsPageViewProvider(
          id, comlink.proxy(
                  (context: ViewContexts[typeof ContributableViewContainer
                                             .InsightsPage]) =>
                      toProxyableSubscribable(provider.provideView(context),
                                              result => result || null))))
    }
    case ContributableViewContainer.GlobalPage: {
      return syncSubscription(this.proxy.$registerGlobalPageViewProvider(
          id, comlink.proxy(
                  (context: ViewContexts[typeof ContributableViewContainer
                                             .GlobalPage]) =>
                      toProxyableSubscribable(provider.provideView(context),
