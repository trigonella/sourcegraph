...
const unsubscribe =
    extensionAPI.commands
        .registerCommand('c', () => 'a')
            await expect(extensionAPI.commands.executeCommand('c'))
        .resolves
        .toBe('a')
            await expect(services.commands.executeCommand({command : 'c'}))
        .resolves.toBe('a')

// Unregister the command and ensure it's removed.
unsubscribe.unsubscribe() await extensionAPI.internal
    .sync() await expect(extensionAPI.commands.executeCommand('c'))
