...
panelView.title = 't'
panelView.content = 'c'
panelView.priority = 3 await extensionAPI.internal.sync()

const values =
    await services.panelViews.getPanelViews(ContributableViewContainer.Panel)
        ...
        extensionAPI.languages.registerLocationProvider(
    ...
    extensionAPI.app.registerViewProvider('v', {
      ...
      provideView : parameters =>
          of({title : `t${parameters.x}`, content : []}),
    })

    const view = await services.view.get('v', {x : 'y'})
