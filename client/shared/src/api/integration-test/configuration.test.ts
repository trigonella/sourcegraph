...
expect(() => extensionAPI.configuration.subscribe(() => undefined))
    .not.toThrow()
  ...
  calls.push(edit)
    ...
    calls.length = 0 // clear
                   ...
                   await extensionAPI.configuration.get()
                       .update('a', 3) await extensionAPI.internal.sync()
    ...
    const {extensionAPI} =
        await integrationTestContext({settings : mockSettings})

    let calls = 0
    extensionAPI.configuration.subscribe(() => calls++)
    expect(calls).toBe(1) // called initially

    mockSettings.next(EMPTY_SETTINGS_CASCADE) await extensionAPI.internal.sync()
            expect(calls).toBe(2)
        })
    })
})
