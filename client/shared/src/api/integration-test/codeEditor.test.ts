...
const editor =
    viewerService.viewers.get('viewer#0')!
    viewerService.setSelections(editor, [ new Selection(1, 2, 3, 4) ])
viewerService.setSelections(editor, [])

const values =
    await from(extensionAPI.app.windows[0].activeViewComponentChanges)
        .pipe(switchMap(viewer => (viewer && viewer.type === 'CodeEditor'
                                       ? viewer.selectionsChanges
                                       : [])),
              distinctUntilChanged(), take(3), toArray())
        .toPromise()
            assertToJSON(
    ...
    expect(await services.textDocumentDecoration
    ...
    editor.setDecorations(decorationType, []) await extensionAPI.internal.sync()
            expect(
        ...
        const [dt1, dt2] =
            [
              extensionAPI.app.createDecorationType(),
              extensionAPI.app.createDecorationType()
            ]
        ...
        // Change decorations only for dt1, and check that merged decorations
        // are coherent
        editor
            .setDecorations(dt1, [
              {
        ...
        // remove decorations for dt2, and verify that decorations for dt1 are
        // still present
        editor.setDecorations(dt2, []) await extensionAPI.internal.sync()
            expect(
                await services.textDocumentDecoration.getDecorations({ uri: 'file:///f' }).pipe(take(1)).toPromise()
            ).toEqual([
                {
        ...
        // This call to setDecorations does not supply a type, mimicking
        // extensions that may have been developed against an older version of
        // the API
        editor
            .setDecorations(null as any,
                            [
                              {
                                range : new Range(1, 2, 3, 4),
                                after : {
            ...
            async function getFirstCodeEditor(extensionAPI: typeof sourcegraph):
                Promise<sourcegraph.CodeEditor> {
