...
import {BehaviorSubject, from, of, Subscription, throwError} from 'rxjs'
...
export function assertToJSON(a: any,
                             expected: any): void{const raw = JSON.stringify(a)
...
interface TestInitData {
    ...
    interface Mocks extends Pick<PlatformContext,|'settings'|'updateSettings'|
                                 'requestGraphQL'|'getScriptURLForExtension'|
                                 'clientApplication'|'sideloadedExtensionURL'> {
    }

    const NOOP_MOCKS: Mocks = {
      settings : of({final : {}, subjects : []}),
      updateSettings : () =>
          Promise.reject(new Error('Mocks#updateSettings not implemented')),
      requestGraphQL : () =>
          throwError(new Error('Mocks#queryGraphQL not implemented')),
    ...
    integrationTestContext(partialMocks: Partial<Mocks> = NOOP_MOCKS,
      ...
      const {api} =
          await createExtensionHostClientConnection(Promise.resolve({
            endpoints : clientEndpoints,
            subscription : new Subscription(),
          }),
                                                    services, initData, mocks)
      ...
      return { extensionAPI, services, extensionHost: api, }
    }

    /**
     * Returns a {@link Promise} and a function. The {@link Promise} blocks
     * until the returned function is called.
     *
     * @internal
     */
    export function createBarrier(): {wait: Promise<void>; done : () => void} {
      let done!: () => void const wait =
          new Promise<void>(resolve => (done = resolve))
      return { wait, done }
    }

    export function collectSubscribableValues<T>(
        subscribable: sourcegraph.Subscribable<T>): T[] {
      const values: T[] = []
