...
export class ClientExtensions {
   ...
   * @param extensions An observable that emits the set of extensions that
   *     should be activated
   * upon subscription and whenever it changes.
   */
  constructor(private proxy: Remote<ExtensionExtensionsAPI>,
    ...
    this.subscriptions.add(
        from(extensionRegistry.activeExtensions)
            ...
            .subscribe(([ oldExtensions, newExtensions ]) => {
               ...
               * {@link activeExtensions} never deactivates extensions, so this
               * will never be called (in the current implementation).
               */
              for (const extension of toDeactivate) {
                this.proxy.$deactivateExtension(extension.id)
                    .catch(error => {
                               console.warn(`Error deactivating extension ${
                                                JSON.stringify(extension.id)}:`,
                                            error)})
              }
              ...
              for (const extension of toActivate) {
                console
                    .log('Activating Sourcegraph extension:',
                         extension.id) this.proxy
  ...
  public unsubscribe(): void { this.subscriptions.unsubscribe() }
}
