 ...
 * Unsubscribing will send a RELEASE message over the MessagePort, then close it
 * and remove all event listeners from it.
 */
export class ProxySubscription extends Subscription {
  constructor(proxy: Pick<ProxyMethods, typeof releaseProxy>) {
    super(() => {proxy[releaseProxy]()})
  }
}

/**
 * An object that is backed by a comlink Proxy and exposes its Subscription so
...
/**
 * An ordinary Observable linked to an Observable in another thread through a
 * comlink Proxy.
 */
export interface RemoteObservable<T> extends Observable<T>, ProxySubscribed {}

/**
 * When a Subscribable is returned from the other thread (wrapped with
 * `proxySubscribable()`), this thread gets a `Promise` for a `Subscribable`
 * _proxy_ where `subscribe()` returns a `Promise<Unsubscribable>`. This
 * function wraps that proxy in a real Rx Observable where `subscribe()` returns
 * a `Subscription` directly as expected.
 *
 * The returned Observable is augmented with the `releaseProxy` method from
 * comlink to release the underlying `MessagePort`.
 *
 * @param proxyPromise The proxy to the `ProxyObservable` in the other thread
 * @param addToSubscription If provided, directly adds the `ProxySubscription`
 *     to this Subscription.
 */
export const wrapRemoteObservable =
    <T>(proxyPromise: Promise<Remote<ProxySubscribable<T>>>,
    ...
    [symbolObservable](): Subscribable<T>{return this},
      ...
      let proxyObserver: Parameters<typeof proxySubscribable['subscribe']>[
        0
        ...
        proxyObserver = {
          [proxyMarker] : true,
          next : partialObserver.next ? value => partialObserver.next(value)
                                      : noop,
          error : partialObserver.error
                      ? error => partialObserver.error(asError(error))
                      : noop,
          complete : partialObserver.complete ? () => partialObserver.complete()
                                              : noop,
        }
      }
...
/**
 * Releases the underlying MessagePort of a remote Observable when it completes
 * or is unsubscribed from.
 *
 * Important: This will prevent resubscribing to the Observable. Only use this
 * operator in a scope where it is known that no resubscriptions can happen
 * after completion, e.g. in a `switchMap()` callback.
 *
 * Must be used as the first parameter to `pipe()`, because the source must be a
 * `RemoteObservable`.
 */
// needed for the type parameter
// eslint-disable-next-line unicorn/consistent-function-scoping
export const finallyReleaseProxy = <T>() => (source: Observable<T>&
                                             Partial<ProxySubscribed>):
    Observable<T> => {
...
/**
 * Helper function to register a remote provider returning an Observable,
 * proxied by comlink, in a provider registry.
 *
 * @param registry The registry to register the provider on.
 * @param registrationOptions The registration options to pass to
 *     `registerProvider()`
 * @param remoteProviderFunction The provider function in a remote thread,
 *     proxied by `comlink`.
 *
 * @returns A Subscription that can be proxied through comlink which will
 *     unregister the provider.
 */
export function
registerRemoteProvider<TRegistrationOptions, TLocalProviderParameters extends
                           UnproxyOrClone<TProviderParameters>,
                           TProviderParameters, TProviderResult>(
    registry: FeatureProviderRegistry<TRegistrationOptions,
                                      (parameters: TLocalProviderParameters) =>
                                          Observable<TProviderResult>>,
    registrationOptions: TRegistrationOptions,
    remoteProviderFunction:
        Remote<((parameters: TProviderParameters) =>
                    ProxySubscribable<TProviderResult>)&ProxyMarked>):
    Unsubscribable&ProxyMarked {
  // This subscription will unregister the provider when unsubscribed.
  const subscription = new Subscription()

  subscription.add(registry.registerProvider(
      registrationOptions,
      parameters =>
