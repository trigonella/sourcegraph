...
export interface ClientContentAPI extends ProxyMarked {
  $registerLinkPreviewProvider(
      ...
      provider: Remote<
          ((url: string) => ProxySubscribable<LinkPreview|null|undefined>)&
          ProxyMarked>): Unsubscribable&ProxyMarked
}

/** @internal */
export function createClientContent(registry: LinkPreviewProviderRegistry):
    ClientContentAPI&ProxyMarked {
  return {
    [proxyMarker]: true,
        $registerLinkPreviewProvider: (urlMatchPattern, providerFunction) =>
            registerRemoteProvider(registry, {urlMatchPattern},
                                   providerFunction),
  }
}
