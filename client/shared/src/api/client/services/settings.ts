 ...
 *
 * Each successive array element specifies an index in an object or array to
 * descend into. For example, in the object `{"a": ["x", "y"]}`, the key path
 * `["a", 1]` refers to the value `"y"`.
 */
...
export async function updateSettings(
    ...
    edit: SettingsEdit): Promise<void> {
  ...
  const subject = settings.subjects[settings.subjects.length - 1] await update(
      subject.subject.id, edit)
}
