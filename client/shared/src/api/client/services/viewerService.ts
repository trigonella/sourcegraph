...
import {ModelService, PartialModel, TextModel} from './modelService'
...
/**
 * Describes a directory viewer that has been added to the {@link
 * ViewerService}.
 */
export interface DirectoryViewer extends ViewerId, DirectoryViewerData {}

export type ViewerWithPartialModel =
    CodeEditorWithPartialModel|DirectoryViewer // Directories don't have a model

/**
 * A code editor with a partial model.
 *
 * To get the editor's full model, look up the model in the {@link
 * ModelService}.
 */
...
}&ViewerId)|({type : 'deleted'}&ViewerId)
...
export interface ViewerService {
   ...
   * {@link ViewerService#viewerUpdates} or {@link
   * ViewerService#activeViewerUpdates}
   */
  readonly viewers: ReadonlyMap<string, Viewer>
      ...
      readonly viewerUpdates: Subscribable<ViewerUpdate[]>
          ...
          readonly activeViewerUpdates: Subscribable<Viewer|undefined>
               ...
               * @returns The added code viewer (which must be passed as the
               *     first argument to other
  ...
  observeViewer(viewer: ViewerId): Observable<ViewerData>
      ...
      /**
       * Removes a viewer.
       * Also removes the corresponding model if no other viewer is referencing
       * it.
       *
...
class ViewerNotFoundError extends Error {
  ...
  constructor(viewerId: string) { super(`Viewer not found: ${viewerId}`) }
}
...
export function createViewerService(
    modelService: Pick<ModelService, 'removeModel'>): ViewerService {
  // Don't use lodash.uniqueId because that makes it harder to hard-code
  // expected ID values in test code (because the IDs change depending on test
  // execution order).
  let id = 0
   ...
   */
  const getViewer = (viewerId: ViewerId['viewerId']):
      Viewer => {
  ...
  return {
    viewers, viewerUpdates, activeViewerUpdates, addViewer: viewerData => {
      ...
      try {
        ...
        return viewerUpdates.pipe(
            filter(updates =>
                       updates.some(update => update.viewerId === viewerId)),
            takeWhile(updates => updates.every(
                          update => update.viewerId !== viewerId ||
                                    update.type !== 'deleted')),
            map(() => getViewer(viewerId)), startWith(viewer))
      } catch (error) {
      ...
      if (viewer.type !== 'CodeEditor') {
        throw new Error(`Editor ID ${viewerId} is type ${
            String(viewer.type)}, expected CodeEditor`)
      }
      viewers.set(viewerId, {...viewer, selections})
      viewerUpdates.next(
          [ {type : 'updated', viewerId, viewerData : {selections}} ])
    }
    , removeViewer({viewerId}: ViewerId): void {
      ...
      if (activeViewerUpdates.value &&
          activeViewerUpdates.value.viewerId === viewerId) {
        activeViewerUpdates.next(undefined)
      }
      if (viewer.type === 'CodeEditor' &&
          modelReferences.decrement(viewer.resource)) {
        modelService.removeModel(viewer.resource)
      }
    }
    , removeAllViewers(): void{const updates : ViewerUpdate[] = [
      ...viewers.keys()
    ].map(viewerId => ({type : 'deleted', viewerId}))
    viewers.clear()
...
/**
 * Helper function to get the active viewer's {@link TextDocumentPositionParams}
 * from
 * {@link ViewerService#viewers}. If there is no active viewer or it has no
 * position, it returns null.
 */
export function getActiveCodeEditorPosition(activeViewer: Viewer|undefined):
    TextDocumentPositionParameters|null {
  if (!activeViewer || activeViewer.type !== 'CodeEditor') {
    return null
  }
  const sel = activeViewer.selections[0] if (!sel) { return null }
  // TODO(sqs): Return null for empty selections (but currently all selected
  // tokens are treated as an empty selection at the beginning of the token, so
  // this would break a lot of things, so we only do this for empty selections
  // when the start character is -1). HACK(sqs): Character === -1 means that the
  // whole line is selected (this is a bug in the caller, but it is useful
  // here).
  const isEmpty = sel.start.line === sel.end.line &&
                  sel.start.character === sel.end.character &&
                  sel.start.character === -1
  if (isEmpty) {
    return null
  }
  return { textDocument: {uri: activeViewer.resource}, position: sel.start, }
}
...
export function observeEditorAndModel(
    {viewerId}: ViewerId, {observeViewer}: Pick<ViewerService, 'observeViewer'>,
    {observeModel}: Pick<ModelService, 'observeModel'>):
    Observable<CodeEditorWithModel> {
  return observeViewer({viewerId})
      .pipe(map(viewer => {
              if (viewer.type !== 'CodeEditor') {
                throw new Error(`Editor ID ${viewerId} is type ${
                    String(viewer.type)}, expected CodeEditor`)
            ...
            switchMap(
                viewer =>
                    observeModel(viewer.resource)
                        .pipe(map(model => ({viewerId, ...viewer, model})))))
}
