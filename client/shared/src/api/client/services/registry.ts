...
export abstract class FeatureProviderRegistry<O, P> {
  ...
  /**
   * Bulk-register providers. This is useful when registering an initial set of
   * providers. Registering them all at once means there is only one emission
...
/**
 * A registry for providers that provide features within a document. Use this
 * class instead of
 * {@link FeatureProviderRegistry} when all calls to the provider are scoped to
 * a document.
 *
 * For example, hovers are scoped to a document (i.e., the document URI is one
 * of the required arguments passed to the hover provider), so this class is
