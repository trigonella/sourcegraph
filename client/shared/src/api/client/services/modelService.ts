 ...
 *
 * How does this relate to editors (in {@link ViewerService}? A model is the
 * file, an editor is the window that the file is shown in. Things like the
 * content and language are properties of the model; things like decorations and
 * the selection ranges are properties of the editor.
 */
export interface TextModel extends
    Pick<TextDocument, 'uri'|'languageId'|'text'> {}
...
export type TextModelUpdate =|({type : 'added'}&TextModel)|({
  type: 'updated';
  text: string
}&Pick<TextModel, 'uri'>)|({type : 'deleted'}&Pick<TextModel, 'uri'>)

/**
 * The model service manages document contents and metadata.
 *
 * See {@link Model} for an explanation of the difference between a model and an
 * editor.
 */
export interface ModelService {
   ...
   * {@link ModelService#modelUpdates}, {@link ModelService#activeLanguages}
   * or {@link ModelService#observeModel} instead.
   */
  ...
  addModel(model: TextModel): void
       ...
       * @param text The new text content (which will overwrite the model's
       *     previous content).
...
export function createModelService(): ModelService {
  ...
  const getModel = (uri: string): TextModel => {
    ...
    models, modelUpdates, activeLanguages, getPartialModel: uri => {
      ...
      try {
        ...
        return modelUpdates.pipe(
            ...
            takeWhile(updates =>
                          updates.every(update => update.uri !== uri ||
                                                  update.type !== 'deleted')),
            map(() => getModel(uri)), startWith(model))
      } catch (error) {
      ...
      // Update activeLanguages if no other existing model has the same
      // language.
    ...
    }, hasModel: uri => models.has(uri), removeModel: uri => {
      const model = getModel(uri)
      models.delete(uri)
