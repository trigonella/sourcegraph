...
export interface ExecutableExtension
  extends Pick<ConfiguredExtension, "id" | "manifest"> {
...
export interface PartialContext
  extends Pick<
    PlatformContext,
    "requestGraphQL" | "getScriptURLForExtension" | "settings"
  > {
...
export class ExtensionsService implements IExtensionsService {
  ...
  protected configuredExtensions: Subscribable<
    ConfiguredExtension[]
    ...
    return combineLatest([
      ...
      map(([settings, configuredExtensions, sideloadedExtension]) => {
        ...
        if (sideloadedExtension) {
          if (
            !isErrorLike(sideloadedExtension.manifest) &&
            ...
            enabled = enabled.filter(
              extension => extension.id !== constructedID
            );
          }
  ...
  public get activeExtensions(): Subscribable<ExecutableExtension[]> {
    ...
    return combineLatest([
      from(this.modelService.activeLanguages),
      ...
      map(([, extensions]) =>
        extensions
          ? extensions.filter(extension =>
              activatedExtensionIDs.has(extension.id)
            )
          : []
      ),
      distinctUntilChanged((a, b) =>
        isEqual(
          new Set(a.map(extension => extension.id)),
  ...
  private memoizedGetScriptURLForExtension = memoizeObservable<
    string,
    string | null
  >(
    url =>
  ...
  return enabledExtensions.filter(extension => {
    try {
      ...
      if (!extension.manifest.activationEvents) {
        console.warn(
          `Extension ${extension.id} has no activation events, so it will never be activated.`
        );
        return false;
      }
