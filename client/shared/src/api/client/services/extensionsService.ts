 ...
 */
export interface ExecutableExtension extends
    Pick<ConfiguredExtension, 'id'|'manifest'>{
...
const getConfiguredSideloadedExtension =
    (baseUrl: string): Observable<ConfiguredExtension> =>
        fromFetch(`${baseUrl}/package.json`,
                  {selector : response => checkOk(response).json()})
            .pipe(map(
                (response:
                     SideloadedExtensionManifest): ConfiguredExtension => ({
                  id : response.name,
                  manifest : {
                    ...response,
...
export interface PartialContext extends Pick<
    PlatformContext, 'requestGraphQL'|'getScriptURLForExtension'|'settings'> {
 ...
 *
 * @internal This is an internal implementation detail and is different from the
 * product feature called the "extension registry" (where users can search for
...
export class ExtensionsService implements IExtensionsService {
  constructor(
      ...
      private fetchSideloadedExtension:
          (baseUrl: string) => Subscribable<ConfiguredExtension|null> =
              getConfiguredSideloadedExtension) {}
  ...
  protected configuredExtensions: Subscribable<ConfiguredExtension[]> = viewerConfiguredExtensions({
    ...
    return combineLatest([
        ...
        .pipe(map(([ settings, configuredExtensions, sideloadedExtension ]) => {
          let enabled = configuredExtensions.filter(
              extension => isExtensionEnabled(settings.final, extension.id))

          if (sideloadedExtension) {
            if (!isErrorLike(sideloadedExtension.manifest) &&
                sideloadedExtension.manifest?.publisher) {
              // Disable extension with the same ID while this extension is
              // sideloaded
              const constructedID =
                  `${sideloadedExtension.manifest.publisher}/${
                      sideloadedExtension.id}`
              enabled =
                  enabled.filter(extension => extension.id !== constructedID)
            }
  ...
  /**
   * Returns an observable that emits the set of extensions that should be
   * active, based on the previous and current state and each available
   * extension's activationEvents.
   *
   * An extension is activated when one or more of its activationEvents is true.
   * After an extension has been activated, it remains active for the rest of
   * the session (i.e., for as long as the browser tab remains open) as long as
   * it remains enabled. If it is disabled, it is deactivated. (I.e.,
   * "activationEvents" are retrospective/sticky.)
   *
   * @todo Consider whether extensions should be deactivated if none of their
   * activationEvents are true (or that plus a certain period of inactivity).
   */
  public get activeExtensions(): Subscribable<ExecutableExtension[]> {
    // Extensions that have been activated (including extensions with zero
    // "activationEvents" that evaluate to true currently).
    const activatedExtensionIDs = new Set<string>()
    return combineLatest([
             from(this.modelService.activeLanguages), this.enabledExtensions
           ])
        .pipe(
            tap(([ activeLanguages, enabledExtensions ]) => {
              const activeExtensions = this.extensionActivationFilter(
            ...
            map(([, extensions ]) =>
                    extensions ? extensions.filter(
                                     extension => activatedExtensionIDs.has(
                                         extension.id))
                               : []),
            distinctUntilChanged(
                (a, b) => isEqual(new Set(a.map(extension => extension.id)),
                                  new Set(b.map(extension => extension.id)))),
            switchMap(
                extensions => combineLatestOrDefault(extensions.map(
                    extension =>
                        this.memoizedGetScriptURLForExtension(
                                getScriptURLFromExtensionManifest(extension))
                            .pipe(map(scriptURL => scriptURL === null ? null : {
                              id : extension.id,
...
function asObservable(input: string|ObservableInput<string>):
    Observable<string>{return typeof input === 'string' ? of(input)
                                                        : from(input)}

function extensionsWithMatchedActivationEvent(
    enabledExtensions: ConfiguredExtension[],
    visibleTextDocumentLanguages: ReadonlySet<string>): ConfiguredExtension[] {
  const languageActivationEvents =
      new Set([...visibleTextDocumentLanguages ].map(
  ...
  return enabledExtensions.filter(extension => {
    try {
      if (!extension.manifest) {
        ...
        if (match) {
          console.warn(
              `Extension ${extension.id} has been renamed to sourcegraph/${
                  match[1]}. It's safe to remove ${
                  extension.id} from your settings.`)
        }
        else {
          console.warn(`Extension ${
              extension
                  .id} was not found. Remove it from settings to suppress this warning.`)
        }
        return false
      }
      ...
      if (!extension.manifest.activationEvents) {
        console.warn(`Extension ${
            extension
                .id} has no activation events, so it will never be activated.`)
        return false
      }
