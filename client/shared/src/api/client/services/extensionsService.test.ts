...
class TestExtensionsService extends ExtensionsService {
  constructor(mockConfiguredExtensions: ConfiguredExtension[],
              ...
              extensionActivationFilter:
                  (enabledExtensions: ConfiguredExtension[],
                   activeLanguages:
                       ReadonlySet<string>) => ConfiguredExtension[],
    ...
    super({
      requestGraphQL : () => { throw new Error('not implemented') },
      settings,
      getScriptURLForExtension : scriptURL => scriptURL,
      sideloadedExtensionURL,
    },
          modelService, extensionActivationFilter,
          fetchSideloadedExtension) this.configuredExtensions =
        of(mockConfiguredExtensions)
  }
}
  ...
  activationEvents : []
       ...
       () => scheduler().run(
           ({cold, expectObservable}) =>
               expectObservable(
                   from(new TestExtensionsService(
                            [
                            ...
                            cold<SettingsCascadeOrError>('-a-b-|', {
                              a : {
                                final : {extensions : {x : true}},
                                subjects : []
                              },
                              b : {
                                final : {extensions : {x : true, y : true}},
                                subjects : []
                              },
                            }),
                            (enabledExtensions, activeLanguages) =>
                                enabledExtensions.filter(
                                    extension =>
                                        activeLanguages.has(extension.id)),
                            cold('-a--|', {a : ''}), () => of(null))
                            .activeExtensions))
                   .toBe('-a-b-|', {
