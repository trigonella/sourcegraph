...
describe('ContributionRegistry', () => {
    ...
    const registry = new ContributionRegistry(
        createTestViewerService({}), TEST_MODEL_SERVICE,
        of(EMPTY_SETTINGS_CASCADE), of({}))
    const entry1: ContributionsEntry =
    { contributions: FIXTURE_CONTRIBUTIONS_1 } const entry2:
        ContributionsEntry = { contributions: FIXTURE_CONTRIBUTIONS_2 }

    const unregister1 =
        subscriptions.add(registry.registerContributions(entry1))
    expect(registry.entries.value).toEqual([ entry1 ])

    const unregister2 =
        subscriptions.add(registry.registerContributions(entry2))
  ...
  public getContributionsFromEntries(entries: Observable<ContributionsEntry[]>):
  ...
  a: [ {contributions : FIXTURE_CONTRIBUTIONS_1} ],
      ...
      c: [
        {contributions : FIXTURE_CONTRIBUTIONS_1},
        {contributions : FIXTURE_CONTRIBUTIONS_2}
      ],
                }
  ...
  public getContributionsFromEntries(entries: Observable<ContributionsEntry[]>):
    ...
    return super.getContributionsFromEntries(entries, undefined)
        ...
        const values: Record<Marble, Contributions|Contributions[]> = {
          b: FIXTURE_CONTRIBUTIONS_1, c: [ FIXTURE_CONTRIBUTIONS_2 ],
    ...
    return super.getContributionsFromEntries(entries, undefined)
        ...
        test('continues after error thrown during evaluation', () => {
            scheduler().run(({ cold, expectObservable }) => {
                const registry = new (class extends ContributionRegistry {
  constructor() {
    super(createTestViewerService({}), TEST_MODEL_SERVICE,
...
test('handles a single item', () => {expect(mergeContributions([
                                       FIXTURE_CONTRIBUTIONS_1
                                     ])).toEqual(FIXTURE_CONTRIBUTIONS_1)})
...
it('handles empty array of menu contributions', () => {
  ...
  } expect(filterContributions({menus : {commandPalette : []}}))
                                                  .toEqual(expected)
})
...
test('handles empty array of command contributions', () => {
  const expected : Evaluated<Contributions> = {
    actions : [],
  } expect(evaluateContributions({},
                                 {actions : []})).toEqual(expected)
})
  ...
  actions: [
    ...
    } expect(evaluateContributions(FIXTURE_CONTEXT,
    ...
    describe(
        ...
        () => {it('should not parse the `id` or `command` values', () => {
          const expected : Contributions =
                    ...
                    } expect(parseContributionExpressions({
                      actions : [ {id : '${replaceMe}', command : '${c}'} ],
                    })).toEqual(expected)
        })})
