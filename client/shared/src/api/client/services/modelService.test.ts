...
modelService.addModel({uri : 'u', text : 't', languageId : 'l'})
        ...
        it('refuses to add model with duplicate URI', () => {
            const modelService = createModelService()
        modelService.addModel({uri : 'u', text : 't', languageId : 'l'})
        expect(() => {modelService.addModel(
                   {uri : 'u', text : 't2', languageId : 'l2'})})
    ...
    modelService.addModel({uri : 'u', languageId : 'x', text : 't'})
        ...
        it('emits when a model is removed', async () => {
            const modelService = createModelService()
        const modelRemoved =
            from(modelService.modelUpdates)
                .pipe(takeWhile(
                    updates => updates.every(
                        ({uri, type}) => uri !== 'u' || type !== 'deleted')))
                .toPromise()
        modelService.addModel({uri : 'u', languageId : 'x', text : 't'})
    ...
    const values = from(modelService.activeLanguages)
                       .pipe(map(activeLanguages => [...activeLanguages]),
                             bufferCount(3), first())
                       .toPromise()
