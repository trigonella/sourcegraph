...
import { bufferCount, first, map, takeWhile } from "rxjs/operators";
...
describe("ModelService", () => {
  ...
  describe("updateModel", () => {
    test("existing model", () => {
      ...
      expect([...modelService.models.values()]).toEqual([
        { uri: "u", text: "t2", languageId: "l" }
      ]);
    });

    test("nonexistent model", () => {
      const modelService = createModelService();
      expect(() => modelService.updateModel("x", "t2")).toThrowError(
        "model does not exist with URI x"
      );
    });
  });
  ...
  describe("modelUpdates", () => {
    it("emits when a model is added", async () => {
      ...
      expect(await modelAdded).toMatchObject([
    ...
    it("emits when a model is removed", async () => {
      ...
      const modelRemoved = from(modelService.modelUpdates)
        .pipe(
          takeWhile(updates =>
            updates.every(({ uri, type }) => uri !== "u" || type !== "deleted")
          )
        )
        .toPromise();
      modelService.addModel({ uri: "u", languageId: "x", text: "t" });
