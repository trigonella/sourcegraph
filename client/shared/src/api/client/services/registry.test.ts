...
class FeatureProviderRegistry extends AbstractFeatureProviderRegistry<
  TextDocumentRegistrationOptions,
  {}
> {
  /**
  ...
  test("registers and unregisters providers", () => {
    ...
    const unregister1 = subscriptions.add(
      registry.registerProvider(
        FIXTURE.PartialEntry.registrationOptions,
        provider1
      )
    );
...
class DocumentFeatureProviderRegistry extends AbstractDocumentFeatureProviderRegistry<{
  a: number;
}> {
  constructor(
    entries?: Observable<
  ...
  describe("providersForDocument", () => {
    test("is initially empty", () =>
      scheduler().run(({ expectObservable }) =>
        expectObservable(
          new DocumentFeatureProviderRegistry().providersForDocument({
            uri: "file:///a"
          })
        ).toBe("a", {
          a: []
      ...
      scheduler().run(({ expectObservable, cold }) =>
        expectObservable(
          new DocumentFeatureProviderRegistry(
            cold<Entry<TextDocumentRegistrationOptions, { a: number }>[]>(
              "--b-c-|",
              {
                b: [
                  {
