  ...
  private entries = new BehaviorSubject<CommandEntry[]>([])
    ...
    for (const entry of this.entries.value) {
      if (entry.command === entryToRegister.command) {
        throw new Error(
            `command is already registered: ${JSON.stringify(entry.command)}`)
      }
    }
    ...
    return {
      unsubscribe: () => {
        this.entries.next(
            this.entries.value.filter(entry => entry !== entryToRegister))
      },
    }
  }
 ...
 *
 * Most callers should use CommandRegistry's executeCommand method, which uses
 * the registered commands.
 */
export function executeCommand(commands: CommandEntry[],
