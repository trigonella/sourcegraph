...
expect(score([ {pattern : '**/*.txt'} ], new URL('file:///f.txt'), 'l'))
    .toBe(10)
expect(score([ {pattern : '*.txt'} ], new URL('file:///f.txt'), 'l')).toBe(10)
expect(score([ {baseUri : 'git://repo?revision', pattern : '*.txt'} ],
             new URL('git://repo?revision#f.txt'), 'l'))
    .toBe(10)
expect(score([ {baseUri : 'file:///a/b', pattern : '*.txt'} ],
             new URL('file:///a/b/c.txt'), 'l'))
    .toBe(5)
expect(score([ {baseUri : 'git://repo?revision', pattern : '**/*.txt'} ],
             new URL('git://repo?revision#f.txt'), 'l'))
    .toBe(10)
expect(score([ {baseUri : 'git://repo', pattern : '**/*.txt'} ],
             new URL('git://repo?revision#f.txt'), 'l'))
    .toBe(10)
expect(score([ {baseUri : 'git://repo?revision'} ],
             new URL('git://repo?revision#f.txt'), 'l'))
    .toBe(5)
expect(
    score(
...
expect(score([ {pattern : 'f.txt', language : 'x'} ], new URL('file:///f.txt'),
             'l'))
    .toBe(0)
...
interface OffsetPositionTestCase {
        ...
        describe('offsetToPosition', () => {
          for (const [index, {text, ...testCase}] of OFFSET_TO_POSITION_TESTS
                   .entries()) {
            if (!('offset' in testCase)) {
              continue
            }
            test(index.toString(),
                 () => expect(offsetToPosition(text, testCase.offset))
                           .toEqual(testCase.pos))
          }
        })

        describe('positionToOffset', () => {
          for (const [index, {text, ...testCase}] of POSITION_TO_OFFSET_TESTS
                   .entries()) {
            test(index.toString(),
                 () => expect(positionToOffset(text, testCase.pos))
