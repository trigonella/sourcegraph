...
/**
 * The URI scheme for the resources that hold the body of comments (such as
...
/**
 * Returns whether any of the document selectors match (or "select") the
 * document.
 */
export function match(
    selectors: DocumentSelector|IterableIterator<DocumentSelector>,
    document: Pick<TextDocument, 'uri'|'languageId'>): boolean {
  for (const selector of isSingleDocumentSelector(selectors) ? [ selectors ]
                                                             : selectors) {
...
function isSingleDocumentSelector(value: DocumentSelector|
                                  IterableIterator<DocumentSelector>):
    value is DocumentSelector{
        return Array.isArray(value) &&
               (value.length === 0 || isDocumentSelectorElement(value[0]))}

function isDocumentSelectorElement(value: any):
    value is DocumentSelector[0] {
      return typeof value === 'string' || isDocumentFilter(value)
    }

function isDocumentFilter(value: any):
    value is DocumentFilter {
...
function match1(selector: DocumentSelector,
...
/**
 * Returns the score that indicates "how well" the document selector matches a
 * document (by its URI and language ID). A higher score indicates a more
 * specific match. The score is a heuristic.
 *
 * For example, a document selector ['*'] matches all documents, so it is not a
 * very specific match for any document (but it *does* match all documents). Its
 * score will be lower than a more specific match, such as the document selector
 * [{language: 'python'}] against a Python document.
 *
 * Taken from
 ...
 */
export function score(selector: DocumentSelector, candidateUri: URL,
...
function score1(selector: DocumentSelector[0], candidateUri: URL,
                candidateLanguage: string):
    number {
      if (typeof selector === 'string') {
        // Shorthand notation: "mylang" -> {language: "mylang"}, "*"
        // -> {language: "*""}.
        if (selector === '*') {
          return 5
        }
      ...
      if (pattern) {
        const filePath =
            decodeURIComponent(candidateUri.protocol === 'git:'
...
export function offsetToPosition(text: string, offset: number):
    Position {
      ...
      return {
        line, character: pre ? pre[0].length + (line === 0 ? 0 : -1) : 0
      }
    }
