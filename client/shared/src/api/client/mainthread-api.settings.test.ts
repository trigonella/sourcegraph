...
const updateSettings: PlatformContext['updateSettings'] =
    (...args) => {
      calledWith = args
      return Promise.resolve()
    } const platformContext:
        Pick<PlatformContext, 'updateSettings'|'settings'> =
{
  settings: of({
    subjects : [
      {
...
const {api} =
    initMainThreadAPI(pretendRemote({}), platformContext, defaultDependencies())
...
const edit: SettingsEdit = {
  ...
  settings: of(...values), updateSettings: () => Promise.resolve(),
        }

        const passedToExtensionHost: SettingsCascade<object>[] = []
    initMainThreadAPI(
        pretendRemote<FlatExtensionHostAPI>({
          syncSettingsData : data => { passedToExtensionHost.push(data) },
        }),
        platformContext, defaultDependencies())
    ...
    const platformContext: Pick<PlatformContext, 'updateSettings'|'settings'> =
    {
      settings: values.asObservable(), updateSettings: () => Promise.resolve(),
    }
    ...
    const one = { subjects: [], final: {a: 'one'}, }
