...
// for now it will partially mimic Services object but hopefully will be
// incrementally reworked in the process
    ...
    export const initMainThreadAPI =
        (extensionHost: Remote<FlatExtensionHostAPI>,
         ...
         dependencies: MainThreadAPIDependencies): {
          ...
          // Settings
          subscription.add(from(platformContext.settings)
                               .pipe(switchMap(settings => {
                                 if (isSettingsValid(settings)) {
                                   return extensionHost.syncSettingsData(
                                       settings)
                                 }
                                 return []
                               }))
          ...
          subscription.add(
              from(versionContext)
                  .pipe(concatMap(
                      context => extensionHost.syncVersionContext(context)))
                  .subscribe())
          ...
          const api: MainThreadAPI = {
            ...
            executeCommand : (command, args) =>
                commands.executeCommand({command, arguments : args}),
