...
describe("Expression", () => {
  ...
  const TESTS = {
    ...
    'x == "y"': true,
    "json(o)": '{"k":"v"}',
    // TODO: Support operator precedence. See ./parser.test.ts for a
    // commented-out precedence test case.
    //
    // 'x == "y" || x == "z"': true,
    'x == "y" && x == "z"': false,
    'x == "y" && x != "z"': true,
    ...
    "`_${`-${x}-`}_`": "_-y-_",
    "a || isnotdefined": 1, // short-circuit (if not, the use of an undefined
    // ident would cause an error)
    "get(array, 0)": 7,
