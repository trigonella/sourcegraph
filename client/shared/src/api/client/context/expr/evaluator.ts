...
export class Expression<T> {
  ...
  public exec<C>(context: Context<C>): T { return exec<C>(this.root, context) }
}
 ...
 */
export class TemplateExpression<S extends string = string> extends
    Expression<S> {}
 ...
 *
 * A template is a string that interpolates expressions in ${...}. It uses the
 * same syntax as JavaScript templates.
 */
...
const FUNCS: {[name: string]: (...args: any[]) => any} = {
  ...
  json
  : (object: any) : string => JSON.stringify(object),
}

function
exec<C>(node: ExpressionNode, context: Context<C>): any {
  if ('Literal' in node) {
    switch (node.Literal.type) {
    case TokenType.String:
      return node.Literal.value
      case TokenType
          .Number: return parseFloat(node.Literal.value) default: throw new SyntaxError(
              `Unrecognized literal of type ${TokenType[node.Literal.type]}`)
    }
  }

  if ('Template' in node) {
    const parts: any[] = [] for (const expression of node.Template.parts) {
  ...
  if ('Binary' in node) {
    ...
    case '&&':
      ...
      case '/': return left / right()
      case '^': return left ^ right()
      case '%': return left % right() default: throw new SyntaxError(
                                  `Invalid operator: ${node.Binary.operator}`)
    }
  }
  ...
  if ('Unary' in node) {
    ...
    case '!':
      ...
      case '-': return -expression default: throw new SyntaxError(
          `Invalid operator: ${node.Unary.operator}`)
    }
  }
  ...
  if ('FunctionCall' in node) {
    ...
    const func = FUNCS[expression.name] if (typeof func === 'function') {
