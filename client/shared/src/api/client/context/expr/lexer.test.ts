...
describe("Lexer", () => {
  ...
  describe("templates", () => {
    ...
    test("throws on unclosed expression", () =>
      expect(() => scanAll(lexer, "x${")).toThrow());
  });
  ...
  describe("constants", () => {
    test("OPERATOR_CHARS", () => {
      const maxOpLength = Math.max(
        ...Object.keys(OPERATORS).map(operator => operator.length)
      );
      for (const operator of Object.keys(OPERATORS)) {
        if (operator.length === 1) {
          expect(
            OPERATOR_CHARS[operator] === true ||
              (OPERATOR_CHARS[operator] as { [ch: string]: OperatorTree })[
                "\u0000"
              ] === true
          ).toBeTruthy();
          ...
          expect(
            ...
            })[operator.charAt(1)] === true ||
              ((OPERATOR_CHARS[operator.charAt(0)] as {
                [ch: string]: OperatorTree;
              })[operator.charAt(1)] as { [ch: string]: OperatorTree })[
                "\u0000"
              ] === true
          ).toBeTruthy();
          ...
          expect(
            ((OPERATOR_CHARS[operator.charAt(0)] as {
              ...
              (((OPERATOR_CHARS[operator.charAt(0)] as {
                ...
                operator.charAt(2)
              ] as { [ch: string]: OperatorTree })["\u0000"] === true
          ).toBeTruthy();
