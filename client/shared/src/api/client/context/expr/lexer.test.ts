...
describe('Lexer', () => {
    ...
    test('scans an expression', () => {
        lexer.reset('ab * 2')
    expect(lexer.peek()).toEqual({type : TokenType.Identifier,
                                  value : 'ab'} as Token)
    const token = lexer.next()
    expect(token).toEqual(
        {type : TokenType.Identifier, value : 'ab', start : 0, end : 2} as
        Token)
    ...
    describe('constants', () => {
        test('OPERATOR_CHARS', () => {
  const maxOpLength =
      Math.max(...Object.keys(OPERATORS).map(operator => operator.length))
  for (const operator of Object.keys(OPERATORS)) {
    if (operator.length === 1) {
      expect(OPERATOR_CHARS[operator] === true ||
             (OPERATOR_CHARS[operator] as
              {[ch: string] : OperatorTree})['\u0000'] === true)
          .toBeTruthy()
    } else if (operator.length === 2) {
      expect((OPERATOR_CHARS[operator.charAt(0)] as
              {[ch: string] : OperatorTree})[operator.charAt(1)] === true ||
             ((OPERATOR_CHARS[operator.charAt(0)] as
               {[ch: string] : OperatorTree})[operator.charAt(1)] as
              {[ch: string] : OperatorTree})['\u0000'] === true)
          .toBeTruthy()
    } else if (operator.length === 3) {
      expect(((OPERATOR_CHARS[operator.charAt(0)] as
               {[ch: string] : OperatorTree})[operator.charAt(1)] as
              {[ch: string] : OperatorTree})[operator.charAt(2)] === true ||
             (((OPERATOR_CHARS[operator.charAt(0)] as
                {[ch: string] : OperatorTree})[operator.charAt(1)] as
               {[ch: string] : OperatorTree})[operator.charAt(2)] as
              {[ch: string] : OperatorTree})['\u0000'] === true)
          .toBeTruthy()
    } else if (operator.length > maxOpLength) {
      throw new Error(
          `operators of length ${operator.length} are not yet supported`)
    }
  }
        })
    })
