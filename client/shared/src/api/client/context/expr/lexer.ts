...
export enum TokenType {
   ...
   *
   * See https://tc39.github.io/ecma262/#sec-template-literal-lexical-components
   * for documentation on the ECMAScript lexical components for templates, upon
   * which this is based.
   */
  TemplateHead,

  /**
     The end of a previous template expression until the next template
     expression.
   */
  TemplateMiddle,
...
export interface Token {
   ...
   * The token's value.
   *
   * For string and template literals, this is the parsed string value (after
   * accounting for escape sequences but not template expressions). For number
   * literals, this is the (unparsed) string representation.
   */
  value: any
...
/**
 * All valid operators in expressions. The values are the operator precedence
 * (or, for operators that are not operators, 0). This must be kept in sync with
 * OPERATOR_CHARS.
 *
...
/**
 * A tree with the next valid operator characters for multi-character operators.
 * This must be kept in sync with OPERATORS.
 *
 * Exported for testing only.
 */
...
function
isDecimalDigit(character: string):
    boolean { return character >= '0' && character <= '9'}

function
isIdentifierStart(character: string):
    boolean { return character === '_' || isLetter(character)}

function
isIdentifierPart(character: string):
    boolean {
...
export class Lexer {
  ...
  public reset(string: string): void {
    this.expression = string this.length = string.length this._index =
        0 this.curlyStack = 0
  }
  ...
  public next(): Token|undefined {
    ...
    throw new SyntaxError(`Unexpected character ${
        JSON.stringify(this.peekNextChar())} (at ${this.index})`)
  }
  ...
  public peek(): Omit<Token, 'start'|'end'>|undefined {
    ...
    this._index = savedIndex this.curlyStack = savedCurlyStack
  ...
  private getNextChar(): string {
    ...
    if (index < this.length) {
      character = this.expression.charAt(index) this._index += 1
    }
    return character
  }

  private createToken(type: TokenType, value: any): Token {
    return { type, value, start: this.marker, end: this._index, }
  }
  ...
  private scanOperator(): Token|undefined {
    ...
    while (searchTree && searchTree !== true) {
      const character = this.peekNextChar()
      searchTree = searchTree[character] if (searchTree) {
        value += character this.getNextChar()
      }
    }
    if (value === '}') {
      this.curlyStack--
  ...
  protected backtick(): boolean { return true }
    ...
    while (this._index < this.length) {
      ...
      if (character === '\\') {
        ...
        if (character === '$') {
          ...
          if (character2 === '{') {
            this.curlyStack++ this.getNextChar()
            terminated = true
    ...
    if (character === 'e' || character === 'E') {
      ...
      if (character === '+' || character === '-' || isDecimalDigit(character)) {
        ...
        throw new SyntaxError(`Unexpected ${
            character} after the exponent sign (at ${this.index})`)
      }
    }

    if (number === '.') {
      throw new SyntaxError(
          `Expected decimal digits after the dot sign (at ${this.index})`)
    }
...
function backslashEscapeCodeString(character: string): string {
  ...
  case 'n':
    return '\n'
    case 'r': return '\r'
    case 't': return '\t' default: return character
  }
}
