 ...
 *
 * TODO: Operator precedence is not handled correctly. Use parentheses to be
 * explicit about your desired precedence.
 */
export class Parser {
  protected lexer!: Lexer
    ...
    if (token !== undefined) {
      throw new SyntaxError(`Unexpected token at end of input: ${
          JSON.stringify(token.value)} (at ${this.lexer.index})`)
    }
  ...
  private parseArgumentList(): ExpressionNode[] {
    const args: ExpressionNode[] = [] while (true) {
      const expression = this.parseExpression()
      if (expression === undefined) {
        throw new Error(`Parse error on token in arguments list: ${
            JSON.stringify(this.lexer.peek())} (at ${this.lexer.index})`)
      }
  ...
  private parseFunctionCall(name: string): ExpressionNode {
    ...
    if (!matchOp(token, '(')) {
      throw new SyntaxError(`Expected "(" in function call ${
          JSON.stringify(name)} (at ${this.lexer.index})`)
    }

    token = this.lexer.peek()
    const args: ExpressionNode[] =
        matchOp(token, ')') ? [] : this.parseArgumentList()
  ...
  private parseTemplateParts(): ExpressionNode[] {
    const parts: ExpressionNode[] = [] while (true) {
  ...
  protected parseTemplate(): ExpressionNode {
    ...
    if (token === undefined) {
      throw new SyntaxError(
          `Unexpected termination of expression at beginning of template (at ${
              this.lexer.index})`)
    }

    if (token.type === TokenType.NoSubstitutionTemplate) {
      this.lexer.next()
      // The caller doesn't need to distinguish between NoSubstitutionTemplate
      // and String tokens, so collapse both token types into String.
    ...
    if (token.type === TokenType.TemplateHead ||
      ...
      if (token.value) {
        parts.unshift(
            {Literal : {type : TokenType.String, value : token.value}})
      }
      return { Template: {parts}, }
    }

    throw new SyntaxError(`Unexpected token at beginning of template: ${
        JSON.stringify(token.value)} (at ${this.lexer.index})`)
  }
  ...
  private parsePrimary(): ExpressionNode {
    ...
    if (token === undefined) {
      throw new SyntaxError(
          `Unexpected termination of expression (at ${this.lexer.index})`)
    }
    ...
    throw new SyntaxError(`Parse error on token: ${
        JSON.stringify(token.value)} (at ${this.lexer.index})`)
  }
  ...
  private parseUnary(): ExpressionNode {
    const token = this.lexer.peek()
    if (token !== undefined &&
        (matchOp(token, '-') || matchOp(token, '+') || matchOp(token, '!'))) {
      this.lexer.next()
  ...
  private parseMultiplicative(): ExpressionNode {
    ...
    while (token !== undefined && (matchOp(token, '*') || matchOp(token, '/') ||
                                   matchOp(token, '%'))) {
      this.lexer.next()
      expression = {
        Binary : {
  ...
  // Expression ::= Additive
  private parseExpression(): ExpressionNode { return this.parseAdditive() }
}
...
export class TemplateParser extends Parser {
  public parse(templateString: string): ExpressionNode {
    ...
    const token = this.lexer.next()
    if (token !== undefined) {
      throw new SyntaxError(`Unexpected token at end of template input: ${
          JSON.stringify(token.value)} (at ${this.lexer.index})`)
    }
...
function matchOp(token: Pick<Token, 'type'|'value'>|undefined,
                 operator: Operator): boolean {
  return token !== undefined && token.type === TokenType.Operator &&
