...
export function computeContext<T>(
  ...
  if (component?.type === "CodeEditor") {
    ...
    if (component.selections.length > 0) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
      data["component.selection.start"] = (component.selections[0]
        ? component.selections[0].start
        : null) as any; // eslint-disable-line @typescript-eslint/no-explicit-any
      // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
      data["component.selection.end"] = (component.selections[0]
        ? component.selections[0].end
        : null) as any; // eslint-disable-line @typescript-eslint/no-explicit-any
      data["component.selection.start.line"] = component.selections[0]
        ? component.selections[0].start.line
        : null;
      data["component.selection.start.character"] = component.selections[0]
        ? component.selections[0].start.character
        : null;
      data["component.selection.end.line"] = component.selections[0]
        ? component.selections[0].end.line
        : null;
