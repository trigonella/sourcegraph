...
/**
 * The context service owns the context data for the context, which consists of
 * arbitrary key-value pairs that describe application state.
 */
...
export function createContextService({
  ...
  const data = new BehaviorSubject<Context>({
    ...
    // Arbitrary, undocumented versioning for extensions that need different
    // behavior for different Sourcegraph versions.
    //
    // TODO: Make this more advanced if many extensions need this (although we
    // should try to avoid extensions needing this).
    'clientApplication.extensionAPIVersion.major' : 3,
  })
  return {
    data, updateContext(update: {[k: string]: unknown}): void {
      if (isMatch(this.data.value, update)) {
        return
      }
      const result:
          any = {} for (const [key, oldValue] of Object.entries(data.value)) {
        if (update[key] !== null) {
          result[key] = oldValue
        }
      }
