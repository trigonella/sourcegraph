...
export async function createExtensionHostClientConnection(
  ...
  const {
    endpoints,
    subscription: endpointsSubscription
  } = await endpointsPromise;
  subscription.add(endpointsSubscription);

  /** Proxy to the exposed extension host API */
  const initializeExtensionHost = comlink.wrap<ExtensionHostAPIFactory>(
    endpoints.proxy
  );

  const initialSettings = await from(platformContext.settings)
    .pipe(first())
    .toPromise();
  ...
  const clientContext = new ClientContext((updates: ContextValues) =>
    services.context.updateContext(updates)
  );
  ...
  subscription.add(
    merge(
      ...
      .pipe(
        concatMap(modelUpdates =>
          proxy.documents.$acceptDocumentData(modelUpdates)
        )
      )
      .subscribe()
  );
  ...
  const clientWindows = new ClientWindows(
    (parameters: ShowNotificationParameters) =>
      services.notifications.showMessages.next({ ...parameters }),
    (parameters: ShowMessageRequestParameters) =>
      new Promise<MessageActionItem | null>(resolve => {
        services.notifications.showMessageRequests.next({
          ...parameters,
          resolve
        });
      }),
    ...
    ({ title }: ProgressOptions) => {
      ...
      services.notifications.progresses.next({
        title,
        progress: reporter.asObservable()
      });
      return reporter;
    }
  );
  ...
  const clientCodeEditor = new ClientCodeEditor(
