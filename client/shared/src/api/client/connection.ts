...
import {from, merge, of, Subject, Subscription} from 'rxjs'
...
export interface ActivatedExtension {
  /**
   * The extension's extension ID (which uniquely identifies it among all
...
export async function createExtensionHostClientConnection(
    endpointsPromise: Promise<ClosableEndpointPair>, services: Services,
    initData: Omit<InitData, 'initialSettings'>,
    platformContext: Pick<PlatformContext, 'settings'|'updateSettings'>):
    Promise<{
      subscription : Unsubscribable; api : comlink.Remote<FlatExtensionHostAPI>
    }> {
  ...
  const {endpoints, subscription : endpointsSubscription} =
  ...
  const initializeExtensionHost =
      comlink.wrap<ExtensionHostAPIFactory>(endpoints.proxy)

  const initialSettings =
      await from(platformContext.settings).pipe(first()).toPromise()
  const proxy = await initializeExtensionHost({
    ...initData,
    // TODO what to do in error case?
    initialSettings : isSettingsValid(initialSettings)
                          ? initialSettings
                          : {final : {}, subjects : []},
  })

  const clientContext = new ClientContext(
      (updates: ContextValues) => services.context.updateContext(updates))
  subscription.add(clientContext)

  // Sync models and viewers to the extension host
  subscription.add(
      merge(of([...services.model.models.entries() ].map(
                ([, model ]): TextModelUpdate => ({type : 'added', ...model}))),
            from(services.model.modelUpdates))
          .pipe(concatMap(modelUpdates => proxy.documents.$acceptDocumentData(
                              modelUpdates)))
          .subscribe())
  subscription.add(
      merge(of([...services.viewer.viewers.entries() ].map(
          ...
          .pipe(concatMap(viewerUpdates =>
                              proxy.windows.$acceptWindowData(viewerUpdates)))
          .subscribe())
    ...
    services.notifications.progresses.next(
        {title, progress : reporter.asObservable()})
            return reporter
        }
    )

            const clientViews = new ClientViews(services.panelViews,
                                                services.textDocumentLocations,
                                                services.viewer, services.view)

            const clientCodeEditor =
                new ClientCodeEditor(services.textDocumentDecoration)
            subscription.add(clientCodeEditor)

            const clientLanguageFeatures = new ClientLanguageFeatures(
                services.textDocumentDefinition,
                services.textDocumentReferences, services.textDocumentLocations,
                services.completionItems)
            subscription.add(
                new ClientExtensions(proxy.extensions, services.extensions))

            const clientContent = createClientContent(services.linkPreviews)

            const {api : newAPI, subscription : apiSubscriptions} =
                initMainThreadAPI(proxy, platformContext, services)
