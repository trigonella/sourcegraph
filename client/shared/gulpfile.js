...
const {DEFAULT_OPTIONS, DEFAULT_TYPE_MAP} =
...
const GRAPHQL_SCHEMA_PATH =
    path.join(__dirname, '../../cmd/frontend/graphqlbackend/schema.graphql')

/**
 * Generates the TypeScript types for the GraphQL schema.
 * These are used by older code, new code should rely on the new query-specific
 * generated types.
 *
...
async function graphQlSchema() {
  ...
  const formatOptions = await resolveConfig(
      __dirname, {config : __dirname + '/../../prettier.config.js'})
  const typings =
      ...
      generateNamespace(
          ...
          },
          {
            ...
            enumTypeBuilder : (name, values) => `export ${
                DEFAULT_OPTIONS.enumTypeBuilder(name, values)
                    .replace(/^const enum/, 'enum')}`,
            typeBuilder : (name, body) =>
                `export ${DEFAULT_OPTIONS.typeBuilder(name, body)}`,
            wrapList : type => `${type}[]`,
            postProcessor : code =>
                format(code, {...formatOptions, parser : 'typescript'}),
 ...
 */
async function graphQlOperations() { await generateGraphQlOperations() }

/**
 * Allow json-schema-ref-parser to resolve the v7 draft of JSON Schema
 * using a local copy of the spec, enabling developers to run/develop
 * Sourcegraph offline
 */
const draftV7resolver =
    {
      order : 1,
      read : () => readFile(path.join(
          __dirname, '../../schema/json-schema-draft-07.schema.json')),
    ...
    schema() {
      const outputDirectory =
          path.join(__dirname, '..', 'web', 'src', 'schema')
  ...
  // HACK: Rewrite absolute $refs to be relative. They need to be absolute for
  // Monaco to resolve them when the schema is in a oneOf (to be merged with
  // extension schemas).
