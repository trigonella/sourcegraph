 ...
 *
 * Sourcegraph extensions enhance your code host, code reviews, and Sourcegraph
 * itself by adding features such as:
 ...
 *
 * Check out the [extension authoring
 * documentation](https://docs.sourcegraph.com/extensions/authoring) to get
 * started.
 */
declare module 'sourcegraph' {
  ...
  /**
   * @deprecated Use the global [native `URL`
  ...
  export class Position {
     ...
     * or zero when this and the given position are equal.
     */
     ...
     * @param characterDelta Delta value for the character value, default is
     *     `0`.
     * @returns A position which line and character is the sum of the current
     *     line and
   ...
   * [start](#Range.start).isBeforeOrEqual([end](#Range.end))
   *
  ...
  export class Range {
     ...
     * using `new Range(new Position(startLine, startCharacter), new
     ...
     */
    constructor(startLine: number, startCharacter: number, endLine: number,
                endCharacter: number)
     ...
     *
     * @param start A position that should be used as start. The default value
     *     is the [current start](#Range.start).
     * @param end A position that should be used as end. The default value is
     *     the [current end](#Range.end).
  ...
  export class Selection extends Range {
     ...
     */
    constructor(anchorLine: number, anchorCharacter: number, activeLine: number,
                activeCharacter: number)

    /**
     * A selection is reversed if
     * [active](#Selection.active).isBefore([anchor](#Selection.anchor)).
     */
    isReversed: boolean
  }
  ...
  export class Location {
     ...
     * @param rangeOrPosition The range or position. Positions will be converted
     *     to an empty range.
     */
  ...
  export interface TextDocument {
     ...
     * extension](https://docs.sourcegraph.com/integration/browser_extension),
     * the value is `undefined` because determining the text contents (in
     * general) is not possible without additional access to the code host API.
     ...
     *
     * The position will be adjusted using {@link
     * TextDocument#validatePosition}.
     *
    ...
    /**
     * Ensure a position is contained in the range of this document. If not,
     ...
     *
     * @param range A range
     * @returns The text inside the provided range, or the whole text if no
     *     range is provided
     */
   ...
   * [language](#TextDocument.languageId), the scheme of its resource, or a
   * glob-pattern that is applied to the [path](#TextDocument.fileName). A
   * document filter matches if all the provided properties (those of
   * `language`, `scheme` and `pattern` that are not `undefined`) match. If all
   * properties are `undefined`, the document filter matches all documents.
   *
   * Examples:
   * ```ts
  ...
  export interface DocumentFilter {
    ...
    /**
     * A base URI (e.g. root URI of a workspace folder) that the document must
     * be within.
     */
    baseUri?: URL|string
  }
   ...
   * If the filter is a string and not a {@link DocumentFilter}, it will be
   * treated as a language id.
   *
   * @example let sel: DocumentSelector = [{ language: 'typescript' }, {
   * language: 'json', pattern: '**∕tsconfig.json' }];
   */
  export type DocumentSelector = (string|DocumentFilter)[]

      /**
       * Options for an input box displayed as a result of calling {@link
       * Window#showInputBox}.
       */
  ...
  export interface Progress {
    ...
    /**
     * Integer from 0 to 100. If not set, the previous percentage is still
     * shown.
     */
    percentage?: number
  }
  ...
  export interface ProgressReporter {
    ...
    next(status: Progress): void
        ...
        /**
         * Turns the progress display into an error display for the given error
         * or message. Use if the operation failed. No further progress updates
  ...
  export interface Window {
    ...
    activeViewComponentChanges: Subscribable<ViewComponent|undefined>
        ...
        /**
         * Show a notification message to the user that does not require
         * interaction or steal focus.
         *
         * @param message The message to show. Markdown is supported.
         * @param type a {@link NotificationType} affecting the display of the
         *     notification.
         */
            showNotification(message: string, type: NotificationType): void

        /**
         * Show progress in the window. Progress is shown while running the
         * given callback and while the promise it returned isn't resolved nor
         * rejected.
         *
         * @param task A callback returning a promise. Progress state can be
         *     reported with
         * the provided [ProgressReporter](#ProgressReporter)-object.
         *
         * @returns The Promise the task-callback returned.
         */
        withProgress<R>(options: ProgressOptions,
                        task: (reporter: ProgressReporter) => Promise<R>):
            Promise<R>

        /**
         * Show progress in the window. The returned ProgressReporter can be
         * used to update the progress bar, complete it or turn the notification
         * into an error notification in case the operation failed.
         *
         * @returns A ProgressReporter that allows updating the progress
         *     display.
         */
        showProgress(options: ProgressOptions): Promise<ProgressReporter>

        /**
         * Show a modal message to the user that the user must dismiss before
         * continuing.
         *
         ...
         * (e.g., because the user pressed the ESC key). Otherwise the returned
         * value will be the string provided by the user.
         *
         * @param options Configures the behavior of the input box.
         * @returns The string provided by the user, or `undefined` if the input
         *     box was canceled.
         */
   ...
   *
   * Each {@link ViewComponent} has a distinct {@link ViewComponent#type} value
   * that indicates what kind of component it is ({@link CodeEditor}, etc.).
   */
  ...
  /**
   * A text document decoration changes the appearance of a range in the
   * document and/or adds other content to it.
   */
  export interface TextDocumentDecoration extends ThemableDecorationStyle {
    ...
    /**
     * If true, the decoration applies to all lines in the range (inclusive),
     * even if not all characters on the line are included.
     */
    isWholeLine?: boolean
  ...
  /**
   * A decoration attachment adds content after a {@link
   * TextDocumentDecoration}.
   */
  export interface DecorationAttachmentRenderOptions extends
  ...
  export interface Directory {
     ...
     *
     * @todo The format of this URI will be changed in the future. It must not
     * be relied on.
     */
    readonly uri: URL
  }
  ...
  /**
   * A text editor for code files (as opposed to a rich text editor for
   * documents or other kinds of file format editors).
   */
  export interface CodeEditor {
    ...
    /**
     * The text document that is open in this editor. The document remains the
     * same for the entire lifetime of this editor.
     */
    readonly document: TextDocument

    /**
     * The primary selection in this text editor. This is equivalent to
     * `CodeEditor.selections[0] || null`.
     *
    ...
    /**
     * The selections in this text editor. A text editor has zero or more
     * selections. The primary selection
     * ({@link CodeEditor#selection}), if any selections exist, is always at
     * index 0.
     *
     * @todo Make this non-readonly.
     */
     ...
     * The primary selection ({@link CodeEditor#selection}), if any selections
     * exist, is always at index 0 of the emitted array.
     */
    readonly selectionsChanges: Subscribable<Selection[]>

        /**
         * Add a set of decorations to this editor. If a set of decorations
         ...
         *
         */
            setDecorations(decorationType: TextDocumentDecorationType,
                           decorations: TextDocumentDecoration[]): void
  }
  ...
  export interface PanelView extends Unsubscribable {
    ...
    /**
     * The priority of this panel view. A higher value means that the item is
     * shown near the beginning (usually the left side).
     */
    priority: number

    /**
     * Display the results of the location provider (with the given ID) in this
     * panel below the
  ...
  export type ChartContent = LineChartContent<any, string>|
      BarChartContent<any, string>|PieChartContent<any>
  ...
  export interface LineChartContent<D extends object, XK extends keyof D> {
     ...
     * A link URL should take the user to more details about the specific data
     * point.
     */
    linkURLs?: string[]
  ...
  export interface BarChartContent<D extends object, XK extends keyof D> {
    ...
    series: {
      /**
         The key in each data object for the values this bar should be
         calculated from.
       */
      dataKey: keyof D
     ...
     * When two bars have the same same `stackId`, the two bars are stacked in
     * order.
     */
    stackId?: string
  ...
  export interface PieChartContent<D extends object> {
    ...
    pies: {/** The key of each sector's va lue. */
           dataKey: keyof D
  ...
  export interface View {
     ...
     *
     * Support for non-MarkupContent elements is experimental and subject to
     * change or removal without notice.
     */
    content: (|MarkupContent|ChartContent|{
      component: string;
      props: {[name: string]: string|number|boolean|null|undefined}
    })[]
  }

  /**
   * A view provider registered with {@link
   * sourcegraph.app.registerViewProvider}.
   */
  export type ViewProvider =|InsightsPageViewProvider|HomepageViewProvider|
      GlobalPageViewProvider|DirectoryViewProvider

  /**
   * Experimental view provider shown on the dashboard on the insights page.
   * This API is experimental and is subject to change or removal without
   * notice.
   */
  ...
  /**
   * Experimental view provider shown on the homepage (below the search box in
   * the Sourcegraph web app). This API is experimental and is subject to change
  ...
  /**
   * Experimental global view provider. Global view providers are shown on a
   * dedicated page in the app. This API is experimental and is subject to
   * change or removal without notice.
   */
  export interface GlobalPageViewProvider {
     ...
     * Provide content for the view.
     *
     * @param params Parameters from the page (such as URL query parameters).
     *     The schema of these parameters is
   ...
   *
   * The schema of these parameters is experimental and subject to change
   * without notice.
   */
  ...
  export interface DirectoryViewProvider {
     ...
     *
     * @param context The context of the directory. The schema of these
     *     parameters is experimental and subject to
  ...
  /**
   * The currently active window, or `undefined`. The active window is the
   * window that has focus, or when none has focus, the window that was most
   * recently focused.
   */
  ...
  export const activeWindowChanges: Subscribable<Window|undefined>
      ...
      /**
       * Create a panel view for the view contribution with the given {@link
       * id}.
       *
       * @todo Consider requiring extensions to specify these statically in
       * package.json's contributions section to improve the activation
       * experience.
       *
       * @param id The ID of the view. This may be shown to the user (e.g., in
       *     the URL fragment when the panel is
       * active).
   ...
   *
   * Use this to create a unique handle to a set of decorations, that can be
   * applied to text editors using {@link setDecorations}.
   */
   ...
   */
    ...
    export namespace workspace {
          ...
          export const textDocuments: TextDocument[]
               ...
               *
               * @example The repository that is currently being viewed is a
               * root.
              ...
              /**
               * An event that is fired when a workspace root is added or
               ...
               *
               * A version context is a set of repositories and revisions on a
               * Sourcegraph instance. when set, extensions use it to scope
               * search queries, code intelligence actions, etc.
               *
               * See more information at
               * http://docs.sourcegraph.com/user/search#version-contexts.
               */
        ...
        /**
         * The full configuration value, containing all settings for the current
         * subject.
         *
        ...
        /**
         * Updates the configuration value for the given key. The updated
         * configuration value is persisted by the client.
         *
         * @template C The configuration schema.
         * @template K Valid key on the configuration object.
         * @param key The name of the configuration property to update.
         * @param value The new value, or undefined to remove it.
         * @returns A promise that resolves when the client acknowledges the
         *     update.
         */
    ...
    interface ConfigurationService extends Subscribable<void> {
         ...
         * Directory settings
         *
         * @todo Add a way to get/update configuration for a specific scope or
         * subject.
        ...
        /**
         * A provider result represents the values that a provider, such as
         * the {@link HoverProvider}, may return. The result may be a single
         * value, a Promise that resolves to a single value, or a Subscribable
         * that emits zero or more values.
         */
        export type ProviderResult<T> =|T|undefined|null|
            Promise<T|undefined|null>|Subscribable<T|undefined|null>
        ...
        /**
         * The type of a notification shown through {@link
        ...
        export interface ThemableBadgeAttachmentStyle {
          /**
           * The icon (a base64-encoded image icon) to display next to the
           * wrapped value.
           *
           * @deprecated Use {@link BadgeAttachmentRenderOptions#kind} to pick a
           *     predefined icon
           */
          icon?: string
        ...
        /**
         * An attachment adds content to a hover tooltip or result in a
         * locations panel.
         */
        export interface BadgeAttachmentRenderOptions extends
        ...
        /**
         * A wrapper around a providable type (currently hover and locations)
         * with additional context to enable displaying badges next to the
         * wrapped result value in the UI.
         */
        ...
        export interface Hover {
           ...
           * will use the range at the current position or the current position
           * itself.
           */
          range?: Range
        ...
        export interface HoverAlert {
          ...
          /**
           * When an alert has a dismissal type, dismissing it will prevent all
           * alerts of that type from being shown. If no type is provided, the
        ...
        export interface HoverProvider {
          provideHover(document: TextDocument,
                       position: Position): ProviderResult<Badged<Hover>>
        }

        /**
         * The definition of a symbol represented as one or many
         * [locations](#Location). For most programming languages there is only
         * one location at which a symbol is defined. If no definition can be
        ...
        export interface DefinitionProvider {
           ...
           * @returns A definition location, or an array of definitions, or
           *     `null` if there is no definition.
           */
          provideDefinition(document: TextDocument,
                            position: Position): ProviderResult<Definition>
        }
         ...
         * extensions and the [find
         * references](https://code.visualstudio.com/docs/editor/editingevolved#_peek)-feature.
         */
        export interface ReferenceProvider {
          /**
           * Provides a set of workspace-wide references for the given position
           * in a document.
           *
           * @param document The document in which the command was invoked.
           * @param position The position at which the command was invoked.
           * @param context Additional information and parameters for the
           *     request.
           * @returns An array of reference locations.
           */
          provideReferences(document: TextDocument, position: Position,
                            context: ReferenceContext):
              ProviderResult<Badged<Location>[]>
        }

        /**
         * A location provider implements features such as "find
         * implementations" and "find type definition". It is the general form
         * of {@link DefinitionProvider} and {@link ReferenceProvider}.
         */
        export interface LocationProvider {
          /**
           * Provide related locations for the symbol at the given position and
           * document.
           *
           * @param document The document in which the command was invoked.
           * @param position The position at which the command was invoked.
           ...
           */
          provideLocations(document: TextDocument,
                           position: Position): ProviderResult<Location[]>
        }

        /**
         * A completion item is a suggestion to complete text that the user has
         * typed.
         *
        ...
        export interface CompletionItem {
          /**
           * The label of this completion item, which is rendered prominently.
           * If no
           * {@link CompletionItem#insertText} is specified, the label is the
           * text inserted when the user selects this completion.
           */
          label: string

          /**
           * The description of this completion item, which is rendered less
           * prominently but still alongside the {@link CompletionItem#label}.
           */
          description?: string

          /**
           * A string to insert in a document when the user selects this
           * completion. When not set, the
        ...
        /**
         * A collection of [completion items](#CompletionItem) to be presented
         * in the editor.
         */
        ...
        /**
         * A completion item provider provides suggestions to insert or apply at
         * the cursor as the user is typing.
         *
         * Providers are queried for completions as the user types in any
         * document matching the document selector specified at registration
         * time.
         */
        export interface CompletionItemProvider {
           ...
           * @param position The position at which the command was invoked.
           *
           * @returns An array of completions, a [completion
           *     list](#CompletionList), or a thenable that resolves to either.
        ...
        /**
         * A document highlight provider provides ranges to highlight in the
         * current document like all occurrences of a variable or all
         * exit-points of a function.
         *
         * Providers are queried for document highlights on symbol hovers in any
         * document matching the document selector specified at registration
         * time.
         */
        export interface DocumentHighlightProvider {
           ...
           * @param position The position at which the command was invoked.
           *
           * @returns An array of document highlights, or a thenable that
           *     resolves to document highlights.
           * The lack of a result can be signaled by returning `undefined`,
           * `null`, or an empty array.
           */
        ...
        /**
         * Registers a hover provider, which returns a formatted hover message
         * (intended for display in a tooltip) when the user hovers on code.
         *
         * Multiple providers can be registered for a language. In that case,
         * providers are queried in parallel and the results are merged. A
         * failing provider (rejected promise or exception) will not cause the
         * whole operation to fail.
         *
         * @param selector A selector that defines the documents this provider
         *     is applicable to.
         * @param provider A hover provider.
         * @returns An unsubscribable to unregister this provider.
         */
        export function registerHoverProvider(selector: DocumentSelector,
         ...
         * @returns An unsubscribable to unregister this provider.
         */
        export function registerDefinitionProvider(selector: DocumentSelector,
                                                   provider:
                                                       DefinitionProvider):
            Unsubscribable
         ...
         * @returns An unsubscribable to unregister this provider.
         */
        export function registerReferenceProvider(selector: DocumentSelector,
                                                  provider: ReferenceProvider):
            Unsubscribable

        /**
         * Registers a generic provider of a list of locations. It is the
         * general form of
         * {@link registerDefinitionProvider} and {@link
         * registerReferenceProvider}. It is intended for "find
         ...
         * builtin command, passing the {@link id} as the first argument. For
         * more information, see
         * https://docs.sourcegraph.com/extensions/authoring/builtin_commands#executeLocationProvider.
         *
         * @param id An identifier for this location provider that distinguishes
         *     it from other location providers.
         * @param selector A selector that defines the documents this provider
         *     is applicable to.
         ...
         */
        export function
            registerLocationProvider(id: string, selector: DocumentSelector,
                                     provider: LocationProvider): Unsubscribable

        /**
         * Registers a completion item provider.
         *
         * Multiple providers can be registered with overlapping document
         * selectors. In that case, providers are queried in parallel and the
         * results are merged. A failing provider will not cause the whole
         * operation to fail.
         *
         * @param selector A selector that defines the documents this provider
         *     applies to.
         ...
         */
        export function
            registerCompletionItemProvider(selector: DocumentSelector,
    ...
    /**
     * Transforms a search query into another, valid query. If there are no
     * transformations to be made the original query is returned.
     *
    ...
    export namespace search {
           ...
           *
           * Multiple transformers can be registered. In that case, all
           * transformations will be applied and the result is a single query
           * that has been altered by all transformers. The order in which
           * transforms are applied is not defined.
           *
           * @param provider A query transformer.
           */
          export function registerQueryTransformer(provider: QueryTransformer):
              Unsubscribable
        }

        /**
         * Commands are functions that are implemented and registered by
         * extensions. Extensions can invoke any command (including commands
         * registered by other extensions). The extension can also define
         * contributions (in package.json), such as actions and menu items, that
         * invoke a command.
         */
        export namespace commands {
        /**
         * Registers a command that can be invoked by an action or menu item, or
         * directly (with
         * {@link commands.executeCommand}).
         *
         * @param command A unique identifier for the command.
         * @param callback A command function. If it returns a {@link Promise},
         *     execution waits until it is resolved.
         * @returns Unsubscribable to unregister this command.
         * @throws Registering a command with an existing command identifier
         *     throws an error.
         */
         ...
         * @returns A {@link Promise} that resolves to the result of the given
         *     command.
         * @throws If no command exists with the given command identifier, an
         *     error is thrown.
         */
        export function executeCommand<T = any>(command: string,
                                                ...args: any[]): Promise<T>
        }
        ...
        export interface LinkPreview {
          ...
          /**
           * The hover content of this link preview, which is shown when the
           * cursor hovers the link.
           *
           * @todo Add support for Markdown. Currently only plain text is
           * supported.
           */
        ...
        export interface LinkPreviewProvider {
          /**
           * Provides a preview of the information available at the URL of a
           * link in a document.
           *
           * @todo Add a `context` parameter so that the provider knows what
           * document contains the link (so that it can handle links in code
           * files differently from rendered Markdown documents, for example).
           *
        ...
        /**
         * EXPERIMENTAL. This API is subject to change without notice and has no
         * compatibility guarantees.
         *
         * Registers a provider for link previews ({@link LinkPreviewProvider})
         * for all URLs in a document matching the {@link urlMatchPattern}. A
         * link preview is a description of the information available at a URL.
         *
         * @todo Support a more powerful syntax for URL match patterns, such as
         * Chrome's (https://developer.chrome.com/extensions/match_patterns).
         *
         * @param urlMatchPattern A pattern that matches URLs for which the
         *     provider is called to
         * obtain a preview. Currently it matches all URLs that start with the
         * match pattern (i.e., string prefix matches). No wildcards are
         * supported.
         * @param provider The link preview provider.
         */
        export function registerLinkPreviewProvider(urlMatchPattern: string,
                                                    provider:
                                                        LinkPreviewProvider):
            Unsubscribable
        }
        ...
        /**
         * Internal API for Sourcegraph extensions. Most of these will be
         * removed for the beta release of Sourcegraph extensions. They are
         * necessary now due to limitations in the extension API and its
         * implementation that will be addressed in the beta release.
         *
         * @internal
         * @hidden
         */
         ...
         * sent to the client. It helps enforce serialization of messages.
         *
         * @internal
         */
        export function sync(): Promise<void>
            ...
            /**
             * Updates context values for use in context expressions and
             * contribution labels.
             *
             * @param updates The updates to apply to the context. If a context
             *     property's value is null, it is deleted from the context.
             */
            export function updateContext(updates: ContextValues): void

            /**
             * The URL to the Sourcegraph site that the user's session is
             * associated with. This refers to Sourcegraph.com
             * (`https://sourcegraph.com`) by default, or a self-hosted instance
             * of Sourcegraph.
             *
    ...
    export interface Subscribable<T> {
           ...
           *
           * @returns An unsubscribable that, when its {@link
           *     Unsubscribable#unsubscribe} method is called, causes
           * the subscription to stop reacting to the stream.
           */
          subscribe(observer?: PartialObserver<T>): Unsubscribable
          /** @deprecated Use an observer instead of a complete callback */
          subscribe(next: null|undefined, error: null|undefined,
                    complete: () => void): Unsubscribable
          /** @deprecated Use an observer instead of an error callback */
          subscribe(next: null|undefined, error: (error: any) => void,
                    complete?: (() => void)|null): Unsubscribable
          /** @deprecated Use an observer instead of a complete callback */
          // eslint-disable-next-line @typescript-eslint/unified-signatures
          subscribe(next: (value: T) => void, error: null|undefined,
                    complete: () => void): Unsubscribable
          subscribe(next?: ((value: T) => void)|null,
                    error?: ((error: any) => void)|null,
                    complete?: (() => void)|null): Unsubscribable
        }

        /**
         * The extension context is passed to the extension's activate function
         * and contains utilities for the extension lifecycle.
         *
         * @since Sourcegraph 3.0. Use `export function activate(ctx?:
         * ExtensionContext) { ... }` for prior versions (to ensure your code
         * handles the pre-3.0-preview case when `ctx` is undefined).
         */
        export interface ExtensionContext {
          /**
           * An object that maintains subscriptions to resources that should be
           * freed when the extension is deactivated.
           *
           * When an extension is deactivated, first its exported `deactivate`
           * function is called (if one exists). The `deactivate` function may
           * be async, in which case deactivation blocks on it finishing. Next,
           * regardless of whether the `deactivate` function finished
           * successfully or rejected with an error, all unsubscribables passed
           * to {@link ExtensionContext#subscriptions#add} are unsubscribed
           * from.
           *
           * (An extension is deactivated when the user disables it, or after an
           * arbitrary time period if its activationEvents no longer evaluate to
           * true.)
           */
          subscriptions: {
            /**
             * Mark a resource's teardown function to be called when the
             * extension is deactivated.
             *
             * @param unsubscribable An {@link Unsubscribable} that frees
             *     (unsubscribes from) a resource, or a
             * plain function that does the same. Async functions are not
             * supported. (If deactivation requires async operations, make the
             * `deactivate` function async; that is supported.)
             */
