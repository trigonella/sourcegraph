...
export function createPlatformContext(): PlatformContext{
  ...
  settings: concat(fetchViewerSettings(), updatedSettings)
      .pipe(map(gqlToCascade), publishReplay(1), refCount()),
      updateSettings:
          async (subject, edit) => {
            // Unauthenticated users can't update settings. (In the browser
            // extension, they can update client settings even when not
            // authenticated. The difference in behavior in the web app vs.
            // browser extension is why this logic lives here and not in
            // shared/.)
            if (!window.context.isAuthenticatedUser) {
              const editDescription =
                  typeof edit === 'string'
                      ? 'edit settings'
                      : 'update setting `' + edit.path.join('.') + '`'
              const url = new URL(window.context.externalURL)
              throw new Error(
                  `Unable to ${
                      editDescription} because you are not signed in.` +
                  '\n\n' +
                  `[**Sign into Sourcegraph${
                      url.hostname === 'sourcegraph.com'
                          ? ''
                          : ` on ${url.host}`}**](${
                                           `${
                                               url.href.replace(
            ...
            updatedSettings.next(await fetchViewerSettings().toPromise())
                          ...
                          createExtensionHost: () =>
                              Promise.resolve(createExtensionHost()),
...
`
    ...
    /**
     * Fetches the viewer's settings from the server. Callers should use
     * settingsRefreshes#next instead of calling this function, to ensure that
     * the result is propagated consistently throughout the app instead of only
