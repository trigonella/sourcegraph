...
const fetchActivationStatus = (
  isSiteAdmin: boolean
): Observable<ActivationCompletionStatus> =>
  queryGraphQL(
    isSiteAdmin
      ? gql`
    ...
    map(data => {
      ...
      const completed: ActivationCompletionStatus = {
        ...
        FoundReferences:
          ...
          usageStats &&
          (usageStats.findReferencesActions > 0 ||
            usageStats.codeIntelligenceActions > 10)
      };
      if (isSiteAdmin) {
        completed.ConnectedCodeHost =
          data.externalServices && data.externalServices.totalCount > 0;
        if (authProviders) {
          completed.EnabledSharing =
            data.users.totalCount > 1 ||
            authProviders.filter(provider => !provider.isBuiltin).length > 0;
        }
      }
      return completed;
    })
  ...
  `).pipe(
    ...
    map(data => {
      ...
      const repositoryURLs = data.repositories.nodes
        .filter(
          repository => repository.gitRefs && repository.gitRefs.totalCount > 0
        )
        .sort(
          (repository1, repository2) =>
            repository2.gitRefs.totalCount - repository1.gitRefs.totalCount
        )
...
const getActivationSteps = (
  ...
  const sources: (ActivationStep & { siteAdminOnly?: boolean })[] = [
    ...
    },
    {
      ...
      detail: (
        <span>
          Head to the <a href="/search">homepage</a> and perform a search query
          on your code. <strong>Example:</strong> type 'lang:' and select a
          language
        </span>
      )
    },
...
export const withActivation = <P extends ActivationProps>(
  ...
  > {
    ...
    private componentUpdates = new Subject<
      Readonly<WithActivationProps & Subtract<P, ActivationProps>>
    >();
    ...
    public componentDidMount(): void {
      ...
      const serverCompletionStatus: Observable<ActivationCompletionStatus> = combineLatest(
        [authenticatedUser, this.refetches.pipe(startWith(undefined))]
      ).pipe(
        switchMap(([authenticatedUser]) =>
          authenticatedUser
            ? fetchActivationStatus(authenticatedUser.siteAdmin)
            : []
        )
      );
      const localCompletionStatus: Observable<Partial<
        ActivationCompletionStatus
      >> = merge(
        ...
        scan<
          Partial<ActivationCompletionStatus> | null,
          Partial<ActivationCompletionStatus>
    ...
    private updateCompletionStatus = (
      update: Partial<ActivationCompletionStatus>
    ): void => this.updates.next(update);
