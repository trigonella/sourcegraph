 ...
 */
export function getHover(context: RepoSpec&ResolvedRevisionSpec&FileSpec&
                         UIPositionSpec,
  ...
  return concat(
      ...
      from(extensionsController.extHostAPI)
          .pipe(switchMap(extensionHost =>
                              wrapRemoteObservable(extensionHost.getHover({
                                textDocument : {
...
export function getDocumentHighlights(
  ...
  return concat(
      [ [] ],
      from(extensionsController.extHostAPI)
          .pipe(switchMap(extensionHost => wrapRemoteObservable(
