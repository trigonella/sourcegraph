    ...
    run: config => {
      const edits = [
        editWithComments(
          ...
          {
            COMMENT:
              "// See https://docs.sourcegraph.com/admin/auth/saml/one_login for instructions"
          }
        )
      ];
    ...
    run: config => {
      ...
      const edits = [
        editWithComments(config, ["auth.providers", -1], value, {
          COMMENT:
            "// See https://docs.sourcegraph.com/admin/auth/saml/okta for instructions"
        })
      ];
      return { edits, selectText: "<identity provider metadata URL>" };
    }
    ...
    run: config => {
      const edits = [
        editWithComments(
          ...
          {
            COMMENT:
              "// See https://docs.sourcegraph.com/admin/auth/saml for instructions"
          }
        )
      ];
    ...
    run: config => {
      const edits = [
        editWithComments(
          ...
          {
            COMMENT:
              "// See https://docs.sourcegraph.com/admin/auth#openid-connect for instructions"
          }
        )
      ];
  ...
  public componentDidMount(): void {
      ...
      this.remoteUpdates
        .pipe(
          ...
          concatMap(newContents => {
            ...
            return updateSiteConfiguration(
              lastConfigurationID,
              newContents
            ).pipe(
              catchError(error => {
              ...
              tap(() => {
                ...
                const lastCampaignsEnabled =
                  (lastConfiguration &&
                    (jsonc.parse(
                      lastConfiguration.effectiveContents
                    ) as SiteConfiguration)?.["campaigns.enabled"]) === true;
                const newCampaignsEnabled =
                  (jsonc.parse(newContents) as SiteConfiguration)?.[
                    "campaigns.enabled"
                  ] === true;
    ...
    this.subscriptions.add(
      this.siteReloads
        .pipe(
          tap(() =>
            this.setState({ reloadStartedAt: Date.now(), error: undefined })
          ),
          mergeMap(reloadSite),
          delay(2000),
          mergeMap(() =>
  ...
  public render(): JSX.Element | null {
    ...
    if (this.state.reloadStartedAt) {
      alerts.push(
        <div
          key="error"
          className="alert alert-primary site-admin-configuration-page__alert"
        >
          <p>
            <LoadingSpinner className="icon-inline" /> Waiting for site to
            reload...
          </p>
          {Date.now() - this.state.reloadStartedAt > EXPECTED_RELOAD_WAIT && (
            <p>
              <small>
                It's taking longer than expected. Check the server logs for
                error messages.
              </small>
            </p>
          )}
    ...
    if (
      ...
      alerts.push(
        <div
          ...
          className="alert alert-danger site-admin-configuration-page__alert"
        >
          <p>The server reported issues in the last-saved config:</p>
          <ul>
            {this.state.site.configuration.validationMessages.map(
              (message, index) => (
                <li
                  key={index}
                  className="site-admin-configuration-page__alert-item"
                >
                  {message}
                </li>
              )
    ...
    if (legacyKubernetesConfigProps.length > 0) {
      alerts.push(
        ...
        >
          ...
          <code>values.yaml</code> config file used for Kubernetes cluster
          deployments of Sourcegraph:{" "}
          <code>{legacyKubernetesConfigProps.join(" ")}</code>. You can
          disregard the validation warnings for these properties reported by the
    ...
    return (
      <div className="site-admin-configuration-page">
        ...
        <p>
          ...
          <Link to="/help/admin/config/site_config">documentation</Link> for
          more information.
        </p>
