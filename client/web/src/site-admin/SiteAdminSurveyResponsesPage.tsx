...
class SurveyResponseNode extends React.PureComponent<
  SurveyResponseNodeProps,
  SurveyResponseNodeState
> {
...
class UserSurveyResponseNode extends React.PureComponent<
  UserSurveyResponseNodeProps,
  UserSurveyResponseNodeState
> {
  public state: UserSurveyResponseNodeState = {
    displayAll: false
  };

  private showMoreClicked = (): void =>
    this.setState(state => ({ displayAll: !state.displayAll }));
        ...
        <tr>
          ...
          <td>
            {this.props.node.usageStatistics?.lastActiveTime ? (
              <Timestamp
                date={this.props.node.usageStatistics.lastActiveTime}
              />
            ) : (
              "?"
            )}
              ...
              {responses.map((response, index) => (
                <dl key={index}>
                  <div className="pl-3 border-left site-admin-survey-responses-connection__wide-border">
                    ...
                    <br />
                    {(response.reason || response.better) && (
                      <div className="mt-2" />
                    )}
                    {response.reason && response.reason !== "" && (
                      <>
                        <dt>
                          What is the most important reason for the score you
                          gave Sourcegraph?
                        </dt>
...
class SiteAdminSurveyResponsesSummary extends React.PureComponent<
  {},
  SiteAdminSurveyResponsesSummaryState
> {
  ...
  constructor(props: {}) {
    ...
    this.subscriptions.add(
      fetchSurveyResponseAggregates().subscribe(summary =>
      ...
      <div className="msite-admin-survey-responses-summary mb-2">
        ...
        <div className="site-admin-survey-responses-summary__container">
          ...
          <SingleValueCard
            ...
            valueClassName={
              anyResults ? `text-${scoreToClassSuffix(roundAvg)}` : ""
            }
          />
          <SingleValueCard
            className="site-admin-survey-responses-summary__item"
...
class FilteredSurveyResponseConnection extends FilteredConnection<
  GQL.ISurveyResponse,
  {}
> {}
class FilteredUserSurveyResponseConnection extends FilteredConnection<
  GQL.IUser,
  {}
> {}
 ...
 */
export class SiteAdminSurveyResponsesPage extends React.Component<
  Props,
  State
> {
  public state: State = {};
  ...
  public render(): JSX.Element | null {
    return (
      <div className="site-admin-survey-responses-page">
        ...
        <p>
          After using Sourcegraph for a few days, users are presented with a
          request to answer "How likely is it that you would recommend
          Sourcegraph to a friend?" on a scale from 0–10 and to provide some
          feedback. Responses are visible below (and are also sent to
          Sourcegraph).
        </p>
