...
function createAccessToken(
  ...
  scopes: string[],
  note: string
): Observable<GQL.ICreateAccessTokenResult> {
 ...
 */
export class UserSettingsCreateAccessTokenPage extends React.PureComponent<
  Props,
  State
> {
  public state: State = {
              ...
              createAccessToken(
                ...
                tap(result => {
                  ...
                  this.props.history.push(
                    `${this.props.match.url.replace(/\/new$/, "")}`
                  );
      ...
      <div className="user-settings-create-access-token-page">
        ...
        {siteAdminViewingOtherUser && (
          <SiteAdminAlert className="sidebar__alert">
            Creating access token for other user{" "}
            <strong>{this.props.user.username}</strong>
          </SiteAdminAlert>
        )}
        <Form onSubmit={this.onSubmit}>
          <div className="form-group">
            <label htmlFor="user-settings-create-access-token-page__note">
              Token description
            </label>
            <input
              type="text"
          ...
          <div className="form-group">
            <label
              className="mb-1"
              htmlFor="user-settings-create-access-token-page__note"
            >
              Token scope
            </label>
            <div>
            ...
            <div className="form-check">
              ...
              >
                <strong>{AccessTokenScopes.UserAll}</strong> — Full control of
                all resources accessible to the user account
              </label>
            </div>
            {this.props.user.siteAdmin && (
              <div className="form-check">
                <input
                  ...
                  checked={this.state.scopes.includes(
                    AccessTokenScopes.SiteAdminSudo
                  )}
                ...
                >
                  <strong>{AccessTokenScopes.SiteAdminSudo}</strong> — Ability
                  to perform any action as any other user
                </label>
              </div>
            )}
          </div>
  ...
  private onScopesChange: React.ChangeEventHandler<
    HTMLInputElement
  > = event => {
    ...
    this.setState(previousState => ({
      scopes: checked
        ? [...previousState.scopes, value]
        : previousState.scopes.filter(scope => scope !== value)
    }));
  };
