...
class UserEmailNode extends React.PureComponent<
  UserEmailNodeProps,
  UserEmailNodeState
> {
  ...
  public render(): JSX.Element | null {
    ...
    if (this.props.node.verified) {
      ...
      verifiedFragment = (
        <span className="badge badge-info">Verification pending</span>
      );
    } else {
      verifiedFragment = (
...
export class UserSettingsEmailsPage extends React.Component<Props, State> {
  ...
  public componentDidMount(): void {
    ...
    this.subscriptions.add(
      siteFlags.subscribe(siteFlags => this.setState({ siteFlags }))
    );
  }
        ...
        {this.state.siteFlags &&
          !this.state.siteFlags.sendsEmailVerificationEmails && (
            <div className="alert alert-warning mt-2">
              Sourcegraph is not configured to send email verifications. Newly
              added email addresses must be manually verified by a site admin.
            </div>
          )}
