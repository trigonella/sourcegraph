...
export const EditUserProfileForm: React.FunctionComponent<Props> = ({
  ...
  const onChange = useCallback<
    React.ComponentProps<typeof UserProfileFormFields>["onChange"]
  ...
  const onSubmit = useCallback<React.FormEventHandler>(
    async event => {
      ...
      try {
        const updatedUser = await requestGraphQL<
          UpdateUserResult,
          UpdateUserVariables
        >(
          gql`
  ...
  return (
    <Form className="w-100" onSubmit={onSubmit}>
      ...
      {isErrorLike(opState) && (
        <div className="mt-3 alert alert-danger">Error: {opState.message}</div>
      )}
      {opState === "success" && (
