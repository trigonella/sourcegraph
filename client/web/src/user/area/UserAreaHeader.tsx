...
export interface UserAreaHeaderNavItem
  extends NavItemWithIconDescriptor<UserAreaHeaderContext> {}

/**
 * Header for the user area.
 */
export const UserAreaHeader: React.FunctionComponent<Props> = ({
  url,
  navItems,
  className = "",
  ...props
        ...
        <>
          <h2 className="user-area-header__title">
            {props.user.avatarURL && (
              <UserAvatar
                className="user-area-header__avatar"
                user={props.user}
              />
            )}
            {props.user.displayName ? (
              <div>
                <div className="user-area-header__title-subtitle">
                  {props.user.username}
                </div>
                <span className="test-user-area-header__display-name">
                  {props.user.displayName}{" "}
                </span>
              </div>
            ) : (
