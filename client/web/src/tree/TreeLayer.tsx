...
export class TreeLayer extends React.Component<TreeLayerProps, TreeLayerState> {
      ...
      this.componentUpdates
        .pipe(
          ...
          switchMap(props => {
            ...
            return merge(
              treeFetch,
              of(LOADING).pipe(delay(300), takeUntil(treeFetch))
            );
          })
        )
        .subscribe(
  ...
  public componentDidUpdate(previousProps: TreeLayerProps): void {
    ...
    if (
      previousProps.activePath !== this.props.activePath &&
    ...
    if (
      this.node === this.props.selectedNode &&
      isDirectory &&
      this.props.onHover
    ) {
    ...
    return (
      <div>
        <table
          ...
          onMouseOver={entryInfo.isDirectory ? this.invokeOnHover : undefined}
        >
          <tbody>
