...
export type DeployType =
    'kubernetes'|'docker-container'|'docker-compose'|'pure-docker'|'dev'

/**
 * Defined in cmd/frontend/internal/app/jscontext/jscontext.go JSContext struct
 */
export interface SourcegraphContext extends
    Pick<Required<SiteConfiguration>, 'experimentalFeatures'> {
  xhrHeaders: {[key: string]: string} csrfToken: string
  userAgentIsBot: boolean

  /**
   * Whether the user is authenticated. Use authenticatedUser in ./auth.ts to
  ...
  /**
   * Externally accessible URL for Sourcegraph (e.g., https://sourcegraph.com
  ...
  site: Pick<SiteConfiguration,
             'auth.public'|'update.channel'|'disableNonCriticalTelemetry'>
  ...
  authProviders: {serviceType: 'github'|'gitlab'|'http-header'|'openidconnect'|
                  'saml'|'builtin'
  displayName: string
  isBuiltin: boolean
        ...
        branding?: {/** The URL of the favicon to be used for your instance */
                    favicon?: string
        ...
        /**
         * Prevents the icon in the top-left corner of the screen from
         * spinning.
         */
