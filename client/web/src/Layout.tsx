...
import {
  AuthenticatedUser,
  authRequired as authRequiredObservable
} from "./auth";
...
export const Layout: React.FunctionComponent<LayoutProps> = props => {
  const routeMatch = props.routes.find(({ path, exact }) =>
    matchPath(props.location.pathname, { path, exact })
  )?.path;
  const isSearchRelatedPage =
    (routeMatch === "/:repoRevAndRest+" || routeMatch?.startsWith("/search")) ??
    false;
  ...
  if (
    ...
    return (
      <Redirect
        to={{
          ...props.location,
          pathname: props.location.pathname.slice(0, -1)
        }}
      />
    );
  }
      ...
      {parseHash(props.location.hash).viewState &&
        props.location.pathname !== "/sign-in" && (
          <ResizablePanel
            {...props}
            repoName={`git://${
              parseBrowserRepoURL(props.location.pathname).repoName
            }`}
