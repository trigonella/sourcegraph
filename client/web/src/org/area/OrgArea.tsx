...
export class OrgArea extends React.Component<Props> {
  ...
  public componentDidMount(): void {
    ...
    this.subscriptions.add(
      combineLatest([
        nameChanges,
        merge(this.refreshRequests.pipe(mapTo(false)), of(true))
      ])
        .pipe(
          switchMap(([name, forceRefresh]) => {
            ...
            return queryOrganization({ name }).pipe(
              ...
              startWith<PartialStateUpdate>(
                forceRefresh ? { orgOrError: undefined } : {}
              )
            );
          })
        )
        .subscribe(
          stateUpdate => {
            if (
              stateUpdate.orgOrError &&
              !isErrorLike(stateUpdate.orgOrError)
            ) {
              const childBreadcrumbSetters = this.props.setBreadcrumb({
                key: "OrgArea",
                link: {
                  to: stateUpdate.orgOrError.url,
                  label: stateUpdate.orgOrError.name
                }
              });
