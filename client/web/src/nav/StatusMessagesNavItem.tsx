...
export function fetchAllStatusMessages(): Observable<
  StatusMessagesResult["statusMessages"]
> {
...
export class StatusMessagesNavItem extends React.PureComponent<Props, State> {
    ...
    this.subscriptions.add(
      (this.props.fetchMessages ?? fetchAllStatusMessages)()
        .pipe(
          ...
          repeatUntil(messagesOrError => isErrorLike(messagesOrError), {
            delay: REFRESH_INTERVAL_MS
          }),
          repeatWhen(completions =>
            completions.pipe(delay(REFRESH_INTERVAL_AFTER_ERROR_MS))
          ),
  ...
  private renderIcon(): JSX.Element | null {
    ...
    if (
      this.state.messagesOrError.some(
        ({ __typename }) => __typename === "ExternalServiceSyncError"
      )
    ) {
      return (
        <CloudAlertIcon
          ...
          data-tooltip={
            this.state.isOpen ? undefined : "Syncing repositories failed!"
          }
        />
      );
    }
    if (
      this.state.messagesOrError.some(
        ({ __typename }) => __typename === "CloningProgress"
      )
    ) {
      return (
        <CloudSyncIcon
          className="icon-inline"
          data-tooltip={
            this.state.isOpen ? undefined : "Cloning repositories..."
          }
        />
      );
    }
  ...
  public render(): JSX.Element | null {
    return (
      ...
      >
        ...
        <DropdownMenu
          right={true}
          className="status-messages-nav-item__dropdown-menu"
        >
