...
export const GlobalNavbar: React.FunctionComponent<Props> = ({
  ...
  const query = useMemo(() => parseSearchURLQuery(location.search || ""), [
    location.search
  ]);
  ...
  const logo = (
    <LinkOrSpan
      to={authRequired ? undefined : "/search"}
      className="global-navbar__logo-link"
    >
      <BrandLogo
        branding={branding}
          ...
          {authRequired ? (
            ...
            <div className="global-navbar__search-box-container d-none d-sm-flex flex-row">
              {splitSearchModes && (
                <SearchModeToggle
                  {...props}
                  interactiveSearchMode={interactiveSearchMode}
                />
              )}
