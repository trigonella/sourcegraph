...
type Props<P> = P & {
  component: React.ComponentType<P & ReactStripeElements.InjectedStripeProps>;
};

/**
 * Wraps a React tree (of elements) and injects the Stripe API.
 */
export const StripeWrapper = <P extends object>(
  props: Props<P>
): JSX.Element | null => {
  ...
  useEffect(() => {
    ...
    const initStripe = (): void =>
      setStripe((window as any).Stripe(billingPublishableKey));
  ...
  >(() => props.component);
  const [InjectedComponent, setInjectedComponent] = useState<
