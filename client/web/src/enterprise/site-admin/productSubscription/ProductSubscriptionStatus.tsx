 ...
 */
export const ProductSubscriptionStatus: React.FunctionComponent<Props> = ({
  className,
  showTrueUpStatus,
  history
}) => {
  ...
  if (isErrorLike(statusOrError)) {
    return (
      <ErrorAlert
        error={statusOrError}
        prefix="Error checking product license"
        history={history}
      />
    );
  }
            ...
            {license ? (
              <>
                <div>
                  ...
                  {numberWithCommas(actualUserCount)} used /{" "}
                  {numberWithCommas(license.userCount - actualUserCount)}{" "}
                  remaining
                </div>
                <a
