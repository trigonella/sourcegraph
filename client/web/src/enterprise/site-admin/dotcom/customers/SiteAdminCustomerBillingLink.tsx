 ...
 */
export const SiteAdminCustomerBillingLink: React.FunctionComponent<Props> = ({
  customer,
  onDidUpdate
}) => {
  ...
  const onLinkBillingClick = useCallback(() => {
    const billingCustomerID = window.prompt(
      "Enter new Stripe billing customer ID:",
      "cus_ABCDEF12345678"
    );
  ...
  const onUnlinkBillingClick = useCallback(
    () => nextUpdate({ user: customer.id, billingCustomerID: null }),
    [customer.id, nextUpdate]
  );
    ...
    <div className="site-admin-customer-billing-link">
      <div className="d-flex align-items-center">
        {customer.urlForSiteAdminBilling && (
          <a
            href={customer.urlForSiteAdminBilling}
        ...
        <button
          ...
          onClick={
            customerHasLinkedBilling ? onUnlinkBillingClick : onLinkBillingClick
          }
...
function setCustomerBilling(
  args: GQL.ISetUserBillingOnDotcomMutationArguments
): Observable<void> {
