...
export const SiteAdminProductSubscriptionPage: React.FunctionComponent<Props> = ({
        ...
        return archivals.pipe(
          ...
          switchMap(() =>
            archiveProductSubscription({ id: productSubscription.id }).pipe(
              mapTo(undefined),
              tap(() =>
                history.push("/site-admin/dotcom/product/subscriptions")
              ),
              catchError(error => [asError(error)]),
              startWith(LOADING)
            )
  ...
  const toggleShowGenerate = useCallback(
    (): void => setShowGenerate(previousValue => !previousValue),
    []
  );
      ...
      {productSubscription === LOADING ? (
        ...
        <>
          ...
          <div className="mb-3">
            ...
            {isErrorLike(archival) && (
              <ErrorAlert className="mt-2" error={archival} history={history} />
            )}
          </div>
                  ...
                  <td className="w-100">
                    {productSubscription.account && (
                      <AccountEmailAddresses
                        emails={productSubscription.account.emails}
                      />
                    )}
                  </td>
                </tr>
            ...
            <div className="card-header d-flex align-items-center justify-content-between">
              ...
              {showGenerate ? (
                ...
                <button
                  ...
                  onClick={toggleShowGenerate}
                >
                  <AddIcon className="icon-inline" /> Generate new license
                  manually
                </button>
              )}
            </div>
...
function queryProductSubscription(
  uuid: string
): Observable<GQL.IProductSubscription> {
  return queryGraphQL(
    gql`
...
function archiveProductSubscription(
  args: GQL.IArchiveProductSubscriptionOnDotcomMutationArguments
): Observable<void> {
  return mutateGraphQL(
    ...
    map(({ data, errors }) => {
      if (
        !data ||
        !data.dotcom ||
        !data.dotcom.archiveProductSubscription ||
        (errors && errors.length > 0)
      ) {
        throw createAggregateError(errors);
      }
