...
export type UploadConnection = Omit<
  GQL.ILSIFUploadConnection,
  "__typename" | "nodes"
> & {
  nodes: Upload[];
};
...
export function fetchLsifUploads({
  ...
  first,
  after
}: { repository?: string } & GQL.ILsifUploadsOnRepositoryArguments): Observable<
  UploadConnection
> {
  if (!repository) {
    return queryGraphQL(
  ...
  return queryGraphQL(
    ...
    map(({ node }) => {
      ...
      if (node.__typename !== "Repository") {
        throw new Error(
          `The given ID is a ${node.__typename}, not a Repository`
        );
      }
  ...
  return queryGraphQL(
    ...
    map(({ node }) => {
      ...
      if (node.__typename !== "LSIFUpload") {
        throw new Error(
          `The given ID is a ${node.__typename}, not an LSIFUpload`
        );
      }
...
export type IndexConnection = Omit<
  GQL.ILSIFIndexConnection,
  "__typename" | "nodes"
> & {
  nodes: Index[];
};
...
export function fetchLsifIndexes({
  ...
  first,
  after
}: { repository?: string } & GQL.ILsifIndexesOnRepositoryArguments): Observable<
  IndexConnection
> {
  if (!repository) {
    return queryGraphQL(
...
export function fetchLsifIndex({
  id
}: {
  id: string;
  ...
  return queryGraphQL(
    ...
    map(({ node }) => {
      ...
      if (node.__typename !== "LSIFIndex") {
        throw new Error(
          `The given ID is a ${node.__typename}, not an LSIFIndex`
        );
      }
