  ...
  return (
    <>
      <div className="alert alert-info p-3 mb-3 d-block d-md-flex align-items-center body-lead">
        ...
        {!campaign && (
          <p className="mb-0 flex-grow-1 mr-3 create-update-campaign-alert__copy">
            Review the proposed changesets below. Click 'Apply spec' or run 'src
            campaigns apply' against your campaign spec to create the campaign
            and perform the indicated action on each changeset.
          </p>
        )}
        {campaign && (
          <p className="mb-0 flex-grow-1 mr-3 create-update-campaign-alert__copy">
            This operation will update the existing campaign{" "}
            <Link to={campaign.url}>{campaign.name}</Link>. Click 'Apply spec'
            or run 'src campaigns apply' against your campaign spec to update
        ...
        <div className="create-update-campaign-alert__btn">
          <button
            ...
            data-tooltip={
              !viewerCanAdminister
                ? "You have no permission to apply this campaign."
                : undefined
            }
          >
            Apply spec
      ...
      {isErrorLike(isLoading) && (
        <ErrorAlert error={isLoading} history={history} />
      )}
    </>
  );
};
