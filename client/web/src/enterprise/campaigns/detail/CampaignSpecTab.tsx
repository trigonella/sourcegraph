...
export interface CampaignSpecTabProps {
  campaign: Pick<
    CampaignFields,
    "name" | "createdAt" | "lastApplier" | "lastAppliedAt"
  >;
...
export const CampaignSpecTab: React.FunctionComponent<CampaignSpecTabProps> = ({
  ...
  const downloadUrl = useMemo(
    () => "data:text/plain;charset=utf-8," + encodeURIComponent(originalInput),
    [originalInput]
  );
  ...
  const input = useMemo(
    () =>
      inputIsJSON
        ? JSON.stringify(JSON.parse(originalInput), null, 2)
        : originalInput,
    ...
    <>
      <div className="d-flex flex-wrap justify-content-between align-items-baseline mb-2 test-campaigns-spec">
        <p className="mb-2 campaign-spec-tab__header-col">
          ...
          <Timestamp date={lastAppliedAt} /> by applying the following campaign
          spec:
        </p>
      ...
      <CodeSnippet
        code={input}
        language={inputIsJSON ? "json" : "yaml"}
        className="mb-3"
      />
    </>
  );
