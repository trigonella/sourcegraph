  ...
  <>
    <select
      className={classNames(
        "form-control changeset-filter__dropdown",
        className
      )}
      value={selected}
      onChange={event =>
...
export const ChangesetUIStateFilter: React.FunctionComponent<ChangesetUIStateFilterProps> = ({
  ...
  <>
    ...
    <div
      className={classNames("d-none d-lg-block btn-group flex-wrap", className)}
      role="group"
    >
      <button
        type="button"
...
function changesetUIStateToChangesetFilters(
  ...
  switch (state) {
    ...
    case ChangesetUIState.PROCESSING:
      return {
        reconcilerState: [
          ChangesetReconcilerState.QUEUED,
          ChangesetReconcilerState.PROCESSING
        ],
        externalState: null,
        publicationState: null
      };
