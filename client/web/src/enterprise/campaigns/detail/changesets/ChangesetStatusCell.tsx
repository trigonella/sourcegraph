...
export interface ChangesetStatusCellProps {
  ...
  changeset: Pick<
    ChangesetFields,
    "publicationState" | "externalState" | "reconcilerState"
  >;
}
...
const iconClassNames =
  "m-0 text-nowrap flex-column align-items-center justify-content-center";
...
export const ChangesetStatusClosed: React.FunctionComponent<{
  label?: JSX.Element;
  className?: string;
}> = ({ label = <span className="text-muted">Closed</span>, className }) => (
  <div className={classNames(iconClassNames, "text-danger", className)}>
    <SourcePullIcon />
    {label}
  </div>
);
export const ChangesetStatusMerged: React.FunctionComponent<{
  label?: JSX.Element;
  className?: string;
}> = ({ label = <span className="text-muted">Merged</span>, className }) => (
  <div className={classNames(iconClassNames, "text-merged", className)}>
    <SourceMergeIcon />
    {label}
  </div>
);
export const ChangesetStatusOpen: React.FunctionComponent<{
  label?: JSX.Element;
  className?: string;
}> = ({ label = <span className="text-muted">Open</span>, className }) => (
  <div className={classNames(iconClassNames, "text-success", className)}>
    <SourcePullIcon />
    {label}
  </div>
);
export const ChangesetStatusDraft: React.FunctionComponent<{
  label?: JSX.Element;
  className?: string;
}> = ({ label = <span className="text-muted">Draft</span>, className }) => (
  <div className={classNames(iconClassNames, "text-muted", className)}>
    <SourcePullIcon />
    {label}
  </div>
);
export const ChangesetStatusDeleted: React.FunctionComponent<{
  label?: JSX.Element;
  className?: string;
}> = ({ label = <span className="text-muted">Deleted</span>, className }) => (
  <div className={classNames(iconClassNames, "text-muted", className)}>
    <DeleteIcon />
    {label}
  </div>
);
export const ChangesetStatusError: React.FunctionComponent<{
  label?: JSX.Element;
  className?: string;
}> = ({ label = <span className="text-danger">Error</span>, className }) => (
  <div className={classNames(iconClassNames, "text-danger", className)}>
    <ErrorIcon />
    {label}
  </div>
