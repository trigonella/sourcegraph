...
export const CampaignCloseAlert: React.FunctionComponent<CampaignCloseAlertProps> = ({
  ...
  const onChangeCloseChangesets = useCallback<
    ...
    <>
      <div className="card mb-3">
        <div className="card-body p-3">
          <p>
            <strong>
              After closing this campaign, it will be read-only and no new
              campaign specs can be applied.
            </strong>
          </p>
          {totalCount > 0 && (
            <>
          ...
          <div className="d-flex justify-content-end">
            ...
            <button
              ...
              disabled={isClosing === true || !viewerCanAdminister}
            >
              {isClosing === true && <LoadingSpinner className="icon-inline" />}{" "}
              Close campaign
            </button>
          </div>
        </div>
      </div>
      {isErrorLike(isClosing) && (
        <ErrorAlert error={isClosing} history={history} />
      )}
    </>
  );
};
