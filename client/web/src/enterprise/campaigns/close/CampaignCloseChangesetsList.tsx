...
import {
  ChangesetCloseNodeProps,
  ChangesetCloseNode
} from "./ChangesetCloseNode";
...
import {
  CampaignCloseHeaderWillCloseChangesets,
  CampaignCloseHeaderWillKeepChangesets
} from "./CampaignCloseHeader";
...
export const CampaignCloseChangesetsList: React.FunctionComponent<Props> = ({
  ...
  const nextContainerElement = useMemo(
    () => containerElements.next.bind(containerElements),
    [containerElements]
  );

  const hoverOverlayElements = useMemo(
    () => new Subject<HTMLElement | null>(),
    []
  );
  ...
  const nextCloseButtonClick = useCallback(
    (event: MouseEvent): void => closeButtonClicks.next(event),
    [closeButtonClicks]
  );
    ...
    () =>
      ...
      >({
        ...
        getHover: hoveredToken =>
          getHover(getLSPTextDocumentPositionParameters(hoveredToken), {
            extensionsController
          }),
        getDocumentHighlights: hoveredToken =>
          getDocumentHighlights(
            getLSPTextDocumentPositionParameters(hoveredToken),
            { extensionsController }
          ),
        getActions: context =>
  ...
  const hoverState = useObservable(
