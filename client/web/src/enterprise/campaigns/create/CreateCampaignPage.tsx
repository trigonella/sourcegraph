...
const SampleTabHeader: React.FunctionComponent<SampleTabHeaderProps> = ({
  sample,
  active,
  setSelectedSample
}) => {
...
export const CreateCampaignPage: React.FunctionComponent<CreateCampaignPageProps> = ({
  ...
  useBreadcrumb(
    useMemo(
      () => ({ element: <>Create campaign</>, key: "createCampaignPage" }),
      []
    )
  );
  return (
    <>
      ...
      <div className="pt-3">
        ...
        <p>
          ...
          ) describes what the campaign does. You'll provide it when previewing,
          creating, and updating campaigns. We recommend committing it to source
          control.
        </p>
        <h4>Examples:</h4>
        ...
        />
        <p>
          Follow the URL printed in your terminal to see the preview and (when
          you're ready) create the campaign.
        </p>
        <hr className="mt-4" />
        <p className="text-muted">
          Want more help? See{" "}
          <a
            href="/help/user/campaigns"
            rel="noopener noreferrer"
            target="_blank"
          >
