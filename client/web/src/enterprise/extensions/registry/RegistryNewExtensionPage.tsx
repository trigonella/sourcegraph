...
} from "../extension/RegistryExtensionForm";
...
interface State {
  ...
  creationOrError?:
    | "loading"
    | GQL.IExtensionRegistryCreateExtensionResult
    | ErrorLike;
}
  ...
  class RegistryNewExtensionPage extends React.PureComponent<Props, State> {
    ...
    public componentDidMount(): void {
      ...
      this.subscriptions.add(
        concat(
          ...
          queryViewerRegistryPublishers().pipe(
            map(result => ({
              publishersOrError: result,
              publisher: result[0]?.id
            })),
      ...
      this.subscriptions.add(
        this.props.setBreadcrumb({
          key: "create-new-extension",
          element: <>Create new extension</>
        })
      );
    ...
    public render(): JSX.Element | null {
      ...
      if (
        ...
        const publisher = this.state.publishersOrError.find(
          publisher => publisher.id === this.state.publisher
        );
        if (publisher) {
    ...
    private onPublisherChange: React.ChangeEventHandler<
      HTMLSelectElement
    > = event => this.setState({ publisher: event.currentTarget.value });
