...
export const ProductSubscriptionBeforeAfterInvoiceItem: React.FunctionComponent<{
  ...
  const planChanged =
    beforeInvoiceItem.plan.billingPlanID !==
    afterInvoiceItem.plan.billingPlanID;
  const userCountChanged =
    beforeInvoiceItem.userCount !== afterInvoiceItem.userCount;
  return !planChanged && !userCountChanged ? (
    ...
    <ul className={`pl-3 ${className}`}>
      {planChanged && (
        <li>
          {afterInvoiceItem.plan.pricePerUserPerYear >
          beforeInvoiceItem.plan.pricePerUserPerYear
            ? "Upgrade"
            : "Downgrade"}{" "}
          plan from {beforeInvoiceItem.plan.name} to{" "}
          {afterInvoiceItem.plan.name}.
        </li>
      )}
      {userCountChanged && (
        <li>
          {afterInvoiceItem.userCount > beforeInvoiceItem.userCount
            ? "Increase"
            : "Decrease"}{" "}
          user count from {numberWithCommas(beforeInvoiceItem.userCount)} to{" "}
          {numberWithCommas(afterInvoiceItem.userCount)}.
        </li>
      )}
      <li>
        ...
        <ExpirationDate
          showTime={false}
          date={parseISO(afterInvoiceItem.expiresAt)}
          lowercase={true}
        />
        ).
      </li>
