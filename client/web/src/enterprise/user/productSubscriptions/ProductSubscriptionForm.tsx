 ...
 */
const _ProductSubscriptionForm: React.FunctionComponent<Props &
  ...
  const [billingPlanID, setBillingPlanID] = useState<string | null>(
    initialValue?.billingPlanID || null
  );

  /** The user count input by the user. */
  const [userCount, setUserCount] = useState<number | null>(
    initialValue?.userCount || DEFAULT_USER_COUNT
  );

  /** The validity of the payment and billing information. */
  const [paymentValidity, setPaymentValidity] = useState<PaymentValidity>(
          ...
          switchMap(() =>
            ...
            (paymentValidity !== PaymentValidity.NoPaymentRequired
              ...
              switchMap(({ token, error }) => {
                ...
                if (!accountID) {
                  return throwError(
                    new Error("no account (unauthenticated user)")
                  );
                }
                if (!billingPlanID) {
  ...
  return (
    <div className="product-subscription-form">
            ...
            {!accountID && (
              <div className="form-group mt-3">
                ...
                <small className="form-text text-muted">
                  A user account on Sourcegraph.com is required to create a
                  subscription so you can view the license key and invoice.
                </small>
                <hr className="my-3" />
                <small className="form-text text-muted">
                  Next, you'll enter payment information and buy the
                  subscription.
                </small>
              </div>
            )}
            ...
            <div className="form-group mt-3">
              ...
              >
                {paymentToken === LOADING || submissionState === LOADING ? (
                  <>
                    <LoadingSpinner className="icon-inline mr-2" />{" "}
                    Processing...
                  </>
                ) : paymentValidity !== PaymentValidity.NoPaymentRequired ? (
                  primaryButtonText
      ...
      {isErrorLike(paymentToken) && (
        <ErrorAlert className="mt-3" error={paymentToken} history={history} />
      )}
      {isErrorLike(submissionState) && (
        <ErrorAlert
          className="mt-3"
