...
export const summarizeSearchResultsStatsLanguages = (
  ...
  const totalLines = languages.reduce(
    (sum, language) => sum + language.totalLines,
    0
  );
  const minLines = minFraction * totalLines;
  const languagesAboveMin = languages.filter(
    language => language.totalLines >= minLines
  );
  const otherLines =
    totalLines -
    languagesAboveMin.reduce((sum, language) => sum + language.totalLines, 0);
  return [
    ...languagesAboveMin,
    {
 ...
 */
export const SearchStatsLanguages: React.FunctionComponent<Props> = ({
  query,
  stats
}) => {
  const chartData = summarizeSearchResultsStatsLanguages(
    stats.languages,
    0.02
  ).map((language, index) => ({
    ...language,
  ...
  const totalLines = stats.languages.reduce(
    (sum, language) => sum + language.totalLines,
    0
  );
  ...
  const percent = useCallback(
    (lines: number) =>
      totalLines !== undefined && totalLines !== 0
        ? `${Math.round((100 * lines) / totalLines)}%`
        : "",
    [totalLines]
  );
  ...
  const tooltipFormatter = useCallback<TooltipFormatter>(
    value =>
      typeof value === "number"
        ? `${numberWithCommas(value)} ${pluralize("line", value)}`
        : "",
    []
  );
