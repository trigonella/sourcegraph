 ...
 */
export function isProductLicenseExpired(expiresAt: string|number|
                                        Date): boolean {
  return !isAfter(typeof expiresAt === 'string' ? parseISO(expiresAt)
                                                : expiresAt,
                  Date.now())
}
...
export function formatRelativeExpirationDate(date: string|number|Date): string {
  return `${
      formatDistanceStrict(
          ...
          Date.now())} ${isProductLicenseExpired(date) ? 'ago' : 'remaining'}`
}
...
export function mailtoSales(args: {subject?: string}): string {
  return `mailto:sales@sourcegraph.com?subject=${
