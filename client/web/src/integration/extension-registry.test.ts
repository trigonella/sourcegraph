...
const wordCountBundleUrl =
    'https://sourcegraph.com/-/static/extension/sqs-word-count.js'
...
const registryExtensionNodes: RegistryExtensionFieldsForList[] =
    ...
    const extensionNodes:
        ExtensionsResult['extensionRegistry']['extensions']['nodes'] = [
          {
            extensionID : 'sourcegraph/typescript',
            id : 'test-extension-1',
            manifest : {
...
afterEach(() => testContext?.dispose())
        ...
        subjects : [
          ...
          },
          {
            ...
            latestSettings : {
              id : 310,
              contents :
                  JSON.stringify({extensions : {'sqs/word-count' : enabled}}),
            },
          },
        ],
  ...
  testContext.server.get(wordCountBundleUrl)
      .intercept((request, response) => {
                     response.type('application/javascript; charset=utf-8')
                         .send('exports.activate = () => {}')})
    }
    ...
    assert(!(await driver.page.$(
        ...
        it('only shows extensions from the selected categories', async () => {
            ...
            await driver.page.waitForSelector('[data-test="extension-toggle-sqs/word-count"]')
        assert(await driver.page.$(
                   '[data-test="extension-toggle-sqs/word-count"]'),
    ...
    const request = await testContext.waitForGraphQLRequest(
        async () => {await driver.replaceText({
