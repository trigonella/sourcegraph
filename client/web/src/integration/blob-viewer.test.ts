...
const files = [ 'README.md', fileName ]
    ...
    const commonBlobGraphQlResults:
        Partial<WebGraphQlOperations&SharedGraphQlOperations> = {
          ...
          ResolveRev : () =>
              createResolveRevisionResult(repositorySourcegraphUrl),
          FileExternalLinks : ({filePath}) => createFileExternalLinksResult(
              `https://${repositoryName}/blob/master/${filePath}`),
          TreeEntries : () => createTreeEntriesResult(
              repositorySourcegraphUrl, [ 'README.md', fileName ]),
          Blob : ({filePath}) => createBlobContentResult(
    ...
    assert.strictEqual(blobContent,
    ...
    // collect all files/links visible the the "Files" tab
            ...
            // Describes the ways the blob viewer can be extended through
            // Sourcegraph extensions.
    describe('extensibility', () => {
        const getHoverContents = async (): Promise<string[]> => {
  // Search for any child of e2e-tooltip-content: as e2e-tooltip-content has
  ...
  function activate(context: sourcegraph.ExtensionContext):
      void{
          context.subscriptions.add(sourcegraph.languages.registerHoverProvider(
              [ {language : 'typescript'} ], {
                provideHover : () => ({
                  contents : {
  ...
  exports.activate = activate
                    ...
                    it('truncates long file paths properly', async function() {
                      await driver.page
                          .goto(`${driver.sourcegraphBaseUrl}/${
                              repositoryName}/-/blob/this_is_a_long_file_path/apps/rest-showcase/src/main/java/org/demo/rest/example/OrdersController.java`)
                              await driver.page
                          .waitForSelector('.test-repo-blob')
                              await driver.page.waitForSelector(
                                  '.test-breadcrumb')
                                  await percySnapshot(driver.page,
                                                      this.test!.fullTitle())
                    })

                    it.skip(
                        'shows a hover overlay from a hover provider when a token is hovered',
                        async () => {
                            await driver.page
                                .goto(`${driver.sourcegraphBaseUrl}/${
                                    repositoryName}/-/blob/${fileName}`)
                                    await driver.page.waitForSelector(
                                        '.test-repo-blob')
                            // TODO
                        })

                    it('shows a hover overlay from a hover provider and updates the URL when a token is clicked',
                       async function() {
                         await driver.page
                             .goto(`${
                                 driver
                                     .sourcegraphBaseUrl}/github.com/sourcegraph/test/-/blob/test.ts`)

                             // Click on "log" in "console.log()" in line 2
                             await driver.page
                             .waitForSelector('.test-log-token',
                                              {visible : true})
                                 await driver.page
                             .click('.test-log-token')

                                 await driver
                             .assertWindowLocation(
                                 '/github.com/sourcegraph/test/-/blob/test.ts#L2:9')
                         assert.deepStrictEqual(await getHoverContents(),
                                                [ 'Test hover content\n' ])
                             await percySnapshot(driver.page,
                                                 this.test!.fullTitle())
                       })
                    ...
                    it.skip(
                        ...
                        async () => {
                            await driver.page.goto(`${
                                driver
                                    .sourcegraphBaseUrl}/github.com/sourcegraph/test/-/test.ts#2:9`)
                            // TODO
                        })
        ...
        it(`shows a popover about the browser extension when the user has seen ${
               HOVER_THRESHOLD} hovers and clicks "View on [code host]" button`,
           async () => {
             await driver.page
                 .goto(`${
                     driver
                         .sourcegraphBaseUrl}/github.com/sourcegraph/test/-/blob/test.ts`)
                     await driver.page
                 .evaluate(() => localStorage.removeItem('hover-count'))
                     await driver.page
                 .reload()
             ...
             const newPage =
                 new Promise<Page>(
                     resolve => driver.browser.once(
                         'targetcreated', target => resolve(target.page())))
                     await driver.page
                         .click('.test-go-to-code-host', {button : 'middle'})
                             await (await newPage)
                         .close()

             assert(
                 !(await driver.page.$(
                     '.test-install-browser-extension-popover')),
                 'Expected popover to not be displayed before user reaches hover threshold')

                 // Click 'console' and 'log' 5 times combined
                 await driver.page.waitForSelector('.test-log-token',
                                                   {visible : true})
             for (let index = 0; index < HOVER_THRESHOLD; index++) {
               await driver.page
                   .click(index % 2 === 0 ? '.test-log-token'
                                          : '.test-console-token')
                       await driver.page.waitForSelector('.hover-overlay',
                                                         {visible : true})
             }
             ...
             const popoverHeader = await driver.page.evaluate(
                 () => document
                           .querySelector(
                               '.test-install-browser-extension-popover-header')
                           ?.textContent)
             assert.strictEqual(
                 popoverHeader,
  ...
  await driver.page
      .goto(`${
          driver
              .sourcegraphBaseUrl}/github.com/sourcegraph/test/-/blob/test.ts`)
          await driver.page
      .evaluate(HOVER_COUNT_KEY => localStorage.removeItem(HOVER_COUNT_KEY),
                HOVER_COUNT_KEY) await driver.page.reload()
  ...
  for (let index = 0; index < HOVER_THRESHOLD; index++) {
    await driver.page
        .click(index % 2 === 0 ? '.test-log-token' : '.test-console-token')
            await driver.page.waitForSelector('.hover-overlay',
                                              {visible : true})
  }
  await driver.page
      ...
      // Alert should be visible now that the user has seen $HOVER_THRESHOLD
      // hovers
      await driver.page
      .waitForSelector('.install-browser-extension-alert', {timeout : 5000})

      // Dismiss alert
      await driver.page.click('.test-close-alert') await driver.page
      .reload()

      // Alert should not show up now that the user has dismissed it once
      await driver.page
      .waitForSelector('.repo-header')
      ...
      // After this point, we know whether or not the alert will be displayed
      // for this page load.
