...
export const DiffStat: React.FunctionComponent<Props> = React.memo(
  function DiffStat({
    added,
    changed,
    deleted,
    ...
    const squares = new Array<
      "bg-success" | "bg-warning" | "bg-danger" | "diff-stat__empty"
    >(addedSquares)
      .fill("bg-success")
      .concat(
        new Array<"bg-warning">(changedSquares).fill("bg-warning"),
        new Array<"bg-danger">(deletedSquares).fill("bg-danger"),
        new Array<"diff-stat__empty">(NUM_SQUARES - numberOfSquares).fill(
          "diff-stat__empty"
        )
      );
    ...
    if (changed > 0) {
      labels.push(
        `${numberWithCommas(changed)} ${pluralize("change", changed)}`
      );
    }
    if (deleted > 0) {
      labels.push(
        `${numberWithCommas(deleted)} ${pluralize("deletion", deleted)}`
      );
    }
    return (
      <div
      ...
      >
        {expandedCounts ? (
          <span className="diff-stat__total font-weight-bold">
            ...
            <span
              className={classNames("text-danger", !separateLines && "mr-1")}
            >
              &minus;{numberWithCommas(deleted)}
            </span>
          </span>
        ) : (
          <small className="diff-stat__total">
            {numberWithCommas(total + changed)}
          </small>
        )}
        <div>
