...
export class FileDiffConnection extends React.PureComponent<Props> {
  public render(): JSX.Element | null {
    return (
      <FilteredFileDiffConnection {...this.props} onUpdate={this.onUpdate} />
    );
  }

  private onUpdate = (
    fileDiffsOrError: Connection<FileDiffFields> | ErrorLike | undefined
  ): void => {
      ...
      if (fileDiffsOrError && !isErrorLike(fileDiffsOrError)) {
        for (const fileDiff of fileDiffsOrError.nodes) {
          if (fileDiff.oldPath && nodeProps.extensionInfo) {
            ...
            if (
              !nodeProps.extensionInfo.extensionsController.services.model.hasModel(
                uri
              )
            ) {
