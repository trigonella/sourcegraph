...
const DiffBoundary: React.FunctionComponent<DiffBoundaryProps> = props => (
  <tr className="diff-boundary">
    {props.lineNumbers && (
      <td
        className={`diff-boundary__num ${props.lineNumberClassName}`}
        colSpan={2}
      />
    )}
    <td
      className={`diff-boundary__content ${props.contentClassName}`}
      data-diff-marker=" "
    >
      {props.oldRange.lines !== undefined &&
        props.newRange.lines !== undefined && (
          <code>
            @@ -{props.oldRange.startLine},{props.oldRange.lines} +
            {props.newRange.startLine},{props.newRange.lines}{" "}
        ...
        return (
          <tr
            ...
            className={`diff-hunk__line ${
              line.kind === DiffHunkLineType.UNCHANGED
                ? "diff-hunk__line--both"
                : ""
            } ${
              line.kind === DiffHunkLineType.DELETED
                ? "diff-hunk__line--deletion"
                : ""
            } ${
              line.kind === DiffHunkLineType.ADDED
                ? "diff-hunk__line--addition"
                : ""
            } ${
              (line.kind !== DiffHunkLineType.ADDED &&
          ...
          >
              ...
              <>
                {line.kind !== DiffHunkLineType.ADDED ? (
                  <td
                    ...
                    id={oldAnchor}
                    onClick={() =>
                      persistLines && history.push({ hash: oldAnchor })
                    }
                  />
                ) : (
                ...
                {line.kind !== DiffHunkLineType.DELETED ? (
                  <td
                    ...
                    id={newAnchor}
                    onClick={() =>
                      persistLines && history.push({ hash: newAnchor })
                    }
                  />
                ) : (
                  <td className="diff-hunk__num diff-hunk__num--empty" />
            ...
            >
              ...
              />
              {decorationsForLine
                .filter(property("after", isDefined))
                .map((decoration, index) => {
                  const style = decorationAttachmentStyleForTheme(
                    decoration.after,
                    isLightTheme
                  );
                  return (
