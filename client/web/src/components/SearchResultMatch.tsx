...
export class SearchResultMatch extends React.Component<
  SearchResultMatchProps,
  SearchResultMatchState
> {
  ...
  private bodyIsCode(): boolean {
    return (
      this.props.item.body.text.startsWith("```") &&
      this.props.item.body.text.endsWith("```")
    );
  }
        ...
        .pipe(
          ...
          switchMap(([props]) => {
            ...
            if (this.bodyIsCode()) {
              ...
              const codeContentAndAnyNewLines = new RegExp(
                escapeRegExp(codeContent) + "\\n*"
              );
              if (codeContent) {
          ...
          catchError(() =>
            of("<pre>" + sanitizeHtml(props.item.body.text) + "</pre>")
          )
        )
        .subscribe(
  ...
  private highlightNodes(): void {
    if (this.tableContainerElement) {
      const visibleRows = this.tableContainerElement.querySelectorAll(
        "table tr"
      );
  ...
  private getFirstLine(): number {
    ...
    return Math.max(
      0,
      Math.min(...this.props.highlightRanges.map(range => range.line)) - 1
    );
  }
  ...
  private getLastLine(): number {
    ...
    const lastLine =
      Math.max(...this.props.highlightRanges.map(range => range.line)) + 1;
    return this.props.highlightRanges
      ? Math.min(lastLine, this.props.highlightRanges.length)
      : lastLine;
  }
