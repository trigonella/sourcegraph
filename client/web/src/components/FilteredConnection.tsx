...
class FilteredConnectionFilterControl extends React.PureComponent<
  FilterProps,
  FilterState
> {
...
class ConnectionNodes<
  ...
  N,
  NP = {}
> extends React.PureComponent<ConnectionNodesProps<C, N, NP>> {
  public render(): JSX.Element | null {
    ...
    if (
      ...
      (!this.props.noSummaryIfAllNodesVisible ||
        ...
        summary = TotalCountSummaryComponent ? (
          ...
          <p className="filtered-connection__summary">
            <small>
              <span>
                {totalCount}{" "}
                {pluralize(this.props.noun, totalCount, this.props.pluralNoun)}{" "}
    ...
    return (
      <>
        ...
        {this.props.connection && this.props.connection.nodes.length > 0 && (
          <ListComponent
            className={`filtered-connection__nodes ${this.props.listClassName ||
              ""}`}
          >
            {HeadComponent && (
              <HeadComponent
            ...
            {FootComponent && (
              <FootComponent nodes={this.props.connection.nodes} />
            )}
          </ListComponent>
        )}
        {!this.props.connectionQuery && summary}
        {!this.props.loading &&
          ...
          hasNextPage && (
            <button
              type="button"
              className={`btn btn-secondary btn-sm filtered-connection__show-more ${this
...
interface FilteredConnectionState<C extends Connection<N>, N>
  extends ConnectionStateCommon {
...
> {
    ...
    this.state = {
      ...
      query:
        (!this.props.hideSearch &&
          ...
          searchParameters.get(QUERY_KEY)) ||
        "",
      activeFilter:
        (this.props.useURLQuery &&
          getFilterFromURL(
        ...
        undefined,
      first:
        (this.props.useURLQuery && parseQueryInt(searchParameters, "first")) ||
        this.props.defaultFirst!,
      visible:
        (this.props.useURLQuery &&
          parseQueryInt(searchParameters, "visible")) ||
        0
    };
  }
  ...
  public componentDidMount(): void {
    ...
    this.subscriptions.add(
      combineLatest([
        ...
        .pipe(
          ...
          >(
            (
              { query, filter, queryCount },
              [currentQuery, currentFilter, { forceRefresh }]
            ) => ({
              query: currentQuery,
          ...
          switchMap(({ query, filter, shouldRefresh, queryCount }) => {
            const result = this.props
              .queryConnection({
                ...
                first:
                  (queryCount === 1 && this.state.visible) || this.state.first,
          ...
          scan<
            PartialStateUpdate & { shouldRefresh: boolean },
            PartialStateUpdate & { previousPage: N[] }
          >(
            (
              { previousPage },
              { shouldRefresh, connectionOrError, ...rest }
            ) => {
              ...
              if (
                ...
                if (!shouldRefresh) {
                  connectionOrError.nodes = previousPage.concat(
        ...
        .subscribe(
          ({ connectionOrError, previousPage, ...rest }) => {
            if (this.props.useURLQuery) {
              const searchFragment = this.urlQuery({
                visible: previousPage.length
              });
    ...
    this.subscriptions.add(
      this.showMoreClicks
        ...
        .subscribe(({ first }) =>
          this.setState({ first, loading: true }, () =>
            refreshRequests.next({ forceRefresh: false })
          )
        )
    );
  ...
  public render(): JSX.Element | null {
    ...
    const compactnessClass = `filtered-connection--${
      this.props.compact ? "compact" : "noncompact"
    }`;
    return (
      <div
        className={`filtered-connection test-filtered-connection ${compactnessClass} ${this
          .props.className || ""}`}
      >
  ...
  private onDidSelectFilter = (filter: FilteredConnectionFilter): void =>
    this.activeFilterChanges.next(filter);
