...
export const ExternalServicePage: React.FunctionComponent<Props> = ({
  ...
  const onSubmit = useCallback(
    async (event?: React.FormEvent<HTMLFormElement>): Promise<void> => {
      ...
      if (isExternalService(externalServiceOrError)) {
        try {
          setIsUpdating(true);
          const updatedService = await updateExternalService({
            input: externalServiceOrError
          });
          setIsUpdating(false);
  ...
  const externalService =
    (!isErrorLike(externalServiceOrError) && externalServiceOrError) ||
    undefined;
  ...
  return (
    <div className="site-admin-configuration-page">
      ...
      {externalServiceOrError === undefined && (
      ...
      {externalService && (
        <ExternalServiceWebhook externalService={externalService} />
      )}
    </div>
  );
};
