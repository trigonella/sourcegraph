...
export interface AddExternalServicesPageProps
  extends ThemeProps,
    TelemetryProps {
  history: H.History;
...
export const AddExternalServicesPage: React.FunctionComponent<AddExternalServicesPageProps> = ({
  ...
  ] = useLocalStorage("hasDismissedCodeHostPrivacyWarning", false);
    ...
    <div className="add-external-services-page mt-3">
      ...
      {!hasDismissedPrivacyWarning && (
        <div className="alert alert-info">
          {!userID && (
            <p>
              This Sourcegraph installation will never send your code,
              repository names, file names, or any other specific code data to
              Sourcegraph.com or any other destination. Your code is kept
          ...
          <ul>
            <li>
              Periodically fetching a list of repositories to ensure new,
              removed, and renamed repositories are accessible on Sourcegraph.
            </li>
            <li>
              Cloning the repositories you specify to create a local cache.
            </li>
            <li>
              Periodically pulling cloned repositories to ensure search results
              are current.
            </li>
            <li>
              Fetching{" "}
      ...
      {Object.entries(nonCodeHostExternalServices).length > 0 && (
        <>
          ...
          {Object.entries(nonCodeHostExternalServices).map(
            ([id, externalService]) => (
              <div className="add-external-services-page__card" key={id}>
                <ExternalServiceCard to={getAddURL(id)} {...externalService} />
              </div>
