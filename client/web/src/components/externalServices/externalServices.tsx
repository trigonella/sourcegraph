...
}): JSX.Element => <code className="hljs-type">{props.children}</code>;

const Value = (props: {
  children: React.ReactChildren | string | string[];
}): JSX.Element => <code className="hljs-attr">{props.children}</code>;
  ...
  <div>
    <ol>
      ...
      <li>
        ...
        ) with <b>repo</b> scope, and set it to be the value of the{" "}
        <Field>token</Field> field in the configuration below.
      </li>
      <li>
        Specify which repositories Sourcegraph should index using one of the
        following fields:
        <ul>
          <li>
          ...
          <li>
            ...
            <br />
            For example,
            <Value>"org:sourcegraph created:&gt;2019-11-01"</Value> selects all
            repositories in organization "sourcegraph" created after November 1,
            2019.
            <br />
            You may also use <Value>"affiliated"</Value> to select all
            repositories affiliated with the access token.
          </li>
          <li>
  ...
  <div>
    <ol>
      ...
      <li>
        ...
        scope, and set it to be the value of the <Field>token</Field> field in
      ...
      <li>
        ...
        <ul>
          <li>
            ...
            <br />
            <Value>"groups/&lt;mygroup&gt;/projects"</Value> selects all
            projects in a group.
            <br />
            <Value>"projects?membership=true&archived=no"</Value> selects all
            unarchived projects of which the token's user is a member.
            <br />
            <Value>"search?scope=projects&search=my_search_query"</Value>{" "}
            selects all projects matching a search query.
          </li>
          <li>
      ...
      <ol>
        <li>
          ...
          <ul>
            ...
            <li>
              Scroll to the section, "Access keys for CLI, SDK, & API access".
            </li>
            <li>
              Use an existing key ID if you still have access to its Secret
              Access Key. Otherwise, click "Create access key" to create a new
              key. Record the Secret Access Key and key ID in a safe place.
            </li>
            <li>
              Set <Field>accessKeyID</Field> and <Field>secretAccessKey</Field>{" "}
              in the configuration below to the access key ID and Secret Access
              Key.
            </li>
          </ul>
        </li>
        <li>
          Set the region to your AWS region. The region (e.g.,{" "}
          <Value>us-west-2</Value>) should be visible in the URL when you visit
          AWS CodeCommit. You can visit AWS CodeCommit by logging into AWS,
          clicking on "Services" in the top navbar, and clicking on
          "CodeCommit".
        </li>
        <li>
        ...
        <li>
          You can optionally exclude repositories using the{" "}
          <Field>exclude</Field> field.
        </li>
      </ol>
      <p>
        See{" "}
    ...
    <div>
      <ol>
        <li>
          ...
          ) with <b>read</b> scope over your repositories and teams. Set it to
          be the value of the <Field>appPassword</Field> field in the
          configuration below.
        </li>
        <li>
          Set the <Field>username</Field> field to be the username corresponding
          to <Field>appPassword</Field>.
        </li>
        <li>
          Set the <Field>teams</Field> field to be the list of teams whose
          repositories Sourcegraph should index.
        </li>
      </ol>
      <p>
        See{" "}
...
const BITBUCKET_SERVER: AddExternalServiceOptions = {
  ...
  instructions: (
    <div>
      <ol>
        <li>
          In the configuration below, set <Field>url</Field> to the URL of
          Bitbucket Server.
        </li>
        <li>
        ...
        <li>
          Set <Field>username</Field> to the user that created the personal
          access token.
        </li>
        <li>
          Specify which repositories Sourcegraph should clone using the
          following fields.
          <ul>
            <li>
              <Field>repositoryQuery</Field>: a list of strings that are one of
              the following:
              <ul>
                <li>
                  <Value>"all"</Value> selects all repositories visible to the
                  token
                </li>
                <li>
                  A query string like{" "}
                  <Value>
                    "
                    {
                      "?name=<repo name>&projectname=<project>&visibility=private"
                    }
                    "
                  </Value>{" "}
                  that specifies search query parameters. See{" "}
                  <a
                ...
                <li>
                  <Value>"none"</Value> selects no repositories (should only be
                  used if you are listing repositories one-by-one)
                </li>
              </ul>
            </li>
            <li>
            ...
            <li>
              <Field>exclude</Field>: a list of repositories or repository name
              patterns to exclude
            </li>
            <li>
              <Field>excludePersonalRepositories</Field>: if true, excludes
              personal repositories from being indexed
            </li>
          </ul>
        </li>
      </ol>
...
const SRC_SERVE_GIT: AddExternalServiceOptions = {
  ...
  instructions: (
    <div>
      <p>
        In the configuration below, set <Field>url</Field> to be the URL of src
        serve-git.
      </p>
      <p>
        Install the{" "}
        ...
        . src serve-git allows you to serve any git repositories that you have
        on disk.
      </p>
    </div>
  ),
...
const GITOLITE: AddExternalServiceOptions = {
  ...
  instructions: (
    <div>
      <ol>
        <li>
          In the configuration below, set <Field>host</Field> to be the username
          and host of the Gitolite server.
        </li>
        <li>
          Set the <Field>prefix</Field> field to the prefix you desire for the
          repository names on Sourcegraph. This is typically the hostname of the
          Gitolite server.
        </li>
      </ol>
      <p>
...
const GENERIC_GIT: AddExternalServiceOptions = {
  ...
  instructions: (
    <div>
      <ol>
        <li>
          In the configuration below, set <Field>url</Field> to be the URL of
          your Git host.
        </li>
        <li>
          Add the paths of the repositories you wish to index to the{" "}
          <Field>repos</Field> field. These will be appended to the host URL to
          obtain the repository clone URLs.
        </li>
      </ol>
      <p>
        See{" "}
...
export const nonCodeHostExternalServices: Record<
  string,
  AddExternalServiceOptions
> = {
...
export const defaultExternalServices: Record<
  ExternalServiceKind,
  AddExternalServiceOptions
> = {
