...
function isElementBreadcrumb(
  breadcrumb: Breadcrumb
): breadcrumb is ElementBreadcrumb {
...
export interface BreadcrumbSetters {
  ...
  ) => BreadcrumbSetters & Unsubscribable;
}
...
export const useBreadcrumbs = (): BreadcrumbsProps & BreadcrumbSetters => {
  const [breadcrumbsByDepth, setBreadcrumbsByDepth] = useState<
    BreadcrumbAtDepth[]
  >([
    {
      depth: 0,
      breadcrumb: {
        key: "home",
        element: <Link to="/search">Home</Link>,
        divider: null
      }
    }
  ]);
   ...
   */
  const createBreadcrumbSetters = useCallback(
    (depth: number = 1): BreadcrumbSetters => {
      /** Shared logic between plain function and hook */
      function internalSetBreadcrumb(
        breadcrumb: NullableBreadcrumb
      ): () => void {
        ...
        // cleanup
        return () => {
          setBreadcrumbsByDepth(breadcrumbs =>
            breadcrumbs.filter(breadcrumb => breadcrumb !== entry)
          );
        };
      }
  ...
  const breadcrumbSetters = useMemo(() => createBreadcrumbSetters(), [
...
export const Breadcrumbs: React.FC<{
  ...
  <nav
    className="d-flex py-2 container-fluid flex-nowrap flex-shrink-past-contents"
