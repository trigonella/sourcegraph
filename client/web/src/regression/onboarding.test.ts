...
/**
 * Gets the activation status for the current user from the GUI. There's no easy
 * way to fetch this from the API, so we use a coarse method for extracting it
 * from the GUI.
 */
async function getActivationStatus(
    driver: Driver): Promise<{complete : number; total : number}>{
  ...
  const lineItems = [...dropdownMenu.querySelectorAll(
      '.activation-dropdown-item') ] const complete =
      lineItems
          .flatMap(element => [...element.querySelectorAll(
                       '.mdi-icon.text-success')])
          .length
  const incomplete =
      lineItems
          .flatMap(
              element => [...element.querySelectorAll('.mdi-icon.text-muted')])
          .length
  return { complete, total: complete + incomplete, }
    })
}
...
describe('Onboarding', () => {
    ...
    const testExternalServiceConfig = {
      kind: GQL.ExternalServiceKind.GITHUB,
          uniqueDisplayName: '[TEST] GitHub (onboarding.test.ts)', config: {
            url: 'https://github.com',
            token: config.gitHubToken,
            repos:
                [
                  'auth0/go-jwt-middleware', 'kyoshidajp/ghkw',
    ...
    /**
     * Only run site-admin onboarding test if the appropriate environment
     * variable is set. This test assumes an instance of Sourcegraph that does
     * not yet have external services.
     */
    ...
    testAdminOnboarding('Site-admin onboarding', async function() {
      ...
      const testUser =
          await getUser(gqlClient, 'test-onboarding-regression-test-user')
      if (!testUser) {
      ...
      if (activationStatus.total !== 4) {
        throw new Error(
            `Expected 4 onboarding steps for site admins, but only found ${
                activationStatus.total}`)
      }

      // Verify add-external-service onboarding step
      await driver.page
          .waitForSelector(activationNavBarSelector)
          // Check that onboarding status menu contains menu item and it goes
          // where it should
          await retry(
              async () => {
                  ...
                  await driver.page.click(activationNavBarSelector)
                      await driver.page
                          .click(activationNavBarSelector)
                              await (await driver.findElementWithText(
                                  'Connect your code host'))
                          .click()}) await driver
          .waitUntilURL(driver.sourcegraphBaseUrl +
          ...
          await screenshots.verifyScreenshot({
            filename :
                'confetti-appears-after-adding-first-external-service.png',
            description : 'confetti coming out of "Setup" navbar item',
          })
    })

    test('Non-admin user onboarding', async function () {
  this.timeout(30 * 1000) await ensureTestExternalService(
      gqlClient, testExternalServiceConfig, config)
  const repoSlugs = testExternalServiceConfig.config.repos await waitForRepos(
      gqlClient, [ 'github.com/' + repoSlugs[repoSlugs.length - 1] ], config)
  ...
  const statusBarSelector =
      '.test-activation-progress-bar'

      // Initial status indicator
      await driver.page
          .goto(config.sourcegraphBaseUrl + '/search') await screenshots
          .verifySelector('initial-progress-bar-is-gray-circle.png',
                          'gray circle', statusBarSelector, 2000)
      ...
      await screenshots
          ...
          .verifySelector('progress-bar-after-initial-search-is-half-green.png',
                          '50% green circle', statusBarSelector)

      // Do a find references
      await driver.page
          .goto(config.sourcegraphBaseUrl +
                '/github.com/auth0/go-jwt-middleware/-/blob/jwtmiddleware.go')
  ...
  const findReferencesSelector =
      '.test-tooltip-find-references' await driver.page
          .waitForSelector(findReferencesSelector)
              await driver.page.click(findReferencesSelector) await driver.page
      ...
      await screenshots
          ...
          .verifySelector(
              ...
              '100% green circle', statusBarSelector) await driver.page
          .reload()
