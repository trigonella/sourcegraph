...
/**
 * Create the user with the specified password. Returns a destructor that
 * destroys the test user. Assumes basic auth.
 */
export async function ensureLoggedInOrCreateTestUser(
    ...
    gqlClient: GraphQLClient,
    {
        ...
        deleteIfExists?: boolean
          ...
          const userDestructor = (): Promise<void> =>
              deleteUser(gqlClient, username, false)

          if (!username.startsWith('test-')) {
            throw new Error(`Test username must start with "test-" (was ${
          ...
          if (deleteIfExists) {
            ...
            try {
              await driver.ensureLoggedIn(
                  {username, password : testUserPassword})
              return userDestructor
            } catch (error) {
              console.log(`Login failed (error: ${
                  asError(error).message}), will attempt to create user ${
                  JSON.stringify(username)}`)
            }
          }
        ...
        async function createTestUser(
            driver: Driver, gqlClient: GraphQLClient,
            {username, testUserPassword}: {username: string}&
            Pick<Config, 'testUserPassword'>): Promise<void> {
          ...
          const passwordResetURL =
            ...
            `,
                  ...
                  .pipe(
                      ...
                      catchError(
                          error => throwError(new Error(`Could not create user ${
                              JSON.stringify(
                                  username)} (you may need to update the sudo access token used by the test): ${
          ...
          await driver.page.goto(passwordResetURL) await driver.page
              .waitForSelector('.reset-password-page__form')
        ...
        export async function createAuthProvider(
            gqlClient: GraphQLClient, authProvider: GitHubAuthProvider|
            GitLabAuthProvider|OpenIDConnectAuthProvider|
            SAMLAuthProvider): Promise<ResourceDestructor> {
          ...
          const authProviders = siteConfigParsed['auth.providers'] if (
              authProviders?.some(provider =>
                                      provider.type === authProvider.type &&
          ...
          const editFns =
              [
                (contents: string) => jsoncEdit.setProperty(
                    contents, [ 'auth.providers', -1 ], authProvider, {
                      eol : '\n',
                      insertSpaces : true,
                      tabSize : 2,
                    }),
              ] const {destroy} =
                  await editSiteConfig(gqlClient, ...editFns)
          return destroy
        }
        ...
        export async function ensureNewUser(
            ...
            username: string,
            email: string|undefined): Promise<ResourceDestructor> {
          try {
        ...
        /**
         * Ensures a new organization, deleting the existing one if it already
         * exists.
         */
 ...
 */
        displayName?: string
          ...
          const matchingOrgs =
              (await fetchAllOrganizations({requestGraphQL}, {
                 first : 1000
               }).toPromise())
                  .nodes.filter(org => org.name === variables.name)
          if (matchingOrgs.length > 1) {
            throw new Error(`More than one organization name exists with name ${
                variables.name}`)
          }
          if (matchingOrgs.length === 1) {
          ...
          const createdOrg =
              await createOrganization({requestGraphQL}, variables).toPromise()
          return {
            destroy: () =>
                deleteOrganization({requestGraphQL}, createdOrg.id).toPromise(),
                     result: createdOrg,
          }
        }

        export async function getGlobalSettings(gqlClient: GraphQLClient):
            Promise<{
              subjectID : GQL.ID; settingsID : number | null; contents : string
            }> {
          const settings = await getViewerSettings(gqlClient)
          const globalSettingsSubject = first(settings.subjects.filter(
              subject => subject.__typename === 'Site'))
        ...
        export async function editGlobalSettings(
          ...
          const {subjectID, settingsID, contents : origContents} =
        ...
        export async function editSiteConfig(
          ...
          return {
            result: await updateSiteConfiguration(
                gqlClient, origConfig.configuration.id, newContents)
                .toPromise(),
                destroy: async () => {
                  const site =
                      await fetchSiteConfiguration(gqlClient)
                          .toPromise() await updateSiteConfiguration(
                              gqlClient, site.configuration.id,
        ...
        export async function login(driver: Driver,
                                    {
                                      sourcegraphBaseUrl,
                                      authProviderDisplayName,
                                    }: Pick<Config, 'sourcegraphBaseUrl'>&
                                    {authProviderDisplayName : string},
                                    loginToAuthProvider: () =>
                                        Promise<void>): Promise<void> {
          await driver.page
              .goto(sourcegraphBaseUrl +
                    '/-/sign-out') await driver.newPage()
                  await driver.page.goto(sourcegraphBaseUrl) await retry(
                      async () => {
                          await driver.page.reload() await driver
                              .findElementWithText(
                                  'Continue with ' + authProviderDisplayName, {
                                    action : 'click',
                                    selector : 'a',
                                    wait : {timeout : 5000},
                                  })
          ...
          if (driver.page.url() !== sourcegraphBaseUrl + '/search') {
            ...
            try {
              await driver.page.waitForFunction(
                  url => document.location.href === url, {timeout : 5 * 1000},
        ...
        export async function loginToOkta(driver: Driver, username: string,
                                          password: string): Promise<void> {
          await driver.page
              .waitForSelector('#okta-signin-username') await driver
              .replaceText({
              ...
              .waitForSelector('#okta-signin-submit')
                  await driver.page.click('#okta-signin-submit')
        }

        export async function loginToGitHub(driver: Driver, username: string,
                                            password: string): Promise<void> {
          await driver.page.waitForSelector('#login_field') await driver
              .replaceText({
        ...
        export async function loginToGitLab(driver: Driver, username: string,
                                            password: string): Promise<void> {
          await driver.page
              .waitForSelector('input[name="user[login]"]', {timeout : 10000})
                  await driver
              .replaceText({
                selector : '#user_login',
                newText : username,
              ...
              .waitForSelector('input[data-qa-selector="sign_in_button"]')
                  await driver.page.click(
                      'input[data-qa-selector="sign_in_button"]')
        }
