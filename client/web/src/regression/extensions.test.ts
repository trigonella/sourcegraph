...
async function activateAndConfigureExtension({
  ...
  username: string; extensionID : string;
  extensionConfig?: {[key: string] : any}
},
                                             graphQLClient: GraphQLClient) {
  await editUserSettings(username, {
    keyPath : [ {property : 'extensions'}, {property : extensionID} ],
    value : true
  },
                         graphQLClient)
  for (const [property, value] of Object.entries(extensionConfig)) {
    await editUserSettings(username, {keyPath : [ {property} ], value},
                           graphQLClient)
  }
}
...
const config =
    getConfig('sudoToken', 'sudoUsername', 'sourcegraphBaseUrl', 'noCleanup',
              'testUserPassword', 'keepBrowser', 'logBrowserConsole', 'slowMo',
              'headless', 'gitHubToken')
const externalService =
{
  kind: ExternalServiceKind.GITHUB,
    ...
    [
      'theupdateframework/notary', 'GetStream/Winds',
      'codecov/sourcegraph-codecov'
    ]
...
before(async function() {
  ...
  ({driver, gqlClient : graphQLClient, resourceManager} =
  ...
  resourceManager.add('External service', externalService.uniqueDisplayName,
...
test('Codecov extension', async function() {
  this.timeout(30 * 1000) await activateAndConfigureExtension({
        username : testUsername,
  ...
  console.log('------------------------ two -------------------------------')
      ...
      await retry(
          async () =>
              expect(await driver.page.$$('tr[style]')).toHaveLength(264))
  ...
  console
      .log('------------------------ five -------------------------------')
          ...
          await driver.findElementWithText(
              'Codecov: Show line hit/branch counts', {action : 'click'})

  console.log('------------------------ six -------------------------------')

      // Line/hit branch counts should now show up
      await retry(async () => expect(await driver.page.$$(
  ...
  console
      ...
      // Check that the the "View commit report" button links to the correct
      // location
      await driver.page.click('.command-list-popover-button')
  ...
  console
      .log('------------------------ eight -------------------------------')
          ...
          await Promise.all([
            ...
            driver.findElementWithText('Codecov: View commit report',
                                       {action : 'click'}),
          ])

  console.log('------------------------ nine -------------------------------')
  expect(driver.page.url())
      .toEqual(
...
test('Datadog extension', async function() {
  this.timeout(10 * 1000) await activateAndConfigureExtension({
        username : testUsername,
      ...
      await retry(async () =>
                      expect(await driver.page.$$(
                                 '[data-contents=" View metric (Datadog) » "]'))
                          .toHaveLength(9))
})
  ...
  this.timeout(10 * 1000) await activateAndConfigureExtension({
      ...
      // Visit a file containing throw statements that should be matched by the
      // Sentry extension
      await driver.page.goto(
          new URL(
      ...
      await retry(async () =>
                      expect(await driver.page.$$(
                                 '[data-contents=" View logs in Sentry » "]'))
                          .toHaveLength(1))
    })
})
