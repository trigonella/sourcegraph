...
beforeEach(() => {alwaysCleanupManager = new TestResourceManager()})
afterEach(async () => {await alwaysCleanupManager.destroyAll()})

test('2.2.1 User settings are saved and applied', async () => {
  const getSettings =
      async () => {
    await driver.page.waitForSelector(
        '.test-settings-file .monaco-editor .view-lines')
            ...
            // eslint-disable-next-line unicorn/prefer-text-content
                return editor ? editor.innerText : null
                  ...
                  await driver.page.goto(config.sourcegraphBaseUrl +
                                         `/users/${testUsername}/settings`)
  const previousSettings = await getSettings()
  if (!previousSettings) {
    throw new Error('Previous settings were null')
  }
  const newSettings =
      '{\u00A0/*\u00A0These\u00A0are\u00A0new\u00A0settings\u00A0*/}' await driver
          .replaceText({
  ...
  if (currentSettings !== previousSettings) {
    throw new Error(`Settings ${
        JSON.stringify(currentSettings)} did not match (old) saved settings ${
  ...
  await driver
      ...
      .findElementWithText('Save changes', {action : 'click'}) await driver.page
      .waitForFunction(
          () => document.evaluate("//*[text() = ' Saving...']", document)
                    .iterateNext() === null) await driver.page.reload()

  const currentSettings2 = await getSettings()
  if (JSON.stringify(currentSettings2) !== JSON.stringify(newSettings)) {
    throw new Error(`Settings ${
        JSON.stringify(currentSettings2)} did not match (new) saved settings ${
        JSON.stringify(newSettings)}`)
  }

  // When you type (or paste) "{" into the empty user settings editor it adds a
  // "}". That's why we cannot type all the previous text, because then we would
  // have two "}" at the end.
  const textToTypeFromPrevious =
      previousSettings
          .replace(/}$/, '')
      ...
      await driver
          ...
          .findElementWithText('Save changes', {action : 'click'})
              await driver.page.waitForFunction(
                  () =>
                      document.evaluate("//*[text() = ' Saving...']", document)
                          .iterateNext() === null)
  const previousSettings2 = await getSettings() await driver.page.reload()

  const currentSettings3 = await getSettings()
  if (currentSettings3 !== previousSettings2) {
    throw new Error(`Settings ${
        JSON.stringify(currentSettings3)} did not match (old) saved settings ${
        JSON.stringify(previousSettings2)}`)
  }
})
    ...
    test('2.2.2 User profile page', async () => {
        ...
        await screenshots.verifySelector(
            ...
            '.test-user-nav-item-toggle'
                ...
                test('2.2.3. User emails page', async () => {
                  const testEmail =
                      'sg-test-account@protonmail.com' await driver.page
                          .goto(driver.sourcegraphBaseUrl +
                                `/users/${testUsername}/settings/emails`)
                              await driver
                          .replaceText({
                            selector : '.test-user-email-add-input',
                            newText : 'sg-test-account@protonmail.com'
                          }) await driver
                          .findElementWithText('Add', {action : 'click'})
                              await driver.findElementWithText(testEmail,
                                                               {wait : true})
                  try {
                  ...
                  await setUserEmailVerified(gqlClient, testUsername, testEmail,
                                             true)
                      await driver.page.reload()
                          await driver.findElementWithText('Verified',
                                                           {wait : true})
                })

                test(
                    '2.2.4 Access tokens work and invalid access tokens return "401 Unauthorized"',
                    async () => {
                      await driver.page
                          .goto(config.sourcegraphBaseUrl +
                                `/users/${testUsername}/settings/tokens`)
                              await driver
                          .findElementWithText(
                              'Generate new token',
                              {action : 'click', wait : {timeout : 5000}})
                              await driver
                          .findElementWithText('New access token',
                                               {wait : {timeout : 1000}})
                              await driver
                          .replaceText({
                          ...
                          .findElementWithText(
                              'Generate token',
                              {action : 'click', wait : {timeout : 1000}})
                              await driver
                          .findElementWithText(
                              "Copy the new access token now. You won't be able to see it again.",
                              {
                                wait : {timeout : 1000},
                              })
                      ...
                      const token = await driver.page.evaluate(() => {
                        const tokenElement =
                            document.querySelector('.test-access-token')
                        if (!tokenElement) {
                          return null
                        }
        ...
        `
                      ...
                      const gqlClientWithInvalidToken =
                          createGraphQLClient({
                            baseUrl : config.sourcegraphBaseUrl,
  ...
  alwaysCleanupManager
      .add('Global setting', 'quicklinks', async () => {
        const {subjectID : currentSubjectID, settingsID : currentSettingsID} =
            await getGlobalSettings(gqlClient) await overwriteSettings(
                gqlClient, currentSubjectID, currentSettingsID, oldContents)
      })
      ...
      .hover() await driver
      .findElementWithText(quicklinkInfo.description, {
        wait : {timeout : 1000},
      }) await driver
      .findElementWithText(quicklinkInfo.name, {
        action : 'click',
        selector : 'a',
        wait : {timeout : 1000},
