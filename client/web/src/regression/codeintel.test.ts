...
const config =
    getConfig('gitHubToken', 'headless', 'keepBrowser', 'logBrowserConsole',
              'logStatusMessages', 'noCleanup', 'slowMo', 'sourcegraphBaseUrl',
              'sudoToken', 'sudoUsername', 'testUserPassword')
...
const testRepoSlugs =
    ...
    const prometheusCommonHeadCommit =
        'b5fe7d854c42dc7842e48d1ca58f60feae09d77b' // HEAD
const prometheusRedefinitionsHeadCommit =
    'c68f0e063cf8a98e7ce3428cfd50588746010f1f'
...
before(async function() {
  ...
  ({driver, gqlClient, resourceManager : outerResourceManager} =
       await getTestTools(config))
  ...
  outerResourceManager.add(
      'External service', testExternalServiceInfo.uniqueDisplayName,
  ...
  outerResourceManager.add('Global setting', 'showBadgeAttachments',
                           await enableBadgeAttachments(gqlClient))
  outerResourceManager.add('Global setting', 'codeIntel.includeForks',
                           await setIncludeForks(gqlClient, true))
})
...
after(async () => {
  ...
  if (driver) {
    ...
    before(async () => {innerResourceManager.add(
               'Global setting', 'codeIntel.lsif',
               await setGlobalLSIFSetting(gqlClient, false))})
    after(async () => {
      if (!config.noCleanup) {
    ...
    test('File sidebar, multiple levels of directories', async () => {
      ...
      for (const file
               of ['cmd', 'frontend', 'auth', 'providers', 'providers.go']) {
      ...
      await driver.waitUntilURL(
          `${
              config
                  .sourcegraphBaseUrl}/github.com/sourcegraph/sourcegraph@c543dfd3936019befe94b881ade89e637d1a3dc3/-/blob/cmd/frontend/auth/providers/providers.go`,
          {timeout : 2 * 1000})
    })
...
interface CodeNavigationTestCase {
          ...
          /**
           * The token to click. Should be unambiguous within this line for the
           * test to succeed.
           */
          token: string
          ...
          /**
           * A locations (if unambiguous), or a subset of locations that must
           * occur within the definitions panel.
           */
        ...
        interface TestLocation {
          ...
          /**
           * Whether or not this location should be accompanied by a UI badge
           * indicating imprecise code intel. Precise = no badge.
           */
          precise: boolean
        }

        /**
         * Navigate to the given page and test the definitions, references, and
         * hovers of the token on the given line. Will ensure both hover and
         * clicking the token produces the hover overlay. Will check the
         * precision indicator of the hoverlay and each file match in the
         * definition and reference panels. Will compare hover text. Will
         * compare location of each file match or the target of the page
         * navigated to on jump-to-definition (in the case of a single
         * definition).
         */
        async function testCodeNavigation(
            driver: Driver, config: Pick<Config, 'sourcegraphBaseUrl'>, {
              page,
              line,
              token,
              precise,
          ...
          if (expectedReferences && expectedReferences.length > 0) {
            await clickOnEmptyPartOfCodeView(driver) await tokenElement
                .hover() await waitForHover(driver, expectedHoverContains)
                    await (await driver.findElementWithText('Find references'))
                .click()
          ...
          await clickOnEmptyPartOfCodeView(driver) await tokenElement
              .hover() await waitForHover(driver, expectedHoverContains)
                  await (await driver.findElementWithText('Go to definition'))
              .click()
          ...
          else {await driver.page
                    .waitForFunction(
                        defURL => document.location.href.endsWith(defURL),
                        {timeout : 2000}, expectedDefinition.url)
        ...
        /**
         * Return a list of locations (and their precision) that exist in the
         * file list panel. This will click on each repository and collect the
         * visible links in a sequence.
         */
        async function collectLinks(driver: Driver):
            Promise<Set<TestLocation>> {
          await driver.page.waitForSelector('.test-loading-spinner',
                                            {hidden : true})
          ...
          for (const title of panelTabTitles) {
            const tabElement = await driver.page.$$(
                `.test-hierarchical-locations-view-list span[title="${title}"]`)
        ...
        /**
         * Return the list of repository titles on the left-hand side of the
         * definition or reference result panel.
         */
        async function getPanelTabTitles(driver: Driver):
            Promise<string[]>{
                return (await Promise.all(
                            (await driver.page.$$(
                                 '.hierarchical-locations-view > div:nth-child(1) span[title]'))
                                .map(elementHandle => elementHandle.evaluate(
                                         element =>
        ...
        /**
         * Return a list of locations (and their precision) that are current
         * visible in a file list panel. This may be definitions or references.
         */
        function collectVisibleLinks(driver: Driver):
            Promise<TestLocation[]> {
              return driver.page.evaluate(
                  () => [...document.querySelectorAll<HTMLElement>(
                             '.test-file-match-children-item-wrapper')]
                            .map(a => ({
                                   url : a.querySelector(
                                              '.test-file-match-children-item')
                                                 ?.getAttribute('href') ||
                                             '',
                                   precise : a.querySelector('.test-badge-row')
                                                     ?.childElementCount === 0,
                                 })))
            }
         ...
         */
        async function clickOnEmptyPartOfCodeView(driver: Driver):
            Promise<void> {
              await driver.page
                  .click('.test-blob tr:nth-child(1) .line')
                      await driver.page.waitForFunction(
                          () => document
                                    .querySelectorAll(
                                        '.test-tooltip-go-to-definition')
                                    .length === 0)
            }
         ...
         *
         * Will close any toast so that the entire line is visible and will
         * hover over the line to ensure that the line is tokenized (as this is
         * done on-demand).
         */
        async function findTokenElement(driver: Driver, line: number,
                                        token: string):
            Promise<ElementHandle<Element>> {
              try {
                // If there's an open toast, close it. If the toast remains open
                // and our target identifier happens to be hidden by it, we
                // won't be able to select the correct token. This condition was
                // reproducible in the code navigation test that searches for
                // the identifier `StdioLogger`.
              ...
              const selector =
                  `.test-blob tr:nth-child(${
                      line}) span` await driver.page.hover(selector)
              return driver.findElementWithText(token,
                                                {selector, fuzziness : 'exact'})
            }

        /**
         * Wait for the hover tooltip to become visible. Compare the visible
         * text with the expected contents (expected contents must be a
         * substring of the visible contents). If precise is supplied, ensure
         * that the presence of the UI indicator matches this value.
         */
        async function waitForHover(
            driver: Driver, expectedHoverContains: string, precise?: boolean):
            Promise<void> {
              await driver.page
                  .waitForSelector('.test-tooltip-go-to-definition')
                      await driver.page.waitForSelector('.test-tooltip-content')
              expect(normalizeWhitespace(await getTooltip(driver)))
                  .toContain(normalizeWhitespace(expectedHoverContains))
         ...
         */
        async function getTooltip(driver: Driver):
            Promise<string> {
              return driver.page.evaluate(
                  () => (document.querySelector('.test-tooltip-content') as
                         HTMLElement)
                            .textContent ||
                        '')
            }
         ...
         */
        function normalizeWhitespace(string: string):
            string { return string.replace(/\s+/g, ' ') }

        //
        // LSIF utilities

        /**
           Show badge attachments in the UI to distinguish precise and
           search-based results.
         */
        async function enableBadgeAttachments(gqlClient: GraphQLClient):
            Promise<() => Promise<void>> {
              return writeSetting(
                  gqlClient, [ 'experimentalFeatures', 'showBadgeAttachments' ],
                  true)
            }

        /** Replace the codeIntel.includeForks setting with the given value. */
        async function setIncludeForks(gqlClient: GraphQLClient,
                                       enabled: boolean):
            Promise<() => Promise<void>> {
              return writeSetting(gqlClient, [ 'basicCodeIntel.includeForks' ],
                                  enabled)
            }
        /** Replace the codeIntel.lsif setting with the given value. */
        async function setGlobalLSIFSetting(gqlClient: GraphQLClient,
                                            enabled: boolean):
            Promise<() => Promise<void>> {
        ...
        /**
         * Return a promise that updates the global settings to their original
         * value. This return value is suitable for use with the resource
         * manager's destroy queue.
         */
        async function writeSetting(
            gqlClient: GraphQLClient, path: JSONPath,
            value: unknown): Promise<() => Promise<void>> {
          const {subjectID, settingsID, contents : oldContents} =
              await getGlobalSettings(gqlClient)
          const newContents =
              applyEdits(oldContents, setProperty(oldContents, path, value, {
                           eol : '\n',
                           insertSpaces : true,
                           tabSize : 2,
                         }))

                  await overwriteSettings(gqlClient, subjectID, settingsID,
                                          newContents)
          return async () => {
            const {
              ...
              settingsID : currentSettingsID
                ...
                await overwriteSettings(gqlClient, currentSubjectID,
                                        currentSettingsID, oldContents)
          }
        }
