...
/**
 * A fallback settings subject that can be constructed synchronously at
 * initialization time.
 */
export const SITE_SUBJECT_NO_ADMIN:
...
export function viewerSubjectFromSettings(
    cascade: SettingsCascadeOrError, authenticatedUser: AuthenticatedUser|null):
    LayoutProps['viewerSubject'] {
      if (authenticatedUser) {
        return authenticatedUser
      }
      if (cascade && !isErrorLike(cascade) && cascade.subjects &&
...
export function defaultPatternTypeFromSettings(
    ...
    SearchPatternType|undefined {
      // When the web app mounts, if the current page does not have a
      // patternType URL parameter, set the search pattern type to the
      // defaultPatternType from settings (if it is set), otherwise default to
      // literal.
      //
      ...
      if (!parseSearchURLPatternType(window.location.search)) {
        const defaultPatternType =
            settingsCascade.final && !isErrorLike(settingsCascade.final) &&
                settingsCascade
                    .final['search.defaultPatternType'] return defaultPatternType ||
            'literal'
      }
      return
    }
...
export function experimentalFeaturesFromSettings(
    settingsCascade: SettingsCascadeOrError): {splitSearchModes: boolean
...
showMultilineSearchConsole: boolean
  ...
  return {
    splitSearchModes, copyQueryButton, searchStreaming, showRepogroupHomepage,
        showOnboardingTour, showEnterpriseHomePanels,
        showMultilineSearchConsole, showQueryBuilder,
  }
}
