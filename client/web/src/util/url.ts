 ...
 */
export function formatHash(lpr: LineOrPositionOrRange,
                           searchParameters: URLSearchParams): string {
  if (!lpr.line) {
    return `#${searchParameters.toString()}`
  }
  const anyParameters = [...searchParameters ].length > 0
  return `#L${formatLineOrPositionOrRange(lpr)}${
      anyParameters ? '&' + searchParameters.toString() : ''}`
}
...
function formatLineOrPositionOrRange(lpr: LineOrPositionOrRange): string {
  ...
  const emptyRange = range.start.line === range.end.line &&
                     range.start.character === range.end.character
  return emptyRange ? toPositionHashComponent(range.start)
                    : `${toPositionHashComponent(range.start)}-${
                          toPositionHashComponent(range.end)}`
}
 ...
 */
export function replaceRevisionInURL(href: string,
                                     newRevision: string): string {
  const parsed = parseBrowserRepoURL(href)
  const repoRevision = `/${encodeRepoRevision(parsed)}`

  const url = new URL(href, window.location.href)
  url.pathname = `/${encodeRepoRevision({...parsed, revision : newRevision})}${
      url.pathname.slice(repoRevision.length)}`
...
export function parseBrowserRepoURL(href: string): ParsedRepoURI&
    Pick<ParsedRepoRevision, 'rawRevision'> {
  const url = new URL(href, window.location.href)
  ...
  if (indexOfSeparator === -1) {
    ...
    repoRevision = pathname.slice(
        0, indexOfSeparator) // the whole string leading up to the separator
                             // (allows revision to be multiple path parts)
  }
  ...
  const commitID =
      revision && /^[\da-f]{40}$/i.test(revision) ? revision : undefined
  ...
  if (treeSeparator !== -1) {
    filePath =
        decodeURIComponent(pathname.slice(treeSeparator + '/-/tree/'.length))
  }
  if (blobSeparator !== -1) {
    filePath =
        decodeURIComponent(pathname.slice(blobSeparator + '/-/blob/'.length))
  }
  ...
  return {
    repoName, revision, rawRevision, commitID, filePath, commitRange, position,
        range
  }
}
...
/**
 * Parses a repo-revision string like "my/repo@my/revision" to the repo and
 * revision components.
 */
export function parseRepoRevision(repoRevision: string): ParsedRepoRevision {
  const [repository, revision] =
      repoRevision.split('@', 2) as [string, string | undefined] return {
