...
const LayoutWithActivation = window.context.sourcegraphDotComMode
  ? Layout
  : withActivation(Layout);
 ...
 */
class ColdSourcegraphWebApp extends React.Component<
  SourcegraphWebAppProps,
  SourcegraphWebAppState
> {
  private readonly subscriptions = new Subscription();
  private readonly darkThemeMediaList = window.matchMedia(
    "(prefers-color-scheme: dark)"
  );
  private readonly platformContext: PlatformContext = createPlatformContext();
  private readonly extensionsController: ExtensionsController = createExtensionsController(
  ...
  constructor(props: SourcegraphWebAppProps) {
    ...
    const urlPatternType =
      parseSearchURLPatternType(window.location.search) ||
    ...
    const availableVersionContexts =
      window.context.experimentalFeatures.versionContexts;
    const previousVersionContext = localStorage.getItem(
      LAST_VERSION_CONTEXT_KEY
    );
    ...
    this.state = {
      ...
      interactiveSearchMode: currentSearchMode
        ? currentSearchMode === "interactive"
        : false,
  ...
  public componentDidMount(): void {
      ...
      combineLatest([
        ...
        ([settingsCascade, authenticatedUser]) => {
          this.setState(state => ({
            ...
            searchPatternType:
              defaultPatternTypeFromSettings(settingsCascade) ||
              state.searchPatternType,
            viewerSubject: viewerSubjectFromSettings(
              settingsCascade,
              authenticatedUser
            )
          }));
    ...
    this.subscriptions.add(
      authenticatedUser
        ...
        .subscribe(event => {
          if (
            event?.error instanceof HTTPStatusError &&
    ...
    this.extensionsController.services.workspace.versionContext.next(
      this.state.versionContext
    );
  }
  ...
  private toggleSearchMode = (
    ...
    localStorage.setItem(
      SEARCH_MODE_KEY,
      this.state.interactiveSearchMode ? "plain" : "interactive"
    );

    eventLogger.log("SearchModeToggled", {
      mode: this.state.interactiveSearchMode ? "plain" : "interactive"
    });

    if (this.state.interactiveSearchMode) {
      const queries = [
        this.state.navbarSearchQueryState.query,
        generateFiltersQuery(this.state.filtersInQuery)
      ];
            ...
            <Route
              ...
              render={routeComponentProps => (
                <LayoutWithActivation
                  ...
                  searchRequest={
                    this.state.searchStreaming ? searchStream : search
                  }
                  // Extensions
  ...
  private onThemePreferenceChange = (
    themePreference: ThemePreference
  ): void => {
  ...
  private onFiltersInQueryChange = (
    filtersInQuery: FiltersToTypeAndValue
  ): void => {
  ...
  private setVersionContext = (versionContext: string | undefined): void => {
    const resolvedVersionContext = resolveVersionContext(
      versionContext,
      this.state.availableVersionContexts
    );
    if (!resolvedVersionContext) {
      ...
      this.setState({
        versionContext: resolvedVersionContext,
        previousVersionContext: resolvedVersionContext
      });
    }
