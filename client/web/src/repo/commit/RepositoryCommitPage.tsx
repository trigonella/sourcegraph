    ...
    requestGraphQL<RepositoryCommitResult, RepositoryCommitVariables>(
      ...
      map(({ data, errors }) => {
        ...
        if (data.node.__typename !== "Repository") {
          throw new Error(
            `Node is a ${data.node.__typename}, not a Repository`
          );
        }
        if (!data.node.commit) {
...
export class RepositoryCommitPage extends React.Component<Props, State> {
  ...
  private nextRepositoryCommitPageElement = (
    element: HTMLElement | null
  ): void => this.repositoryCommitPageElements.next(element);
    ...
    >({
      ...
      hoverOverlayRerenders: this.componentUpdates.pipe(
        withLatestFrom(
          ...
          this.repositoryCommitPageElements
        ),
      ...
      this.componentUpdates
        .pipe(
          distinctUntilChanged(
            (a, b) =>
              a.repo.id === b.repo.id &&
              a.match.params.revspec === b.match.params.revspec
          ),
              ...
              queryCommit({
                ...
                tap(({ commitOrError }) => {
                  if (isErrorLike(commitOrError)) {
                    ...
                    this.props.onDidUpdateExternalLinks(
                      commitOrError.externalURLs
                    );
                  }
                })
              )
  ...
  public render(): JSX.Element | null {
    return (
      <div
        className="repository-commit-page container mt-3"
      ...
      >
        ...
        {this.state.commitOrError === undefined ? (
          ...
          <ErrorAlert
            className="mt-2"
            error={this.state.commitOrError}
          ...
          <>
            ...
            <FileDiffConnection
              ...
              updateOnChange={`${this.props.repo.id}:${
                this.state.commitOrError.oid
              }:${String(this.props.isLightTheme)}`}
              defaultFirst={15}
              hideSearch={true}
