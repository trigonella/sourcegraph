...
export const GitCommitNode: React.FunctionComponent<GitCommitNodeProps> = ({
  ...
  const [showCommitMessageBody, setShowCommitMessageBody] = useState<boolean>(
    false
  );
  const [
    flashCopiedToClipboardMessage,
    setFlashCopiedToClipboardMessage
  ...
  const oidElement = (
    <code className="git-commit-node__oid">{node.abbreviatedOID}</code>
  );
  return (
    <div
      key={node.id}
      className={`git-commit-node ${
        compact ? "git-commit-node--compact" : ""
      } ${className || ""}`}
    >
