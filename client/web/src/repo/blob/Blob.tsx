...
import {
  ...
  locateTarget
} from "@sourcegraph/codeintellify/lib/token_position";
 ...
 */
const toPortalID = (line: number): string =>
  `line-decoration-attachment-${line}`;
...
interface BlobState
  extends HoverState<HoverContext, HoverMerged, ActionItemAction> {
  /**
...
export class Blob extends React.Component<BlobProps, BlobState> {
  ...
  private nextCodeViewElement = (element: HTMLElement | null): void =>
    this.codeViewElements.next(element);

  /** Emits whenever the ref callback for the blob element is called */
  private blobElements = new Subject<HTMLElement | null>();
  private nextBlobElement = (element: HTMLElement | null): void =>
    this.blobElements.next(element);

  /** Emits whenever the ref callback for the hover element is called */
  private hoverOverlayElements = new Subject<HTMLElement | null>();
  private nextOverlayElement = (element: HTMLElement | null): void =>
    this.hoverOverlayElements.next(element);

  /** Emits when the close button was clicked */
  private closeButtonClicks = new Subject<MouseEvent>();
  private nextCloseButtonClick = (event: MouseEvent): void =>
    this.closeButtonClicks.next(event);
  ...
  constructor(props: BlobProps) {
    ...
    >({
      ...
      getHover: position =>
        getHover(this.getLSPTextDocumentPositionParams(position), this.props),
      getDocumentHighlights: position =>
        getDocumentHighlights(
          this.getLSPTextDocumentPositionParams(position),
          this.props
        ),
      ...
      hoverifier.hoverStateUpdates.subscribe(update => {
        if (
          ...
          if (update.hoveredTokenElement) {
            ...
            update.hoveredTokenElement.addEventListener(
              "click",
              this.goToDefinition
            );
          }
          ...
          if (
            ...
            hash = toPositionOrRangeHash({
              range: {
                start: {
                  line: Math.min(
                    this.state.selectedPosition.line,
                    position.line
                  )
                },
                end: {
    ...
    this.subscriptions.add(
      locationPositions
        .pipe(withLatestFrom(this.codeViewElements))
        .subscribe(([position, codeView]) => {
          ...
          const codeCells = getCodeElementsInRange({
            ...
            position,
            getCodeElementFromLineNumber:
              domFunctions.getCodeElementFromLineNumber
          });
    ...
    this.subscriptions.add(
      combineLatest([modelChanges, locationPositions]).subscribe(
        ([model, position]) => {
      ...
      combineLatest([
        ...
        if (codeView) {
          ...
          for (const decoration of decorations) {
            ...
            const codeCell = domFunctions.getCodeElementFromLineNumber(
              codeView,
              line
            );
            if (!codeCell) {
            ...
            let decorated = false;
            const style = decorationStyleForTheme(
              decoration,
  ...
  private goToDefinition = (event: MouseEvent): void => {
    const goToDefinitionAction =
      ...
      this.state.actionsOrError.find(
        action => action.action.id === "goToDefinition.preloaded"
      );
   ...
   */
  private createLineDecorationAttachmentDOMNode(
    line: number,
  ...
  public shouldComponentUpdate(
    nextProps: Readonly<BlobProps>,
    nextState: Readonly<BlobState>
  ): boolean {
  ...
  public render(): React.ReactNode {
    return (
      <div
        className={`blob ${this.props.className}`}
        ref={this.nextBlobElement}
      >
        <code
          className={`blob__code ${
            this.props.wrapCode ? " blob__code--wrapped" : ""
          } test-blob`}
        ...
        {this.state.decorationsOrError &&
          ...
          this.state.decorationsOrError
            .filter(
              decoration =>
                !!decoration.after &&
                this.state.lineDecorationAttachmentIDs[
