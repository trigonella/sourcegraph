...
export const BlobPage: React.FunctionComponent<Props> = props => {
  ...
  const getPageTitle = (): string => {
    ...
    const repoString =
      repoNameSplit.length > 2 ? repoNameSplit.slice(1).join("/") : repoName;
    if (filePath) {
  ...
  useEffect(
    () => () => props.extensionsController.services.viewer.removeAllViewers(),
    ...
    return (
      <>
        ...
        <HeroPage
          ...
          title="Error"
          subtitle={
            <ErrorMessage error={blobOrError} history={props.history} />
          }
        />
      </>
    );
