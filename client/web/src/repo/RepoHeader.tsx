...
class RepoHeaderContributionStore {
  constructor(
    ...
    private setState: (
      callback: (
        previousState: RepoHeaderContribution[]
      ) => RepoHeaderContribution[]
    ) => void
  ) {}
...
export const RepoHeader: React.FunctionComponent<Props> = ({
  ...
  const [repoHeaderContributions, setRepoHeaderContributions] = useState<
    RepoHeaderContribution[]
  >([]);
  const repoHeaderContributionStore = useMemo(
    () =>
      new RepoHeaderContributionStore(contributions =>
        setRepoHeaderContributions(contributions)
      ),
  ...
  const leftActions = repoHeaderContributions.filter(
    ({ position }) => position === "left"
  );
  const rightActions = repoHeaderContributions.filter(
