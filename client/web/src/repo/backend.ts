...
import {ExternalLinkFields, TreeFields} from '../graphql-operations'
 ...
 *
 * @returns Observable that emits the commit ID. Errors with a
 *     `CloneInProgressError` if the repo is still being cloned.
 */
export const resolveRevision = memoizeObservable(
    ({repoName, revision}: RepoSpec&
     Partial<RevisionSpec>): Observable<ResolvedRevision> =>
        queryGraphQL(gql`
            ...
            .pipe(map(({data, errors}) => {
              ...
              if (data.repositoryRedirect.mirrorInfo.cloneInProgress) {
                throw new CloneInProgressError(
                    repoName,
                    data.repositoryRedirect.mirrorInfo.cloneProgress ||
                        undefined)
              }
              ...
              if (!data.repositoryRedirect.defaultBranch ||
                  !data.repositoryRedirect.commit.tree) {
                throw new RevisionNotFoundError('HEAD')
              }
              return {
                commitID: data.repositoryRedirect.commit.oid,
                    defaultBranch:
                        data.repositoryRedirect.defaultBranch.abbrevName,
    ...
    (context: FetchFileParameters): Observable<HighlightedFileResult> =>
        queryGraphQL(gql`
            ...
            .pipe(map(({data, errors}) => {
              ...
              return {
                isDirectory: file.isDirectory, richHTML: file.richHTML,
                    highlightedFile: file.highlight
              }
            })),
    context =>
        makeRepoURI(context) +
        `?disableTimeout=${String(context.disableTimeout)}&isLightTheme=${
            String(context.isLightTheme)}`)
...
export const fetchHighlightedFileLines = memoizeObservable(
    (context: FetchFileParameters, force?: boolean): Observable<string[]> =>
        fetchHighlightedFile(context, force).pipe(map(result => {
          if (result.isDirectory) {
            return []
          }
          const parsed = result.highlightedFile.html.slice('<table>'.length,
                                                           -'</table>'.length)
    ...
    context =>
        makeRepoURI(context) + `?isLightTheme=${String(context.isLightTheme)}`)

export const fetchFileExternalLinks = memoizeObservable(
    (context: RepoRevision&{filePath : string}):
        Observable<ExternalLinkFields[]> =>
            queryGraphQL(gql`
