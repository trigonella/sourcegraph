...
const queryGitBranches = memoizeObservable(
  (args: { repo: GQL.ID; first: number }): Observable<Data> =>
    queryGraphQL(
      gql`
        query RepositoryGitBranchesOverview(
          $repo: ID!
          $first: Int!
...
interface Props
  extends RepositoryBranchesAreaPageProps,
    RouteComponentProps<{}> {
  history: H.History;
}
...
export class RepositoryBranchesOverviewPage extends React.PureComponent<
  Props,
  State
> {
  public state: State = {};
            ...
            {this.state.dataOrError.activeBranches.length > 0 && (
              <div className="card repository-branches-page__card">
                ...
                <div className="list-group list-group-flush">
                  {this.state.dataOrError.activeBranches.map(
                    (gitReference, index) => (
