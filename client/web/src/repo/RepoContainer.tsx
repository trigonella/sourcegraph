...
import {
  useObservable,
  useEventObservable
} from "../../../shared/src/util/useObservable";
...
export const RepoContainer: React.FunctionComponent<RepoContainerProps> = props => {
    ...
    React.useMemo(
      () =>
        resolveRevision({ repoName, revision }).pipe(
          ...
          repeatUntil(value => !isCloneInProgressErrorLike(value), {
            delay: 1000
          }),
  ...
  useEffect(() => {
    ...
    props.extensionsController.services.workspace.roots,
    repoName,
  ...
  useEffect(() => {
    if (splitSearchModes && interactiveSearchMode) {
      ...
      if (filePath) {
        query = `${query.trimEnd()} file:${quoteIfNeeded(
          globbing ? filePath : "^" + escapeRegExp(filePath)
        )}`;
      }
      onNavbarQueryChange({
        query,
  ...
  const [
    ...
    setHasDismissedExtensionAlert
  ] = useLocalStorage(HAS_DISMISSED_ALERT_KEY, false);
      ...
      <RepoHeaderContributionPortal
        ...
        element={
          <GoToCodeHostAction
            ...
            revision={
              rawRevision || repoOrError.defaultBranch?.displayName || "HEAD"
            }
            filePath={filePath}
