...
export const TreeEntriesSection: React.FunctionComponent<{
  ...
  const directChildren = entries.filter(
    entry => entry.path === [parentPath, entry.name].filter(Boolean).join("/")
  );
  ...
  return (
    <div
      className={
        directChildren.length > MIN_ENTRIES_FOR_COLUMN_LAYOUT
          ? "tree-entries-section--columns"
          : undefined
      }
    >
