...
const breakpoints: Record<BreakpointName, number> = {
  ...
  md: 768,
  lg: 992
}; // no xl because TreePage's max-width is the xl breakpoint.
const columns: Record<BreakpointName, number> = { xs: 1, sm: 6, md: 8, lg: 12 };
const defaultItemsPerRow: Record<BreakpointName, number> = {
  xs: 1,
  sm: 2,
  md: 2,
  lg: 3
};
const minWidths: Record<BreakpointName, number> = {
  xs: 1,
  sm: 2,
  md: 3,
  lg: 3
        ...
        [
          ...
          views.map(
            ({ id }, index): ReactGridLayout => {
              const width =
                columns[breakpointName] / defaultItemsPerRow[breakpointName];
              return {
                i: id,
                h: defaultHeight,
                w: width,
        ...
        <div key={id} className={classNames("card view-grid__item")}>
          {view === undefined ? (
            ...
            <>
              ...
              {view.subtitle && (
                <div className="view-grid__view-subtitle">{view.subtitle}</div>
              )}
              <ViewContent
                {...props}
