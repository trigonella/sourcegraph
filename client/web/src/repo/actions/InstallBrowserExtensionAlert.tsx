...
const CHROME_EXTENSION_STORE_LINK =
...
export const InstallBrowserExtensionAlert: React.FunctionComponent<Props> = ({
  ...
  const copyCore =
    serviceType === "phabricator"
      ...
      : `while browsing files and reading ${
          serviceType === "gitlab" ? "MR" : "PR"
        }s`;
        ...
        <p className="install-browser-extension-alert__text my-0 mr-3">
          {codeHostIntegrationMessaging === "native-integration" ? (
            <>
              ...
              Your site admin set up the Sourcegraph native integration for{" "}
              {displayName}.{" "}
              <a
            ...
            <>
              ...
              to add code intelligence {copyCore} on {displayName} or any other
              connected code host.
            </>
          ) : (
            <>
              Get code intelligence {copyCore} on {displayName} or any other
              connected code host.{" "}
              <a
