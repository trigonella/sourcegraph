...
interface Props
  extends RevisionSpec,
    Partial<FileSpec>,
    GoToCodeHostPopoverProps {
  repo?: Pick<
    GQL.IRepository,
    "name" | "defaultBranch" | "externalURLs"
  > | null;
  filePath?: string;
  commitRange?: string;
  position?: Position;
...
export const GoToCodeHostAction: React.FunctionComponent<Props> = props => {
  ...
  ] = useLocalStorage(HAS_PERMANENTLY_DISMISSED_POPUP_KEY, false);
   ...
   */
  const fileExternalLinksOrError = useObservable<
  ...
  const defaultBranch =
    (!isErrorLike(props.repo) &&
      props.repo &&
      props.repo.defaultBranch &&
      props.repo.defaultBranch.displayName) ||
    "HEAD";
  ...
  const { displayName, icon } = serviceTypeDisplayNameAndIcon(
    externalURL.serviceType
  );
  ...
  if (
    externalURL.serviceType === "github" ||
    externalURL.serviceType === "gitlab"
  ) {
    // If in a branch, add branch path to the code host URL.
    if (
      props.revision &&
      props.revision !== defaultBranch &&
