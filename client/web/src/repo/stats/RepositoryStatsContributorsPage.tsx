...
interface RepositoryContributorNodeProps
  extends QuerySpec,
...
const queryRepositoryContributors = memoizeObservable(
    ...
    queryGraphQL(
      ...
      map(({ data, errors }) => {
        if (
          !data ||
          !data.node ||
          !(data.node as GQL.IRepository).contributors ||
          errors
        ) {
  ...
  args =>
    `${args.repo}:${String(args.first)}:${String(args.revisionRange)}:${String(
      args.after
    )}:${String(args.path)}`
);

const equalOrEmpty = (a: string | null, b: string | null): boolean =>
  a === b || (!a && !b);
...
class FilteredContributorsConnection extends FilteredConnection<
  ...
  Pick<
    RepositoryContributorNodeProps,
    "repoName" | "revisionRange" | "after" | "path" | "patternType" | "globbing"
  >
> {}
...
export class RepositoryStatsContributorsPage extends React.PureComponent<
  Props,
  State
> {
  private static REVISION_RANGE_INPUT_ID =
    "repository-stats-contributors-page__revision-range";
                  ...
                  <div className="input-group-append">
                    <div className="btn-group">
                      <button
                        ...
                        onClick={() =>
                          this.setStateAfterAndSubmit("7 days ago")
                        }
                      >
                        Last 7 days
                      </button>
                      <button
                        ...
                        onClick={() =>
                          this.setStateAfterAndSubmit("30 days ago")
                        }
                      >
                        Last 30 days
                      </button>
                      <button
                        ...
                        onClick={() =>
                          this.setStateAfterAndSubmit("1 year ago")
                        }
                      >
                        Last year
                      </button>
              ...
              <div className="repository-stats-page__row form-inline">
                <div className="input-group mt-2 mr-sm-2">
                  <div className="input-group-prepend">
                    <label
                      htmlFor={
                        RepositoryStatsContributorsPage.REVISION_RANGE_INPUT_ID
                      }
                      className="input-group-text"
                    >
                      Revision range
