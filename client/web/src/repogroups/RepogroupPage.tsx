...
export const RepogroupPage: React.FunctionComponent<RepogroupPageProps> = (
  props: RepogroupPageProps
) => {
  useEffect(
    () =>
      props.telemetryService.logViewEvent(
        `Repogroup:${props.repogroupMetadata.name}`
      ),
    [props.repogroupMetadata.name, props.telemetryService]
  );
  ...
  const repogroups: { [name: string]: string[] } | undefined = useMemo(
    () =>
      ...
      props.settingsCascade.final["search.repositoryGroups"],
              ...
              <div className="repogroup-page__repo-card card">
                ...
                <div className="repogroup-page__repo-list row">
                  ...
                  <div className="col-lg-6">
                    {repogroupRepoList
                      ?.slice(
                        Math.ceil(repogroupRepoList.length / 2),
                        repogroupRepoList.length
                      )
                      .map(repo => (
                        <RepoLink key={repo} repo={repo} />
                      ))}
...
interface RepogroupPageLogoProps
  extends Exclude<React.ImgHTMLAttributes<HTMLImageElement>, "src"> {
  icon: string;
  text: string;
}
