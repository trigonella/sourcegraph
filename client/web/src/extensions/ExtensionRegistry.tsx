...
export type ExtensionListData =
  | typeof LOADING
  | (ConfiguredExtensionRegistry & { error: string | null })
  | ErrorLike;
...
export const ExtensionRegistry: React.FunctionComponent<Props> = props => {
  ...
  const [enablementFilter, setEnablementFilter] = useState<
      ...
      newQueries =>
        newQueries.pipe(
          ...
          tap(({ query }) => {
            ...
            history.replace({
              search: query
                ? new URLSearchParams({ [URL_QUERY_PARAM]: query }).toString()
                : "",
              hash: location.hash
            });
          }),
          ...
          switchMap(({ query, immediate, settingsCascade }) => {
            ...
            if (!isErrorLike(settingsCascade.final)) {
              if (settingsCascade.final?.extensions) {
                viewerConfiguredExtensions = Object.keys(
                  settingsCascade.final.extensions
                );
              }
            }
  ...
  const onQueryChangeImmediate = useCallback(
    (query: string) =>
      nextQueryInput({ query, immediate: true, settingsCascade }),
    [nextQueryInput, settingsCascade]
  );
    ...
    <>
      <div className="container">
        ...
        <div className="pt-3">
          <div className="mb-5">
            <p>
              Improve your workflow with code intelligence, test coverage, and
            ...
            {!authenticatedUser && (
              <div className="alert alert-info my-4">
                <span>
                  An account is required to create, enable and disable
                  extensions.{" "}
                </span>
