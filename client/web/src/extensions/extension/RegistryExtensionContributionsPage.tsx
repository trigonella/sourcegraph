...
const ContributionsTable: React.FunctionComponent<{
  ...
  <div>
    {contributionGroups.length === 0 && (
      <p>
        This extension doesn't define any settings or actions. No configuration
...
function toContributionsGroups(
  ...
  const settingsGroup: ContributionGroup = {
    title: "Settings",
    ...
    if (Array.isArray(manifest.contributes.actions)) {
      for (const action of manifest.contributes.actions) {
        ...
        if (manifest.contributes.menus) {
          for (const menu of Object.keys(
            manifest.contributes.menus
          ) as ContributableMenu[]) {
        ...
        actionsGroup.rows.push([
          ...
          description.includes("${") ? (
            <>
              ...
              <small className="text-monospace">{description}</small>
            </>
          ) : (
            description
          ),
...
export class RegistryExtensionContributionsPage extends React.PureComponent<
  Props
> {
      ...
      <div className="registry-extension-contributions-page">
        ...
        <div className="mt-3">
          {this.props.extension.manifest === null ? (
            ...
            <ContributionsTable
              contributionGroups={toContributionsGroups(
