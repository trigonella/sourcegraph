...
export class RegistryExtensionManifestPage extends React.PureComponent<
  Props,
  State
> {
  ...
  private static getViewMode(): ViewMode {
    const storedViewMode = localStorage.getItem(
      RegistryExtensionManifestPage.STORAGE_KEY
    );
  ...
  public state: State = {
    viewMode: RegistryExtensionManifestPage.getViewMode()
  };
          ...
          <div>
            {this.props.extension.manifest && (
              <button
                ...
                onClick={this.onViewModeButtonClick}
              >
                <EyeIcon className="icon-inline" /> Use{" "}
                {this.state.viewMode === ViewMode.Plain
                  ? ViewMode.Rich
                  : ViewMode.Plain}{" "}
                viewer
              </button>
  ...
  private onViewModeButtonClick = (): void => {
    this.setState(
      previousState => ({
        viewMode:
          previousState.viewMode === ViewMode.Rich
            ? ViewMode.Plain
            : ViewMode.Rich
      }),
