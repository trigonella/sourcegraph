...
export const ExtensionsList: React.FunctionComponent<Props> = ({
  ...
  if (Object.keys(extensions).length === 0) {
    return (
      <>
        {error && (
          <ErrorAlert className="mb-2" error={error} history={history} />
        )}
        {query ? (
  ...
  const renderLanguages =
    (selectedCategories.length === 0 && showMoreExtensions) ||
    selectedCategories.includes("Programming languages");
  ...
  const categorySections = filteredCategoryIDs
    ...
    .map(category => (
      <div key={category} className="mt-1">
        <h3 className="extensions-list__category font-weight-bold">
          {category}
        </h3>
  ...
  return (
    <>
      ...
      {categorySections.length > 0 ? (
        ...
        <div className="text-muted">
          No extensions match <strong>{query}</strong> in the selected
          categories.
        </div>
      )}
    </>
