...
const NO_VALID_CATEGORIES: 'Other'[] = [ 'Other' ]
     ...
     *
     * `extensionIDsByCategory`: Object mapping category name to array of
     * extension ids in that category `extensions`: Object mapping extension id
     * to the configured extension with that id
     *
     * `configureExtensionRegistry` is passed a cache of configured extensions
     * to avoid parsing manifests multiple times during the lifecycle of the
     * extension registry.
     */
    export function configureExtensionRegistry(
        ...
        ConfiguredExtensionRegistry {
          ...
          const extensionIDsByCategory:
              ConfiguredExtensionRegistry['extensionIDsByCategory'] =
                  createRecord(
                      EXTENSION_CATEGORIES,
          ...
          for (const node of nodes) {
            ...
            let configuredRegistryExtension =
                configuredExtensionCache.get(node.id)
            ...
            let categories:
                ExtensionCategory[] if (!isErrorLike(configuredRegistryExtension
                                                         .manifest) &&
                                        configuredRegistryExtension.manifest
                                            ?.categories) {
              categories =
                  validCategories(
                      configuredRegistryExtension.manifest.categories) ||
                  NO_VALID_CATEGORIES
            }
            else {
            ...
            // Primary category is either specified or inferred by array
            // position
            const primaryCategory = categories[0]

            extensionIDsByCategory[primaryCategory].primaryExtensionIDs.push(
                configuredRegistryExtension.id)
            for (const category of categories) {
              extensionIDsByCategory[category].allExtensionIDs.push(
                  configuredRegistryExtension.id)
            }

            extensions[configuredRegistryExtension.id] =
                configuredRegistryExtension
          }
...
export function applyCategoryFilter(
    extensionIDsByCategory:
        ConfiguredExtensionRegistry['extensionIDsByCategory'],
    categories: ExtensionCategory[], selectedCategories: ExtensionCategory[]):
    Record<ExtensionCategory, string[]> {
      if (selectedCategories.length === 0) {
        // Primary categories
        return createRecord(categories,
                            category => [...extensionIDsByCategory[category]
                                             .primaryExtensionIDs])
      }

      // Categorize in toggle order, make sure the same extension doesn't appear
      // twice.
      const filteredCategorizedExtensions =
          createRecord<ExtensionCategory, string[]>(selectedCategories,
                                                    () => [])
      ...
      for (const category of selectedCategories) {
        for (const extensionID of extensionIDsByCategory[category]
                 .allExtensionIDs) {
...
/**
 * Filters categorized registry extensions by enablement (enabled | disabled |
 * all)
 */
export function applyExtensionsEnablement(
    categorizedExtensions: Record<ExtensionCategory, string[]>,
    filteredCategoryIDs: ExtensionCategory[], enablement: ExtensionsEnablement,
    settings: Settings|ErrorLike|null): Record<ExtensionCategory, string[]> {
  if (enablement === 'all') {
  ...
  return createRecord(
      filteredCategoryIDs,
      category => categorizedExtensions[category].filter(
          extensionID => (enablement === 'enabled') ===
                         isExtensionEnabled(settings, extensionID)))
}
