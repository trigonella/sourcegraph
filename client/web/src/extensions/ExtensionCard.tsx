...
export const ExtensionCard = memo<Props>(function ExtensionCard({
  ...
  const manifest: ExtensionManifest | undefined =
    extension.manifest && !isErrorLike(extension.manifest)
      ? extension.manifest
      : undefined;
  ...
  const [
    optimisticFailure,
    setOptimisticFailure
  ] = useState<OptimisticUpdateFailure<boolean> | null>(null);
  ...
  const onToggleError = useCallback(
    (optimisticUpdateFailure: OptimisticUpdateFailure<boolean>) => {
      ...
      optimisticFailureManager.setTimeout(
        () => setOptimisticFailure(null),
        FEEDBACK_DELAY
      );
    },
    ...
    <div className="d-flex">
      <div
        className={classNames("extension-card card position-relative flex-1", {
          "alert alert-success p-0 m-0 extension-card--enabled":
            change === "enabled"
        })}
      >
              ...
              {extension.manifest ? (
                isErrorLike(extension.manifest) ? (
                  <span
                    ...
                    title={extension.manifest.message}
                  >
        ...
        {optimisticFailure && (
          <div className="alert alert-danger px-2 py-1 extension-card__disabled-feedback">
            <span className="font-weight-semibold">Network Error:</span> {name}{" "}
            is {optimisticFailure.previousValue ? "enabled" : "disabled"} again
          </div>
        )}
      </div>
    </div>
