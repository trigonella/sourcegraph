        ...
        ({ promise, previousValue, optimisticValue }) =>
          new Promise<OptimisticUpdateFailure<T>>(resolve => {
            promise.catch((error: Error) =>
              resolve({ error, previousValue, optimisticValue })
            );
          })
      ),
...
export const ExtensionToggle: React.FunctionComponent<Props> = ({
  ...
  const [askingForPermission, setAskingForPermission] = useState<boolean>(
    false
  );
  ...
  const [nextOptimisticUpdate] = useEventObservable<
    ...
    void
  >(
    useMemo(() => createOptimisticRollbackPipeline(onOptimisticError), [
      onOptimisticError
    ])
  );
  ...
  const updateEnablement = useCallback(
    (enabled: boolean) => {
      const highestPrecedenceSubject = getHighestPrecedenceSubject(
        settingsCascade
      );
      ...
      nextOptimisticUpdate({
        ...
        promise: platformContext.updateSettings(
          highestPrecedenceSubject.subject.id,
          {
...
function getHighestPrecedenceSubject(
  settingsCascade: SettingsCascadeOrError
): ConfiguredSubjectOrError | undefined {
  ...
  if (
    !highestPrecedenceSubject ||
    !highestPrecedenceSubject.subject.viewerCanAdminister
  ) {
    return;
  }
