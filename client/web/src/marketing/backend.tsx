 ...
 */
export function fetchAllSurveyResponses(args: {
  first?: number;
}): Observable<GQL.ISurveyResponseConnection> {
  return queryGraphQL(
    gql`
...
export function fetchAllUsersWithSurveyResponses(args: {
  ...
  return queryGraphQL(
    gql`
      query FetchAllUsersWithSurveyResponses(
        $activePeriod: UserActivePeriod
        $first: Int
        $query: String
      ) {
...
export type SurveyResponseConnectionAggregates = Exclude<
  GQL.ISurveyResponseConnection,
  "nodes"
>;
 ...
 */
export function fetchSurveyResponseAggregates(): Observable<
