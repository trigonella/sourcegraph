...
export const SignUpForm: React.FunctionComponent<SignUpFormProps> = ({
  ...
  const signUpFieldValidators: Record<
  ...
  const [
    ...
    emailInputReference
  ] = useInputValidation(signUpFieldValidators.email);

  const [
    usernameState,
    nextUsernameFieldChange,
    usernameInputReference
  ] = useInputValidation(signUpFieldValidators.username);

  const [
    passwordState,
    nextPasswordFieldChange,
    passwordInputReference
  ] = useInputValidation(signUpFieldValidators.password);
  ...
  const onRequestTrialFieldChange = useCallback(
  ...
  const externalAuthProviders = context.authProviders.filter(
  ...
  return (
    <>
      {error && (
        <ErrorAlert className="mt-4 mb-0" error={error} history={history} />
      )}
        ...
        <div className="form-group d-flex flex-column align-content-start">
          ...
          >
            <UsernameInput
              className={classNames(
                "signin-signup-form__input",
                deriveInputClassName(usernameState)
              )}
        ...
        <div className="form-group d-flex flex-column align-content-start">
          ...
          >
            <PasswordInput
              className={classNames(
                "signin-signup-form__input",
                deriveInputClassName(passwordState)
              )}
