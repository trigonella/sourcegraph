...
const SearchResults = lazyComponent(
  () => import("./search/results/SearchResults"),
  "SearchResults"
);
const SiteAdminArea = lazyComponent(
  () => import("./site-admin/SiteAdminArea"),
  "SiteAdminArea"
);
const ExtensionsArea = lazyComponent(
  () => import("./extensions/ExtensionsArea"),
  "ExtensionsArea"
);
const SearchConsolePage = lazyComponent(
  () => import("./search/SearchConsolePage"),
  "SearchConsolePage"
);
const SignInPage = lazyComponent(
...
const SiteInitPage = lazyComponent(
  () => import("./site-admin/init/SiteInitPage"),
  "SiteInitPage"
);

interface LayoutRouteComponentProps<
  RouteParameters extends { [K in keyof RouteParameters]?: string }
>
...
export interface LayoutRouteProps<
  Parameters_ extends { [K in keyof Parameters_]?: string }
> {
  path: string;
  exact?: boolean;
...
export const routes: readonly LayoutRouteProps<any>[] = [
  ...
  },
  {
    ...
    render: props =>
      props.showQueryBuilder ? (
        lazyComponent(
          () => import("./search/queryBuilder/QueryBuilderPage"),
          "QueryBuilderPage"
        )(props)
      ) : (
        <Redirect to="/search" />
  ...
  },
  {
    path: "/extensions",
    render: props => (
      <ExtensionsArea {...props} routes={props.extensionsAreaRoutes} />
    )
  },
  {
    path: "/help",
  ...
  },
  {
    ...
    render: props => (
      <RepogroupPage {...props} repogroupMetadata={python2To3Metadata} />
    ),
  ...
  },
  {
    path: "/kubernetes",
    render: props => (
      <RepogroupPage {...props} repogroupMetadata={kubernetes} />
    ),
    condition: props => window.context.sourcegraphDotComMode
  },
  {
  ...
  },
  {
    path: "/react-hooks",
    render: props => (
      <RepogroupPage {...props} repogroupMetadata={reactHooks} />
    ),
    condition: props => window.context.sourcegraphDotComMode
  },
  {
