...
describe("search/helpers", () => {
  ...
  describe("submitSearch()", () => {
    test("should update history", () => {
      ...
      expect(history.location.search).toEqual(
        "?q=querystring&patternType=literal"
      );
    });
    test("should keep trace param when updating history", () => {
      ...
      expect(history.location.search).toEqual(
        "?q=querystring&patternType=literal&trace=1"
      );
    });
  });

  const searchTypes: NonNullable<SearchType>[] = [
    "diff",
    "commit",
    "symbol",
    "repo",
  ...
  describe("getSearchTypeFromQuery()", () => {
    test("parses the search type in simple queries", () => {
      for (const searchType of searchTypes) {
        expect(getSearchTypeFromQuery(`type:${searchType}`)).toEqual(
          searchType
        );
      }
    });
    ...
    test("parses the search type in complex queries", () => {
      ...
      expect(
        getSearchTypeFromQuery(
          "repo:^github.com/sourcegraph/sourcegraph type:diff test"
        )
      ).toEqual("diff");
      expect(
        getSearchTypeFromQuery(
          "type:diff repo:^github.com/sourcegraph/sourcegraph test"
        )
      ).toEqual("diff");
    });
    ...
    test("returns symbol when multiple search types, including symbol, are specified", () => {
      ...
      expect(
        getSearchTypeFromQuery(
          "type:diff type:symbol repo:^github.com/sourcegraph/sourcegraph test"
        )
      ).toEqual("symbol");
    });
    ...
    test("returns the first search type specified when multiple search types, not including symbol, are specified", () => {
      expect(
        getSearchTypeFromQuery(
          "type:diff type:commit repo:^github.com/sourcegraph/sourcegraph test"
        )
      ).toEqual("diff");
    });
  });
  ...
  describe("toggleSearchType()", () => {
    test("returns the original query when the query already contains the correct type", () => {
      ...
      for (const searchType of searchTypes) {
        expect(toggleSearchType(`test type:${searchType}`, searchType)).toEqual(
          `test type:${searchType}`
        );
      }
    });
    ...
    test("appends type:$TYPE to the query when no type exists in the query", () => {
      for (const searchType of searchTypes) {
        expect(toggleSearchType("test", searchType)).toEqual(
          `test type:${searchType}`
        );
      }
    });
    ...
    test("replaces existing type in query with new type in simple queries", () => {
      expect(toggleSearchType("test type:commit", "diff")).toEqual(
        "test type:diff"
      );
      expect(toggleSearchType("type:diff test", "commit")).toEqual(
        "type:commit test"
      );
    });
    ...
    test("replaces existing type in query with new type in complex queries", () => {
      expect(
        toggleSearchType("test type:symbol repo:^sourcegraph/test", "diff")
      ).toEqual("test type:diff repo:^sourcegraph/test");
      expect(
        toggleSearchType("test type:symbol repo:^sourcegraph/test", null)
      ).toEqual("test  repo:^sourcegraph/test");
    });

    test("replaces the first type in query with new type in queries with multiple type fields", () => {
      expect(
        toggleSearchType(
          "test type:symbol type:commit repo:^sourcegraph/test",
          "diff"
        )
      ).toEqual("test type:diff type:commit repo:^sourcegraph/test");
    });
  });
  ...
  describe("suggestions", () => {
    ...
    const getArchivedSuggestions = () =>
      filterStaticSuggestions(
        { query: "archived:", cursorPosition: 9 },
        searchFilterSuggestions
      );
    const getFilterSuggestionStartingWithR = () =>
      filterStaticSuggestions(
        { query: filterQuery, cursorPosition: 6 },
        searchFilterSuggestions
      );
    ...
    describe(`${filterStaticSuggestions.name}()`, () => {
      test('filters suggestions for filters starting with "r"', () => {
        ...
        expect(
          getFilterSuggestionStartingWithR().map(suggestion => suggestion.value)
        ).toEqual(expect.arrayContaining(filtersStartingWithR));
      });

      test("filters suggestions for filter aliases", () => {
        const [{ value }] = filterStaticSuggestions(
          { query: "l", cursorPosition: 1 },
          searchFilterSuggestions
        );
      ...
      test('filters suggestions for word "test"', () => {
        expect(
          filterStaticSuggestions(
            { query: filterQuery, cursorPosition: 4 },
            searchFilterSuggestions
          )
        ).toHaveLength(0);
      });
      ...
      test('filters suggestions for the "archived:" filter', () => {
        ...
        expect(archivedSuggestions).toEqual(
          expect.arrayContaining(searchFilterSuggestions.archived.values)
        );
      });
    });
    ...
    describe(`${insertSuggestionInQuery.name}()`, () => {
      describe("inserts suggestions for a filter name", () => {
        const suggestion = getFilterSuggestionStartingWithR().find(
          ({ value }) => value === "repo:"
        );
        assert(suggestion);
  ...
  describe(`${validFilterAndValueBeforeCursor.name}()`, () => {
    ...
    it("returns values when a filter value is being typed", () => {
      expect(
        validFilterAndValueBeforeCursor({ query, cursorPosition: 10 })
      ).toEqual({
        filterIndex: 0,
    ...
    it("returns values when a filter is selected but no value char is typed yet", () => {
      expect(
        validFilterAndValueBeforeCursor({ query, cursorPosition: 9 })
      ).toEqual({
        filterIndex: 0,
    ...
    it("does not return a value if typed whitespace char", () => {
      expect(
        validFilterAndValueBeforeCursor({ query, cursorPosition: 13 })
      ).toStrictEqual(null);
    });
    it("correctly resolves filter aliases", () => {
      const query = "l:go";
      expect(
        validFilterAndValueBeforeCursor({ query, cursorPosition: query.length })
      ).toEqual({
        filterIndex: 0,
  ...
  describe("formatQueryForFuzzySearch", () => {
    ...
    it("isolates filters that are in isolatedFuzzySearchFilters", () => {
      expect(
        isolatedFuzzySearchFilters.map(formatForSearchWithFilter)
      ).toStrictEqual(
    ...
    it("replaces filter being typed with its `filterAliasForSearch`", () => {
      expect(
        Object.keys(filterAliasForSearch).map(formatForSearchWithFilter)
      ).toStrictEqual(
        Object.values(filterAliasForSearch).map(
          alias => `archived:Yes ${alias!}:value Props`
        )
      );
    });
  ...
  describe("toggleSearchFilter", () => {
    it("adds filter if it is not already in query", () => {
      expect(toggleSearchFilter("repo:test ", "lang:c++")).toStrictEqual(
        "repo:test lang:c++ "
      );
    });

    it("adds filter if it is not already in query, even if it matches substring for an existing filter", () => {
      expect(toggleSearchFilter("repo:test lang:c++ ", "lang:c")).toStrictEqual(
        "repo:test lang:c++ lang:c "
      );
    });

    it("removes filter from query it it exists", () => {
      expect(
        toggleSearchFilter("repo:test lang:c++ lang:c ", "lang:c")
      ).toStrictEqual("repo:test lang:c++");
    });
  });
});
