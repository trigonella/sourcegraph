...
class MockVisibilitySensor extends React.Component<{
  ...
  public componentWillUnmount(): void {
    if (this.props.onChange) {
      VISIBILITY_CHANGED_CALLBACKS.splice(
        VISIBILITY_CHANGED_CALLBACKS.indexOf(this.props.onChange),
        1
      );
    }
  }
...
describe("SearchResultsList", () => {
  ...
  const defaultProps: SearchResultsListProps = {
    ...
    extensionsController: {
      executeCommand: sinon.spy(),
      services: extensionsController.services
    },
  ...
  it("shows error message when the search GraphQL request returns an error", () => {
    const { container } = render(
      <BrowserRouter>
        <SearchResultsList
          {...defaultProps}
          resultsOrError={{ message: "test error", name: "TestError" }}
        />
      </BrowserRouter>
    );
  ...
  it("renders correct number of search results if there are multiple", () => {
    const props = {
      ...defaultProps,
      resultsOrError: MULTIPLE_SEARCH_REQUEST()
    };
    const { container } = render(
      <BrowserRouter>
  ...
  it("does not add filters to query in search suggestions link", () => {
    ...
    expect(link.href).toStrictEqual(
      "http://localhost/search?q=repo:test1%7Ctest2&patternType=regexp"
    );
  });
  ...
  it("shows both alerts and results if both are present", () => {
    ...
    expect(link.compareDocumentPosition(result)).toStrictEqual(
      link.DOCUMENT_POSITION_FOLLOWING
    );
  });
});
