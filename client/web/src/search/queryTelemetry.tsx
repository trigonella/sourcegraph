...
import {
  ...
  ParserResult,
  Sequence
} from "../../../shared/src/search/parser/parser";
...
function filterExistsInQuery(
  parsedQuery: ParserResult<Sequence>,
    ...
    for (const member of members) {
      if (member.token.type === "filter") {
        ...
        if (
          ...
          resolvedFilter.type === filterToMatch
        ) {
          return true;
        }
      }
...
function queryStringTelemetryData(query: string, caseSensitive: boolean) {
  ...
  return {
    ...
    field_repogroup: filterExistsInQuery(parsedQuery, "repogroup")
      ...
      : undefined,
    field_repohascommitafter: filterExistsInQuery(
      parsedQuery,
      "repohascommitafter"
    )
      ? {
