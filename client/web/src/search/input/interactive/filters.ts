...
export type FiniteFilterType =|FilterType.archived|FilterType.fork|
                              FilterType.type|FilterType.index|
                              FilterType.visibility|FilterType.stable

const withBooleanSuggestions = (type: FiniteFilterType):
    Suggestion[] => ['no', 'only', 'yes'].map(value => ({type, value}))

export const finiteFilters: Record < FiniteFilterType, {default : string
        values: Suggestion[]
    }
> = {
    archived: {
...
export const isFiniteFilter = (filter: FilterType): filter is FiniteFilterType =>
    !!resolveFilter(filter) && ['fork', 'archived', 'type', 'index', 'visibility'].includes(filter)
        ...
        export function isTextFilter(filter: FilterType):
            boolean { return !!resolveFilter(filter) && !isFiniteFilter(filter)}

        /**
         * Some filter types should have their suggestions searched without
         * influence from the rest of the query, as they will then influence the
         * scope of other filters.
         *
         * Same as {@link isolatedFuzzySearchFilters} but using FilterTypes
         * rather than SuggestionTypes.
         */
        export const isolatedFuzzySearchFiltersFilterType =
            [ FilterType.repo, FilterType.repogroup ]
