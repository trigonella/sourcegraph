...
export function generateStepTooltip(options: {
  ...
  const bottomRow = generateBottomRow(
    options.tour,
    options.stepNumber,
 ...
 */
export function generateBottomRow(
  tour: Shepherd.Tour,
  stepNumber: number,
  totalStepCount: number
): HTMLElement {
 ...
 */
export const isValidLangQuery = (query: string): boolean =>
  LANGUAGES.map(lang => `lang:${lang}`).includes(query);
...
export const advanceLangStep = (
  query: string,
  tour: Shepherd.Tour | undefined
): void => {
  if (
    query !== "lang:" &&
    isValidLangQuery(query.trim()) &&
