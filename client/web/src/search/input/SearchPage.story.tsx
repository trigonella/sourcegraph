...
const { add } = storiesOf("web/search/input/SearchPage", module).addParameters({
  design: {
    type: "figma",
    url:
      "https://www.figma.com/file/sPRyyv3nt5h0284nqEuAXE/12192-Sourcegraph-server-page-v1?node-id=255%3A3"
  },
...
add("Server without panels", () => (
  <WebStory>{webProps => <SearchPage {...defaultProps(webProps)} />}</WebStory>
));

add("Server without panels, with query builder", () => (
  <WebStory>
    {webProps => (
      <SearchPage {...defaultProps(webProps)} showQueryBuilder={true} />
    )}
  </WebStory>
));
...
add("Server with panels", () => (
  <WebStory>
    {webProps => (
      <SearchPage {...defaultProps(webProps)} showEnterpriseHomePanels={true} />
    )}
  </WebStory>
));
