...
export const SearchPageInput: React.FunctionComponent<Props> = (
  ...
  useEffect(() => {
    setUserQueryState({
      query: props.queryPrefix || "",
      cursorPosition: props.queryPrefix?.length || 0
    });
  }, [props.queryPrefix]);

  const quickLinks =
    (isSettingsValid<Settings>(props.settingsCascade) &&
      props.settingsCascade.final.quicklinks) ||
    [];
  ...
  const [hasCancelledTour, setHasCancelledTour] = useLocalStorage(
    HAS_CANCELLED_TOUR_KEY,
    false
  );
  ...
  const onSubmit = useCallback(
    (event?: React.FormEvent<HTMLFormElement>): void => {
      ...
      submitSearch({
        ...
        source: "home",
        searchParameters: tourWasActive
          ? [{ key: "onboardingTour", value: "true" }]
          : undefined
      });
    },
