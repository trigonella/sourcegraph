...
} from "../../repogroups/HomepageConfig";
...
export const SearchPage: React.FunctionComponent<SearchPageProps> = props => {
  const SearchExampleClicked = useCallback(
    (url: string) => (): void =>
      props.telemetryService.log("ExampleSearchClicked", { url }),
    [props.telemetryService]
  );
  const LanguageExampleClicked = useCallback(
    (language: string) => (): void =>
      props.telemetryService.log("ExampleLanguageSearchClicked", { language }),
    [props.telemetryService]
  );

  useEffect(() => props.telemetryService.logViewEvent("Home"), [
    props.telemetryService
  ]);
  ...
  return (
    <div className="web-content search-page d-flex flex-column align-items-center pb-5">
          ...
          <>
            <div className="search-page__repogroup-content container-fluid mt-5">
              <div className="d-flex align-items-baseline mb-3">
                <h3 className="search-page__help-content-header mr-2">
                  Search in repository groups
                </h3>
                ...
                <div className="col-xs-12 col-lg-5 col-xl-6">
                  ...
                  <ul className="list-group-flush p-0 mt-2">
                    <li className="list-group-item px-0 pt-3 pb-2">
                      ...
                      <p className="mt-2">
                        Find usages of the alert() method that displays an alert
                        box.
                      </p>
                    </li>
                    <li className="list-group-item px-0 pt-3 pb-2">
                      ...
                      >
                        <span className="search-keyword">repogroup:</span>python
                        from \B\.\w+ import \w+
                      </Link>{" "}
                      <p className="mt-2">
                        Search for explicit imports with one or more leading
                        dots that indicate current and parent packages involved,
                        across popular Python repositories.
                      </p>
                    </li>
                    <li className="list-group-item px-0 pt-3 pb-2">
                      <Link
                      ...
                      >
                        ...
                        <span className="search-keyword">type:</span>
                        diff <span className="search-keyword">after:</span>"1
                        week ago"
                      </Link>{" "}
                      <p className="mt-2">
                        Browse diffs for recent code changes in the 'golang/go'
                        GitHub repository.
                      </p>
                    </li>
                    <li className="list-group-item px-0 pt-3 pb-2">
                      ...
                      <p className="mt-2">
                        Use a ReplicationController configuration to ensure
                        specified number of pod replicas are running at any one
                        time.
                      </p>
                    </li>
                  </ul>
      ...
      {props.showEnterpriseHomePanels && props.authenticatedUser && (
        <HomePanels {...props} />
      )}
