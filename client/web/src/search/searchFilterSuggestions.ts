...
export type SearchFilterSuggestions = Record<
  ...
  { default?: string; values: Suggestion[] }
>;
...
export const searchFilterSuggestions: SearchFilterSuggestions = {
    ...
    values: [
      {
        value: "repo:",
        description:
          "regex-pattern (include results whose repository path matches)"
      },
      {
        value: "-repo:",
        description:
          "regex-pattern (exclude results whose repository path matches)"
      },
      {
        value: "repogroup:",
      ...
      },
      {
        value: "repohasfile:",
        description:
          "regex-pattern (include results from repos that contain a matching file)"
      },
      {
        value: "-repohasfile:",
        description:
          "regex-pattern (exclude results from repositories that contain a matching file)"
      },
      {
      ...
      },
      {
        value: "timeout:",
        description:
          '"string specifying time duration" (duration before timeout)'
      },
      {
        value: "after:",
        description:
          '"string specifying time frame" (time frame to match commits after)'
      },
      {
        value: "before:",
        description:
          '"string specifying time frame" (time frame to match commits before)'
      },
      {
        value: "author:",
  ...
  lang: {
    values: Object.keys(languageIcons).map(value => ({
  ...
  before: {
    values: [
      ...
      { value: '"1 day ago"' },
      { value: '"last thursday"' }
    ].map(assign({ type: FilterType.before }))
  },
  after: {
    values: [
      { value: '"1 week ago"' },
      { value: '"1 day ago"' },
      { value: '"last thursday"' }
    ].map(assign({ type: FilterType.after }))
  },
  content: {
    values: []
  },
  patterntype: {
    values: [
      ...
      { value: "structural" },
      { value: "regexp" }
    ].map(assign({ type: FilterType.patterntype }))
  },
  index: {
    default: "yes",
    values: [{ value: "no" }, { value: "only" }, { value: "yes" }].map(
