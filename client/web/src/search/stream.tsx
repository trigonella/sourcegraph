...
function toGQLFileMatchBase(
  fm: Omit<FileMatch, "lineMatches">
): GQL.IFileMatch {
  let revision = "";
  if (fm.branches) {
...
// eslint-disable-next-line @typescript-eslint/consistent-type-assertions
const toMarkdown = (text: string | MarkdownText): GQL.IMarkdown =>
  ({ __typename: "Markdown", text } as GQL.IMarkdown);
 ...
 */
export const switchToGQLISearchResults: OperatorFunction<
  SearchEvent,
  GQL.ISearchResults
> = pipe(
    ...
    switch (newEvent.type) {
      ...
      case "repomatches":
        return {
          ...
          results: results.results.concat(
            newEvent.matches.map(toGQLRepositoryMatch)
          )
        };
      ...
      case "commitmatches":
        return {
          ...
          results: results.results.concat(
            newEvent.matches.map(toGQLCommitMatch)
          )
        };
      ...
      case "symbolmatches":
        return {
          ...
          results: results.results.concat(
            newEvent.matches.map(toGQLSymbolMatch)
          )
        };
...
const observeMessages = <T extends {}>(
  eventSource: EventSource,
  eventName: SearchEvent["type"]
): Observable<T> =>
  fromEvent(eventSource, eventName).pipe(
    map((event: Event) => {
      if (!(event instanceof MessageEvent)) {
        throw new TypeError(
          `internal error: expected MessageEvent in streaming search ${eventName}`
        );
      }
      try {
        ...
        throw new Error(
          `Could not parse ${eventName} message data in streaming search`
        );
      }
    })
  );
...
export function search(
  ...
  return new Observable<SearchEvent>(observer => {
    ...
    const parameterEncoded = parameters
