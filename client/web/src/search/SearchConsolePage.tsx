...
} from "./results/SearchResultsList";
...
export const SearchConsolePage: React.FunctionComponent<SearchConsolePageProps> = props => {
  ...
  const triggerSearch = useCallback(() => {
    props.history.push(
      "/search/console?q=" + encodeURIComponent(searchQuery.value)
    );
  }, [props.history, searchQuery]);
  // Fetch search results when the `q` URL query parameter changes
  const resultsOrError = useObservable<
    "loading" | GQL.ISearchResults | ErrorLike
  >(
    useMemo(() => {
  ...
  const onExpandAllResultsToggle = useCallback((): void => {
    setAllExpanded(allExpanded => {
      props.telemetryService.log(
        allExpanded ? "allResultsExpanded" : "allResultsCollapsed"
      );
      return allExpanded;
    });
