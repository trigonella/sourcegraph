...
export const RecentSearchesPanel: React.FunctionComponent<Props> = ({
  ...
  const recentSearches = useObservable(
    useMemo(
      ...
      [authenticatedUser?.id, fetchRecentSearches, itemsToLoad]
    )
  );
  ...
  useEffect(() => {
    ...
    if (processedResults && itemsToLoad === pageSize) {
      telemetryService.log("RecentSearchesPanelLoaded", {
  ...
  const emptyDisplay = (
    <div className="panel-container__empty-container text-muted">
      <small className="mb-2">
        Your recent searches will be displayed here. Here are a few searches to
        get you started:
      </small>
    ...
    <>
      ...
      {recentSearches?.pageInfo.hasNextPage && (
        <ShowMoreButton
          ...
          className="test-recent-searches-panel-show-more"
        />
      )}
    </>
  );
  ...
  for (const node of eventLogResult.nodes) {
    if (node.argument) {
      ...
      const searchText: string =
        parsedArguments?.code_search?.query_data?.combined;
