...
describe("SavedSearchesPanel", () => {
  ...
  it('should show correct mode and number of entries when clicking on "my searches" and "all searches" buttons', () => {
    ...
    let savedSearchEntries = container.querySelectorAll(
      ".test-saved-search-entry"
    );
    expect(savedSearchEntries.length).toBe(2);
    const mySearchesButton = container.querySelector(
      ".test-saved-search-panel-my-searches"
    )!;
    ...
    const allSearchesButton = container.querySelector(
      ".test-saved-search-panel-all-searches"
    )!;
