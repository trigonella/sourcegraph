...
export const RepositoriesPanel: React.FunctionComponent<Props> = ({
  ...
  const loadingDisplay = (
    <LoadingPanelView text="Loading recently searched repositories" />
  );
  ...
  const emptyDisplay = (
    <div className="panel-container__empty-container text-muted">
      <small className="mb-2">
        ...
        <p className="mb-1">
          Add the code host to scope to a single repository:
        </p>
        <p className="mb-1 text-monospace">
  ...
  const [repoFilterValues, setRepoFilterValues] = useState<string[] | null>(
    null
  );

  const searchEventLogs = useObservable(
    useMemo(
      () => fetchRecentSearches(authenticatedUser?.id || "", itemsToLoad),
      [fetchRecentSearches, authenticatedUser?.id, itemsToLoad]
    )
  );
  ...
  useEffect(() => {
    ...
    if (repoFilterValues && itemsToLoad === pageSize) {
      telemetryService.log("RepositoriesPanelLoaded", {
        ...
        >
          <small>
            ...
            >
              ...
              <span className="repositories-panel__search-value">
                {repoFilterValue}
              </span>
            </Link>
          </small>
        ...
        if (
          ...
          (member.token.filterType.token.value === FilterType.repo ||
            member.token.filterType.token.value ===
              FILTERS[FilterType.repo].alias)
        ) {
          if (
            member.token.filterValue?.token.type === "literal" &&
            !recentlySearchedRepos.includes(
              member.token.filterValue.token.value
            )
          ) {
