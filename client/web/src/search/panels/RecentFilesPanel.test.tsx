...
describe("RecentFilesPanel", () => {
  test("duplicate files are only shown once", () => {
    ...
    expect(listItems.at(0).text()).toStrictEqual(
      "ghe.sgdev.org/sourcegraph/gorilla-mux › go.mod"
    );
    expect(listItems.at(1).text()).toStrictEqual(
      "github.com/sourcegraph/sourcegraph › .eslintrc.js"
    );
  });
    ...
    const recentFiles = {
      nodes: [
        ...
        },
        {
          ...
          timestamp: "2020-09-10T22:55:06Z",
          url:
            "https://sourcegraph.test:3443/bitbucket.sgdev.org/SOURCEGRAPH/jsonrpc2"
        }
      ],
      pageInfo: {
        endCursor: null,
  ...
  test("Show More button shown when more items can be loaded", () => {
    ...
    const component = mount(<RecentFilesPanel {...props} />);
    const showMoreButton = component
      .getDOMNode()
      .querySelectorAll(".test-recent-files-panel-show-more");
