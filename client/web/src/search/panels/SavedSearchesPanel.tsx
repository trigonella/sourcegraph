...
export const SavedSearchesPanel: React.FunctionComponent<Props> = ({
  ...
  const savedSearches = useObservable(
    useMemo(() => fetchSavedSearches(), [fetchSavedSearches])
  );
  ...
  useEffect(() => {
    ...
    if (savedSearches) {
      telemetryService.log("SavedSearchesPanelLoaded", {
        empty: savedSearches.length === 0,
        showAllSearches
      });
    }
  ...
  const logEvent = useCallback(
    (event: string, props?: any) => (): void =>
      telemetryService.log(event, props),
    [telemetryService]
  );
  ...
  const emptyDisplay = (
    <div className="panel-container__empty-container text-muted">
      <small>
        Use saved searches to alert you to uses of a favorite api, or changes to
        code you need to monitor.
      </small>
      {authenticatedUser && (
        <Link
          to={`/users/${authenticatedUser.username}/searches/add`}
          onClick={logEvent("SavedSearchesPanelCreateButtonClicked", {
            source: "empty view"
          })}
      ...
      <dl className="list-group-flush flex-grow-1">
        {savedSearches
          ?.filter(search =>
            showAllSearches
              ? true
              : search.namespace.id === authenticatedUser?.id
          )
          .map(search => (
            <dd
              key={search.id}
              className="text-monospace test-saved-search-entry"
            >
              <div className="d-flex justify-content-between">
                <small>
                  <Link
                    to={
                      "/search?" +
                      buildSearchURLQuery(search.query, patternType, false)
                    }
                    className=" p-0"
