...
export const RecentFilesPanel: React.FunctionComponent<Props> = ({
  ...
  const recentFiles = useObservable(
    useMemo(
      () => fetchRecentFileViews(authenticatedUser?.id || "", itemsToLoad),
      [authenticatedUser?.id, fetchRecentFileViews, itemsToLoad]
    )
  );

  const [processedResults, setProcessedResults] = useState<RecentFile[] | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    if (processedResults && itemsToLoad === pageSize) {
      telemetryService.log("RecentFilesPanelLoaded", {
        empty: processedResults.length === 0
      });
    }
  }, [processedResults, telemetryService, itemsToLoad]);

  const logFileClicked = useCallback(
    () => telemetryService.log("RecentFilesPanelFileClicked"),
    [telemetryService]
  );
  ...
  const emptyDisplay = (
    <div className="panel-container__empty-container align-items-center text-muted">
      ...
      <small className="mb-2">
        This panel will display your most recently viewed files.
      </small>
    </div>
  );
      ...
      {recentFiles?.pageInfo.hasNextPage && (
        <div className="test-recent-files-show-more-container">
          <ShowMoreButton
            ...
            className="test-recent-files-panel-show-more"
          />
        </div>
      )}
    </div>
  ...
  for (const node of eventLogResult.nodes) {
    if (node.argument) {
      ...
      if (
        ...
        repoName &&
        !recentFiles.some(
          file => file.repoName === repoName && file.filePath === filePath
