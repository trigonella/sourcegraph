...
const MaybeLink: React.FunctionComponent<React.AnchorHTMLAttributes<
  unknown
>> = ({ children, ...props }) =>
...
const dateTickFormat = new Intl.DateTimeFormat(undefined, {
  month: "numeric",
  day: "numeric"
});
const dateTickFormatter = dateTickFormat.format.bind(dateTickFormat);

const tooltipLabelFormat = new Intl.DateTimeFormat(undefined, {
  month: "short",
  day: "numeric",
  year: "numeric"
});
const dateTooltipLabelFormatter = tooltipLabelFormat.format.bind(
  tooltipLabelFormat
);
...
export const CartesianChartViewContent: React.FunctionComponent<{
  ...
  const yAxisTicks = useMemo(() => {
    const allValues = series.flatMap(series =>
      content.data.map(datum => datum[series.dataKey])
    );
    ...
    <ResponsiveContainer width="100%">
      ...
      >
        ...
        <XAxis
          ...
          tickFormatter={
            content.xAxis.scale === "time" ? dateTickFormatter : toLocaleString
          }
        />
        <YAxis
        ...
        <Tooltip
          ...
          labelFormatter={
            content.xAxis.scale === "time"
              ? (dateTooltipLabelFormatter as LabelFormatter)
              : undefined
          }
...
const percentageLabel: ContentRenderer<PieLabelRenderProps> = props =>
  props.name +
  (props.percent ? ": " + (props.percent * 100).toFixed(0) + "%" : "");
        ...
        {content.pies.map(({ fillKey, dataKey, nameKey, linkURLKey, data }) => (
          <Pie
            ...
            activeShape={({ key, ...props }) => (
              <MaybeLink
                key={key}
                href={linkURLKey && props.payload[linkURLKey]}
                onClick={linkHandler}
              >
                <Sector {...props} />
              </MaybeLink>
          ...
          >
            {fillKey &&
              data.map(entry => (
                <Cell key={entry[nameKey]} fill={entry[fillKey]} stroke="" />
              ))}
          </Pie>
        ))}
      </PieChart>
