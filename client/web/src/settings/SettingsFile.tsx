...
const disposableToFn = (disposable: _monaco.IDisposable) => () =>
  disposable.dispose();
...
export class SettingsFile extends React.PureComponent<Props, State> {
    ...
    const refreshedAfterSave = this.componentUpdates.pipe(
      ...
      filter(
        ({ settings, commitError }) =>
          !!settings &&
          !commitError &&
          ((typeof this.state.editingLastID === "number" &&
            settings.id > this.state.editingLastID) ||
            (typeof settings.id === "number" &&
              this.state.editingLastID === null))
      )
    );
    this.subscriptions.add(
  ...
  public render(): JSX.Element | null {
    ...
    const contents =
      this.state.contents === undefined
        ? this.getPropsSettingsContentsOrEmpty()
        : this.state.contents;
    ...
    return (
      <div className="settings-file test-settings-file d-flex flex-grow-1 flex-column">
        ...
        <React.Suspense
          fallback={<LoadingSpinner className="icon-inline mt-2" />}
        >
      ...
      this.subscriptions.add(
        disposableToFn(
          this.monaco.editor.onDidCreateModel(async model => {
            ...
            const { MonacoSettingsEditor } = await import(
              "./MonacoSettingsEditor"
            );

            if (
              this.editor &&
              MonacoSettingsEditor.isStandaloneCodeEditor(this.editor)
            ) {
  ...
  private getPropsSettingsContentsOrEmpty(
