 ...
 */
export class AccessTokenCreatedAlert extends React.PureComponent<
  AccessTokenCreatedAlertProps
> {
  public render(): JSX.Element | null {
    const isSudoToken = this.props.token.scopes.includes(
      AccessTokenScopes.SiteAdminSudo
    );
    return (
      <div className={`access-token-created-alert ${this.props.className}`}>
        <p>
          <CheckmarkCircleIcon className="icon-inline" /> Copy the new access
          token now. You won't be able to see it again.
        </p>
        <CopyableText
          className="test-access-token"
          text={this.props.tokenSecret}
          size={48}
        />
        <h5 className="mt-4">
