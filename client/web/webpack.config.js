...
const config = {
  context : __dirname, // needed when running `gulp webpackDevServer` from the
                       // root dir
  mode,
  optimization : {
    ...
    minimizer : [
      new TerserPlugin({
        ...
        terserOptions : {
          compress : {
            // // Don't inline functions, which causes name collisions with
            // uglify-es:
  ...
  entry : {
    // Enterprise vs. OSS builds use different entrypoints. The enterprise
    // entrypoint imports a
    // strict superset of the OSS entrypoint.
    app : [
      'react-hot-loader/patch',
      isEnterpriseBuild ? path.join(enterpriseDirectory, 'main.tsx')
  ...
  resolve : {
    ...
    alias : {
      // react-visibility-sensor's main field points to a UMD bundle instead of
      // ESM
  ...
  module : {
    rules : [
      // Run hot-loading-related Babel plugins on our application code only
      // (because they'd be
      ...
      },
      {
        ...
        use : [
          ...(mode === 'production' ? [ 'thread-loader' ] : []), babelLoader
        ],
      },
      {
        test : /\.(sass|scss)$/,
      ...
      {
        // CSS rule for monaco-editor and other external plain CSS (skip SASS
        // and PostCSS for build perf)
        test : /\.css$/,
