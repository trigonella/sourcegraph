 ...
 */
const logWebpackStats = stats => { log(stats.toString(WEBPACK_STATS_OPTIONS)) }
...
/**
 * Watch files and update the webpack bundle on disk without starting a dev
 * server.
 */
async function
watchWebpack() {
...
webpackDevelopmentServer() {
  ...
  const options = {
    ...
    proxy : {
      '/' : {
        ...
        onProxyReqWs : (_proxyRequest, _request, socket) => socket.on(
            'error', error => console.error('WebSocket proxy error:', error)),
      },
    },
    sockHost,
    sockPort,
  ...
  const server =
      new WebpackDevServer(createWebpackCompiler(webpackConfig), options)
 ...
 */
const build =
    gulp.series(gulp.parallel(schema, graphQlOperations, graphQlSchema),
...
const development = gulp.series(
    // Ensure the typings that TypeScript depends on are build to avoid
    // first-time-run errors
    gulp.parallel(schema, graphQlSchema),
    gulp.parallel(watchSchema, watchGraphQlSchema, watchGraphQlOperations,
                  webpackDevelopmentServer))
