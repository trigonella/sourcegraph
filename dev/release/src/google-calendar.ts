...
const SCOPES =
    [ 'https://www.googleapis.com/auth/calendar.events' ] const TOKEN_PATH =
        '.secrets/google-calendar-token.json'

export async function getClient():
    Promise<OAuth2Client> {
      const credentials = JSON.parse(await readLine(
...
async function
getAccessToken(oauth2Client: OAuth2Client):
    Promise<Credentials> {
      try {
...
async function
getAccessTokenNoCache(oauth2Client: OAuth2Client):
    Promise<Credentials> {
      ...
      const code = await readLine(
          'Log in via the browser page that just opened and enter the code that appears: ')
...
export async function ensureEvent({
  ...
  title,
                                  ...
                                  auth: OAuth2Client): Promise<void> {
  const existingEvents = await listEvents(auth)
  const foundEvents =
      (existingEvents || []).filter(({summary}) => summary === title)
  ...
  const calendar =
      google.calendar({version : 'v3', auth}) await calendar.events.insert({
        calendarId : 'primary',
        requestBody : {
          anyoneCanAddSelf,
