...
import {ensureEvent, EventOptions, getClient} from './google-calendar'
...
const formatDate = (date: Date): string => `${date.toLocaleString('en-US', {
  ...
  dryRun: {changesets: boolean}
}

type StepID =|'help'
    // release tracking
    |'tracking:release-timeline'|'tracking:release-issue'|'tracking:patch-issue'
    // branch cut
    |'changelog:cut'
    // release
    |'release:status'|'release:create-candidate'|'release:publish'
    // testing
    |'_test:google-calendar'|'_test:slack'

    interface Step {
  id: StepID
  run?: ((config: Config, ...args: string[]) => Promise<void>)|
      ((config: Config, ...args: string[]) => void)
  argNames?: string[]
}
    ...
    [
      {
        ...
        run : () => {
          ...
          console.error(
              steps.filter(({id}) => !id.startsWith('_'))
                  .map(({id, argNames}) =>
                           '\t' + id +
      ...
      },
      {
        ...
        run : async config => {
          const googleCalendar =
              await getClient() await ensureEvent({
                title : 'TEST EVENT',
                startDateTime : new Date(config.releaseDateTime).toISOString(),
                endDateTime : addMinutes(new Date(config.releaseDateTime), 1)
                                  .toISOString(),
              },
                                                  googleCalendar)
        },
      },
      {
        ...
        run :
            async (_config,
                   message) => { await postMessage(message, '_test-channel') },
      },
      {
        ...
        run : async config => {
          ...
          const events: EventOptions[] =
              [
                {
                  title :
                      'Release captain: prepare for branch cut (5 working days until release)',
                  description : 'See the release tracking issue for TODOs',
                  startDateTime : new Date(config.fiveWorkingDaysBeforeRelease)
                                      .toISOString(),
                  endDateTime :
                      addMinutes(new Date(config.fiveWorkingDaysBeforeRelease),
                                 1)
                          .toISOString(),
                },
                {
                  title :
                      'Release captain: branch cut (4 working days until release)',
                  description : 'See the release tracking issue for TODOs',
                  startDateTime : new Date(config.fourWorkingDaysBeforeRelease)
                                      .toISOString(),
                  endDateTime :
                      addMinutes(new Date(config.fourWorkingDaysBeforeRelease),
                                 1)
                          .toISOString(),
                },
                ...eachDayOfInterval({
                  start :
                      addDays(new Date(config.fourWorkingDaysBeforeRelease), 1),
                ...
                {
                  title :
                      'Release captain: tag final release (1 working day before release)',
                  description : 'See the release tracking issue for TODOs',
                  startDateTime :
                      new Date(config.oneWorkingDayBeforeRelease).toISOString(),
                  endDateTime :
                      addMinutes(new Date(config.oneWorkingDayBeforeRelease), 1)
                          .toISOString(),
                },
                {
                  title : `Cut release branch ${config.majorVersion}.${
                      config.minorVersion}`,
                  description :
                ...
                {
                  title : `Release Sourcegraph ${config.majorVersion}.${
                      config.minorVersion}`,
                  description :
                      '(This is not an actual event to attend, just a calendar marker.)',
                  anyoneCanAddSelf : true,
                  attendees : [ config.teamEmail ],
                  startDateTime :
                      new Date(config.releaseDateTime).toISOString(),
                  endDateTime : addMinutes(new Date(config.releaseDateTime), 1)
                                    .toISOString(),
                },
              ]
      ...
      },
      {
        ...
        run : async ({
          ...
          const {url, created} = await ensureTrackingIssue({
            ...
            fourWorkingDaysBeforeRelease :
                new Date(fourWorkingDaysBeforeRelease),
            fiveWorkingDaysBeforeRelease :
                new Date(fiveWorkingDaysBeforeRelease),
          })
          console.log(created ? `Created tracking issue ${url}`
                              : `Tracking issue already exists: ${url}`)
...
Tracking issue: ${url}
Key dates:
- Release branch cut, testing commences: ${
                    formatDate(new Date(fourWorkingDaysBeforeRelease))}
...
- Release: ${formatDate(new Date(releaseDateTime))}`,
      ...
      },
      {
        ...
        run : async ({captainGitHubUsername, slackAnnounceChannel},
                     version) => {
          ...
          if (created) {
            await postMessage(
                `:captain: Patch release ${
                    parsedVersion
                        .version} will be published soon. If you have changes that should go into this patch release, please add your item to the checklist in the issue description: ${
                    url}`,
                slackAnnounceChannel)
            console.log(`Posted to Slack channel ${slackAnnounceChannel}`)
          }
            ...
            changes : [
              {
                ...
                commitMessage :
                    `Update latest release to ${parsedVersion.version}`,
                edits : [
                  (directory: string) => {
                    console.log(`Updating '${changelogFile} for ${
                    ...
                    changelogContents = changelogContents.replace(
                        unreleasedHeader, releaseHeader)

                    // Add a blank changelog template for the next release
                    changelogContents = changelogContents.replace(
                        changelog.divider, changelog.releaseTemplate)
      ...
      },
      {
        ...
        run : async (config, version) => {
          ...
          if (!trackingIssueURL) {
            throw new Error(
                `Tracking issue for version ${config.majorVersion}.${
                    config.minorVersion} not found--has it been create yet?`)
          }
          ...
          const blockingIssuesURL =
              `https://github.com/issues?q=${encodeURIComponent(blockingQuery)}`

          const openQuery = `is:open org:sourcegraph is:issue milestone:${
              config.majorVersion}.${config.minorVersion}`
          const openIssues = await listIssues(githubClient, openQuery)
          const openIssuesURL =
              `https://github.com/issues?q=${encodeURIComponent(openQuery)}`
...
${
                  issueCategories
                      .map(category =>
                               ...
                               (category.issues.length === 1
                                    ? `There is 1 ${category.name} issue: ${
                                          category.issuesURL}`
                                    : `There are ${category.issues.length} ${
                                          category.name} issues: ${
                                          category.issuesURL}`))
                      .join('\n')}` await postMessage(
                  message, config.slackAnnounceChannel)
        },
      },
      {
        ...
        run : async (_config, version) => {
          ...
          const branch =
              JSON.stringify(`${parsedVersion.major}.${parsedVersion.minor}`)
          console.log(`Creating and pushing tag ${tag}`)
              await execa('bash',
                          [
                            '-c',
                            `git diff --quiet && git checkout ${
                                branch} && git pull --rebase && git tag -a ${
                                tag} -m ${tag} && git push origin ${tag}`,
                          ],
                          {stdio : 'inherit'})
        },
      },
        ...
        run : async ({slackAnnounceChannel, dryRun}, version) => {
            ...
            changes : [
              {
                ...
                commitMessage :
                    `Update latest release to ${parsedVersion.version}`,
                edits : [
                  `find . -type f -name '*.md' ! -name 'CHANGELOG.md' -exec ${
                      sed} -i -E 's/sourcegraph\\/server:[0-9]+\\.[0-9]+\\.[0-9]+/sourcegraph\\/server:${
                      parsedVersion.version}/g' {} +`,
                  `${sed} -i -E 's/version \`[0-9]+\\.[0-9]+\\.[0-9]+\`/version \`${
                      parsedVersion.version}\`/g' doc/index.md`,
                  parsedVersion.patch === 0
                      ? `comby -in-place '{{$previousReleaseRevspec := ":[1]"}} {{$previousReleaseVersion := ":[2]"}} {{$currentReleaseRevspec := ":[3]"}} {{$currentReleaseVersion := ":[4]"}}' '{{$previousReleaseRevspec := ":[3]"}} {{$previousReleaseVersion := ":[4]"}} {{$currentReleaseRevspec := "v${
                            parsedVersion
                                .version}"}} {{$currentReleaseVersion := "${
                            parsedVersion.major}.${
                            parsedVersion
                                .minor}"}}' doc/_resources/templates/document.html`
                      : `comby -in-place 'currentReleaseRevspec := ":[1]"' 'currentReleaseRevspec := "v${
                            parsedVersion
                                .version}"' doc/_resources/templates/document.html`,
                  `comby -in-place 'latestReleaseKubernetesBuild = newBuild(":[1]")' "latestReleaseKubernetesBuild = newBuild(\\"${
                      parsedVersion
                          .version}\\")" cmd/frontend/internal/app/updatecheck/handler.go`,
                  `comby -in-place 'latestReleaseDockerServerImageBuild = newBuild(":[1]")' "latestReleaseDockerServerImageBuild = newBuild(\\"${
                      parsedVersion
                          .version}\\")" cmd/frontend/internal/app/updatecheck/handler.go`,
                ],
                title : `Update latest release to ${parsedVersion.version}`,
              },
              {
                ...
                edits : [
                  ...
                  `.github/workflows/scripts/update-docker-tags.sh ${
              ...
              },
              {
                ...
                commitMessage :
                    `Update latest release to ${parsedVersion.version}`,
                edits : [
                  `${sed} -i -E 's/export SOURCEGRAPH_VERSION=[0-9]+\\.[0-9]+\\.[0-9]+/export SOURCEGRAPH_VERSION=${
                      parsedVersion.version}/g' resources/amazon-linux2.sh`,
                ],
                title : `Update latest release to ${parsedVersion.version}`,
              },
              {
                ...
                edits : [
                  `${sed} -i -E 's/export SOURCEGRAPH_VERSION=[0-9]+\\.[0-9]+\\.[0-9]+/export SOURCEGRAPH_VERSION=${
                      parsedVersion.version}/g' resources/user-data.sh`,
                ],
                title : `Update latest release to ${parsedVersion.version}`,
              },
              {
                ...
                commitMessage :
                    `Update latest release to ${parsedVersion.version}`,
                edits : [
                  `${sed} -i -E 's/"defaultContentBranch":"[[:alnum:]\\.]+"/"defaultContentBranch":"${
                      parsedVersion.major}.${
                      parsedVersion
                          .minor}"/g' configure/docs-sourcegraph-com/docs-sourcegraph-com.Deployment.yaml`,
                ],
                title : `Update latest release to ${parsedVersion.version}`,
              },
            ],
          ...
          if (!dryRun.changesets) {
            await postMessage(
                `${
                    parsedVersion
                        .version} has been released, update deploy-sourcegraph-docker as needed, cc @stephen`,
                slackAnnounceChannel)
          }
        },
      },
    ...
    async function
    run(config: Config, stepIDToRun: StepID, ...stepArguments: string[]):
        Promise<void> {
          await Promise.all(
              steps.filter(({id}) => id === stepIDToRun).map(async step => {
                if (step.run) {
    ...
    main():
        Promise<void> {
          ...
          const step =
              args[0] if (!steps.map(({id}) => id as string).includes(step)) {
            console.error('Unrecognized step', JSON.stringify(step))
            return
          }
          const stepArguments =
              args.slice(1) await run(config, step as StepID, ...stepArguments)
        }
