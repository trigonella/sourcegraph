...
interface Step {
  id: StepID;
  run?:
    | ((config: Config, ...args: string[]) => Promise<void>)
...
const steps: Step[] = [
      ...
      await ensureEvent(
        {
          ...
          endDateTime: addMinutes(
            new Date(config.releaseDateTime),
            1
          ).toISOString()
        },
        googleCalendar
  ...
  },
  {
    ...
    run: async config => {
      ...
      const events: EventOptions[] = [
        {
          ...
          startDateTime: new Date(
            config.fiveWorkingDaysBeforeRelease
          ).toISOString(),
          endDateTime: addMinutes(
        ...
        },
        {
          ...
          description: "See the release tracking issue for TODOs",
          startDateTime: new Date(
            config.fourWorkingDaysBeforeRelease
          ).toISOString(),
          endDateTime: addMinutes(
          ...
          })),
        {
          ...
          description: "See the release tracking issue for TODOs",
          startDateTime: new Date(
            config.oneWorkingDayBeforeRelease
          ).toISOString(),
          endDateTime: addMinutes(
            new Date(config.oneWorkingDayBeforeRelease),
            1
          ).toISOString()
        },
        {
      ...
      for (const event of events) {
        console.log(
          `Create calendar event: ${event.title}: ${event.startDateTime!}`
        );
  ...
  },
  {
    ...
    run: async config => {
      ...
      await postMessage(
        `:captain: ${config.majorVersion}.${
          config.minorVersion
        } Release :captain:
Release captain: @${config.captainSlackUsername}
Tracking issue: ${trackingIssueURL}
Key dates:
- Release branch cut, testing commences: ${formatDate(
          new Date(config.fourWorkingDaysBeforeRelease)
        )}
...
- Release: ${formatDate(new Date(config.releaseDateTime))}`,
  ...
  },
  {
    ...
    run: async (config, version) => {
      ...
      const issuesURL = `https://github.com/issues?q=${encodeURIComponent(
        query
      )}`;
