...
export async function ensureTrackingIssue({
  ...
  const milestone = milestones.data.filter(
...
export async function listIssues(
  ...
  return (
    await octokit.search.issuesAndPullRequests({ per_page: 100, q: query })
  ).data.items;
}
...
export async function getIssueByTitle(
  ...
  const response = await octokit.search.issuesAndPullRequests({
    per_page: 100,
    q: `type:issue repo:sourcegraph/sourcegraph is:open ${JSON.stringify(
      title
    )}`
  });
