...
fourWorkingDaysBeforeRelease: Date
  ...
  created: boolean }> {
      ...
      const releaseIssueBody =
          releaseIssueTemplate.replace(/\$MAJOR/g, majorVersion)
              .replace(/\$MINOR/g, minorVersion)
              .replace(/\$RELEASE_DATE/g, formatDate(releaseDateTime))
              .replace(/\$FIVE_WORKING_DAYS_BEFORE_RELEASE/g,
                       formatDate(fiveWorkingDaysBeforeRelease))
              .replace(/\$FOUR_WORKING_DAYS_BEFORE_RELEASE/g,
                       formatDate(fourWorkingDaysBeforeRelease))
              .replace(/\$ONE_WORKING_DAY_BEFORE_RELEASE/g,
                       formatDate(oneWorkingDayBeforeRelease))
      ...
      const milestone = milestones.data.filter(milestone => milestone.title ===
                                                            milestoneTitle)
      if (milestone.length === 0) {
        console.log(`Milestone ${
            JSON.stringify(
                milestoneTitle)} is closed or not found—you'll need to manually create it and add this issue to it.`)
      }
...
export async function ensurePatchReleaseIssue({
    ...
    const issueBody =
        issueTemplate.replace(/\$MAJOR/g, version.major.toString())
...
async function ensureIssue(
  ...
  created: boolean }> {
        ...
        export async function listIssues(octokit: Octokit, query: string):
            Promise<Octokit.SearchIssuesAndPullRequestsResponseItemsItem[]> {
          return (await octokit.search.issuesAndPullRequests(
                      {per_page : 100, q : query}))
              .data.items
        }

        export function trackingIssueTitle(major: string,
                                           minor: string): string {
        ...
        export async function getIssueByTitle(octokit: Octokit, title: string):
            Promise<string|null> {
          const response = await octokit.search.issuesAndPullRequests({
            per_page : 100,
            q : `type:issue repo:sourcegraph/sourcegraph is:open ${
                JSON.stringify(title)}`,
          })

          const matchingIssues =
              response.data.items.filter(issue => issue.title === title)
          if (matchingIssues.length === 0) {
            return null
          }
          if (matchingIssues.length > 1) {
            throw new Error(
                `Multiple issues matched issue title ${JSON.stringify(title)}`)
          }
        ...
        export async function createChangesets(options: ChangesetsOptions):
          ...
          const results: CreatedChangeset[] =
            ...
            await createBranchWithChanges(octokit,
                                          {...change, dryRun : options.dryRun})
            let prURL = ''
            if (!options.dryRun) {
          ...
          for (const result of results) {
            console.log(`${result.repository} (${
                result.branch}): created pull request ${result.pullRequestURL}`)
          }

          return results
        }

        async function createBranchWithChanges(octokit: Octokit, {
          owner,
          repo,
          base : baseRevision,
          head : headBranch,
          ...
          const tmpdir = await mkdtemp(
              path.join(os.tmpdir(), `sg-release-${owner}-${repo}-`))
          console.log(`Created temp directory ${tmpdir}`)
          const depthFlag = '--depth 10'

          // Determine whether or not to create the base branch, or use the
          // existing one
          let baseExists = true
          try {
          ...
          const checkoutCommand =
              baseExists === true
                  ? // check out the existing branch - fetch fails if we are
                    // already checked out, in which case just check out
                  `git fetch ${depthFlag} origin ${baseRevision}:${
                      baseRevision} || git checkout ${baseRevision}`
    ...
    git clone ${depthFlag} git@github.com:${owner}/${repo} || git clone ${
              depthFlag} https://github.com/${owner}/${repo};
    cd ./${repo};
    ${checkoutCommand};` await execa('bash', [ '-c', setupScript ],
          ...
          for (const edit of edits) {
            ...
            case 'function':
              edit(workdir)
              break case 'string': {const editScript = `set -ex

                ${edit};` await execa('bash', [ '-c', editScript ],
        ...
        git --no-pager diff;` await execa('bash', [ '-c', showChangesScript ],
                                          {stdio : 'inherit', cwd : workdir})
          } else {
            // Publish changes
        ...
        git push origin HEAD:${headBranch};` await execa(
                'bash', [ '-c', publishScript ],
                {stdio : 'inherit', cwd : workdir})
          }
        }
