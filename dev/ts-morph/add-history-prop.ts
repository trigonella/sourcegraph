 ...
 */
export function addMissingHistoryProp(diagnostic: Diagnostic,
  ...
  if (sourceFile?.getBaseName().endsWith('.test.tsx')) {
    ...
    let importDecl = sourceFile.getImportDeclaration(
        decl => decl.getModuleSpecifierValue() === 'history')
    if (!importDecl) {
    ...
    const namedBindings =
        importDecl.getImportClauseOrThrow().getNamedBindingsOrThrow()
    if (Node.isNamedImports(namedBindings) &&
        !namedBindings.getChildrenOfKind(SyntaxKind.ImportSpecifier)
             .some(spec => spec.getText() === 'createMemoryHistory')) {
      importDecl.addNamedImport('createMemoryHistory')
    }
    const namespace =
        Node.isNamespaceImport(namedBindings) ? namedBindings.getName() : null
    initializer = namespace ? `{${namespace}.createMemoryHistory()}`
                            : '{createMemoryHistory()}'
  }
  else {
    // Application code
    ...
    if (defaultImport) {
      ...
      importDecl = importDecl.replaceWithText("import * as H from 'history'") as
                   ImportDeclaration
    }
    const namedBindings =
        importDecl.getImportClauseOrThrow().getNamedBindingsOrThrow()
    ...
    const namespace =
        Node.isNamespaceImport(namedBindings) ? namedBindings.getName() : null
    const classDecl = jsxNode.getFirstAncestor(
        anyOf(Node.isClassDeclaration, Node.isClassExpression))
    if (classDecl) {
      ...
      const propsSymbol =
          Node.isTypeReferenceNode(propsTypeArg)
              ? propsTypeArg.getFirstChildByKindOrThrow(SyntaxKind.Identifier)
                    .getSymbolOrThrow()
              : propsTypeArg.getSymbolOrThrow()
      if (!propsSymbol.getMember('history')) {
        const propsDecl =
            propsSymbol.getDeclarations()[0] if (Node.isInterfaceDeclaration(
                                                     propsDecl) ||
      ...
      // Function component
      const functionDecl = jsxNode.getFirstAncestorOrThrow(
          (node: Node): node is VariableDeclaration => {
            const isVarDecl = Node.isVariableDeclaration(node)
            try {
              const isFuncComp =
                  node.getType().getSymbol()?.getName() === 'FunctionComponent'
      ...
      const propsSymbol =
          functionDecl.getType().getTypeArguments()[0].getSymbolOrThrow()
      if (!propsSymbol.getMember('history')) {
        const propsDecl =
            propsSymbol.getDeclarations()[0] if (Node.isInterfaceDeclaration(
                                                     propsDecl) ||
      ...
      const paramNode =
          functionDecl.getFirstDescendantByKindOrThrow(SyntaxKind.Parameter)
      const paramName = paramNode.getNameNode()
      if (Node.isObjectBindingPattern(paramName)) {
        const restSpread = paramName.getFirstChild(
            (node): node is BindingElement =>
                Node.isBindingElement(node) && !!node.getDotDotDotToken())
        if (restSpread) {
          // Reference rest spread
          initializer = `{${restSpread.getName()}.history}`
        }
        else {
          if (!paramName.getElements().some(element => element.getName() ===
                                                       'history')) {
            // Add to destructured props
            // ts-morph currently has no nice API for this so we use the
            // low-level TS API https://github.com/dsherret/ts-morph/issues/775
