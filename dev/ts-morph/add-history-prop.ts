 ...
 */
export function addMissingHistoryProp(
  ...
  if (sourceFile?.getBaseName().endsWith(".test.tsx")) {
    ...
    const namespace = Node.isNamespaceImport(namedBindings)
      ? namedBindings.getName()
      : null;
    initializer = namespace
      ? `{${namespace}.createMemoryHistory()}`
      : "{createMemoryHistory()}";
  } else {
    // Application code
    ...
    const defaultImport = importDecl
      .getImportClauseOrThrow()
      .getDefaultImport();
    if (defaultImport) {
      // history should not be imported with a default import
      importDecl = importDecl.replaceWithText(
    ...
    if (classDecl) {
      ...
      if (!propsSymbol.getMember("history")) {
        ...
        if (
          Node.isInterfaceDeclaration(propsDecl) ||
          Node.isTypeLiteralNode(propsDecl)
        ) {
      ...
      const propsSymbol = functionDecl
        .getType()
        .getTypeArguments()[0]
        .getSymbolOrThrow();
      if (!propsSymbol.getMember("history")) {
      ...
      const paramNode = functionDecl.getFirstDescendantByKindOrThrow(
      ...
      if (Node.isObjectBindingPattern(paramName)) {
        ...
        if (restSpread) {
          ...
          if (
            !paramName
              .getElements()
              .some(element => element.getName() === "history")
          ) {
